    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Sinria"
"""
...
def _fallback_runtime_home() -> Path:
    explicit_home = os.environ.get("SINRIA_HOME") or os.environ.get("HERMES_HOME")
    if explicit_home:
        return Path(explicit_home).expanduser()
    cli_name = os.environ.get("HERMES_CLI_NAME", "hermes").strip().lower()
    if cli_name == "sinria":
        return Path.home() / ".sinria"
...
def _get_token_dir() -> Path:
    ...
    Uses the active runtime home so each profile gets its own OAuth tokens.
...
async def _redirect_handler(authorization_url: str) -> None:
    ...
    if _oauth_port and (os.getenv("SSH_CLIENT") or os.getenv("SSH_TTY")):
        print(
            ...
            f"\n"
            f"  Then open the URL above. See: https://sinria-agent.nousresearch.com/docs/guides/oauth-over-ssh\n",
            file=sys.stderr,
        )
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    client_name = cfg.get("client_name", "Sinria")
    scope = cfg.get("scope")
