...
def _fire_approval_hook(hook_name: str, **kwargs) -> None:
    ...
    """
    # Persistent audit first: the JSONL sink (tools/audit_log.py) works even
    # when the plugin system is unavailable and never raises. This is the
    # durable record healthcare/confidential deployments audit against.
    try:
        from tools.audit_log import record_audit_event

        record_audit_event(hook_name, **kwargs)
    except Exception:
        pass
    try:
...
def request_gateway_approval(
    ...
    *,
    ...
    """Request interactive approval inside a gateway session.

    This is the generic approval primitive for non-terminal gates such as
    Sinria external-egress decisions. It reuses the same per-session queue and
    Discord/Slack button plumbing as dangerous command approvals, but callers
    provide their own preview and policy key.
    """
    session_key = get_current_session_key(default="")
    if not session_key or not _is_gateway_approval_context():
        return {
            "approved": False,
            "message": "BLOCKED: Approval required, but no interactive gateway approval session is active. Do NOT retry automatically.",
            "pattern_key": pattern_key,
            "description": description,
        }

    if allow_session and is_approved(session_key, pattern_key):
        return {"approved": True, "message": None, "preapproved": True}
    ...
    if notify_cb is None:
        submit_pending(session_key, {
            "command": preview,
            "pattern_key": pattern_key,
            "pattern_keys": pattern_keys or [pattern_key],
            "description": description,
        ...
        return {
            ...
            "message": "Approval required, but this gateway session has no approval notifier registered.",
            "pattern_key": pattern_key,
            "description": description,
        }

    all_keys = pattern_keys or [pattern_key]
    approval_data = {
        "command": preview,
        "pattern_key": pattern_key,
    ...
    if choice in {"session", "always"} and allow_session:
        for key in all_keys:
...
def check_all_command_guards(command: str, env_type: str,
    ...
    """
    if (os.getenv("HERMES_CLI_NAME") or "").strip().lower() == "sinria":
        try:
            from agent.sinria_egress import SinriaEgressBlocked, guard_terminal_command_egress

            guard_terminal_command_egress(command)
        except SinriaEgressBlocked as exc:
            decision = exc.decision
            status = "approval_required" if decision.action == "ask" else "blocked"
            return {
                "approved": False,
                "message": str(exc),
                "status": status,
                ...
                "pattern_key": "sinria_external_egress",
            }
        except Exception:
