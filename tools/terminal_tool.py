...
Singularity, Daytona, Vercel Sandbox, and Workshop environments. Supports
...
- "workshop": Execute in a Canonical Workshop (LXD) sandbox on Ubuntu hosts
...
# Backend signature per cached environment: (env_type, workshop_name-or-"").
# A cache hit must match the currently-requested signature — reusing a cached
# env across a backend switch would silently ignore the requested isolation
# (e.g. a workshop-pinned task running in a previously-cached local env).
_env_signatures: Dict[str, tuple] = {}
...
def _get_env_config() -> Dict[str, Any]:
    ...
    elif env_type == "workshop":
        # Workshop mounts the launch directory at /project; init_session
        # self-recovers to the exec user's home when the mount is absent.
        default_cwd = "/project"
    else:
        default_cwd = "/root"
    ...
    if env_type == "docker" and mount_docker_cwd:
        ...
        if (
            ...
            host_cwd = candidate
            cwd = "/workspace"
    elif env_type in {"modal", "docker", "singularity", "daytona", "vercel_sandbox", "workshop"} and cwd:
    ...
    return {
        ...
        # Workshop (Canonical LXD sandbox) backend config
        "workshop_name": os.getenv("TERMINAL_WORKSHOP_NAME", "").strip(),
        "workshop_auto_start": os.getenv("TERMINAL_WORKSHOP_AUTO_START", "false").lower() in {"true", "1", "yes"},
        "workshop_stop_on_cleanup": os.getenv("TERMINAL_WORKSHOP_STOP_ON_CLEANUP", "false").lower() in {"true", "1", "yes"},
    }
...
def _create_environment(env_type: str, image: str, cwd: str, timeout: int,
    ...
    Args:
        env_type: One of "local", "docker", "singularity", "modal",
            "daytona", "vercel_sandbox", "workshop", "ssh"
    ...
    if env_type == "local":
        ...
        # Lazy import so the backend stays optional on non-Ubuntu hosts.
        from tools.environments.workshop import (
            WorkshopEnvironment as _WorkshopEnvironment,
        )
        workshop_name = (cc.get("workshop_name") or "").strip()
        if not workshop_name:
            raise ValueError(
                "Workshop environment requires TERMINAL_WORKSHOP_NAME to be "
                "set to a pre-provisioned workshop (see `workshop list`)."
            )
        return _WorkshopEnvironment(
            workshop_name=workshop_name,
            cwd=cwd,
            timeout=timeout,
            auto_start=cc.get("workshop_auto_start", False),
            stop_on_cleanup=cc.get("workshop_stop_on_cleanup", False),
        )
    ...
    with _env_lock:
        for task_id, last_time in list(_last_activity.items()):
            if current_time - last_time > lifetime_seconds:
                ...
                _env_signatures.pop(task_id, None)
...
def terminal_tool(
    ...
    try:
        ...
        # Cache signature for environment reuse: switching backends (or the
        # target workshop) must retire the cached env instead of reusing it.
        desired_env_signature = (
            env_type,
            config.get("workshop_name", "") if env_type == "workshop" else "",
        )
        ...
        with _env_lock:
            if effective_task_id in _active_environments:
                cached_sig = _env_signatures.get(effective_task_id)
                if cached_sig is None or cached_sig == desired_env_signature:
                    _last_activity[effective_task_id] = time.time()
                    env = _active_environments[effective_task_id]
                    needs_creation = False
                else:
                    # Backend switch (e.g. sandbox policy now pins workshop):
                    # retire the stale env and rebuild with the requested one.
                    stale_env = _active_environments.pop(effective_task_id, None)
                    _env_signatures.pop(effective_task_id, None)
        ...
        if stale_env is not None:
            logger.info(
                "Backend switch for task %s: retiring cached env, creating %s",
                effective_task_id[:8],
                desired_env_signature[0],
            )
            try:
                stale_env.cleanup()
            except Exception:
        ...
        if needs_creation:
            ...
            with task_lock:
                # Double-check after acquiring the per-task lock (signature must
                # still match — a concurrent creator may have built a different
                # backend; we then rebuild and retire theirs on insert).
                with _env_lock:
                    if (
                        effective_task_id in _active_environments
                        and _env_signatures.get(effective_task_id)
                        in (None, desired_env_signature)
                    ):
                        _last_activity[effective_task_id] = time.time()
                        env = _active_environments[effective_task_id]
                        needs_creation = False
                ...
                if needs_creation:
                    ...
                    try:
                        ...
                        if env_type in {"docker", "singularity", "modal", "daytona", "vercel_sandbox", "workshop"}:
                            container_config = {
                                ...
                                "workshop_auto_start": config.get("workshop_auto_start", False),
                                "workshop_stop_on_cleanup": config.get("workshop_stop_on_cleanup", False),
                            }
                    ...
                    prev_env = None
                    with _env_lock:
                        prev_env = _active_environments.get(effective_task_id)
                        _active_environments[effective_task_id] = new_env
                        _env_signatures[effective_task_id] = desired_env_signature
                        _last_activity[effective_task_id] = time.time()
                        env = new_env
                    if prev_env is not None and prev_env is not new_env:
                        # Lost a creation race against a different-signature env;
                        # ours matches the requested config — retire theirs.
                        try:
                            prev_env.cleanup()
                        except Exception:
                            pass
