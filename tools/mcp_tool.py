...
transport, discovers their tools, and registers them into the sinria-agent
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            try:
                ...
                default_dir = ".sinria" if os.getenv("HERMES_CLI_NAME", "").strip().lower() == "sinria" else ".hermes"
                hermes_home = os.path.expanduser(
                    os.getenv("HERMES_HOME", os.path.join(os.path.expanduser("~"), default_dir))
                )
            candidates = [
...
def _cache_mcp_image_block(block) -> str:
    ...
    return a ``MEDIA:<path>`` tag that Sinria gateways know how to render.
