...
def _sinria_redact_process_notification_field(value: str) -> str:
    if str(os.getenv("HERMES_CLI_NAME", "")).lower() != "sinria":
        return value
    try:
        from agent.sinria_egress import looks_confidential
        if looks_confidential(value):
            return "[sinria-redacted-confidential-process-notification]"
    except Exception:
        pass
    return value
...
def format_process_notification(evt: dict) -> "str | None":
    ...
    _cmd = _sinria_redact_process_notification_field(evt.get("command", "unknown"))

    if evt_type == "watch_disabled":
        message = _sinria_redact_process_notification_field(evt.get("message", ""))
        return f"[IMPORTANT: {message}]"

    if evt_type == "watch_match":
        _pat = evt.get("pattern", "?")
        _out = _sinria_redact_process_notification_field(evt.get("output", ""))
