...
def _handle_send(args):
    ...
    try:
        ...
        guard_messaging_egress(target_label, cleaned_message)
        ...
        try:
            ...
            if isinstance(egress_exc, SinriaEgressBlocked):
                return json.dumps(_error(str(egress_exc)))
        except Exception:
            pass
        logger.debug("Sinria messaging egress guard skipped due to error: %s", egress_exc)
...
async def _send_feishu(pconfig, chat_id, message, media_files=None, thread_id=None):
    ...
    try:
        ...
        if not FEISHU_AVAILABLE:
            return {"error": "Feishu dependencies not installed. Run: pip install 'sinria-agent[feishu]'"}
