...
# Temp roots are the only carve-out from the sensitive-path write guard.  The
# source of truth (``tempfile.gettempdir()``) is process mutable: tests/plugins
# can monkeypatch ``tempfile.tempdir`` or env vars before first use.  Never trust
# an arbitrary value such as ``/etc`` or ``/private/var/db`` as a carve-out root.
# Keep this intentionally narrow: it covers normal POSIX temp locations plus the
# macOS per-user temp subtree that resolves under the otherwise protected
# ``/private/var/`` prefix.
_TRUSTED_TEMP_ROOTS = (
    "/tmp",
    "/private/tmp",
...
def _is_trusted_temp_root(path: str) -> bool:
    """Return True only for canonical OS temp roots we intentionally exempt."""
...
def _temp_dir_carveout_roots() -> tuple[str, ...]:
    """Realpath-resolved temp roots that are exempt from the sensitive guard.

    On macOS ``tempfile.gettempdir()`` lives under ``/private/var/folders/…/T``
    (``/var`` is a symlink to ``/private/var``), so legitimate temp/scratch
    writes — including everything in pytest's ``tmp_path`` — resolve under the
    ``"/private/var/"`` prefix in :data:`_SENSITIVE_PATH_PREFIXES` and would be
    wrongly refused. Carve out *only* the process temp dir so writes there are
    allowed while ``/private/var/db``, ``/private/var/root``, etc. stay
    protected. We do NOT drop the ``/private/var/`` prefix itself.
    """
    roots: list[str] = []
    # tempfile.gettempdir() already reads $TMPDIR internally; listing $TMPDIR
    # separately is redundant and lets a monkeypatch.setenv("TMPDIR", …) in
    # tests accidentally widen the carve-out to an unintended directory.
    for raw in (tempfile.gettempdir(),):
        if not raw:
            continue
        try:
            ...
            continue
        if real and _is_trusted_temp_root(real) and real not in roots:
            roots.append(real)
    return tuple(roots)


def _is_within_temp_carveout(path: str) -> bool:
    """True if *path* (realpath-resolved) sits inside a temp carveout root."""
    try:
        real = os.path.realpath(path)
    except (OSError, ValueError):
        return False
    for root in _temp_dir_carveout_roots():
        if real == root or real.startswith(root + os.sep):
            return True
    return False
...
def _check_sensitive_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    # Carve-out: the OS temp dir (macOS resolves it under /private/var/folders)
    # is a legitimate write target even though it matches the /private/var/
    # prefix below. The realpath-resolved target (``resolved``) is the actual
    # write destination, so gate on that. When the caller passed an *absolute*
    # path we additionally require its own normalization to land in the temp
    # dir — this defeats a literal sensitive path (e.g. "/private/var/db/x")
    # that some upstream cwd quirk might otherwise resolve under temp. For
    # relative paths ``normalized`` is cwd-relative and not meaningful here, so
    # only ``resolved`` (task-cwd aware) is consulted. Other /private/var
    # locations like /private/var/db stay guarded.
    if _is_within_temp_carveout(resolved):
        if os.path.isabs(normalized):
            if _is_within_temp_carveout(normalized):
                return None
        else:
            return None
