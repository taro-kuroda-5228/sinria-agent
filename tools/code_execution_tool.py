...
Lets the LLM write a Python script that calls Sinria tools via RPC,
...
"""Auto-generated Sinria tools RPC stubs."""
...
"""Auto-generated Sinria tools RPC stubs (file-based transport)."""
...
def execute_code(
    ...
    try:
        ...
        # Ensure the sinria-agent root is importable in the sandbox so
...
def _get_execution_mode() -> str:
    ...
    Mode semantics:
      ...
      - ``strict``: scripts run in an isolated temp directory with
        ``sys.executable`` (sinria-agent's python). Reproducible and the
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    # temp-dir staging and sinria-agent's own python.
    if mode == "strict":
        try:
            from hermes_constants import display_hermes_home

            env_hint = f"{display_hermes_home()}/.env"
        except Exception:
            cli_name = (os.getenv("SINRIA_CLI_NAME") or os.getenv("HERMES_CLI_NAME") or "").strip().lower()
            env_hint = "~/.sinria/.env" if cli_name == "sinria" else "~/.hermes/.env"
        cwd_note = (
            "Scripts run in their own temp dir, not the session's CWD — use absolute paths "
            f"(os.path.expanduser('{env_hint}')) or terminal()/read_file() for user files."
        )
    else:
        cwd_note = (
    ...
    description = (
        "Run a Python script that can call Sinria tools programmatically. "
