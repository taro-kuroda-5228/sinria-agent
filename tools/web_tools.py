...
def web_search_tool(query: str, limit: int = 5) -> str:
    ...
    try:
        ...
        guard_web_query_egress("web_search", query)
    except Exception as exc:
        if exc.__class__.__name__ == "SinriaEgressBlocked":
            # Do not include the raw query in the tool error or debug logs.
...
async def web_extract_tool(
    ...
    try:
        ...
        for _url in urls:
            guard_web_query_egress("web_search", _url)
    except Exception as exc:
        if exc.__class__.__name__ == "SinriaEgressBlocked":
            return tool_error(str(exc))
        raise
...
async def web_crawl_tool(
    ...
    try:
        from agent.sinria_egress import guard_web_query_egress
        guard_web_query_egress("web_search", url)
        if instructions:
            guard_web_query_egress("web_search", instructions)
    except Exception as exc:
        if exc.__class__.__name__ == "SinriaEgressBlocked":
            return tool_error(str(exc))
        raise
