---
name: honcho
description: Configure and use Honcho memory with Sinria -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.
version: 2.0.0
author: Sinria Agent
license: MIT
...
# Honcho Memory for Sinria

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Sinria profile its own peer identity while sharing a unified view of the user.
...
```bash
sinria honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Sinria profile's AI peer
...
- **AI peer** (`aiPeer`): represents this Sinria instance. Each profile gets its own AI peer so agents develop independent views.
...
Manual override: `sinria honcho map my-project-name`
...
Each Sinria profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
```bash
sinria profile create coder --clone
...
```bash
sinria honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Run `sinria honcho setup`. Ensure `memory.provider: honcho` is in `~/.sinria/config.yaml`. Legacy `~/.hermes/config.yaml` may be read only as an explicit compatibility alias during migration.

### Memory not persisting across sessions
Check `sinria honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `sinria profile create <name> --clone`. For existing profiles: `sinria honcho sync`.
...
| Command | Description |
|---------|-------------|
| `sinria honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `sinria honcho status` | Show resolved config, connection test, peer info for active profile |
| `sinria honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `sinria honcho disable` | Disable Honcho for the active profile |
| `sinria honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `sinria honcho peers` | Show peer identities across all profiles |
| `sinria honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `sinria honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `sinria honcho sessions` | List known directory-to-session-name mappings |
| `sinria honcho map <name>` | Map current working directory to a Honcho session name |
| `sinria honcho identity` | Seed AI peer identity or show both peer representations |
| `sinria honcho sync` | Create host blocks for all Sinria profiles that don't have one yet |
| `sinria honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Sinria + Honcho |
| `sinria memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `sinria memory status` | Show active memory provider and config |
| `sinria memory off` | Disable external memory provider |
