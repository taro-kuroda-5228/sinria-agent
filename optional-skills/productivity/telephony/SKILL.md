---
name: telephony
description: Give Sinria phone capabilities without core tool changes. Provision and persist a Twilio number, send and receive SMS/MMS, make direct calls, and place AI-driven outbound calls through Bland.ai or Vapi.
version: 1.0.0
...
This optional skill gives Sinria practical phone capabilities while keeping telephony out of the core tool list.

It ships with a helper script, `scripts/telephony.py`, that can:
- save provider credentials into `~/.sinria/.env`
...
4. Treat third-party phone numbers as sensitive operational data:
   - do not save them to Sinria memory
...
### 1) "I want Sinria to own a real phone number"
Use **Twilio**.
...
- pairs well with Sinria `text_to_speech` plus a public file host or tunnel
...
```bash
SCRIPT="$(find ~/.sinria/skills -path '*/telephony/scripts/telephony.py' -print -quit)"
```
...
```bash
sinria skills search telephony
sinria skills install official/productivity/telephony
```
...
Then save credentials into Sinria (`~/.sinria/.env`; legacy `~/.hermes/.env` is compatibility-only):
...
3. Buy it and save it into `~/.sinria/.env` + state:
```bash
python3 "$SCRIPT" twilio-buy "+17025551234" --save-env
```
...
Recommended Sinria TTS -> Twilio Play workflow:

1. Generate the audio with Sinria `text_to_speech`.
...
Important note:
- Sinria TTS is great for prerecorded outbound messages
...
- Sinria STT/TTS alone is not being used here as a full duplex phone conversation engine; that would require a much heavier streaming/webhook integration than this skill is trying to introduce
...
7. Summarize the outcome without persisting third-party numbers to Sinria memory.
...
- Do not store arbitrary third-party phone numbers in Sinria memory.
...
3. persist that number to `~/.sinria/.env`
...
Sinria uses `~/.sinria` as the primary runtime home. `~/.hermes` paths are legacy compatibility aliases only and should not be used in new setup instructions.
