...
def _default_home_dir() -> Path:
    if os.environ.get("HERMES_CLI_NAME", "").lower() == "sinria":
...
def _state_dir() -> Path:
    ...
    # Default: $HERMES_HOME/watcher-state/, falling back to the active CLI home.
    hermes_home = os.environ.get("HERMES_HOME")
    if hermes_home:
        return Path(hermes_home) / "watcher-state"
    return _default_home_dir() / "watcher-state"
