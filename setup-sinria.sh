...
# 5. Symlinks the 'sinria' CLI command into a user-facing bin dir
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/sinria-uv-install.$$.log")"
        _uv_installer="$(mktemp 2>/dev/null || echo "/tmp/sinria-uv-installer.$$.sh")"
...
# ============================================================================
# PATH setup — symlink sinria into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up sinria command..."

HERMES_BIN="$SCRIPT_DIR/venv/bin/sinria"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$HERMES_BIN" "$COMMAND_LINK_DIR/sinria"
echo -e "${GREEN}✓${NC} Symlinked sinria → $COMMAND_LINK_DISPLAY_DIR/sinria"
...
# ============================================================================
# Seed bundled skills into ~/.sinria/skills/
# ============================================================================

HERMES_SKILLS_DIR="${SINRIA_HOME:-$HOME/.sinria}/skills"
mkdir -p "$HERMES_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.sinria/skills/ ..."
...
echo "Other commands:"
echo "  sinria status        # Check configuration"
if is_termux; then
    echo "  sinria gateway       # Run gateway in foreground"
else
    echo "  sinria gateway install # Install gateway service (messaging + cron)"
fi
echo "  sinria cron list     # View scheduled jobs"
echo "  sinria doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m sinria_cli.main setup
fi
