# Sinria Security Policy

This document describes Sinria's trust model, names the one
security boundary the project treats as load-bearing, and defines the
scope for vulnerability reports.

## 1. Reporting a Vulnerability

Report privately via [GitHub Security Advisories](https://github.com/taro-kuroda-5228/sinria-agent/security/advisories/new).
Do not open public issues for
security vulnerabilities. **Sinria does not operate a bug
bounty program.**
...
- Environment details (`sinria version`, commit SHA, OS, Python
  version).
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted Sinria access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in Sinria's documentation
...
Sinria supports two OS-level isolation postures. They address
...
Sinria supports this in two ways:

- **Sinria's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, Sinria's in-process heuristics
...
Sinria filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in Sinria
itself. Bugs in Sinria's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   Sinria does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, Sinria's own documentation, or reasonable
  operator expectations would predict — including cases where
  Sinria has documented a stance about how its output should
...
- **Documented break-glass settings.** Operator-selected trade-offs
  ...
  sinria-home security, and similar. Reports against those
...
- Sinria includes supply-chain guards for MCP server
