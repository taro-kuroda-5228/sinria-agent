...
You no longer need to wrap `sinria update` in `screen` or `tmux` to survive a terminal drop.
...
Compare against the latest release at the [GitHub releases page](https://github.com/taro-kuroda-5228/sinria-agent/releases).
...
sinria config check
sinria config migrate   # Interactively add any missing options
```
