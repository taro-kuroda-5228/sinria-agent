...
description: "Install and deploy Sinria Agent with Nix — from quick `nix run` to fully declarative NixOS module with container mode"
---

# Nix & NixOS Setup

Sinria Agent ships a Nix flake with three levels of integration:
...
**For non-NixOS users**, this only changes the install step. Everything after (`sinria setup`, `sinria gateway install`, config editing) works identically to the standard install.

**For NixOS module users**, the entire lifecycle is different: configuration lives in `configuration.nix`, secrets go through sops-nix/agenix, the service is a systemd unit, and CLI config commands are blocked. You manage Sinria the same way you manage any other NixOS service.
:::
...
nix run github:taro-kuroda-5228/sinria-agent -- setup
nix run github:taro-kuroda-5228/sinria-agent -- chat

# Or install persistently
nix profile install github:taro-kuroda-5228/sinria-agent
sinria setup
sinria chat
```

After `nix profile install`, `sinria`, `sinria-agent`, and `sinria-acp` are on your PATH. From here, the workflow is identical to the [standard installation](./installation.md) — `sinria setup` walks you through provider selection, `sinria gateway install` sets up a launchd (macOS) or systemd user service, and config lives in `~/.sinria/`.
...
# /etc/nixos/flake.nix (or your system flake)
{
  inputs = {
    ...
    sinria-agent.url = "github:taro-kuroda-5228/sinria-agent";
  };
...
Setting `addToSystemPackages = true` does two things: puts the `sinria` CLI on your system PATH **and** sets `HERMES_HOME` system-wide so the interactive CLI shares state (sessions, skills, cron) with the gateway service. Without it, running `sinria` in your shell creates a separate `~/.sinria/` directory.
:::
...
```bash
sinria mcp add my-oauth-server --url https://mcp.example.com/mcp --auth oauth
scp ~/.sinria/mcp-tokens/my-oauth-server{,.client}.json \
    server:/var/lib/sinria/.sinria/mcp-tokens/
# Ensure: chown sinria:sinria, chmod 0600
```
...
| Blocked command | Why |
|---|---|
| `sinria setup` | Config is declarative — edit `settings` in your Nix config |
| `sinria config edit` | Config is generated from `settings` |
| `sinria config set <key> <value>` | Config is generated from `settings` |
| `sinria gateway install` | The systemd service is managed by NixOS |
| `sinria gateway uninstall` | The systemd service is managed by NixOS |
...
```nix
{
  inputs.sinria-agent.url = "github:taro-kuroda-5228/sinria-agent";
  outputs = { sinria-agent, nixpkgs, ... }: {
    nixpkgs.overlays = [ sinria-agent.overlays.default ];
    # Then:
    #   pkgs.sinria-agent.override { extraPythonPackages = [...]; }
    #   pkgs.sinria-agent.override { extraDependencyGroups = [ "hindsight" ]; }
  };
}
```
