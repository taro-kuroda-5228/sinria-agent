...
The single most important setup step. Use `sinria model` to walk through the choice interactively:
...
|----------|-----------|---------------|
| **Medical Horizon Portal** | Subscription-based, zero-config | OAuth login via `sinria model` |
| **OpenAI Codex** | ChatGPT OAuth, uses Codex models | Device code auth via `sinria model` |
| **Anthropic** | Claude models directly — Max plan + extra usage credits (OAuth), or API key for pay-per-token | `sinria model` → OAuth login (requires Max + extra credits), or an Anthropic API key |
...
| **MiniMax (OAuth)** | MiniMax-M2.7 via browser OAuth — no API key needed | `sinria model` → MiniMax (OAuth) |
...
| **GitHub Copilot** | GitHub Copilot subscription (GPT-5.x, Claude, Gemini, etc.) | OAuth via `sinria model`, or `COPILOT_GITHUB_TOKEN` / `GH_TOKEN` |
| **GitHub Copilot ACP** | Copilot ACP agent backend (spawns local `copilot` CLI) | `sinria model` (requires `copilot` CLI + `copilot login`) |
...
Sinria Agent requires a model with at least **64,000 tokens** of context. Models with smaller windows cannot maintain enough working memory for multi-step tool-calling workflows and will be rejected at startup. Most hosted models (Claude, GPT, Gemini, Qwen, DeepSeek) meet this easily. If you're running a local model, set its context size to at least 64K (e.g. `--ctx-size 65536` for llama.cpp or `-c 65536` for Ollama).
:::

:::tip
You can switch providers at any time with `sinria model` — no lock-in. For a full list of all supported providers and setup details, see [AI Providers](../integrations/providers.md).
:::
...
- **Secrets and tokens** → `~/.sinria/.env` (`~/.hermes/.env` remains a compatibility alias)
- **Non-secret settings** → `~/.sinria/config.yaml` (`~/.hermes/config.yaml` compatibility alias)

The easiest way to set values correctly is through the CLI:

```bash
sinria config set model anthropic/claude-opus-4.6
sinria config set terminal.backend docker
sinria config set OPENROUTER_API_KEY sk-or-...
```
