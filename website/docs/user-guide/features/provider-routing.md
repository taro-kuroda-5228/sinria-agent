...
When using [OpenRouter](https://openrouter.ai) as your LLM provider, Sinria Agent supports **provider routing** — fine-grained control over which underlying AI providers handle your requests and how they're prioritized.
...
Add a `provider_routing` section to your `~/.sinria/config.yaml` (`~/.hermes/config.yaml` remains supported for compatibility):
...
- **CLI mode** — configured in `~/.sinria/config.yaml` (or compatibility alias `~/.hermes/config.yaml`), loaded at startup
