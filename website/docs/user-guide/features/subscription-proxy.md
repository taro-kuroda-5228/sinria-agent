...
description: "Use your Medical Horizon subscription (or other OAuth provider) as an OpenAI-compatible endpoint for external apps"
---
...
chat completions — use your Sinria-managed provider subscription as their
...
| Use case | "Use Sinria as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the Medical Horizon OAuth flow. Sinria stores
the refresh token in `~/.sinria/auth.json` (`~/.hermes/auth.json` remains supported for compatibility) — the same place all Sinria
...
```
Starting Sinria proxy for Medical Horizon Portal
...
Model:      Sinria-4-70B    # or Sinria-4.3-36B, Sinria-4-405B
```
...
```
Sinria proxy upstream adapters

  [nous    ] Nous Portal — ready (bearer expires 2026-05-15T06:43:21Z)
```

If you see `not logged in`, run `sinria login nous`. If you see
