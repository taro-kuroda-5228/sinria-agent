...
description: "Customize Sinria Agent's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

Sinria Agent's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.
...
If you want to change who Sinria is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

Sinria now seeds a default `SOUL.md` automatically in:

```text
~/.sinria/SOUL.md
```

More precisely, it uses the current instance's `HERMES_HOME`, so if you run Sinria with a custom home directory, it will use:
...
- Sinria creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- Sinria loads `SOUL.md` only from `HERMES_HOME`
- Sinria does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, Sinria falls back to a built-in default identity
...
If Sinria loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `HERMES_HOME`, the personality belongs to the Sinria instance itself.

That also makes it easier to teach users:
- "Edit `~/.sinria/SOUL.md` to change Sinria's default personality."
...
- how Sinria should handle uncertainty, disagreement, or ambiguity
...
If the file is empty, whitespace-only, or cannot be read, Hermes falls back to a built-in default identity ("You are Sinria, an independently developed AI agent for confidential organizations..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
