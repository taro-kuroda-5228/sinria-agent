...
description: "Overview of Sinria Agent's tools — what's available, how toolsets work, and terminal backends"
---
...
Sinria ships with a broad built-in tool registry covering web search, browser automation, terminal execution, file editing, memory, delegation, RL training, messaging delivery, Home Assistant, and more.
...
Paid Medical Horizon subscribers can use web search, image generation, TTS, and browser automation through the **[Tool Gateway](tool-gateway.md)** — no separate API keys needed. Run `sinria model` to enable it, or configure individual tools with `sinria tools`.
:::
...
sinria chat --toolsets "web,terminal"
...
See [Toolsets Reference](/docs/reference/toolsets-reference) for the full set, including platform presets such as `sinria-cli`, `sinria-telegram`, and dynamic MCP toolsets like `mcp-<server>`.
...
**One persistent container, shared across the whole process.** Sinria starts a single long-lived container on first use (`docker run -d ... sleep 2h`) and routes every terminal, file, and `execute_code` call through `docker exec` into that same container. Working-directory changes, installed packages, environment tweaks, and files written to `/workspace` all carry over from one tool call to the next, across `/new`, `/reset`, and `delegate_task` subagents, for the lifetime of the Sinria process. The container is stopped and removed on shutdown.
...
```
```bash
# Set credentials in ~/.sinria/.env (or ~/.hermes/.env for compatibility)
...
# Configure
sinria config set terminal.backend singularity
sinria config set terminal.singularity_image ~/python.sif
```
...
modal setup
sinria config set terminal.backend modal
```

### Vercel Sandbox

```bash
pip install 'sinria-agent[vercel]'
sinria config set terminal.backend vercel_sandbox
sinria config set terminal.vercel_runtime node24
```

Authenticate with all three of `VERCEL_TOKEN`, `VERCEL_PROJECT_ID`, and `VERCEL_TEAM_ID`. This access-token setup is the supported path for deployments and normal long-running Sinria processes on Render, Railway, Docker, and similar hosts. Supported runtimes are `node24`, `node22`, and `python3.13`; Sinria defaults to `/vercel/sandbox` as the remote workspace root.

For one-off local development, Hermes also accepts short-lived Vercel OIDC tokens:

```bash
VERCEL_OIDC_TOKEN="$(vc project token <project-name>)" sinria chat
```
