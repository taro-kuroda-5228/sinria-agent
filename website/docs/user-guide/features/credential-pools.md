...
Credential pools let you register multiple API keys or OAuth tokens for the same provider. When one key hits a rate limit or billing quota, Sinria automatically rotates to the next healthy key — keeping your session alive without switching providers.
...
If you already have an API key set in `.env`, Sinria auto-discovers it as a 1-key pool. To benefit from pooling, add more keys:

```bash
# Add a second OpenRouter key
sinria auth add openrouter --api-key sk-or-v1-your-second-key

# Add a second Anthropic key
sinria auth add anthropic --type api-key --api-key sk-ant-api03-your-second-key

# Add an Anthropic OAuth credential (requires Claude Max plan + extra usage credits)
sinria auth add anthropic --type oauth
...
anthropic (3 credentials):
  #1  sinria_pkce          oauth   sinria_pkce ←
...
Run `sinria auth` with no subcommand for an interactive wizard:
...
| Command | Description |
|---------|-------------|
| `sinria auth` | Interactive pool management wizard |
| `sinria auth list` | Show all pools and credentials |
| `sinria auth list <provider>` | Show a specific provider's pool |
| `sinria auth add <provider>` | Add a credential (prompts for type and key) |
| `sinria auth add <provider> --type api-key --api-key <key>` | Add an API key non-interactively |
| `sinria auth add <provider> --type oauth` | Add an OAuth credential via browser login |
| `sinria auth remove <provider> <index>` | Remove credential by 1-based index |
| `sinria auth reset <provider>` | Clear all cooldowns/exhaustion status |

## Rotation Strategies

Configure via `sinria auth` → "Set rotation strategy" or in `config.yaml`:
...
Sinria automatically discovers credentials from multiple sources and seeds the pool on startup:
...
| Sinria PKCE OAuth | `~/.sinria/auth.json` | Yes (Anthropic) |
| Custom endpoint config | `model.api_key` in config.yaml | Yes (custom endpoints) |
| Manual entries | Added via `sinria auth add` | Persisted in auth.json |

Auto-seeded entries are updated on each pool load — if you remove an env var, its pool entry is automatically pruned. Manual entries (added via `sinria auth add`) are never auto-pruned.
...
2. **`hermes_cli/auth_commands.py`** — Legacy-compatible internal CLI module for Sinria auth commands and interactive wizard
3. **`hermes_cli/runtime_provider.py`** — Legacy-compatible internal module for pool-aware credential resolution
