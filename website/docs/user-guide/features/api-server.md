...
sidebar_position: 14
title: "API Server"
description: "Expose sinria-agent as an OpenAI-compatible API for any frontend"
---

# API Server

The API server exposes sinria-agent as an OpenAI-compatible HTTP endpoint. Any frontend that speaks the OpenAI format — Open WebUI, LobeChat, LibreChat, NextChat, ChatBox, and hundreds more — can connect to sinria-agent and use it as a backend.
...
Add to `~/.sinria/.env` (`~/.hermes/.env` remains supported for compatibility):

```bash
API_SERVER_ENABLED=true
API_SERVER_KEY=change-me-local-dev
# Optional: only if a browser must call Sinria directly
...
curl http://localhost:8642/v1/chat/completions \
  ...
  -d '{"model": "sinria-agent", "messages": [{"role": "user", "content": "Hello!"}]}'
```
