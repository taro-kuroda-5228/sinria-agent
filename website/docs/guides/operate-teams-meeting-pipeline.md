...
```ini
[Unit]
Description=Sinria Teams pipeline subscription maintenance
After=network-online.target

[Service]
Type=oneshot
User=sinria
EnvironmentFile=/etc/sinria/env
ExecStart=/usr/local/bin/sinria teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/sinria-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run Sinria Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now sinria-teams-pipeline-maintain.timer
systemctl list-timers sinria-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/sinria teams-pipeline maintain-subscriptions >> /var/log/sinria/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.sinria/.env` at the top of a wrapper script that crontab calls.
