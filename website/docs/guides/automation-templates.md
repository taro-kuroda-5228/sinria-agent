...
```bash
sinria cron create "0 2 * * *" \
  "You are a project manager triaging the taro-kuroda-5228/sinria-agent GitHub repo.

1. Run: gh issue list --repo taro-kuroda-5228/sinria-agent --state open --json number,title,labels,author,createdAt --limit 30
...
```bash
sinria webhook subscribe github-pr-review \
  --events "pull_request" \
...
```bash
sinria cron create "0 9 * * 1" \
  "Scan the taro-kuroda-5228/sinria-agent repo for documentation drift.

1. Run: gh pr list --repo taro-kuroda-5228/sinria-agent --state merged --json number,title,files,mergedAt --limit 30
