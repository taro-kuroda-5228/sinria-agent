...
## Session Training Export

For organization-specific local agent optimization, export stored sessions as
ML-ready JSONL with final results, observable tool actions, outcomes, and
optional explicit reasoning fields:

```bash
sinria sessions export local-agent-training.jsonl --format training
sinria sessions export local-agent-training-with-reasoning.jsonl --format training --include-reasoning
sinria sessions export one-session.jsonl --format training --session-id 20260523_191940
```

Each line uses schema `sinria.agent_training.v1` and includes:

- `input` / `output`: first user request and final assistant answer.
- `messages`: raw stored messages for replay/debugging.
- `conversations`: compact ShareGPT-style transcript.
- `action_trace`: ordered tool calls with parsed arguments, matched tool results, and success flags.
- `reasoning_trace`: empty by default; populated only when `--include-reasoning` is set and explicit provider reasoning fields were stored.
- `outcome`: completion status, end reason, token/cost counters, and tool counts.

Reasoning policy: Sinria can export explicit stored provider reasoning fields
when the operator asks for them, but it does not reconstruct hidden model
internals. For regulated deployments, keep raw exports local, sanitize/redact
before sharing externally, and fine-tune/evaluate on the observable action trace
plus outcome fields when raw reasoning is unavailable or not appropriate.
