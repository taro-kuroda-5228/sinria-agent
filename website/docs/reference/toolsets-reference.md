...
description: "Reference for Sinria core, composite, platform, and dynamic toolsets"
---
...
- **Platform** — A complete tool configuration for a specific deployment context (e.g., the CLI default toolset used for interactive sessions)
...
```bash
sinria chat --toolsets web,file,terminal
sinria chat --toolsets debugging        # composite — expands to file + terminal + web
sinria chat --toolsets all              # everything
```
...
```bash
sinria tools                            # curses UI to enable/disable per platform
```
...
| `code_execution` | `execute_code` | Run Python scripts that call Sinria tools programmatically. |
...
| `x_search` | `x_search` | Search X (Twitter) posts and threads via xAI's built-in `x_search` Responses tool. Off by default; opt in via `sinria tools`. Schema only registered when xAI credentials (SuperGrok OAuth or `XAI_API_KEY`) are configured. |
...
## Relationship to `sinria tools`

The `sinria tools` command provides a curses-based UI for toggling individual tools on or off per platform. This operates at the tool level (finer than toolsets) and persists to `config.yaml`. Disabled tools are filtered out even if their toolset is enabled.
