...
description: "Reference for Sinria MCP configuration keys, filtering semantics, and utility-tool policy"
---
...
- [Use MCP with Sinria](/docs/guides/use-mcp-with-hermes)
...
Sinria may register these utility wrappers per MCP server:
...
Even when `resources: true` or `prompts: true`, Sinria only registers those utility tools if the MCP session actually exposes the corresponding capability.
...
If filtering removes all server-native tools and no utility tools are registered, Sinria does not create an empty MCP runtime toolset for that server.
...
Behavior:
- Sinria uses the MCP SDK's OAuth 2.1 PKCE flow (metadata discovery, dynamic client registration, token exchange, and refresh)
- On first connect, a browser window opens for authorization
- Tokens are persisted to `~/.sinria/mcp-tokens/<server>.json` and reused across sessions (`~/.hermes/...` remains a legacy compatibility alias)
