...
description: "Frequently asked questions and solutions to common issues with Sinria Agent"
---
...
### What LLM providers work with Sinria?

Sinria Agent works with any OpenAI-compatible API. Supported providers include:

- **[OpenRouter](https://openrouter.ai/)** — access hundreds of models through one API key (recommended for flexibility)
- **Nous Portal** — Nous Research's inference endpoint (upstream provider integration)
- **OpenAI** — GPT-5.4, GPT-5-codex, GPT-4.1, GPT-4o, etc.
- **Anthropic** — Claude models (direct API, OAuth via `sinria login anthropic`, OpenRouter, or any compatible proxy)
...
Set your provider with `sinria model` or by editing `~/.sinria/.env` (`~/.hermes/.env` remains a legacy compatibility alias). See the [Environment Variables](./environment-variables.md) reference for all provider keys.

### Does it work on Windows?

**Not natively.** Sinria Agent requires a Unix-like environment. On Windows, install [WSL2](https://learn.microsoft.com/en-us/windows/wsl/install) and run Sinria from inside it. The standard install command works perfectly in WSL2:

```bash
curl -fsSL https://raw.githubusercontent.com/taro-kuroda-5228/sinria-agent/main/scripts/install.sh | bash
```

### I run Sinria in WSL2. What's the best way to control my normal Windows Chrome?

Prefer an MCP bridge over `/browser connect`.

Recommended pattern:

- run Sinria inside WSL2
...
- let Sinria use the resulting MCP browser tools

This is more reliable than trying to force Sinria core browser transport to attach directly across the WSL2/Windows boundary.

See:

- [Use MCP with Sinria](../guides/use-mcp-with-hermes.md#wsl2-bridge-hermes-in-wsl-to-windows-chrome)
...
Yes — Sinria now has a tested Termux install path for Android phones.
...
API calls go **only to the LLM provider you configure** (e.g., OpenRouter, your local Ollama instance). Sinria Agent does not collect telemetry, usage data, or analytics. Your conversations, memory, and skills are stored locally in `~/.sinria/` (`~/.hermes/` remains a legacy compatibility alias).

### Can I use it offline / with local models?

Yes. Run `sinria model`, select **Custom endpoint**, and enter your server's URL:
...
Sinria persists the endpoint, provider, and base URL in `config.yaml` so it survives restarts. If your local server has exactly one model loaded, `/model custom` auto-detects it. You can also set `provider: custom` in config.yaml — it's a first-class provider, not an alias for anything else.
...
:::tip Ollama users
If you set a custom `num_ctx` in Ollama (e.g., `ollama run --num_ctx 16384`), make sure to set the matching context length in Sinria — Ollama's `/api/show` reports the model's *maximum* context, not the effective `num_ctx` you configured.
:::

:::tip Timeouts with local models
Sinria auto-detects local endpoints and relaxes streaming timeouts (read timeout raised from 120s to 1800s, stale stream detection disabled). If you still hit timeouts on very large contexts, set `HERMES_STREAM_READ_TIMEOUT=1800` in your `.env` (legacy-compatible name). See the [Local LLM guide](../guides/local-llm-on-mac.md#timeouts) for details.
:::

### How much does it cost?

Sinria Agent itself is **free and open-source** (MIT license). You pay only for the LLM API usage from your chosen provider. Local models are completely free to run.

### Can multiple people use one instance?

Yes. The [messaging gateway](../user-guide/messaging/index.md) lets multiple users interact with the same Sinria Agent instance via Telegram, Discord, Slack, WhatsApp, or Home Assistant. Access is controlled through allowlists (specific user IDs) and DM pairing (first user to message claims access).
...
Yes. Import the `AIAgent` class and use Sinria programmatically:
...
#### `sinria: command not found` after installation
...
**Cause:** Sinria requires Python 3.11 or newer.
...
**Cause:** Sinria builds a per-session environment snapshot by running `bash -l` once at startup. A bash login shell reads `/etc/profile`, `~/.bash_profile`, and `~/.profile`, but **does not source `~/.bashrc`** — so tools that install themselves there (`nvm`, `asdf`, `pyenv`, `cargo`, custom `PATH` exports) stay invisible to the snapshot. This most commonly happens when Sinria runs under systemd or in a minimal shell where nothing has pre-loaded the interactive shell profile.

**Solution:** Sinria auto-sources `~/.bashrc` by default. If that's not enough — e.g. you're a zsh user whose PATH lives in `~/.zshrc`, or you init `nvm` from a standalone file — list the extra files to source in `~/.sinria/config.yaml` (`~/.hermes/config.yaml` remains a legacy compatibility alias):
...
**Solution:** Exit your session and use `sinria model` from your terminal to add new providers:

```bash
# Exit the Sinria chat session first (Ctrl+C or /quit)
...
After adding a new provider via `sinria model`, start a new chat session — `/model` will now show all your configured providers.

:::tip Quick reference
| Want to... | Use |
|-----------|-----|
| Add a new provider | `sinria model` (from terminal) |
| Enter/change API keys | `sinria model` (from terminal) |
...
# Or set directly
sinria config set OPENROUTER_API_KEY sk-or-...xxxx
```

:::warning
Make sure the key matches the provider. An OpenAI key won't work with OpenRouter and vice versa. Check `~/.sinria/.env` (`~/.hermes/.env` legacy alias) for conflicting entries.
:::
...
# Set a valid model
sinria config set HERMES_MODEL anthropic/claude-opus-4.7

# Or specify per-session
sinria chat --model openrouter/meta-llama/llama-3.1-70b-instruct
```
...
- Using `sinria chat --provider <alternative>` to route to a different backend

#### Context length exceeded

**Cause:** The conversation has grown too long for the model's context window, or Sinria detected the wrong context length for your model.
...
sinria chat --model openrouter/google/gemini-3-flash-preview
```

If this happens on the first long conversation, Sinria may have the wrong context length for your model. Check what it detected:
...
1. **Search existing issues:** [GitHub Issues](https://github.com/taro-kuroda-5228/sinria-agent/issues)
2. **File a bug report:** Include your OS, Python version (`python3 --version`), Sinria version (`sinria --version`), and the full error message
