...
Sinria fetches curated model lists for **OpenRouter** and **Nous Portal** from a JSON manifest hosted alongside the docs site. This lets maintainers update picker lists without shipping a new `sinria-agent` release.

When the manifest is unreachable (offline, network blocked, hosting failure), Sinria silently falls back to the in-repo snapshot that ships with the CLI. The manifest never breaks the picker — worst case you see whatever list was bundled with your installed version.

## Live manifest URL

```
https://taro-kuroda-5228.github.io/sinria-agent/docs/api/model-catalog.json
```
...
- **`version`** — integer schema version. Future schemas bump this; Sinria refuses manifests with versions it doesn't understand and falls back to the hardcoded snapshot.
- **`metadata`** — free-form dict at the manifest, provider, and model level. Any keys. Sinria ignores unknown fields, so you can annotate entries (`"tier": "paid"`, `"tags": [...]`, etc.) without coordinating a schema change.
...
| When | What happens |
|---|---|
| `/model` or `sinria model` | Fetches if disk cache is stale, else uses cache |
...
Cache location: `~/.sinria/cache/model_catalog.json` (`~/.hermes/...` remains a legacy compatibility alias).
...
model_catalog:
  enabled: true
  url: https://taro-kuroda-5228.github.io/sinria-agent/docs/api/model-catalog.json
  ttl_hours: 24
  providers: {}
```
