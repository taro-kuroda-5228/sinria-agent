...
运行 `sinria model` 并选择 Nous Portal 作为提供商时，Sinria 会主动询问是否启用 Tool Gateway：
...
在 `~/.sinria/config.yaml` 中直接设置 `use_gateway`（旧版 `~/.hermes/config.yaml` 仍作为兼容路径）：
...
网关使用 Nous Portal 凭据认证（在 `sinria model` 完成后写入 `~/.sinria/auth.json`，旧版 `~/.hermes/auth.json` 仍可迁移/兼容）。
...
在 `sinria tools` 中选择非网关提供商时，`use_gateway` 会自动设为 `false`，避免配置自相矛盾。
...
若使用自建或自定义网关，可在 `~/.sinria/.env` 中用环境变量覆盖端点：
...
经网关路由的工具会停止工作，直到你 [续订](https://portal.nousresearch.com/manage-subscription) 或通过 `sinria tools` 改回直连 Key。
...
Modal（无服务器终端后端）可作为 Nous 订阅的可选附加能力，但**不会**由 Tool Gateway 安装向导一并打开——请单独通过 `sinria setup terminal` 或在 `config.yaml` 中配置。
