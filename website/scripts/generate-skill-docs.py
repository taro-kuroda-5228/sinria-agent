...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Sinria Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Sinria ships with a large built-in skill library copied into `~/.sinria/skills/` on install (`~/.hermes/skills/` remains a legacy compatibility alias). Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Sinria also syncs bundled skills on `sinria update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.sinria/skills/` tree, it is still shipped with Sinria; restore it with `sinria skills reset <name> --restore`.",
        "",
...
def build_catalog_md_optional(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Official optional skills shipped with Sinria Agent — install via sinria skills install official/<category>/<skill>"',
        "---",
        "",
        "# Optional Skills Catalog",
        "",
        "Optional skills ship with Sinria Agent under `optional-skills/` but are **not active by default**. Install them explicitly:",
        "",
        "```bash",
        "sinria skills install official/<category>/<skill>",
        "```",
        "",
        "For example:",
        "",
        "```bash",
        "sinria skills install official/blockchain/solana",
        "sinria skills install official/mlops/flash-attention",
        "```",
        "",
        ...
        "",
        "```bash",
        "sinria skills uninstall <skill-name>",
        "```",
        "",
    ]
    for category in sorted(by_cat):
