"""Centralized logging setup for Sinria Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.sinria/logs/`` (profile-aware via ``get_sinria_home()``; ``get_hermes_home()`` remains a legacy compatibility alias).
...
def setup_logging(
    ...
    *force* is ``True``.
    ...
    # --- code_defects.jsonl — structured defect telemetry (self-repair loop
    # Phase 1). Same trigger class as errors.log (ERROR+ with tracebacks) but
    # sanitized/structured so the self-improvement loop can read it. Behind
    # repair.telemetry (default on); best-effort so logging setup never breaks.
    try:
        from agent.defect_capture import DefectCaptureHandler, repair_telemetry_enabled

        if repair_telemetry_enabled() and not any(
            isinstance(existing, DefectCaptureHandler) for existing in root.handlers
        ):
            root.addHandler(DefectCaptureHandler(session_kind=mode or "unknown"))
    except Exception:
        pass
