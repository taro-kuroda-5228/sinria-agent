...
def _curator_cli_command() -> str:
    return f"{_cli_command_name()} curator"



def _should_write_sanitized_training_log() -> bool:
    """Return whether sanitized curator improvement logs should be written."""
    try:
        ...
        return bool(retention.get("retain_sanitized_training_log", True))
    except Exception:
        return True
...
def _load_config() -> Dict[str, Any]:
    """Read curator.* config from the active runtime config.yaml. Tolerates missing file."""
    try:
        from hermes_cli.config import load_config
        cfg = load_config()
    except Exception as e:
...
def _build_rename_summary(
    *,
    ...
    Format::
        ...
        full report: sinria curator status
        keep an umbrella stable: sinria curator pin document-tools
    ...
    lines.append(f"full report: {_curator_cli_command()} status")
    ...
    if consolidated:
        ...
        if umbrellas:
            example = umbrellas[0]
            lines.append(
                f"keep an umbrella stable: {_curator_cli_command()} pin {example}"
            )
    return "\n".join(lines)
...
def _write_run_report(
    *,
    ...
    """
    if not _should_write_sanitized_training_log():
        return None
