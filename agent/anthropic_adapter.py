"""Anthropic Messages API adapter for Sinria.
...
def read_claude_code_credentials() -> Optional[Dict[str, Any]]:
    ...
    Keychain is preferred when it is valid. If Keychain contains an expired
    record but the file contains a valid refreshed credential, use the file.
    This avoids a stale Keychain entry shadowing Claude Code's current
    subscription token and making Sinria report "No Anthropic credentials".
    ...
    if file_creds and is_claude_code_token_valid(file_creds):
        if kc_creds:
            logger.debug("Using valid Claude Code credentials file over expired Keychain record")
        return file_creds

    # Preserve historical behavior for refresh handling: if every discovered
    # credential is expired, return Keychain first so _resolve_claude_code_token_
    # from_credentials() can attempt to refresh it before falling back to file.
    if kc_creds:
        return kc_creds
    return file_creds
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    # 1. Sinria-managed OAuth/setup token env var
...
# Anthropic moved the PKCE token endpoint from console.anthropic.com to
# platform.claude.com.  Keep the old URL as a fallback for older/self-hosted
# compatibility, but always try the current Claude endpoint first.
_OAUTH_TOKEN_URL = "https://platform.claude.com/v1/oauth/token"
_OAUTH_TOKEN_URL_FALLBACKS = (
    _OAUTH_TOKEN_URL,
    "https://console.anthropic.com/v1/oauth/token",
)
...
def run_hermes_oauth_login_pure() -> Optional[Dict[str, Any]]:
    ...
    print()
    print("Authorize Sinria with your Claude Pro/Max subscription.")
    print()
    ...
    try:
        ...
        last_error = None
        result = None
        for token_url in _OAUTH_TOKEN_URL_FALLBACKS:
            req = urllib.request.Request(
                token_url,
                data=exchange_data,
                headers={
            ...
            try:
                ...
                last_error = exc
                logger.debug("Anthropic OAuth token exchange failed at %s: %s", token_url, exc)
                continue
        if result is None:
            raise last_error or RuntimeError("Anthropic OAuth token exchange failed")
    except Exception as e:
...
def read_hermes_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read Sinria-managed OAuth credentials from ~/.hermes/.anthropic_oauth.json."""
    ...
    *base_url* is a Kimi / Moonshot host), unsigned thinking blocks
    ...
    for m in messages:
        ...
        if role == "assistant":
            ...
            # can validate the message history.  See sinria-agent#13848.
            #
    ...
    # are preserved.  See sinria-agent#13848 (Kimi) and #16748 (DeepSeek).
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("Sinria", "Claude Code")
                text = text.replace("Hermes agent", "Claude Code")
                text = text.replace("sinria-agent", "claude-code")
