...
def _runtime_cli_name() -> str:
    name = os.getenv("HERMES_CLI_NAME", "").strip().lower()
    return "sinria" if name == "sinria" else "hermes"


def _runtime_command(command: str) -> str:
    return f"{_runtime_cli_name()} {command}"
...
def init_agent(
                ...
                if _explicit and _explicit not in {"auto", "openrouter", "custom"}:
                    ...
                    if not _fb_resolved:
                        raise RuntimeError(
                            ...
                            f"variable, or switch to a different provider with `{_runtime_command('model')}`."
                        )
                if not getattr(agent, "_fallback_activated", False):
                    # No provider configured — reject with a clear message.
                    raise RuntimeError(
                        f"No LLM provider configured. Run `{_runtime_command('model')}` to "
                        f"select a provider, or run `{_runtime_command('setup')}` for first-time "
                        "configuration."
                    )
    ...
    # Todo list for task planning (one per agent/session). With a session
    # id, state is also persisted under SINRIA_HOME so in-flight task
    # state survives process death (architecture-centric P1).
    from tools.todo_tool import TodoStore
    _todo_storage_path = None
    if getattr(agent, "session_id", None):
        try:
            from hermes_constants import get_sinria_home
            _safe_session = re.sub(r"[^A-Za-z0-9._-]", "_", str(agent.session_id))[:120]
            _todo_storage_path = get_sinria_home() / "todos" / f"{_safe_session}.json"
        except Exception:
            _todo_storage_path = None
    agent._todo_store = TodoStore(storage_path=_todo_storage_path)
    ...
    # Context-window gate + capability profile (architecture-centric):
    # models below the absolute floor cannot run; models below the
    # recommended minimum run in small-context mode with tier-scaled
    # iteration caps and prompt budgets unless model.small_context_mode
    # is "strict" (which preserves the old hard reject).
    ...
    _small_context_mode = "auto"
    if isinstance(_model_cfg, dict):
        _small_context_mode = str(_model_cfg.get("small_context_mode") or "auto")
    agent.small_context_mode = _small_context_mode
    # Text-mode tool calling for models without native function calling
    # (model.text_tool_calls in config.yaml, default off). Read here so the
    # flag is fixed for the session before the system prompt is built.
    agent.text_tool_calls_enabled = bool(
        _model_cfg.get("text_tool_calls", False) if isinstance(_model_cfg, dict) else False
    )
    # Verify-after-act (architecture-centric P1): one bounded verification
    # nudge before accepting an unverified completion claim. Defaults on —
    # it implements the repository's practical-completion invariant
    # (agent.verify_after_act: false to disable).
    _behavior_cfg = _agent_cfg.get("agent", {}) if isinstance(_agent_cfg, dict) else {}
    agent.verify_after_act_enabled = bool(
        (_behavior_cfg or {}).get("verify_after_act", True)
    )
    # Escalation-model recommendation target for routing signals
    # (model.escalation_model, default unset). P1 records recommendations
    # only; automatic mid-turn switching is a later wave.
    agent.escalation_model = (
        str(_model_cfg.get("escalation_model") or "").strip() or None
        if isinstance(_model_cfg, dict) else None
    )
    # Hybrid strategist routing (docs/plans/2026-07-07-hybrid-strategist-
    # routing.md): model.strategist_model side-calls a planner/corrector
    # while the loop stays on the executor model. Default off.
    from agent.strategist import configure_strategist
    configure_strategist(agent, _model_cfg, _behavior_cfg)
    # Distillation intake (architecture-centric P2): capture verified
    # turns as sanitized local SFT raw material. Body-content training is
    # opt-in per the data-use policy — default off
    # (training.capture_verified_trajectories: true to opt in).
    _training_cfg = _agent_cfg.get("training", {}) if isinstance(_agent_cfg, dict) else {}
    agent.training_capture_enabled = bool(
        (_training_cfg or {}).get("capture_verified_trajectories", False)
    )
    _ctx_warning = validate_context_length(
        _ctx,
        small_context_mode=_small_context_mode,
        model=agent.model,
    ...
    if (
        iteration_budget is None
        and max_iterations == 90
        and agent.capability_profile.max_iterations_cap < agent.max_iterations
    ):
        agent.max_iterations = agent.capability_profile.max_iterations_cap
