...
def effective_gap_key(record: PracticalOutcomeRecord) -> str:
    """Grouping key for convergence measurement.

    Records carrying a failure signature form their own per-failure-mode class
    (``<gap_summary>|<signature>``): the composed key no longer ends with a
    progress-noise cause, so signature classes are measurable instead of being
    discarded as noise. Signature-less legacy records keep the plain
    ``gap_summary`` and the original noise exclusion.
    """
    signature = getattr(record, "failure_signature", "") or ""
    return f"{record.gap_summary}|{signature}" if signature else record.gap_summary
...
def _approved_correction_available(
    *,
    evidence_id: str,
    durable_evidence_by_id: dict[str, Any],
) -> bool:
    """Verify an approved correction exists in the durable local store.

    Availability is an observability signal only. It does not gate execution,
    alter approvals, or change completion status.
    """
    approved = durable_evidence_by_id.get(evidence_id)
    return bool(approved is not None and approved.human_approved and approved.summary)
...
def compute_loop_status(
    *,
    ...
    for record in gap_records:
        by_summary.setdefault(effective_gap_key(record), []).append(record)
    ...
    for summary, group in sorted(by_summary.items()):
        ...
        if fix_approved:
            ...
            injected = any(
                _approved_correction_available(
            ...
            if not injected:
                effect_gap = "approved_correction_not_available"
            elif recurrence_after_fix > 0:
                effect_gap = "approved_correction_available_but_behavior_recurred"
