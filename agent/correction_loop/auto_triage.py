"""Auto-triage for the Correction Loop review queue.

Implements the review-backlog convergence design
(`docs/plans/2026-07-03-correction-loop-backlog-auto-triage.md`):
...
from .review_queue import dedup_key, load_review_candidates, mutate_review_candidates, review_queue_path
...
_ALLOWED_APPLIES_TO = {"self_improvement", "practical_completion", "correction_loop", "outcome_loop"}
_DENY_MARKERS = (
...
# Public alias: the repair loop (agent/repair/risk.py) reuses the same
# deny-marker vocabulary for defect risk classification (design §5 gate 1).
DENY_MARKERS = _DENY_MARKERS
...
def run_auto_triage(
    *,
    ...
    """Compact and optionally approve candidates in one queue transaction."""
    target = queue_path or review_queue_path()

    def triage(
        rows: list[EvidenceCandidate],
    ) -> tuple[list[EvidenceCandidate], tuple[dict[str, Any], list[EvidenceCandidate]]]:
        ...
        distinct_pending_classes = len(
            {dedup_key(row) for row in compacted if row.approval_state == "proposed"}
        )
        report = {
            "dry_run": not apply,
        ...
        return compacted, (report, promoted)

    if apply:
        def persist_promoted(result: tuple[dict[str, Any], list[EvidenceCandidate]]) -> None:
            _report, promoted = result
            if promoted:
                append_evidence_jsonl([row.evidence for row in promoted], path=evidence_path)

        report, _promoted = mutate_review_candidates(
            triage,
            path=target,
            before_commit=persist_promoted,
        )
        return report

    _, (report, _) = triage(load_review_candidates(path=target))
    return report
