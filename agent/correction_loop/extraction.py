"""Session-to-evidence extraction for Sinria Correction Loop v2.
...
_RULES = [
    ...
    ExtractionRule(
        keywords=("過去の記録", "意図を推論", "1回1回", "コンテキスト", "Correction Loop"),
        applies_to=("correction_loop", "self_improvement"),
        summary="Correction Loop must retrieve prior corrections and infer intent from durable records before action, without forcing repeated user restatement.",
...
def discover_session_evidence_candidates(db, *, limit_per_query: int = 20, since: str | None = None, now: datetime | None = None) -> list[EvidenceCandidate]:
    queries = [
        ...
        "過去の記録 OR 意図を推論 OR Correction Loop",
