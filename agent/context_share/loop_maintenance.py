...
resolver-visible evidence unless a human ran `scripts/sinria_context_share_loop.py`
...
MAINTENANCE_MARKER_RELATIVE_PATH = Path("context_share") / "last_promotion.json"
...
def correction_autopromote_enabled(config: dict | None) -> bool:
    ...
    Reads ``context_share.auto_promote_recurring_corrections`` from a loaded
    ...
    if not isinstance(config, dict):
        return False
    section = config.get("context_share")
