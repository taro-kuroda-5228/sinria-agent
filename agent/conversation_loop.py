...
def _build_api_retry_exhausted_final_response(
    *,
    ...
    This response crosses the gateway boundary, so it must explain the failure
    and recovery options instead of leaving callers with a terse error that
    looks like a crash/shutdown.
    """
    provider_label = provider or "unknown provider"
    model_label = model or "unknown model"
    error_text = summary or "unknown error"
    lower_error = error_text.lower()
    is_sinria_model_provider_guard = (
        "sinria external egress guard" in lower_error
        and "model_provider" in lower_error
    )
    ...
    if is_sinria_model_provider_guard:
        _guard_detail = ""
        _paren = re.search(r"\(([^)]{1,240})\)", error_text)
        if _paren:
            _guard_detail = _paren.group(1)
        elif ":" in error_text:
            _guard_detail = error_text.split(":", 1)[1].strip()
        lines.extend([
            "- Sinria blocked the outbound request to a model_provider before it was sent.",
            "- This usually means the external-egress guard judged the payload or policy as requiring approval/redaction for a model provider.",
            f"- Sanitized guard detail/category: {_guard_detail or 'model_provider egress required approval/redaction'}.",
            f"- Target was {provider_label} / {model_label}.",
            "",
            "Risk assessment:",
            "- The request was not sent to the external model provider, so the suspected sensitive payload stayed inside Sinria's boundary.",
        ...
        lines.extend([
            "- The provider stream connection kept dropping before a complete response arrived.",
            "- This often happens with large generated tool-call payloads or unstable provider/proxy connections.",
            f"- Target was {provider_label} / {model_label}.",
        ])
    else:
        lines.extend([
            "- The model/provider call repeatedly failed before returning a usable assistant response.",
            f"- Target was {provider_label} / {model_label}.",
        ])
    ...
    if is_sinria_model_provider_guard:
        lines.extend([
            ...
            "- Approve/use a trusted cloud model profile for this Sinria dogfooding request if the content is safe to send.",
            "- Switch to a local/internal model endpoint for confidential content.",
            "- Redact secrets, credentials, PHI, patient identifiers, or other confidential spans, then retry.",
            "- Ask me to continue with local-only inspection/planning that does not require external model_provider egress.",
        ...
        lines.extend([
            "- Retry with smaller outputs or ask me to split large file writes into smaller chunks.",
            "- For large local file creation, ask me to use execute_code/Python file writes instead of one huge model tool call.",
            "- Switch providers/models if the stream keeps dropping.",
        ])
    else:
        lines.extend([
            "- Retry after checking provider status/connectivity, or switch to another configured provider/model.",
            "- If this was caused by policy/egress/auth, adjust the request or route it through an approved/local path.",
            "- Ask me to continue with local diagnostics or a fallback plan that does not require the failed API call.",
        ])
    return "\n".join(lines)
...
def run_conversation(
    ...
    # (docs/plans/2026-07-07-hybrid-strategist-routing.md).
    agent._strategist_calls_used = 0
    try:
        from agent.strategist import maybe_request_plan
        _strategist_plan = maybe_request_plan(agent, original_user_message)
    except Exception as exc:
        logger.warning("Strategist plan side-call failed: %s", exc)
        _strategist_plan = None
    if _strategist_plan:
        agent._emit_status("🧭 Strategist plan injected — executor model unchanged")
        messages.append({
            "role": "user",
            "content": (
                "[System: A strategist model produced this plan for the task "
                "above. Follow it unless tool evidence contradicts it.]\n"
                + _strategist_plan
            ),
            "_strategist_plan": True,
        })
    ...
    final_response = None
    interrupted = False
    # Verify-after-act (architecture-centric P1): at most one verification
    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        while retry_count < max_retries:
            ...
            try:
                ...
                if (
                    ...
                    from hermes_constants import display_hermes_home as _dhh_fn
                    _dhh = _dhh_fn()
                    print(f"{agent.log_prefix}     • Check ANTHROPIC_TOKEN in {_dhh}/.env for Sinria-managed OAuth/setup tokens")
                ...
                if (
                    ...
                    agent._persist_session(messages, conversation_history)
                    _final_response = _build_api_retry_exhausted_final_response(
                        max_retries=max_retries,
        ...
        try:
            ...
            # Text-mode tool-call fallback (architecture-centric): models
            # without native function calling emit ```tool_call``` blocks.
            # Parse them into normalized ToolCalls here — before the
            # post_api_request hook and the tool-dispatch branch — so the
            # rest of the loop stays provider-agnostic. Additive: only runs
            # when the response carries no native tool calls.
            if (
                getattr(agent, "text_tool_calls_enabled", False)
                ...
                _text_calls, _text_cleaned = parse_text_tool_calls(assistant_message.content)
                if _text_calls:
                    assistant_message.tool_calls = _text_calls
                    assistant_message.content = _text_cleaned
            ...
            if assistant_message.tool_calls:
                ...
                # the answer is accepted.  The post-loop practical-completion
                # guard stays as the last-resort warning layer for whatever
                # still comes out unverified.
                if (
                    getattr(agent, "verify_after_act_enabled", False)
                    and not getattr(agent, "_verify_after_act_retried", False)
                    and final_response
                    and not agent._interrupt_requested
                ):
                    _seam_tool_turns = sum(
                        1 for m in messages
                    ...
                    try:
                        ...
                        _needs_verification = _seam_tool_turns > 0 and should_nudge_verification(
                            ...
                            tool_turn_count=_seam_tool_turns,
                        )
                    except Exception as exc:
                        logger.warning("verify-after-act classification failed: %s", exc)
                        _needs_verification = False
                    if _needs_verification:
                        agent._verify_after_act_retried = True
                        logger.info(
                            "Verify-after-act: completion claimed without visible "
                            "verification — nudging model to verify before accepting"
                        )
                        ...
                        try:
                            ...
                            record_verify_nudge_event(
                                ...
                                provider=agent.provider,
                                tier=getattr(
                                    getattr(agent, "capability_profile", None), "tier", "large"
                                ),
                            )
                        except Exception as exc:
                            logger.warning("verify-nudge telemetry failed: %s", exc)
                        agent._emit_status(
                            "🔎 Completion claimed without visible verification — "
                            "asking the model to verify before accepting"
                        )
                        messages.append(final_msg)
                        # Hybrid strategist routing: enrich the nudge with
                        # corrective guidance from the strategist model.
                        # Never blocks — None keeps today's plain nudge.
                        _strategist_guidance = None
                        try:
                            from agent.strategist import maybe_request_correction
                            _strategist_guidance = maybe_request_correction(
                                agent,
                                user_message=original_user_message,
                                final_response=final_response,
                            ...
                            logger.warning(
                                "Strategist correction side-call failed: %s", exc
                            )
                        _nudge_content = (
                            "[System: Your answer claims the task is complete but "
                            "cites no verification. Before finalizing, actually "
                            "verify the result with the available tools (run the "
                            "relevant check or test, or read back the changed "
                            "state), then answer again citing what you verified. "
                            "If verification is impossible here, say so explicitly "
                            "instead of claiming completion.]"
                        )
                        if _strategist_guidance:
                            agent._emit_status(
                                "🧭 Strategist guidance added to verification nudge"
                            )
                            _nudge_content += (
                                "\n[Strategist guidance]\n" + _strategist_guidance
                            )
                        messages.append({
                            "role": "user",
                            "content": _nudge_content,
                            "_verify_after_act_synthetic": True,
                        })
                        agent._session_messages = messages
                        agent._save_session_log(messages)
                        continue
    ...
    # Sinria self-improvement loop: record sanitized Goal→Actual→Gap outcome
    ...
    if final_response or interrupted:
        try:
            ...
            # learning substrate for local-first routing. Metadata only.
            try:
                from agent.model_routing import append_routing_signal, build_routing_signal
                _routing_signal = build_routing_signal(
                    model=agent.model,
                    provider=agent.provider,
                    tier=getattr(getattr(agent, "capability_profile", None), "tier", "large"),
                    cause_kind=getattr(_outcome_record, "cause_kind", "none"),
                    escalation_model=getattr(agent, "escalation_model", None),
                )
                if _routing_signal:
                    append_routing_signal(_routing_signal)
                    if _routing_signal.get("escalation_model"):
                        agent._emit_status(
                            "↗️ Routing signal recorded: consider escalating to "
                            f"{_routing_signal['escalation_model']} for this task class"
                        )
            except Exception as exc:
                logger.warning("Routing-signal recording failed: %s", exc)
            # Distillation intake (architecture-centric P2): verified turns
            # become sanitized local SFT raw material. Opt-in config
            # (training.capture_verified_trajectories), fail-closed
            # sanitization, never on cron turns, best-effort.
            try:
                if (
                    getattr(agent, "training_capture_enabled", False)
                    and getattr(_outcome_record, "actual_kind", "") == "verified_practical_completion"
                    and (getattr(agent, "platform", None) or "") != "cron"
                ):
                    from agent.training_capture import capture_verified_trajectory
                    capture_verified_trajectory(
                        messages=messages,
                ...
                logger.warning("Verified-trajectory capture failed: %s", exc)
        except Exception as exc:
            logger.warning("Sinria outcome-gap self-improvement loop failed: %s", exc)
