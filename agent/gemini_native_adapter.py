...
def _runtime_cli_name() -> str:
    return "sinria" if os.environ.get("HERMES_CLI_NAME", "").strip().lower() == "sinria" else "hermes"


def _runtime_env_hint() -> str:
    return "~/.sinria/.env" if _runtime_cli_name() == "sinria" else "~/.hermes/.env"
...
class GeminiNativeClient:
        ...
        **_: Any,
        ...
        if not (api_key or "").strip():
            ...
            raise RuntimeError(
                ...
                f"Set GOOGLE_API_KEY or GEMINI_API_KEY in your environment / {_runtime_env_hint()} "
                "(get one at https://aistudio.google.com/app/apikey), or run "
                f"`{cli_name} setup` to configure the Google provider."
            )
    ...
    def _headers(self) -> Dict[str, str]:
        headers = {
            ...
            "User-Agent": "sinria-agent (gemini-native)",
        }
