...
upstream prefix cache warm.  See ``sinria-agent-dev``'s
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None, current_user_message: Optional[str] = None) -> Dict[str, str]:
    ...
    # Pointer to the sinria-agent skill + docs for user questions about Hermes itself.
    stable_parts.append(HERMES_AGENT_HELP_GUIDANCE)

    # Capability contract (architecture-centric): small-tier models get
    # operational guidance here and per-block char budgets in the volatile
    # tier below.  The profile is fixed at agent init, so everything
    # derived from it is deterministic for the session — the prompt-cache
    # invariant holds.  Absent profile (tests, legacy paths) == unlimited.
    from agent.model_capabilities import (
        SMALL_CONTEXT_OPERATIONS_GUIDANCE,
        apply_char_budget,
    )
    _capability_profile = getattr(agent, "capability_profile", None)
    if _capability_profile is not None and getattr(_capability_profile, "tier", "large") == "small":
        stable_parts.append(SMALL_CONTEXT_OPERATIONS_GUIDANCE)

    def _budgeted(block: str, budget_attr: str, label: str) -> str:
        if not block or _capability_profile is None:
            return block
        return apply_char_budget(block, getattr(_capability_profile, budget_attr, 0) or 0, label)
    ...
    # Text-mode tool-calling contract for models without native function
    # calling (model.text_tool_calls). Fixed at agent init → cache-safe.
    if agent.valid_tool_names and getattr(agent, "text_tool_calls_enabled", False):
        from agent.text_tool_calls import TEXT_TOOL_CALL_GUIDANCE
        stable_parts.append(TEXT_TOOL_CALL_GUIDANCE)
    ...
    if agent._memory_store:
        if agent._memory_enabled:
            mem_block = _budgeted(
                ...
                "memory_char_budget", "memory snapshot",
            )
            if mem_block:
                volatile_parts.append(mem_block)
        # USER.md is always included when enabled.
        if agent._user_profile_enabled:
            user_block = _budgeted(
                agent._memory_store.format_for_system_prompt("user"),
                "user_profile_char_budget", "user profile",
            )
            if user_block:
...
def build_system_prompt(agent: Any, system_message: Optional[str] = None, current_user_message: Optional[str] = None) -> str:
    ...
    warm across turns.
    """
    parts = build_system_prompt_parts(agent, system_message=system_message, current_user_message=current_user_message)
