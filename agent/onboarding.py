...
def _cli_command_name() -> str:
    return (os.getenv("HERMES_CLI_NAME") or "hermes").strip().lower() or "hermes"
...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time the CLI starts and finds ``~/.openclaw/``.

    Points users at ``<cli> claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``<cli> claw cleanup`` is mentioned as the
    ...
    return (
        ...
        f"To port your config, memory, and skills over to {product_name}, run "
        f"`{cli_name} claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        f"run `{cli_name} claw cleanup` (renames it to ~/.openclaw.pre-migration — "
