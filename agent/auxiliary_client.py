...
Resolution order for text tasks (auto mode):
  ...
  3. Nous Portal (runtime-home auth.json active provider)
...
_OR_HEADERS_BASE = {
    "HTTP-Referer": "https://github.com/taro-kuroda-5228/sinria-agent",
    "X-Title": "Sinria",
...
_NVIDIA_NIM_CLOUD_HEADERS = {
    "X-BILLING-INVOKE-ORIGIN": "SinriaAgent",
}
...
_AI_GATEWAY_HEADERS = {
    "HTTP-Referer": "https://github.com/taro-kuroda-5228/sinria-agent",
    "X-Title": "Sinria",
    "User-Agent": f"SinriaAgent/{_HERMES_VERSION}",
}
...
def _codex_cloudflare_headers(access_token: str) -> Dict[str, str]:
    ...
    """
    headers = {
        "User-Agent": "codex_cli_rs/0.0.0 (Sinria)",
    ...
    def create(self, **kwargs) -> Any:
        ...
        def _check_cancelled() -> None:
            if deadline is not None and time.monotonic() >= deadline:
                # The synchronous deadline can fire before the watchdog timer
                # thread does. Run the same close + cache-eviction cleanup here
                # so a dead client is never left cached regardless of which
                # path detects the timeout first (#23432). Both close() and
                # the eviction are idempotent, so a subsequent timer firing is
                # harmless.
...
def _validate_base_url(base_url: str) -> None:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            f"Run `{_cli_cmd('setup')}` or `{_cli_cmd('model')}` and enter a valid http(s) base URL."
        ) from exc
    ...
    if not _stale_base_url_warned:
        ...
        if (_env_base and _cfg_provider
            ...
            logger.warning(
                ...
                "Run: %s to reconfigure, or remove OPENAI_BASE_URL from the active runtime .env",
                _env_base, _cfg_provider, _cli_cmd("model"),
            )
    ...
    if provider == "nous":
        ...
        if client is None:
            logger.warning(
                ...
                f"but Nous Portal not configured (run: {_cli_cmd('auth')})"
            )
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        return (_to_async_client(client, final_model, is_vision=is_vision) if async_mode
        ...
        if raw_codex:
            ...
            if not codex_token:
                logger.warning("resolve_provider_client: openai-codex requested "
                               "but no Codex OAuth token found (run: %s)", _cli_cmd("model"))
                return None, None
    ...
    if provider == "xai-oauth":
        ...
        if client is None:
            logger.warning(
                ...
                "OAuth token found (run: %s -> xAI Grok OAuth — SuperGrok Subscription)",
                _cli_cmd("model"),
            )
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        ...
        if client is None:
            ...
            if _explicit and _explicit not in {"auto", "openrouter", "custom"}:
                raise RuntimeError(
                    ...
                    f"variable, or switch to a different provider with `{_cli_cmd('model')}`."
                )
