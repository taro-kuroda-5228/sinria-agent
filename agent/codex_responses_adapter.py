    ...
    for item in output:
        ...
        if item_type == "message":
            ...
            if isinstance(encrypted, str) and encrypted:
                ...
                # gpt-5.5 (and other Codex models on degraded paths) sometimes emit
                # ``summary=None`` instead of ``summary=[]``.  Default to an empty
                # list so the replayed reasoning item always carries the required
                # ``summary`` key on the next turn (otherwise the Responses API
                # rejects the replay).
                summary = getattr(item, "summary", None)
