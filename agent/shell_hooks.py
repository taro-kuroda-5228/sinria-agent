...
* First-use consent is gated by the allowlist under
  ``~/.sinria/shell-hooks-allowlist.json``.  Non-TTY callers must pass
  ``accept_hooks=True`` (resolved from ``--accept-hooks``,
  ``SINRIA_ACCEPT_HOOKS`` (legacy ``HERMES_ACCEPT_HOOKS`` is also honored),
  or ``hooks_auto_accept: true`` in config)
...
def register_from_config(
    ...
    *,
    ...
    setting.  ``SINRIA_ACCEPT_HOOKS=1`` (plus legacy ``HERMES_ACCEPT_HOOKS=1``)
    and ``hooks_auto_accept: true`` are
        ...
        if not already_allowlisted:
            if not _prompt_and_record(
                ...
                logger.warning(
                    ...
                    "Use --accept-hooks / SINRIA_ACCEPT_HOOKS=1 / "
...
def save_allowlist(data: Dict[str, Any]) -> None:
    ...
    try:
        ...
        logger.warning(
            ...
            "runs without --accept-hooks / SINRIA_ACCEPT_HOOKS).",
            p, exc,
        )
...
def _resolve_effective_accept(
    ...
    Precedence (any truthy source flips us on):
      ...
      2. ``SINRIA_ACCEPT_HOOKS`` env var (legacy ``HERMES_ACCEPT_HOOKS`` also honored)
      3. ``hooks_auto_accept: true`` in ``cli-config.yaml``
    """
    if accept_hooks_arg:
        return True
    env = (os.environ.get("SINRIA_ACCEPT_HOOKS") or os.environ.get("HERMES_ACCEPT_HOOKS") or "").strip().lower()
