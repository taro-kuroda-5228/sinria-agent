# nix/nixosModules.nix — NixOS module for sinria-agent
#
# Two modes:
...
{ inputs, ... }: {
  ...
  let
    cfg = config.services.sinria-agent;
    effectivePackage =
    ...
    sinria-agent = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

    # Deep-merge config type (from 0xrsydn/nix-sinria-agent)
  ...
  in {
    options.services.sinria-agent = with lib; {
      enable = mkEnableOption "Sinria gateway service";

      # ── Package ──────────────────────────────────────────────────────────
      package = mkOption {
        type = types.package;
        default = sinria-agent;
        description = "The sinria-agent package to use.";
      };
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in sinria-agent's
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.sinria-agent.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Assertions ─────────────────────────────────────────────────────
      {
        ...
        in [{
          ...
          message = "services.sinria-agent.extraPlugins: duplicate plugin names detected: ${toString names}. If using fetchFromGitHub, set name = \"plugin-name\" to disambiguate.";
        }];
      }
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            services.sinria-agent: container.enable is true and container.hostUsers
      ...
      {
        system.activationScripts."sinria-agent-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
            ...
            ${lib.concatStringsSep "\n" (map (user:
              ...
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.hermes" ]; then
                  rm -f "${symlinkPath}"
                  echo "sinria-agent: removed symlink ${symlinkPath}"
                fi
            ...
            (lib.concatStringsSep "\n" (map (user:
              ...
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  ...
                  echo "sinria-agent: backing up existing ${symlinkPath} to $_backup"
    ...
    HERMES_NIX_ENV_EOF
      ...
      (lib.mkIf cfg.container.enable {
        ...
        systemd.services.sinria-agent = {
          description = "Sinria Gateway (container)";
          wantedBy = [ "multi-user.target" ];
          after = [ "network-online.target" ]
