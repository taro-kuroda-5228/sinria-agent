...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      sinria-agent = self'.packages.default;
      hermesVenv = sinria-agent.hermesVenv;

      configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

      # Auto-generated config key reference — always in sync with Python
      configKeys = pkgs.runCommand "sinria-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "sinria-cross-eval" { } (
          if failures != [] then
        ...
        package-contents = pkgs.runCommand "sinria-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${sinria-agent}/bin/sinria || (echo "FAIL: sinria binary missing"; exit 1)
          test -x ${sinria-agent}/bin/sinria-agent || (echo "FAIL: sinria-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${sinria-agent}/bin/sinria version 2>&1 | grep -qi "sinria" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "sinria-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in sinria sinria-agent sinria-acp; do
            test -x ${sinria-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "sinria-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking sinria --help ==="
          ${sinria-agent}/bin/sinria --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${sinria-agent}/bin/sinria --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "sinria-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${sinria-agent}/share/sinria-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          SKILL_COUNT=$(find ${sinria-agent}/share/sinria-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "HERMES_BUNDLED_SKILLS" ${sinria-agent}/bin/sinria || \
        ...
        bundled-plugins = pkgs.runCommand "sinria-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${sinria-agent}/share/sinria-agent/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${sinria-agent}/share/sinria-agent/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "HERMES_BUNDLED_PLUGINS" ${sinria-agent}/bin/sinria || \
        ...
        bundled-tui = pkgs.runCommand "sinria-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${sinria-agent}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${sinria-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "HERMES_TUI_DIR" ${sinria-agent}/bin/sinria || \
        ...
        hermes-node = pkgs.runCommand "sinria-node-version" { } ''
          set -e
          echo "=== Checking HERMES_NODE in wrapper ==="
          grep -q "HERMES_NODE" ${sinria-agent}/bin/sinria || \
            (echo "FAIL: HERMES_NODE not set in wrapper"; exit 1)
          echo "PASS: HERMES_NODE present in wrapper"

          HERMES_NODE=$(sed -n "s/^export HERMES_NODE='\(.*\)'/\1/p" ${sinria-agent}/bin/sinria)
        ...
        managed-guard = pkgs.runCommand "sinria-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked "config set" ${sinria-agent}/bin/sinria config set model foo
          check_blocked "config edit" ${sinria-agent}/bin/sinria config edit
        ...
        extra-python-packages = let
          ...
          hermesWithExtra = sinria-agent.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "sinria-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${hermesWithExtra}/bin/sinria || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${hermesWithExtra}/bin/sinria || \
          ...
          if grep -q "PYTHONPATH" ${sinria-agent}/bin/sinria; then
        ...
        extra-dependency-groups = let
          hermesWithGroups = sinria-agent.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "sinria-extra-dependency-groups" { } ''
          set -e
