# nix/sinria-agent.nix — Overridable Sinria package
#
...
stdenv.mkDerivation {
  ...
  installPhase = ''
    ...
    mkdir -p $out/share/sinria-agent $out/bin
    cp -r ${bundledSkills} $out/share/sinria-agent/skills
    cp -r ${bundledPlugins} $out/share/sinria-agent/plugins
    cp -r ${hermesWeb} $out/share/sinria-agent/web_dist
    ...
    ${lib.concatMapStringsSep "\n"
      (name: ''
        makeWrapper ${hermesVenv}/bin/${name} $out/bin/${name} \
          ...
          --set HERMES_BUNDLED_SKILLS $out/share/sinria-agent/skills \
          --set HERMES_BUNDLED_PLUGINS $out/share/sinria-agent/plugins \
          --set HERMES_WEB_DIST $out/share/sinria-agent/web_dist \
  ...
  passthru = {
    ...
    devShellHook = ''
      STAMP=".nix-stamps/sinria-agent"
      STAMP_VALUE="${pyprojectHash}:${uvLockHash}"
      if [ ! -f "$STAMP" ] || [ "$(cat "$STAMP")" != "$STAMP_VALUE" ]; then
        echo "sinria-agent: installing Python dependencies..."
