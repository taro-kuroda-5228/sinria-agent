...
AUTHOR_MAP = {
    "dancing205228@gmail.com": "taro-kuroda-5228",
    ...
    "274096618+sinria-agent-dhabibi@users.noreply.github.com": "dhabibi",
...
def _update_acp_registry_versions(semver: str) -> None:
    ...
    if ACP_REGISTRY_MANIFEST.exists():
        ...
        if "package" in uvx:
            uvx["package"] = f"sinria-agent[acp]=={semver}"
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/NousResearch/sinria-agent",
    ...
    lines.append(f"# Sinria v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for Sinria. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="Sinria Release Tool")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"Sinria v{new_version} ({calver_date})\n\nWeekly release"
        )
