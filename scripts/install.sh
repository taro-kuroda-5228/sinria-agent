...
# Configuration
REPO_URL_SSH="git@github.com:taro-kuroda-5228/sinria-agent.git"
REPO_URL_HTTPS="https://github.com/taro-kuroda-5228/sinria-agent.git"
SINRIA_HOME_DEFAULT="$HOME/.sinria"
HERMES_HOME="${SINRIA_HOME:-${HERMES_HOME:-$SINRIA_HOME_DEFAULT}}"
...
#   code at /usr/local/lib/sinria-agent, command at /usr/local/bin/sinria,
#   data still at /root/.sinria (HERMES_HOME compatibility alias).
#   Matches Claude Code / Codex CLI and keeps Docker bind-mounted /root/ volumes lean.
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   default (non-root):  ~/.sinria/sinria-agent"
            echo "                   default (root, Linux): /usr/local/lib/sinria-agent"
            echo "  --hermes-home PATH  Data directory (default: ~/.sinria, or \$SINRIA_HOME / \$HERMES_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, Sinria installs the code under"
            echo "  /usr/local/lib/sinria-agent and links the command into"
            echo "  /usr/local/bin/sinria (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$HERMES_HOME"
            echo "  (default /root/.sinria via compatibility alias).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$HERMES_HOME/sinria-agent are preserved in-place."
            echo "  --ensure DEPS  Install only specified deps (comma-separated)"
            echo "                   Supported: node, browser, ripgrep, ffmpeg"
            echo "                   Does NOT clone repo or create venv"
            echo "  --postinstall  Run post-install setup only (for pip users)"
            echo "                   Installs optional deps + runs sinria setup"
            echo "                   Does NOT clone repo or create venv"
            exit 0
            ;;
        *)
...
print_banner() {
    ...
    echo "│             ⚕ Sinria Installer                    │"
    echo "├─────────────────────────────────────────────────────────┤"
    echo "│  Confidentiality-first agent OS for regulated teams.     │"
    echo "└─────────────────────────────────────────────────────────┘"
    echo -e "${NC}"
}
...
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $HERMES_HOME/sinria-agent
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $HERMES_HOME/sinria-agent
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/sinria-agent
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $HERMES_HOME/sinria-agent — then preserve it)
#
...
resolve_install_layout() {
    ...
    if is_termux; then
        INSTALL_DIR="$HERMES_HOME/sinria-agent"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$HERMES_HOME/sinria-agent/.git" ]; then
            INSTALL_DIR="$HERMES_HOME/sinria-agent"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/sinria-agent)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/sinria-agent"
        ROOT_FHS_LAYOUT=true
        log_info "Root install on Linux — using FHS layout"
        log_info "  Code:    $INSTALL_DIR"
        log_info "  Command: /usr/local/bin/sinria"
...
get_sinria_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/sinria" ]; then
        echo "$link_dir/sinria"
    else
        echo "sinria"
    fi
}
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  irm https://raw.githubusercontent.com/taro-kuroda-5228/sinria-agent/main/scripts/install.ps1 | iex"
            exit 1
            ;;
        *)
            OS="unknown"
...
check_node() {
    ...
    if [ -x "$HERMES_HOME/node/bin/node" ]; then
        ...
        log_success "Node.js $found_ver found (Sinria-managed)"
        HAS_NODE=true
        return 0
    fi
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Sinria itself does not require or retain root access."
...
setup_path() {
    log_info "Setting up sinria command..."

    if [ "$USE_VENV" = true ]; then
        HERMES_BIN="$INSTALL_DIR/venv/bin/sinria"
    else
        HERMES_BIN="$(which sinria 2>/dev/null || echo "")"
        if [ -z "$HERMES_BIN" ]; then
            log_warn "sinria not found on PATH after install"
            return 0
        fi
    fi
    ...
    if [ ! -x "$HERMES_BIN" ]; then
        log_warn "sinria entry point not found at $HERMES_BIN"
    ...
    # Create a user-facing shim for the sinria command.
    ...
    rm -f "$command_link_dir/sinria"
    cat > "$command_link_dir/sinria" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$HERMES_BIN" "\$@"
EOF
    chmod +x "$command_link_dir/sinria"
    log_success "Installed sinria launcher → $command_link_display_dir/sinria"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "sinria command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v sinria' \
                >/dev/null 2>&1; then
        ...
        log_info "sinria not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# Sinria — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Sinria — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# Sinria — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so sinria works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create .env at the active runtime home (top level, easy to find)
    ...
    # Create config.yaml at the active runtime home (top level, easy to find)
...
configure_browser_env_from_system_browser() {
    ...
    {
        echo ""
        echo "# Sinria browser tools — use the system Chrome/Chromium binary."
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'sinria setup' after install."
        return 0
    fi
...
maybe_start_gateway() {
    ...
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            ...
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                HERMES_CMD="$(get_sinria_command_path)"
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'sinria gateway install' later."
        return 0
    fi
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $HERMES_CMD gateway install 2>/dev/null; then
                ...
                if $HERMES_CMD gateway start 2>/dev/null; then
                    ...
                    log_warn "Service installed but failed to start. Try: sinria gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: sinria gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
...
print_success() {
    ...
    echo ""
    echo -e "   ${GREEN}sinria${NC}              Start chatting"
    echo -e "   ${GREEN}sinria setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}sinria config${NC}       View/edit configuration"
    echo -e "   ${GREEN}sinria config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}sinria gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}sinria update${NC}       Update to latest reviewed version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'sinria' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'sinria' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'sinria' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
...
postinstall_mode() {
    ...
    log_info "Post-install mode: setting up Sinria for package install"
    ...
    if [ -n "$HERMES_CMD" ]; then
        log_info "Running sinria setup..."
        "$HERMES_CMD" setup
    else
        log_warn "sinria command not found on PATH"
