...
param(
    ...
    [string]$Branch = "main",
    [Alias("HermesHome")]
    [string]$SinriaHome = "$env:LOCALAPPDATA\sinria",
    [string]$InstallDir = "$env:LOCALAPPDATA\sinria\sinria-agent",
...
$RepoUrlSsh = "git@github.com:taro-kuroda-5228/sinria-agent.git"
$RepoUrlHttps = "https://github.com/taro-kuroda-5228/sinria-agent.git"
$PythonVersion = "3.11"
$NodeVersion = "22"
...
function Write-Banner {
    ...
    Write-Host "|             * Sinria Installer                    |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host "|  An AI agent platform by Medical Horizon.              |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host ""
}
...
function Install-Git {
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\sinria\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    blocked).  Owning the Sinria copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\sinria\git``
    and re-running this installer fully recovers.

    After install we locate ``bash.exe`` and persist the path in
    ``HERMES_GIT_BASH_PATH`` (User scope; legacy compatibility name) so Sinria can find it in a fresh
    ...
    # Download PortableGit into $SinriaHome\git.  Always works as long as
    # we can reach github.com -- no admin, no winget, no reliance on the
    # user's possibly-broken system Git install.
    Write-Info "Git not found -- downloading PortableGit to $SinriaHome\git\ ..."
    ...
    try {
        ...
        $release = Invoke-RestMethod -Uri $releaseApi -UseBasicParsing -Headers @{ "User-Agent" = "sinria-installer" }

        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent Sinria features (terminal tool, agent-browser) will not work on this machine."
...
function Set-GitBashEnvVar {
    ...
    ``HERMES_GIT_BASH_PATH`` (User env scope; legacy compatibility name) so Sinria can find it even before
    ...
    #
    # Layouts:
    #   PortableGit (our default): $SinriaHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $SinriaHome\git\usr\bin\bash.exe
    $candidates += "$SinriaHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$SinriaHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
    ...
    Write-Warn "Could not locate bash.exe -- Sinria may not find Git Bash."
...
function Test-Node {
    ...
    $managedNode = "$SinriaHome\node\node.exe"
    if (Test-Path $managedNode) {
        $version = & $managedNode --version
        $env:Path = "$SinriaHome\node;$env:Path"
        Write-Success "Node.js $version found (Sinria-managed)"
        $script:HasNode = $true
        return $true
    }
    ...
    # The portable zip path drops node.exe + npm into $SinriaHome\node\
    ...
    Write-Info "Downloading portable Node.js $NodeVersion to $SinriaHome\node\ ..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\sinria-node-extract"
            ...
            if ($extractedDir) {
                if (Test-Path "$SinriaHome\node") { Remove-Item -Recurse -Force "$SinriaHome\node" }
                Move-Item $extractedDir.FullName "$SinriaHome\node"
                ...
                $version = & "$SinriaHome\node\node.exe" --version
                Write-Success "Node.js $version installed to $SinriaHome\node\ (portable, user-scoped)"
                $script:HasNode = $true
...
function Install-Repository {
    ...
    if (Test-Path $InstallDir) {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                Write-Info "terminals, running sinria processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                $zipUrl = "https://github.com/taro-kuroda-5228/sinria-agent/archive/refs/heads/$Branch.zip"
                $zipPath = "$env:TEMP\sinria-agent-$Branch.zip"
                $extractPath = "$env:TEMP\sinria-agent-extract"
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `sinria.exe` exists
...
try:
    ...
    out = []
    for s in specs:
        m = re.search(r'sinria-agent\[([\w-]+)\]', s)
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install sinria-agent package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # isn't set, leaving venv\ empty and sinria.exe broken with
    ...
    # here, not 30 seconds later when the user runs `sinria`.
    if (-not $NoVenv) {
    ...
    # users hit and lazy-import errors from `sinria dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `sinria dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `sinria dashboard` should now work."
            } else {
...
function Set-PathVariable {
    Write-Info "Setting up sinria command..."
    
    if ($NoVenv) {
        $sinriaBin = "$InstallDir"
    } else {
        $sinriaBin = "$InstallDir\venv\Scripts"
    }
    
    # Add the venv Scripts dir to user PATH so sinria is globally available.
    # On Windows, the sinria.exe in venv\Scripts\ has the venv Python baked in.
    $currentPath = [Environment]::GetEnvironmentVariable("Path", "User")
    
    if ($currentPath -notlike "*$sinriaBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$sinriaBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $sinriaBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    $currentSinriaHome = [Environment]::GetEnvironmentVariable("SINRIA_HOME", "User")
    if (-not $currentSinriaHome -or $currentSinriaHome -ne $SinriaHome) {
        [Environment]::SetEnvironmentVariable("SINRIA_HOME", $SinriaHome, "User")
        Write-Success "Set SINRIA_HOME=$SinriaHome"
    }
    $currentHermesHome = [Environment]::GetEnvironmentVariable("HERMES_HOME", "User")
    if (-not $currentHermesHome -or $currentHermesHome -ne $SinriaHome) {
        [Environment]::SetEnvironmentVariable("HERMES_HOME", $SinriaHome, "User")
        Write-Info "Set legacy HERMES_HOME=$SinriaHome for compatibility"
    }
    $env:SINRIA_HOME = $SinriaHome
    $env:HERMES_HOME = $SinriaHome
    
    # Update current session
    $env:Path = "$sinriaBin;$env:Path"
    
    Write-Success "sinria command ready"
}

function Copy-ConfigTemplates {
    Write-Info "Setting up configuration files..."
    
    # Create active runtime home directory structure
    New-Item -ItemType Directory -Force -Path "$SinriaHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$SinriaHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$SinriaHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$SinriaHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$SinriaHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$SinriaHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$SinriaHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$SinriaHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$SinriaHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$SinriaHome\.env"
    ...
    # Create config.yaml
    $configPath = "$SinriaHome\config.yaml"
...
"@
    ...
    Write-Success "Configuration directory ready: $SinriaHome/"
    
    # Seed bundled skills into the active runtime home skills dir (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to $SinriaHome\skills\ ..."
    $pythonExe = "$InstallDir\venv\Scripts\python.exe"
    if (Test-Path $pythonExe) {
        try {
            & $pythonExe "$InstallDir\tools\skills_sync.py" 2>$null
            Write-Success "Skills synced to $SinriaHome\skills\"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$SinriaHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to $SinriaHome\skills\"
            }
        }
    }
}
...
function Install-NodeDeps {
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'sinria setup tools' later."
        return
    }
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\sinria-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\sinria-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\sinria-npm-tui-$(Get-Random).log"
...
function Install-PlatformSdks {
    ...
    # the active Sinria .env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'sinria setup'."
        return
    }
    ...
    # Run Sinria setup using the venv Python directly (no activation needed)
    if (-not $NoVenv) {
...
function Start-GatewayIfConfigured {
    ...
    $sinriaCmd = "$InstallDir\venv\Scripts\sinria.exe"
    if (-not (Test-Path $sinriaCmd)) {
        $sinriaCmd = "sinria"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$SinriaHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'sinria whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: sinria gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$SinriaHome\logs\gateway.log"
            Start-Process -FilePath $sinriaCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$SinriaHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: sinria gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: sinria gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$SinriaHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$SinriaHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$SinriaHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$SinriaHome\sinria-agent\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   sinria              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   sinria setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   sinria config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   sinria config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   sinria gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   sinria update       " -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning Sinria repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
    ...
    @{ Name = "path";             Title = "Adding Sinria to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://raw.githubusercontent.com/taro-kuroda-5228/sinria-agent/main/scripts/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
