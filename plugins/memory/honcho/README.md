...
| 4 | `per-session` strategy | Sinria session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`sinria-agent`) |
...
If `sessionPeerPrefix` is `true`, the peer name is prepended: `eri-sinria-agent`.
...
- **`per-session`** — Sinria session ID (timestamp + hex). Every `hermes` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
