...
try:
    ...
    def get_hermes_home() -> Path:
        ...
        if val:
            return Path(val)
        home_name = ".sinria" if os.environ.get("HERMES_CLI_NAME", "").lower() == "sinria" else ".hermes"
...
def load_user_credentials(email: Optional[str] = None) -> Optional[Any]:
    ...
    try:
        ...
        logger.warning(
            ...
            "attachment delivery is disabled. Install sinria-agent[google_chat]."
        )
        return None
...
def install_deps() -> bool:
    ...
    try:
        ...
        print("  pip install 'sinria-agent[google_chat]'")
        return False
