"""disk_cleanup — ephemeral file cleanup for Sinria.
...
try:
    ...
    def get_hermes_home() -> Path:  # type: ignore[no-redef]
        ...
        cli_name = (os.environ.get("HERMES_CLI_NAME") or "").strip().lower()
        home_name = ".sinria" if cli_name == "sinria" else ".hermes"
        return (Path.home() / home_name).resolve()
...
def quick() -> Dict[str, Any]:
    ...
    _PROTECTED_TOP_LEVEL = {
        ...
        "sinria-agent", "backups", "profiles", ".worktrees",
    }
    empty_removed = 0
    try:
