...
def run_bot() -> int:  # noqa: C901 — orchestration, explicit branches
    ...
    guest_name = os.environ.get("HERMES_MEET_GUEST_NAME", "Sinria")
...
def _looks_like_human_speaker(speaker: str, bot_guest_name: str) -> bool:
    ...
    # Legacy bot names ("Hermes" / "Hermes Agent") are still suppressed as our
    # own echo during the rebrand transition: a session started under the old
    # name, or a stale caption, must not trigger barge-in against ourselves.
    if spk in {"unknown", "you", "hermes", "hermes agent", bot_guest_name.strip().lower()}:
        return False
    return True
