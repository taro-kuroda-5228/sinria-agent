...
class SessionDB:
    ...
    @staticmethod
    def _training_parse_jsonish(value: Any) -> Any:
        """Parse JSON strings when possible while preserving plain text values."""
    ...
    @classmethod
    def _training_message_content(cls, msg: Dict[str, Any]) -> Any:
        content = msg.get("content")
        return cls._training_parse_jsonish(content)

    @classmethod
    def _messages_to_sharegpt_for_training(
        cls,
        messages: List[Dict[str, Any]],
        *,
        include_reasoning: bool,
    ) -> List[Dict[str, Any]]:
        """Convert stored messages to a compact ShareGPT-style training transcript."""
        role_map = {
            "system": "system",
            "user": "human",
            "assistant": "gpt",
        ...
        for msg in messages:
            role = msg.get("role")
            mapped = role_map.get(role if isinstance(role, str) else "", role if isinstance(role, str) and role else "unknown")
            ...
            conversations.append({"from": mapped, "value": value})
        return conversations

    @classmethod
    def _extract_training_action_trace(cls, messages: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """Build a tool/action trace with calls, arguments, matched responses, and success flags."""
        ...
        def _normalize_tool_call(tool_call: Any) -> Optional[Dict[str, Any]]:
            if not isinstance(tool_call, dict):
                return None
            fn_value = tool_call.get("function")
            fn = fn_value if isinstance(fn_value, dict) else {}
            name = tool_call.get("name") or fn.get("name") or tool_call.get("tool_name")
            raw_arguments = tool_call.get("arguments") if "arguments" in tool_call else fn.get("arguments")
            arguments = cls._training_parse_jsonish(raw_arguments)
        ...
        for msg in messages:
            if msg.get("role") == "assistant":
                for tool_call in msg.get("tool_calls") or []:
                    item = _normalize_tool_call(tool_call)
                    if item is None:
                        continue
                    trace.append(item)
                    if item["tool_call_id"]:
                        pending_by_id[item["tool_call_id"]] = item
                continue
            ...
            if item is None:
                item = next((candidate for candidate in trace if candidate.get("response") is None), None)
            if item is None:
                item = {
                    "step": len(trace) + 1,
                ...
                trace.append(item)
            response = cls._training_message_content(msg)
            item["response"] = response
            explicit_failure = False
            if isinstance(response, dict):
                explicit_failure = response.get("success") is False or response.get("error") is not None
            elif isinstance(response, str):
                explicit_failure = "Traceback" in response or response.lower().startswith("error")
            item["success"] = not explicit_failure
        return trace

    @staticmethod
    def _extract_training_reasoning_trace(messages: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        trace: List[Dict[str, Any]] = []
        for idx, msg in enumerate(messages):
            ...
            for field in ("reasoning", "reasoning_content", "reasoning_details", "codex_reasoning_items"):
                ...
                if value in (None, "", [], {}):
                    continue
                try:
                    content = json.loads(value) if isinstance(value, str) and value[:1] in "[{" else value
                except (json.JSONDecodeError, TypeError):
                    content = value
                trace.append({"message_index": idx, "field": field, "content": content})
        return trace

    @classmethod
    def _session_to_training_example(
        cls,
        ...
        *,
        ...
        completed = bool(ended and end_reason not in {"error", "failed", "interrupted", "cancelled"})
        reasoning_policy = {
            "mode": "included_explicit_provider_fields" if include_reasoning else "excluded_by_default",
            "note": "Explicit provider reasoning can be exported with include_reasoning=True; hidden model internals are not reconstructed.",
        }
        return {
            "schema": "sinria.agent_training.v1",
            ...
            "input": user_messages[0].get("content") if user_messages else "",
            "output": assistant_messages[-1].get("content") if assistant_messages else "",
            "messages": messages,
            "conversations": cls._messages_to_sharegpt_for_training(
                messages,
                include_reasoning=include_reasoning,
            ),
            "action_trace": cls._extract_training_action_trace(messages),
            "reasoning_trace": cls._extract_training_reasoning_trace(messages) if include_reasoning else [],
    ...
    def export_training_examples(
        ...
        *,
        ...
        """Export sessions as ML-ready Sinria agent training examples.

        The record keeps both final input/output and the observable action trace
        (tool calls + results). Reasoning fields are excluded by default; when
        requested, only explicit stored provider reasoning fields are included —
        hidden model internals are never reconstructed.
        """
        if session_id:
            ...
            return [
                self._session_to_training_example(
                    session,
                    messages,
                    include_reasoning=include_reasoning,
                )
