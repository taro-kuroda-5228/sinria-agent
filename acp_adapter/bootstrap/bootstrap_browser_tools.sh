...
# into ~/.hermes/node/ for use by Sinria's browser tools.
#
...
while [ $# -gt 0 ]; do
    case "$1" in
        ...
        -h|--help)
            cat <<EOF
Bootstrap Sinria browser tools.
...
ensure_node() {
    ...
    if [ -x "$NODE_PREFIX/bin/node" ]; then
        ...
        log_success "Node.js $found_ver found (Sinria-managed at $NODE_PREFIX)"
        return 0
    fi
...
ensure_agent_browser() {
    ...
    # sudo. Force the prefix to the user-writable Sinria-managed Node
...
main() {
    log_info "Sinria: bootstrapping browser tools"
    ...
    log_info "Sinria will pick up agent-browser from $NODE_PREFIX/bin/ on next launch."
}
