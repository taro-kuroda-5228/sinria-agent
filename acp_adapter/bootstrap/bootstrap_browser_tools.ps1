...
# into ~/.hermes/node/ for use by Sinria's browser tools on Windows.
#
# Targets the registry-install path: users who got Hermes via
# `uvx --from 'sinria-agent[acp]==X' hermes-acp` don't have a repo clone,
...
function Ensure-Node {
    ...
    if (Test-Path $managedNode) {
        ...
        Write-Success "Node.js $v found (Sinria-managed at $NodePrefix)"
...
function Ensure-AgentBrowser {
    ...
    # prefix to the user-writable Sinria-managed Node directory so we never
...
function Write-BrowserEnv {
    ...
    Add-Content -Path $envFile -Value "# Sinria browser tools — use the system Chrome/Chromium/Edge binary."
...
Write-Info "Sinria: bootstrapping browser tools"
...
Write-Info "Sinria will pick up agent-browser from $NodePrefix on next launch."
