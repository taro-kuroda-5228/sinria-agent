...
def _should_retain_legacy_jsonl_transcript() -> bool:
    """Return whether the legacy raw JSONL transcript mirror should be kept.

    SQLite remains the primary internal session store. When the active Sinria
    policy disables local raw-history retention, we stop writing the extra
    legacy JSONL mirror to reduce duplicate sensitive-at-rest footprint while
    preserving internal session functionality.
    """
    try:
        from hermes_cli.config import load_config
        from agent.sinria_egress import resolve_sinria_retention_policy

        retention = resolve_sinria_retention_policy(load_config() or {})
        return bool(retention.get("retain_raw_history_locally", True))
    except Exception:
        return True
...
def _discord_tools_loaded() -> bool:
    ...
    Two conditions must hold:
      1. The `discord` or `discord_admin` toolset is enabled for the
         Discord platform via `sinria tools` (legacy CLI alias also works)
         (opt-in, default OFF).
...
def build_session_context_prompt(
    ...
    *,
    ...
    if context.source.platform == Platform.SLACK:
        ...
        # `discord` / `discord_admin` via `sinria tools` (legacy CLI alias
        # also works) AND the bot
    ...
    lines.append(
        f"- `\"local\"` → Save to local files only ({display_sinria_home()}/cron/output/)"
    )
...
class SessionStore:
    ...
    def append_to_transcript(self, session_id: str, message: Dict[str, Any], skip_db: bool = False) -> None:
        ...
        if _should_retain_legacy_jsonl_transcript():
    ...
    def rewrite_transcript(self, session_id: str, messages: List[Dict[str, Any]]) -> None:
        ...
        # JSONL: overwrite the file when raw-history retention remains enabled.
        if _should_retain_legacy_jsonl_transcript():
            transcript_path = self.get_transcript_path(session_id)
