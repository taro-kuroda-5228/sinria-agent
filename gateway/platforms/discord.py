...
class DiscordAdapter(BasePlatformAdapter):
    ...
    def _register_slash_commands(self) -> None:
        ...
        @tree.command(name="reset", description="Reset your Sinria session")
        ...
        @tree.command(name="status", description="Show Sinria session status")
        ...
        @tree.command(name="stop", description="Stop the running Sinria agent")
        ...
        @tree.command(name="reload-skills", description="Re-scan ~/.sinria/skills/ for new or removed skills")
        ...
        @tree.command(name="update", description="Update Sinria to the latest version")
        async def slash_update(interaction: discord.Interaction):
            await self._run_simple_slash(interaction, "/update", "Update initiated~")

        @tree.command(name="restart", description="Gracefully restart the Sinria gateway")
        ...
        @tree.command(name="thread", description="Create a new thread and start a Sinria session in it")
    ...
    async def _handle_thread_create_slash(
        ...
        # If a message was provided, kick off a new Sinria session in the thread
    ...
    async def send_exec_approval(
        ...
        try:
            ...
            # Discord embed description limit is 4096; show full preview up to that.
            max_desc = 4088
            cmd_display = command if len(command) <= max_desc else command[: max_desc - 3] + "..."
            embed_title = "⚠️ Command Approval Required"
            if metadata and metadata.get("title"):
                embed_title = str(metadata.get("title"))
            embed = discord.Embed(
                title=embed_title,
                description=f"```\n{cmd_display}\n```",
