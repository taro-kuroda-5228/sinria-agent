...
def _reply_anchor_for_event(event) -> str | None:
    ...
    replying to the triggering message. Sinria-created Telegram private-chat
...
GATEWAY_SECRET_CAPTURE_UNSUPPORTED_MESSAGE = (
    ...
    "Load this skill in the local CLI to be prompted, or add the key to ~/.sinria/.env manually. Legacy ~/.hermes/.env remains a compatibility alias when explicitly configured."
)
        ...
        for attempt in range(retries + 1):
            try:
                response = await client.get(
                    url,
                    headers={
                        "User-Agent": "Mozilla/5.0 (compatible; SinriaAgent/1.0)",
                        "Accept": "image/*,*/*;q=0.8",
                    },
                )
