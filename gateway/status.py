...
The PID file lives at ``{SINRIA_HOME}/gateway.pid`` for Sinria. Legacy
``HERMES_HOME`` is still accepted as a compatibility alias while the fork is
being migrated, but new runtime/status code should prefer Sinria-native product
markers so Sinria and upstream Hermes can coexist on the same host.
"""
...
def _runtime_cli_name() -> str:
    """Return the active product CLI name, preferring Sinria-native env vars."""
    return (os.getenv("SINRIA_CLI_NAME") or os.getenv("HERMES_CLI_NAME") or "hermes").strip().lower() or "hermes"


def _runtime_product_slug() -> str:
    return "sinria" if _runtime_cli_name() == "sinria" else "hermes"


def _get_pid_path() -> Path:
    """Return the path to the gateway PID file, respecting SINRIA_HOME/HERMES_HOME."""
...
def _get_lock_dir() -> Path:
    ...
    override = os.getenv("SINRIA_GATEWAY_LOCK_DIR") or os.getenv("HERMES_GATEWAY_LOCK_DIR")
    if override:
        return Path(override)
    state_home = Path(os.getenv("XDG_STATE_HOME", Path.home() / ".local" / "state"))
    return state_home / _runtime_product_slug() / _LOCKS_DIRNAME
...
def _looks_like_gateway_process(pid: int) -> bool:
    """Return True when the live PID still looks like the active product gateway."""
    cmdline = _read_process_cmdline(pid)
    if not cmdline:
        return False

    normalized = cmdline.replace("\\", "/")
    if _runtime_product_slug() == "sinria":
        explicit_hermes_markers = (
            "HERMES_CLI_NAME=hermes",
            "SINRIA_CLI_NAME=hermes",
            " hermes gateway",
            "/hermes gateway",
        )
        explicit_sinria_markers = (
            "SINRIA_CLI_NAME=sinria",
            "HERMES_CLI_NAME=sinria",
            "SINRIA_HOME=",
            " sinria gateway",
            "/sinria gateway",
            "sinria-gateway",
        )
        if any(marker in normalized for marker in explicit_hermes_markers) and not any(
            marker in normalized for marker in explicit_sinria_markers
        ):
            return False
    ...
    patterns = (
        ...
        "sinria gateway",
        "hermes gateway" if _runtime_product_slug() != "sinria" else "__explicit_hermes_gateway_disabled_for_sinria__",
        "sinria-gateway",
        "gateway/run.py",
    )
...
def _record_looks_like_gateway(record: dict[str, Any]) -> bool:
    ...
    product = str(record.get("product") or record.get("cli_name") or "").strip().lower()
    if _runtime_product_slug() == "sinria":
        if product == "hermes":
            return False
        if any(marker in cmdline for marker in ("HERMES_CLI_NAME=hermes", "SINRIA_CLI_NAME=hermes")):
            return False
    patterns = (
        "hermes_cli.main gateway",
        "hermes_cli/main.py gateway",
...
def _build_pid_record() -> dict:
    product = _runtime_product_slug()
    return {
        "pid": os.getpid(),
        "kind": _GATEWAY_KIND,
        "product": product,
        "cli_name": _runtime_cli_name(),
