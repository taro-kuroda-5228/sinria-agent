...
def test_hermes_launcher_wrapper_clears_python_env_before_exec() -> None:
    ...
    # The launcher shim was renamed hermes -> sinria (the HERMES_BIN var that
    # the shim execs is kept for backward compatibility).
    assert 'cat > "$command_link_dir/sinria" <<EOF' in text
    assert 'unset PYTHONPATH' in text
    assert 'unset PYTHONHOME' in text
