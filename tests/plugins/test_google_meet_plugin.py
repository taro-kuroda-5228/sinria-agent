...
def test_looks_like_human_speaker():
    ...
    # Blank, "unknown", "you", the bot's own name, and bare "hermes" token → not human (no barge-in)
    for s in ("", "   ", "Unknown", "unknown", "You", "you", "Sinria", "hermes", "hermes agent"):
        assert not _looks_like_human_speaker(s, "Sinria"), f"{s!r} should NOT be human"
    # Real names → human (barge-in)
    for s in ("Alice", "Bob Lee", "@teknium"):
        assert _looks_like_human_speaker(s, "Sinria"), f"{s!r} SHOULD be human"
