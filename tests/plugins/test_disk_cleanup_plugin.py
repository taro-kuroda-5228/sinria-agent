...
class TestIsSafePath:
    def test_standalone_fallback_uses_sinria_home_for_bare_sinria(
        ...
        def fake_import(name, *args, **kwargs):
            if name == "hermes_constants":
                raise ImportError("simulate standalone plugin helper")
            return real_import(name, *args, **kwargs)

        monkeypatch.setattr(builtins, "__import__", fake_import)
        dg = _load_lib()

        assert dg.get_state_dir() == (tmp_path / ".sinria" / "disk-cleanup").resolve()
