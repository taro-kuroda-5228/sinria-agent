...
class TestBuildContextFilesPrompt:
    ...
    def test_loads_sinria_md(self, tmp_path):
        (tmp_path / ".sinria.md").write_text("Use pytest for testing.")
        result = build_context_files_prompt(cwd=str(tmp_path))
    ...
    def test_loads_sinria_md_uppercase(self, tmp_path):
        (tmp_path / "SINRIA.md").write_text("Always use type hints.")
        result = build_context_files_prompt(cwd=str(tmp_path))
        assert "type hints" in result
    ...
    def test_sinria_md_takes_priority_over_hermes_names(self, tmp_path):
        (tmp_path / ".sinria.md").write_text("From sinria dotfile.")
        (tmp_path / "SINRIA.md").write_text("From sinria uppercase.")
        (tmp_path / ".hermes.md").write_text("From hermes dotfile.")
        (tmp_path / "HERMES.md").write_text("From hermes uppercase.")
        result = build_context_files_prompt(cwd=str(tmp_path))
        assert "From sinria dotfile" in result
        assert "From sinria uppercase" not in result
        assert "From hermes dotfile" not in result
        assert "From hermes uppercase" not in result
    ...
    def test_sinria_md_parent_dir_discovery(self, tmp_path):
        """Walks parent dirs up to git root."""
        (tmp_path / ".git").mkdir()
        (tmp_path / ".sinria.md").write_text("Root project rules.")
    ...
    def test_sinria_md_beats_agents_md(self, tmp_path):
        """When both exist, .sinria.md wins and AGENTS.md is not loaded."""
        (tmp_path / "AGENTS.md").write_text("Agent guidelines here.")
        (tmp_path / ".sinria.md").write_text("Sinria project rules.")
        result = build_context_files_prompt(cwd=str(tmp_path))
        assert "Sinria project rules" in result
    ...
    def test_sinria_md_beats_all_others(self, tmp_path):
        """When all context types exist, .sinria.md wins."""
        (tmp_path / ".sinria.md").write_text("Sinria wins.")
...
class TestFindHermesMd:
    def test_finds_sinria_in_cwd(self, tmp_path):
        (tmp_path / ".sinria.md").write_text("rules")
        assert _find_hermes_md(tmp_path) == tmp_path / ".sinria.md"

    def test_finds_in_cwd(self, tmp_path):
        (tmp_path / ".hermes.md").write_text("rules")
        assert _find_hermes_md(tmp_path) == tmp_path / ".hermes.md"

    def test_finds_sinria_uppercase(self, tmp_path):
        (tmp_path / "SINRIA.md").write_text("rules")
        assert _find_hermes_md(tmp_path) == tmp_path / "SINRIA.md"
    ...
    def test_prefers_sinria_names_first(self, tmp_path):
        (tmp_path / ".sinria.md").write_text("sinria-lower")
        (tmp_path / "SINRIA.md").write_text("sinria-upper")
        (tmp_path / ".hermes.md").write_text("hermes-lower")
        (tmp_path / "HERMES.md").write_text("hermes-upper")
        assert _find_hermes_md(tmp_path) == tmp_path / ".sinria.md"
    ...
    def test_walks_to_git_root_with_sinria(self, tmp_path):
        (tmp_path / ".git").mkdir()
        (tmp_path / ".sinria.md").write_text("root rules")
        sub = tmp_path / "a" / "b"
        sub.mkdir(parents=True)
        assert _find_hermes_md(sub) == tmp_path / ".sinria.md"
