...
def _no_keychain(monkeypatch):
    """Never touch the real macOS Keychain during this module's tests.

    ``resolve_anthropic_token`` / ``resolve_*_with_refresh`` consult
    ``_read_claude_code_credentials_from_keychain`` which, on a developer
    Mac with Claude Code installed, returns a live OAuth token from the
    "Claude Code-credentials" entry. That leaks a real credential into
    tests asserting on a tmp_path-only credential surface (and makes the
    ``returns_none`` cases non-deterministic). Stub it to None module-wide;
    tests that exercise the Keychain reader itself mock it explicitly.
    """
    monkeypatch.setattr(
...
class TestConvertToolsToAnthropicDedup:
    ...
    https://github.com/NousResearch/sinria-agent/issues/18478
    """
