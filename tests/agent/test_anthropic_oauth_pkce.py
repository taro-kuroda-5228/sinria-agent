...
def test_token_exchange_uses_current_platform_endpoint(monkeypatch, tmp_path):
    """Anthropic OAuth login must POST the authorization code to the current
    platform.claude.com token endpoint, not the stale console endpoint.
    """
    monkeypatch.setenv("HERMES_HOME", str(tmp_path))
    captured_url: Dict[str, str] = {}
    captured_token: Dict[str, Any] = {}
    ...
    def fake_input(*_a, **_kw):
        ...
        return f"auth-code-from-anthropic#{qs.get('state', [''])[0]}"
...
def test_token_exchange_falls_back_to_console_endpoint(monkeypatch, tmp_path):
    """If the current endpoint is temporarily unavailable, keep the legacy
    console endpoint as a compatibility fallback.
    """
    monkeypatch.setenv("HERMES_HOME", str(tmp_path))
    ...
    monkeypatch.setattr("webbrowser.open", lambda url: captured_url.setdefault("url", url) or True)
    ...
    def fake_urlopen(req, *_a, **_kw):
        ...
        if req.full_url == "https://platform.claude.com/v1/oauth/token":
            raise OSError("temporary platform outage")
