...
def test_reconcile_absorbed_into_beats_everything_else(curator_env):
    ...
    out = curator_env._reconcile_classification(
        ...
        destinations={"sinria-agent-dev"},
        absorbed_declarations={
            "pr-review-format": {"into": "sinria-agent-dev", "declared": True},
        },
    )
    assert len(out["consolidated"]) == 1
    assert out["pruned"] == []
    ...
    assert e["into"] == "sinria-agent-dev"
...
def test_rename_summary_uses_sinria_command_when_requested(curator_env, monkeypatch):
    monkeypatch.setenv("HERMES_CLI_NAME", "sinria")
    result = curator_env._build_rename_summary(
        before_names={"pdf-extraction", "document-tools"},
        after_report=[{"name": "document-tools", "state": "active"}],
        tool_calls=[
            {
                "name": "skill_manage",
    ...
    assert "full report: sinria curator status" in result
    assert "keep an umbrella stable: sinria curator pin document-tools" in result
