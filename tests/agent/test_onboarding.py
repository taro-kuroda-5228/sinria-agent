...
class TestOpenclawResidueHint:
    ...
    def test_sinria_hint_uses_sinria_commands(self, monkeypatch):
        monkeypatch.setenv("HERMES_CLI_NAME", "sinria")
        msg = openclaw_residue_hint_cli()
        assert "sinria claw migrate" in msg
        assert "sinria claw cleanup" in msg
        assert "hermes claw" not in msg
