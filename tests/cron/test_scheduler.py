...
class TestDeliverResultWrapping:
    ...
    def test_sinria_blocks_confidential_cron_delivery_before_external_send(self, tmp_path, monkeypatch):
        """Sinria cron delivery must not bypass external-egress controls."""
        from gateway.config import Platform

        monkeypatch.setenv("HERMES_HOME", str(tmp_path))
        monkeypatch.setenv("SINRIA_HOME", str(tmp_path))
        audit_path = tmp_path / "logs" / "sinria-egress-audit.jsonl"
        pconfig = MagicMock()
        pconfig.enabled = True
        mock_cfg = MagicMock()
        mock_cfg.platforms = {Platform.TELEGRAM: pconfig}
        ...
        with patch("gateway.config.load_gateway_config", return_value=mock_cfg), \
             ...
             patch("cron.scheduler.load_config", return_value={
                 ...
                 "sinria": {"egress": {"mode": "ask", "confidential_external_send": "ask"}},
             }):
            job = {
                "id": "confidential-cron",
                "deliver": "origin",
                "origin": {"platform": "telegram", "chat_id": "123"},
            }
            result = _deliver_result(job, "社外秘: patient_id=12345 token=SHOULD_NOT_LEAK")

        send_mock.assert_not_called()
        assert "Sinria external egress guard" in result
        assert audit_path.exists()
        audit_text = audit_path.read_text(encoding="utf-8")
        assert "cron_delivery" in audit_text
        assert "SHOULD_NOT_LEAK" not in audit_text
