...
class TestDeleteAndExport:
    ...
    def test_export_training_examples_includes_outcome_actions_and_reasoning_trace(self, db):
        """Training export should preserve result, tool trajectory, and explicit reasoning fields."""
        db.create_session(session_id="s1", source="discord", model="gpt-5.5")
        db.set_session_title("s1", "local agent task")
        db.append_message("s1", role="user", content="調査して直して")
        db.append_message(
            "s1",
            role="assistant",
            content="ファイルを確認します。",
            reasoning="Need to inspect the target file before editing.",
            tool_calls=[
                {
                    "id": "call_1",
                    "type": "function",
        ...
        db.append_message(
            ...
            role="assistant",
            content="修正完了です。",
            reasoning_content="The read result is enough to produce the final answer.",
            finish_reason="stop",
        )
        db.end_session("s1", end_reason="done")

        examples = db.export_training_examples(include_reasoning=True)

        assert len(examples) == 1
        example = examples[0]
        assert example["schema"] == "sinria.agent_training.v1"
        assert example["session_id"] == "s1"
        assert example["input"] == "調査して直して"
        assert example["output"] == "修正完了です。"
        assert example["outcome"]["completed"] is True
        assert example["outcome"]["end_reason"] == "done"
        assert example["action_trace"] == [
            {
                "step": 1,
                "tool_call_id": "call_1",
                "tool_name": "read_file",
                "arguments": {"path": "/tmp/example.py"},
                "response": {"content": "print(1)", "total_lines": 1},
                "success": True,
            }
        ]
        assert example["reasoning_trace"] == [
            {
                "message_index": 1,
                "field": "reasoning",
                "content": "Need to inspect the target file before editing.",
            },
            {
                "message_index": 3,
                "field": "reasoning_content",
                "content": "The read result is enough to produce the final answer.",
            },
        ]
        assert "conversations" in example

    def test_export_training_examples_redacts_reasoning_by_default(self, db):
        db.create_session(session_id="s1", source="cli", model="test")
        db.append_message("s1", role="user", content="hello")
        db.append_message("s1", role="assistant", content="hi", reasoning="private scratchpad")

        example = db.export_training_examples()[0]

        assert example["reasoning_trace"] == []
        assert example["reasoning_policy"] == {
            "mode": "excluded_by_default",
