...
def test_kanban_command_no_action_uses_sinria_in_usage(monkeypatch, capsys):
    monkeypatch.setenv("HERMES_CLI_NAME", "sinria")
    ns = argparse.Namespace(kanban_action=None)
    assert kc.kanban_command(ns) == 0
    err = capsys.readouterr().err
    assert "usage: sinria kanban <action> [options]" in err
    assert "Run 'sinria kanban --help'" in err
    assert "hermes kanban --help" not in err
