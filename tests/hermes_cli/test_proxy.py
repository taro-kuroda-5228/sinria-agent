...
def test_nous_adapter_get_credential_uses_sinria_login_hint(tmp_path, monkeypatch):
    monkeypatch.setenv("HERMES_HOME", str(tmp_path))
    monkeypatch.setenv("HERMES_CLI_NAME", "sinria")
    adapter = NousPortalAdapter()
    with pytest.raises(RuntimeError) as exc:
        adapter.get_credential()
    assert "sinria login nous" in str(exc.value)
    assert "hermes login nous" not in str(exc.value)
...
def test_cmd_proxy_start_refuses_when_unauthenticated_uses_sinria(capsys, tmp_path, monkeypatch):
    ...
    err = capsys.readouterr().err
    assert "sinria login nous" in err
    assert "hermes login nous" not in err


def test_cmd_proxy_start_missing_aiohttp_uses_sinria_hint(capsys, monkeypatch):
    monkeypatch.setenv("HERMES_CLI_NAME", "sinria")
    import hermes_cli.proxy.cli as proxy_cli
    ...
    rc = proxy_cli.cmd_proxy_start(args)
    assert rc == 1
    err = capsys.readouterr().err
    assert "sinria proxy requires aiohttp" in err
    assert "`sinria proxy providers`" in err
    assert "`hermes proxy providers`" not in err


def test_cmd_proxy_help_uses_sinria_commands(capsys, monkeypatch):
    monkeypatch.setenv("HERMES_CLI_NAME", "sinria")
    from hermes_cli.proxy.cli import cmd_proxy

    rc = cmd_proxy(MagicMock(proxy_command=None))
    assert rc == 0
    err = capsys.readouterr().err
    assert "sinria proxy — local OpenAI-compatible proxy" in err
    assert "sinria proxy start [--provider nous]" in err
    assert "sinria proxy status" in err
    assert "sinria proxy providers" in err
    assert "hermes proxy status" not in err
