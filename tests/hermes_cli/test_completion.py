...
class TestGenerateBash:
    ...
    def test_sinria_branding(self, monkeypatch):
        ...
        out = generate_bash(_make_parser())
        assert "# Sinria Agent bash completion" in out
        assert 'eval "$(sinria completion bash)"' in out
        assert "complete -F _hermes_completion sinria" in out
        assert 'local profiles_dir="$HOME/.sinria/profiles"' in out
...
class TestGenerateZsh:
    ...
    def test_sinria_branding(self, monkeypatch):
        ...
        out = generate_zsh(_make_parser())
        assert "#compdef sinria" in out
        assert "# Sinria Agent zsh completion" in out
        assert 'eval "$(sinria completion zsh)"' in out
        assert 'if [[ -d "$HOME/.sinria/profiles" ]]; then' in out
...
class TestGenerateFish:
    ...
    def test_sinria_branding(self, monkeypatch):
        monkeypatch.setenv("HERMES_CLI_NAME", "sinria")
        out = generate_fish(_make_parser())
        assert "# Sinria Agent fish completion" in out
        assert "#   sinria completion fish | source" in out
        assert "complete -c sinria -f" in out
        assert "if test -d $HOME/.sinria/profiles" in out
        assert "complete -c hermes -f" not in out
        assert "$HOME/.hermes/profiles" not in out
