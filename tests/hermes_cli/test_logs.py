...
class TestSinriaBranding:
    def test_missing_log_hint_uses_sinria_chat(self, tmp_path, monkeypatch, capsys):
        monkeypatch.setenv("HERMES_HOME", str(tmp_path))
        monkeypatch.setenv("HERMES_CLI_NAME", "sinria")
        with pytest.raises(SystemExit):
            tail_log("agent")
        out = capsys.readouterr().out
        assert "Logs are created when Sinria runs" in out
        assert "try 'sinria chat' first" in out
        assert "try 'hermes chat' first" not in out

    def test_empty_logs_hint_uses_sinria_chat(self, tmp_path, monkeypatch, capsys):
        monkeypatch.setenv("HERMES_HOME", str(tmp_path))
        monkeypatch.setenv("HERMES_CLI_NAME", "sinria")
        (tmp_path / "logs").mkdir()
        list_logs()
        out = capsys.readouterr().out
        assert "run 'sinria chat' to generate logs" in out
        assert "run 'hermes chat' to generate logs" not in out
