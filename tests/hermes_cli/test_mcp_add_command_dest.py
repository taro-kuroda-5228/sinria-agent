"""Regression test: ``mcp add --command`` must not clobber the
...
result, ``mcp add foo --url ...`` silently launches chat instead
...
def _build_parser():
    """Minimal replica of the parser slice that exhibits
...
class TestMcpAddCommandDest:
    def test_url_invocation_preserves_top_level_command(self):
        """``mcp add foo --url ...`` must keep args.command == "mcp".
