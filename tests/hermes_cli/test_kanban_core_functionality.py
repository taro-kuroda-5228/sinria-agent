...
def test_check_dispatcher_presence_warns_when_no_gateway_uses_sinria(monkeypatch):
    from hermes_cli import kanban as kb_cli
    ...
    running, msg = kb_cli._check_dispatcher_presence()
    assert running is False
    assert "sinria gateway start" in msg
...
def test_cli_daemon_without_force_uses_sinria(kanban_home, monkeypatch, capsys):
    from hermes_cli import kanban as kb_cli
    monkeypatch.setenv("HERMES_CLI_NAME", "sinria")
    ns = argparse.Namespace(
        force=False, interval=60.0, max=None, failure_limit=3,
    ...
    assert rc == 2
    err = capsys.readouterr().err
    assert "sinria gateway start" in err
    assert "hermes gateway start" not in err
