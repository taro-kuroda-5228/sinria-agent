...
def test_dry_run_default_reports_synchronous_wording(monkeypatch, capsys):
    ...
    assert "Read the report with `sinria curator status`" in out



def test_dry_run_wording_uses_sinria_command(monkeypatch, capsys):
    import agent.curator as curator_state
    import hermes_cli.curator as curator_cli
    ...
    out = capsys.readouterr().out
    assert "Read the report with `sinria curator status`" in out
    assert "Read the report with `hermes curator status`" not in out
