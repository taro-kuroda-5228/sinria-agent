...
def test_missing_target_uses_sinria_in_usage(monkeypatch, capsys):
    monkeypatch.setenv("HERMES_CLI_NAME", "sinria")
    args = _parse(["hello world"])
    with pytest.raises(SystemExit) as exc:
        send_cmd.cmd_send(args)
    assert exc.value.code == 2
    err = capsys.readouterr().err
    assert "sinria send: --to PLATFORM[:channel[:thread]] is required" in err
    assert "sinria send --to telegram \"hello\"" in err
    assert "hermes send --to telegram \"hello\"" not in err



def test_send_help_description_uses_sinria_identity(monkeypatch):
    monkeypatch.setenv("HERMES_CLI_NAME", "sinria")
    import argparse

    parser = argparse.ArgumentParser(prog="sinria")
    subparsers = parser.add_subparsers(dest="command")
    send_cmd.register_send_subparser(subparsers)
    help_text = subparsers.choices["send"].format_help()

    assert "messaging platform Sinria is already configured for" in help_text
    assert "~/.sinria/.env + ~/.sinria/config.yaml" in help_text
    assert "messaging platform Hermes is already configured for" not in help_text
...
def test_list_platform_filter_missing_uses_sinria_list_hint(monkeypatch, capsys):
    import sys as _sys
    import types as _types
    ...
    fake_dir.load_directory = lambda: {"platforms": {"telegram": [{"name": "ops"}]}}
    monkeypatch.setitem(_sys.modules, "gateway.channel_directory", fake_dir)

    with pytest.raises(SystemExit) as exc:
        send_cmd.cmd_send(_parse(["--list", "discord"]))
    assert exc.value.code == 1
    err = capsys.readouterr().err
    assert "no targets found for platform 'discord'" in err
    assert "Try `sinria send --list` to inspect available targets." in err
    assert "Try `hermes send --list` to inspect available targets." not in err
...
def test_list_empty_sinria_hint_uses_sinria_gateway_setup(monkeypatch, capsys):
    import sys as _sys
    import types as _types
    ...
    assert "~/.sinria/channel_directory.json" in out
    assert "hermes gateway setup" not in out
    assert "~/.hermes/channel_directory.json" not in out
