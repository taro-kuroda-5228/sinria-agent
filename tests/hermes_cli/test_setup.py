...
def test_sinria_setup_wizard_banner_uses_sinria_branding(monkeypatch, capsys, tmp_path):
    class _StopAfterBanner(RuntimeError):
        pass
    ...
    monkeypatch.setattr(setup_mod, "get_config_path", lambda: tmp_path / "config.yaml")
    ...
    monkeypatch.setattr(setup_mod, "setup_model_provider", lambda *a, **k: (_ for _ in ()).throw(_StopAfterBanner()))
    monkeypatch.setattr("hermes_cli.auth.get_active_provider", lambda: "openai")

    args = SimpleNamespace(reset=False, reconfigure=False, quick=False, non_interactive=False, section=None)
    with pytest.raises(_StopAfterBanner):
        setup_mod.run_setup_wizard(args)

    out = capsys.readouterr().out
    assert "Sinria Agent Setup Wizard" in out
    assert "Sinria Setup Wizard" not in out
    assert "Let's configure your Sinria Agent installation." in out
    assert "You already have Sinria configured." in out
    assert "sinria config edit" in out
    assert "hermes config edit" not in out
