...
These flags share the detection + kill path with the post-update
...
class TestDashboardStatus:
    def test_status_no_processes(self, capsys):
        with patch.dict(os.environ, {"HERMES_CLI_NAME": "sinria"}, clear=False), \
    ...
    def test_status_with_processes(self, capsys):
        ...
        assert exc.value.code == 0
        out = capsys.readouterr().out
        assert "2 sinria dashboard process(es) running" in out
...
class TestLifecycleFlagsTakePrecedence:
    ...
    who typed ``dashboard --stop`` must not end up ALSO starting
    a new server."""
