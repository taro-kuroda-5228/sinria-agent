...
class TestEnvLoaderSanitization:
    ...
    def test_warns_to_stderr_with_sinria_setup_hint(self, monkeypatch, capsys):
        from hermes_cli.env_loader import _sanitize_loaded_credentials, _WARNED_KEYS

        monkeypatch.setenv("HERMES_CLI_NAME", "sinria")
        _WARNED_KEYS.discard("GOOGLE_API_KEY")
        monkeypatch.setenv("GOOGLE_API_KEY", "AIzaSy\u200babcdef")
        _sanitize_loaded_credentials()

        captured = capsys.readouterr()
        assert "`sinria setup`" in captured.err
