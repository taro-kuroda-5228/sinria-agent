...
def test_spotify_refresh_missing_token_hint_uses_sinria(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("HERMES_CLI_NAME", "sinria")

    with pytest.raises(auth_mod.AuthError) as exc:
        auth_mod._refresh_spotify_oauth_state({"client_id": "spotify-client"})

    assert "sinria auth spotify" in str(exc.value)
    assert "hermes auth spotify" not in str(exc.value)



def test_spotify_missing_auth_hint_uses_sinria(tmp_path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("HERMES_CLI_NAME", "sinria")
    monkeypatch.setenv("HERMES_HOME", str(tmp_path))
