...
def test_skills_command_usage_uses_sinria(monkeypatch):
    import hermes_cli.skills_hub as cli_hub

    monkeypatch.setenv("HERMES_CLI_NAME", "sinria")
    args = type("Args", (), {"skills_action": "unknown"})()
    with patch.object(cli_hub, "_console") as mock_console:
        cli_hub.skills_command(args)

    printed = " ".join(str(call.args[0]) for call in mock_console.print.call_args_list)
    assert "Usage: sinria skills" in printed
    assert "Run 'sinria skills <command> --help'" in printed
    assert "Usage: hermes skills" not in printed



def test_skills_snapshot_usage_uses_sinria(monkeypatch):
    import hermes_cli.skills_hub as cli_hub

    monkeypatch.setenv("HERMES_CLI_NAME", "sinria")
    args = type("Args", (), {"skills_action": "snapshot", "snapshot_action": None})()
    with patch.object(cli_hub, "_console") as mock_console:
        cli_hub.skills_command(args)

    printed = " ".join(str(call.args[0]) for call in mock_console.print.call_args_list)
    assert "Usage: sinria skills snapshot [export|import]" in printed
    assert "Usage: hermes skills snapshot [export|import]" not in printed



def test_skills_tap_usage_uses_sinria(monkeypatch):
    import hermes_cli.skills_hub as cli_hub

    monkeypatch.setenv("HERMES_CLI_NAME", "sinria")
    args = type("Args", (), {"skills_action": "tap", "tap_action": None, "repo": "", "name": ""})()
    with patch.object(cli_hub, "_console") as mock_console:
        cli_hub.skills_command(args)

    printed = " ".join(str(call.args[0]) for call in mock_console.print.call_args_list)
    assert "Usage: sinria skills tap [list|add|remove]" in printed
    assert "Usage: hermes skills tap [list|add|remove]" not in printed
...
def test_url_install_actionable_error_uses_sinria(monkeypatch, tmp_path, hub_env):
    installs = _install_mocks(monkeypatch, tmp_path, _make_url_bundle_fetcher())
    monkeypatch.setenv("HERMES_CLI_NAME", "sinria")

    sink = StringIO()
    console = Console(file=sink, force_terminal=False, color_system=None)
    do_install("https://example.com/SKILL.md", console=console, skip_confirm=True)

    assert installs == []
    out = sink.getvalue()
    assert "sinria skills install https://example.com/SKILL.md --name <your-name>" in out
    assert "hermes skills install https://example.com/SKILL.md --name <your-name>" not in out
