...
def test_show_status_termux_gateway_section_uses_sinria(monkeypatch, capsys, tmp_path):
    from hermes_cli import status as status_mod
    import hermes_cli.auth as auth_mod
    import hermes_cli.gateway as gateway_mod
    ...
    monkeypatch.setattr(auth_mod, "get_codex_auth_status", lambda: {}, raising=False)
    monkeypatch.setattr(gateway_mod, "find_gateway_pids", lambda exclude_pids=None: [], raising=False)
    monkeypatch.setattr(status_mod.subprocess, "run", lambda *args, **kwargs: (_ for _ in ()).throw(AssertionError("systemctl should not be called in the Termux status view")))

    status_mod.show_status(SimpleNamespace(all=False, deep=False))

    output = capsys.readouterr().out
    assert "Start with:   sinria gateway" in output
    assert "Start with:   hermes gateway" not in output
...
def test_show_status_reports_nous_auth_error(monkeypatch, capsys, tmp_path):
    ...
    assert "run: hermes auth add nous --type oauth" in output
    assert "Error:      Refresh session has been revoked" in output
    assert "Access exp:" in output
    assert "Key exp:" in output


def test_sinria_status_model_auth_guidance_uses_sinria_command(monkeypatch, capsys, tmp_path):
    from hermes_cli import status as status_mod
    import hermes_cli.auth as auth_mod
    import hermes_cli.gateway as gateway_mod
    ...
    output = capsys.readouterr().out
    assert "run: sinria model" in output
    assert "Medical Horizon Portal" in output
    assert "run: sinria auth add nous --type oauth" in output
    assert "run: sinria auth add minimax-oauth" in output
    assert "run: hermes model" not in output
    assert "run: hermes auth add" not in output
