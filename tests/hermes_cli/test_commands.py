...
class TestCommandRegistry:
    ...
    def test_reload_skills_description_uses_sinria_home(self, monkeypatch):
        monkeypatch.setenv("HERMES_CLI_NAME", "sinria")
        import hermes_cli.commands as commands_mod
        importlib.reload(commands_mod)
        try:
            cmd = next(c for c in commands_mod.COMMAND_REGISTRY if c.name == "reload-skills")
            assert "~/.sinria/skills/" in cmd.description
            assert "~/.hermes/skills/" not in cmd.description
        finally:
            monkeypatch.delenv("HERMES_CLI_NAME", raising=False)
            importlib.reload(commands_mod)

    def test_snapshot_and_goal_descriptions_use_sinria_identity(self, monkeypatch):
        monkeypatch.setenv("HERMES_CLI_NAME", "sinria")
        import hermes_cli.commands as commands_mod
        importlib.reload(commands_mod)
        try:
            snapshot = next(c for c in commands_mod.COMMAND_REGISTRY if c.name == "snapshot")
            goal = next(c for c in commands_mod.COMMAND_REGISTRY if c.name == "goal")
            busy = next(c for c in commands_mod.COMMAND_REGISTRY if c.name == "busy")
            assert "Sinria config/state" in snapshot.description
            assert "Hermes config/state" not in snapshot.description
            assert "standing goal Sinria works on" in goal.description
            assert "standing goal Hermes works on" not in goal.description
            assert "while Sinria is working" in busy.description
            assert "while Hermes is working" not in busy.description
        finally:
            monkeypatch.delenv("HERMES_CLI_NAME", raising=False)
            importlib.reload(commands_mod)
...
class TestSlackNativeSlashes:
    ...
    def test_sinria_catchall_description_uses_sinria_identity(self, monkeypatch):
        monkeypatch.setenv("HERMES_CLI_NAME", "sinria")
        import hermes_cli.commands as commands_mod
        importlib.reload(commands_mod)
        try:
            slashes = commands_mod.slack_native_slashes()
            assert slashes[0][0] == "hermes"
            assert slashes[0][1] == "Talk to Sinria or run a subcommand"
        finally:
            monkeypatch.delenv("HERMES_CLI_NAME", raising=False)
            importlib.reload(commands_mod)
...
class TestSlackAppManifest:
    """Generated Slack app manifest used by the Slack manifest command."""
