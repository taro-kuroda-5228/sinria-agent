...
def test_plugin_install_help_uses_sinria_env_path(monkeypatch, capsys):
    ...
    platform = {
        ...
        "install_hint": None,
        "vars": [],
        "_registry_entry": type("Entry", (), {"setup_fn": None, "required_env": ["DEMO_TOKEN"]})(),
    }

    gateway._configure_platform(platform)
    out = capsys.readouterr().out
    assert "Set these env vars in ~/.sinria/.env: DEMO_TOKEN" in out
...
class TestCmdList:
    ...
    def test_list_empty_plugins_dir_uses_sinria_hint(self, _mock_discover, monkeypatch, capsys):
        from hermes_cli.plugins_cmd import cmd_list
        ...
        cmd_list()
        out = capsys.readouterr().out
        assert "sinria plugins install owner/repo" in out
        assert "hermes plugins install owner/repo" not in out
    ...
    def test_install_disabled_hint_uses_sinria_commands(self, mock_install_core, monkeypatch, capsys, tmp_path):
        from hermes_cli.plugins_cmd import cmd_install
        ...
        mock_install_core.return_value = (plugin_dir, {"name": "demo"}, "demo")
        monkeypatch.setattr("hermes_cli.plugins_cmd._prompt_plugin_env_vars", lambda *a, **k: None)
        ...
        out = capsys.readouterr().out
        assert "sinria plugins enable demo" in out
        assert "sinria gateway restart" in out
        assert "hermes plugins enable demo" not in out
...
class TestProviderDiscovery:
    ...
    @patch("hermes_cli.plugins_cmd._discover_all_plugins", return_value=[])
    def test_toggle_non_tty_uses_sinria_fallback_hints(self, _mock_discover, monkeypatch, capsys):
        ...
        cmd_toggle()
        out = capsys.readouterr().out
        assert "Interactive mode requires a terminal." in out
        assert "sinria plugins list" in out
        assert "sinria plugins enable <name>" in out
        assert "hermes plugins list" not in out
