...
def test_codex_runtime_credentials_use_pool_when_legacy_auth_store_missing(monkeypatch):
    from hermes_cli import auth as auth_mod

    class _Entry:
        access_token = "pool-only-codex-token"
        refresh_token = "pool-only-refresh-token"
        source = "manual:device_code"
        base_url = "https://chatgpt.com/backend-api/codex"
        last_refresh = "2026-06-15T22:06:53Z"
    ...
    assert resolved["provider"] == "openai-codex"
    assert resolved["api_key"] == "pool-only-codex-token"
    assert resolved["source"] == "manual:device_code"
    assert resolved["refresh_token_present"] is True
...
def test_named_custom_provider_can_disable_global_key_fallback(monkeypatch):
    monkeypatch.setenv("OPENAI_API_KEY", "env-openai-key")
    ...
    resolved = rp.resolve_runtime_provider(requested="sakana-fugu")

    assert resolved["provider"] == "custom"
    assert resolved["api_mode"] == "chat_completions"
    assert resolved["base_url"] == "https://api.sakana.ai/v1"
    assert resolved["api_key"] == "no-key-required"
    assert resolved["requested_provider"] == "sakana-fugu"
    assert resolved["source"] == "custom_provider:Sakana Fugu"
    assert resolved["model"] == "fugu"
