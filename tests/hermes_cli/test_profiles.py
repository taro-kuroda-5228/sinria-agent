...
class TestValidateProfileName:
    ...
    def test_reserved_name_error_uses_sinria_identity(self, monkeypatch):
        monkeypatch.setenv("HERMES_CLI_NAME", "sinria")
        with pytest.raises(ValueError, match="Sinria installation itself"):
            validate_profile_name("hermes")
...
class TestCreateProfile:
    ...
    def test_default_raises_value_error_uses_sinria_home(self, profile_env, monkeypatch):
        monkeypatch.setenv("HERMES_CLI_NAME", "sinria")
        with pytest.raises(ValueError, match=r"built-in profile \(~/.sinria\)"):
            create_profile("default", no_alias=True)
    ...
    def test_clone_all_excludes_default_infrastructure(self, profile_env):
        """--clone-all from default profile excludes sinria-agent, .worktrees,
        ...
        (default_home / "sinria-agent" / ".git").mkdir(parents=True)
        (default_home / "sinria-agent" / "venv" / "bin").mkdir(parents=True)
        (default_home / "sinria-agent" / "README.md").write_text("repo")
        ...
        assert not (profile_dir / "sinria-agent").exists()
...
class TestNoSkillsOptOut:
    ...
    def test_no_skills_marker_uses_sinria_commands(self, profile_env, monkeypatch):
        monkeypatch.setenv("HERMES_CLI_NAME", "sinria")
        profile_dir = create_profile("orchestrator", no_alias=True, no_skills=True)
        marker = (profile_dir / NO_BUNDLED_SKILLS_MARKER).read_text()
        assert "sinria profile create --no-skills" in marker
        assert "sinria update" in marker
        assert "hermes profile create --no-skills" not in marker
...
class TestDeleteProfile:
    ...
    def test_default_raises_value_error_uses_sinria_identity(self, profile_env, monkeypatch):
        monkeypatch.setenv("HERMES_CLI_NAME", "sinria")
        with pytest.raises(ValueError, match=r"~/.sinria") as exc:
            delete_profile("default", yes=True)
        assert "sinria uninstall" in str(exc.value)
        assert "hermes uninstall" not in str(exc.value)
...
class TestActiveProfile:
    ...
    def test_set_active_profile_missing_uses_sinria_create_hint(self, profile_env, monkeypatch):
        monkeypatch.setenv("HERMES_CLI_NAME", "sinria")
        with pytest.raises(FileNotFoundError, match="Create it with: sinria profile create missing"):
    ...
    def test_set_nonexistent_uses_sinria_create_hint(self, profile_env, monkeypatch):
        monkeypatch.setenv("HERMES_CLI_NAME", "sinria")
        with pytest.raises(FileNotFoundError) as exc:
            set_active_profile("nonexistent")
        assert "sinria profile create nonexistent" in str(exc.value)
        assert "hermes profile create nonexistent" not in str(exc.value)
...
class TestResolveProfileEnv:
    ...
    def test_nonexistent_uses_sinria_create_hint(self, profile_env, monkeypatch):
        monkeypatch.setenv("HERMES_CLI_NAME", "sinria")
        with pytest.raises(FileNotFoundError, match="Create it with: sinria profile create nonexistent"):
            resolve_profile_env("nonexistent")
...
class TestAliasCollision:
    ...
    def test_subcommand_returns_sinria_branded_message(self, profile_env, monkeypatch):
        monkeypatch.setenv("HERMES_CLI_NAME", "sinria")
        result = check_alias_collision("chat")
        assert result == "'chat' conflicts with a sinria subcommand"
...
class TestExportImport:
    ...
    def test_export_default_excludes_infrastructure(self, profile_env, tmp_path):
        ...
        for d in ("sinria-agent", ".worktrees", "profiles", "bin",
        ...
        excluded_prefixes = [
            "default/sinria-agent", "default/.worktrees", "default/profiles",
    ...
    def test_import_default_without_name_uses_sinria_identity(self, profile_env, tmp_path, monkeypatch):
        monkeypatch.setenv("HERMES_CLI_NAME", "sinria")
        default_dir = get_profile_dir("default")
        (default_dir / "config.yaml").write_text("ok")
        ...
        with pytest.raises(ValueError, match=r"built-in root profile \(~/.sinria\)") as exc:
            import_profile(str(archive))
        assert "sinria profile import <archive> --name <name>" in str(exc.value)
        assert "hermes profile import <archive> --name <name>" not in str(exc.value)
