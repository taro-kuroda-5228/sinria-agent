...
def test_recent_notice_footer_uses_sinria_command(curator_env, monkeypatch):
    now = datetime.now(timezone.utc).isoformat()
    monkeypatch.setenv("HERMES_CLI_NAME", "sinria")
    summary = (
        "auto: 1 marked stale; llm: consolidated 1 into 1\n"
        "archived 1 skill(s):\n"
        "  • pdf-extraction → document-tools\n"
    ...
    curator_env["main"]._print_curator_recent_run_notice()
    out = curator_env["capsys"].readouterr().out
    assert "View anytime: sinria curator status" in out
    assert "View anytime: hermes curator status" not in out
