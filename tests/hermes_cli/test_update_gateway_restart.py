...
def _make_run_side_effect(
    ...
    def side_effect(cmd, **kwargs):
        ...
        # systemctl list-units sinria-gateway* — discover all gateway services
        if "systemctl" in joined and "list-units" in joined:
            if "--user" in joined and systemd_active:
                return subprocess.CompletedProcess(
                    cmd, 0,
                    stdout="sinria-gateway.service loaded active running Sinria Gateway\n",
                    stderr="",
                )
...
class TestLaunchdPlistReplace:
    ...
    def test_plist_exit_timeout_covers_restart_drain_window(self, monkeypatch):
        """launchd must not SIGKILL the gateway before graceful drain finishes."""
        monkeypatch.setattr(gateway_cli, "_get_restart_drain_timeout", lambda: 180)
        plist = gateway_cli.generate_launchd_plist()
        assert "<key>ExitTimeOut</key>" in plist
        assert "<integer>210</integer>" in plist
...
class TestCmdUpdateLaunchdRestart:
    ...
    def test_update_without_launchd_shows_manual_restart(
        ...
        # Pin the Sinria product name (production sets HERMES_CLI_NAME=sinria);
        # otherwise the hint renders "hermes gateway run".
        ...
        captured = capsys.readouterr().out
        assert "Restart manually: sinria gateway run" in captured
    ...
    def test_update_restarts_profile_manual_gateways(
        ...
        monkeypatch.setattr(
            ...
            lambda: tmp_path / "ai.sinria.gateway.plist",
        )
        ...
        signal_kills = [c for c in kill.call_args_list if len(c.args) >= 2 and c.args[1] != 0]
        assert signal_kills == []
    ...
    def test_update_profile_manual_gateway_falls_back_to_sigterm(
        ...
        signal_kills = [c for c in kill.call_args_list if len(c.args) >= 2 and c.args[1] != 0]
        assert len(signal_kills) == 1
        assert signal_kills[0].args[0] == 12345
        assert "Restarting manual gateway profile(s): coder" in captured
    ...
    def test_update_with_systemd_still_restarts_via_systemd(
        ...
        captured = capsys.readouterr().out
        assert "Restarted sinria-gateway" in captured
        ...
        def side_effect(cmd, **kwargs):
            ...
            if "systemctl" in joined and "list-units" in joined:
                if "--user" in joined:
                    return subprocess.CompletedProcess(
                        cmd, 0,
                        stdout="sinria-gateway.service loaded active running\n",
                        stderr="",
                    )
                return subprocess.CompletedProcess(cmd, 0, stdout="", stderr="")
    ...
    def test_update_bypasses_restartsec_after_graceful_drain(
        ...
        # Must have called ``reset-failed sinria-gateway`` AND ``start
        # sinria-gateway`` explicitly so systemd bypasses RestartSec.
        reset_calls = [c for c in calls if "reset-failed" in c and "sinria-gateway" in c]
        start_calls = [
            c for c in calls
            if "start" in c and "sinria-gateway" in c and "restart" not in c
        ]
        assert reset_calls, (
            f"Expected explicit `reset-failed sinria-gateway` after graceful drain; "
            f"systemctl calls were: {calls}"
        )
        assert start_calls, (
            f"Expected explicit `start sinria-gateway` after graceful drain to "
    ...
    def test_excludes_zero_pid(self, monkeypatch):
        ...
        def fake_run(cmd, **kwargs):
            ...
            if "list-units" in joined:
                return subprocess.CompletedProcess(
                    cmd, 0,
                    stdout="sinria-gateway.service loaded inactive dead Sinria Gateway\n",
                    stderr="",
                )
            if "show" in joined and "MainPID" in joined:
    ...
    def test_filters_to_current_profile(self, monkeypatch, tmp_path):
        ...
        def fake_run(cmd, **kwargs):
            return subprocess.CompletedProcess(
                cmd, 0,
                stdout=(
                    "100 /Users/dgrieco/.hermes/sinria-agent/venv/bin/python -m hermes_cli.main --profile orcha gateway run --replace\n"
                    "200 /Users/dgrieco/.hermes/sinria-agent/venv/bin/python -m hermes_cli.main --profile other gateway run --replace\n"
                ),
                stderr="",
            )
...
class TestCmdUpdateLegacyGatewayWarning:
    ...
    ``sinria-gateway.service`` after PR #5646. Every ``hermes update``
    ...
    def test_update_prints_legacy_warning_when_detected(
        ...
        # Pin the Sinria product name (production sets HERMES_CLI_NAME=sinria);
        # otherwise the migrate command renders "hermes gateway migrate-legacy".
        monkeypatch.setenv("HERMES_CLI_NAME", "sinria")
        ...
        captured = capsys.readouterr().out
        assert "Legacy Sinria gateway unit(s) detected" in captured
        assert "hermes.service" in captured
        assert "sinria gateway migrate-legacy" in captured
    ...
    def test_update_silent_when_no_legacy_units(
        ...
        captured = capsys.readouterr().out
        assert "Legacy Sinria gateway" not in captured
    ...
    def test_update_does_not_flag_profile_units(
        ...
        """Profile units (sinria-gateway-coder.service) must not trigger the warning.
        ...
        (user_dir / "sinria-gateway-coder.service").write_text(
            self._OUR_UNIT_TEXT, encoding="utf-8"
        )
        (user_dir / "sinria-gateway.service").write_text(
            self._OUR_UNIT_TEXT, encoding="utf-8"
        )
        ...
        captured = capsys.readouterr().out
        assert "Legacy Sinria gateway" not in captured
        assert "sinria-gateway-coder.service" not in captured  # not flagged
    ...
    def test_update_lists_system_scope_unit_with_sudo_hint(
        ...
        captured = capsys.readouterr().out
        assert "Legacy Sinria gateway" in captured
...
class TestCmdUpdateResetFailedBeforeRestart:
    ...
    def test_reset_failed_runs_before_fallback_restart(
        ...
        assert any(
            "sinria-gateway" in " ".join(str(c) for c in call)
            for call in reset_calls
        ), (
            "Expected `systemctl reset-failed sinria-gateway` before the "
        ...
        for idx, call in enumerate(mock_run.call_args_list):
            ...
            if "systemctl" in joined and "reset-failed" in joined and first_reset_idx is None:
                first_reset_idx = idx
            if "systemctl" in joined and "restart" in joined and "sinria-gateway" in joined:
    ...
    def test_reset_failed_also_runs_before_retry_restart(
        ...
        gateway_restarts = [
            ...
            if "sinria-gateway" in " ".join(str(a) for a in c)
        ]
        gateway_resets = [
    ...
    def test_final_failure_message_tells_user_to_reset_failed(
        ...
        assert "sinria-gateway" in captured
