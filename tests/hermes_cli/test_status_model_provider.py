...
def test_show_status_reports_managed_nous_features_with_sinria_branding(monkeypatch, capsys, tmp_path):
    ...
    monkeypatch.setattr(status_mod, "resolve_requested_provider", lambda requested=None: "nous", raising=False)
    monkeypatch.setattr(status_mod, "resolve_provider", lambda requested=None, **kwargs: "nous", raising=False)
    monkeypatch.setattr(status_mod, "provider_label", lambda provider: "Medical Horizon Portal", raising=False)
    monkeypatch.setattr(
        status_mod,
        "get_nous_subscription_features",
        lambda config: NousSubscriptionFeatures(
            ...
            features={
                "web": NousFeatureState("web", "Web tools", True, True, True, True, False, True, "firecrawl"),
                "image_gen": NousFeatureState("image_gen", "Image generation", False, True, False, False, False, True, "local"),
                "tts": NousFeatureState("tts", "OpenAI TTS", False, True, False, False, False, True, "local"),
                "browser": NousFeatureState("browser", "Browser automation", False, True, False, False, False, True, "local"),
                "modal": NousFeatureState("modal", "Modal execution", False, True, False, False, False, True, "local"),
            },
        ),
        raising=False,
    ...
    out = capsys.readouterr().out
    assert "Medical Horizon Tool Gateway" in out
    assert "Medical Horizon Portal   ✓ managed tools available" in out
    assert "active via Medical Horizon subscription" in out
