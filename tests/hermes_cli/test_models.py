...
class TestModelIds:
    ...
    def test_anthropic_picker_includes_fable_5_first(self):
        from hermes_cli.models import _PROVIDER_MODELS, _fetch_anthropic_models, provider_model_ids

        assert _PROVIDER_MODELS["anthropic"][0] == "claude-fable-5"
        with patch("hermes_cli.models._fetch_anthropic_models", return_value=None):
            assert provider_model_ids("anthropic")[0] == "claude-fable-5"
        ...
        class _Resp:
            ...
            def read(self):
                return b'{"data":[{"id":"claude-opus-4-7"},{"id":"claude-fable-5"},{"id":"claude-sonnet-4-6"}]}'

        with patch("agent.anthropic_adapter.resolve_anthropic_token", return_value="sk-ant-test"), patch(
...
class TestFetchOpenRouterModels:
    ...
    def test_filters_out_models_without_tool_support(self, monkeypatch):
        ...
        sinria-agent is tool-calling-first — surfacing a non-tool model leads to
