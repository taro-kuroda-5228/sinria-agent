...
class TestSystemdServiceRefresh:
    ...
    def test_systemd_start_refreshes_outdated_unit(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(gateway_cli, "get_systemd_unit_path", lambda system=False: unit_path)
        monkeypatch.setattr(gateway_cli, "generate_systemd_unit", lambda system=False, run_as_user=None: "new unit\n")
        # On non-Linux dev machines (macOS) the user-scope path calls
        # _preflight_user_systemd(), which raises UserSystemdUnavailableError
        # because the user D-Bus socket is absent — short-circuiting before the
        # mocked subprocess.run. Stub the availability gate to a no-op, matching
        # the passing sibling tests, so we exercise the unit-refresh logic.
    ...
    def test_systemd_restart_refreshes_outdated_unit(self, tmp_path, monkeypatch):
        ...
        # See test_systemd_start_refreshes_outdated_unit: stub the user-systemd
        # availability gate so the macOS run reaches the unit-refresh path.
        monkeypatch.setattr(gateway_cli, "_preflight_user_systemd", lambda: None)
...
class TestGeneratedSystemdUnits:
    ...
    def test_sinria_systemd_units_use_sinria_runtime_identity(self, tmp_path, monkeypatch):
        sinria_home = tmp_path / ".sinria"
        monkeypatch.setenv("HERMES_CLI_NAME", "sinria")
        monkeypatch.setenv("HERMES_HOME", str(sinria_home))
        monkeypatch.setattr(gateway_cli.shutil, "which", lambda cmd: None)

        user_unit = gateway_cli.generate_systemd_unit(system=False)

        assert "sinria" in user_unit
        assert "Environment=\"SINRIA_HOME=" in user_unit
        assert "Environment=\"HERMES_HOME=" in user_unit  # legacy compatibility alias only
        assert "-m hermes_cli.main" not in user_unit
        assert "hermes gateway" not in user_unit
        assert "/.hermes" not in user_unit

    def test_sinria_system_service_remaps_default_home_to_dot_sinria(self, tmp_path, monkeypatch):
        sinria_home = tmp_path / ".sinria"
        monkeypatch.setenv("HERMES_CLI_NAME", "sinria")
        monkeypatch.setenv("HERMES_HOME", str(sinria_home))
        monkeypatch.setattr(
            gateway_cli,
            "_system_service_identity",
            lambda run_as_user=None: ("alice", "staff", str(tmp_path)),
        )
        monkeypatch.setattr(gateway_cli.shutil, "which", lambda cmd: None)

        unit = gateway_cli.generate_systemd_unit(system=True, run_as_user="alice")

        assert f"Environment=\"SINRIA_HOME={sinria_home}\"" in unit
        assert f"Environment=\"HERMES_HOME={sinria_home}\"" in unit
        assert "sinria" in unit
        assert "-m hermes_cli.main" not in unit
...
class TestLaunchdServiceRecovery:
    ...
    def test_launchd_plist_persists_sinria_identity_env(self, tmp_path, monkeypatch):
        sinria_home = tmp_path / ".sinria"
        monkeypatch.setenv("HERMES_CLI_NAME", "sinria")
        monkeypatch.setenv("HERMES_HOME", str(sinria_home))
        ...
        assert "<string>ai.sinria.gateway</string>" in plist
        assert "sinria" in plist
        assert "<string>hermes_cli.main</string>" not in plist
        assert "<string>hermes</string>" not in plist
        assert "<key>SINRIA_HOME</key>" in plist
        assert "<key>HERMES_HOME</key>" in plist
        assert f"<string>{sinria_home}</string>" in plist
        assert "<key>HERMES_CLI_NAME</key>" in plist
        assert "<string>sinria</string>" in plist
        assert "/.hermes" not in plist
        assert "<key>HERMES_DISABLE_ACTIVE_PROFILE</key>" in plist
        assert "<key>KeepAlive</key>" in plist
        assert "<key>SuccessfulExit</key>" not in plist
    ...
    def test_launchd_status_uses_sinria_start_hint(self, tmp_path, monkeypatch, capsys):
        plist_path = tmp_path / "ai.hermes.gateway.plist"
        plist_path.write_text("<plist>old content</plist>", encoding="utf-8")
        ...
        output = capsys.readouterr().out
        assert "Run: sinria gateway start" in output
...
class TestGatewaySystemServiceRouting:
    def test_systemd_restart_gracefully_restarts_running_service_and_waits(self, monkeypatch, capsys):
        ...
        monkeypatch.setattr(gateway_cli, "_select_systemd_scope", lambda system=False: False)
        monkeypatch.setattr(gateway_cli, "_require_service_installed", lambda action, system=False: None)
        # User-scope path: neutralize the macOS user-systemd D-Bus availability
        # gate so the restart routing logic under test is actually reached.
        monkeypatch.setattr(gateway_cli, "_preflight_user_systemd", lambda: None)
    ...
    def test_wait_for_systemd_restart_timeout_uses_sinria_status_hint(self, monkeypatch, capsys):
        monkeypatch.setenv("HERMES_CLI_NAME", "sinria")
        monkeypatch.setattr(
            gateway_cli,
            "_read_systemd_unit_properties",
        ...
        assert gateway_cli._wait_for_systemd_service_restart(previous_pid=777, timeout=-1) is False
        out = capsys.readouterr().out
        assert "Check status: sinria gateway status" in out
        assert "Check status: hermes gateway status" not in out
    ...
    def test_install_on_termux_uses_sinria_manual_run_hint(self, monkeypatch, capsys):
        ...
        out = capsys.readouterr().out
        assert "Run manually: sinria gateway run" in out
        assert "Run manually: hermes gateway" not in out
    ...
    def test_gateway_status_on_termux_uses_sinria_manual_guidance(self, monkeypatch, capsys):
        ...
        assert "nohup sinria gateway run" in out
        assert "hermes gateway run" not in out

    def test_process_mismatch_message_uses_sinria_gateway_prefix(self, monkeypatch, capsys):
        monkeypatch.setenv("HERMES_CLI_NAME", "sinria")
...
class TestProfileArg:
    ...
    def test_sinria_default_launchd_identity_does_not_collide_with_hermes(self, tmp_path, monkeypatch):
        machine_home = tmp_path / "machine-home"
        machine_home.mkdir()
        sinria_home = machine_home / ".sinria"
        sinria_home.mkdir()

        monkeypatch.setenv("HERMES_CLI_NAME", "sinria")
        monkeypatch.setenv("HERMES_HOME", str(sinria_home))
        monkeypatch.setattr(Path, "home", lambda: machine_home)
        monkeypatch.setattr(gateway_cli, "get_hermes_home", lambda: sinria_home)
        monkeypatch.setattr(pwd, "getpwuid", lambda uid: SimpleNamespace(pw_dir=str(machine_home)))

        assert gateway_cli.get_launchd_label() == "ai.sinria.gateway"
        assert gateway_cli.get_launchd_plist_path() == machine_home / "Library" / "LaunchAgents" / "ai.sinria.gateway.plist"
...
class TestRemapPathForUser:
    ...
    def test_remaps_path_under_current_home(self, monkeypatch, tmp_path):
        ...
        result = gateway_cli._remap_path_for_user(
            str(tmp_path / "root" / ".hermes" / "sinria-agent"),
            str(tmp_path / "alice"),
        )
        assert result == str(tmp_path / "alice" / ".hermes" / "sinria-agent")
    ...
    def test_noop_when_same_user(self, monkeypatch, tmp_path):
        ...
        original = str(tmp_path / "alice" / ".hermes" / "sinria-agent")
...
class TestSystemUnitPathRemapping:
    ...
    def test_system_unit_has_no_root_paths(self, monkeypatch, tmp_path):
        ...
        root_home.mkdir()
        project = root_home / ".hermes" / "sinria-agent"
        ...
        assert "WorkingDirectory=/home/alice/.hermes/sinria-agent" in unit
...
class TestDockerAwareGateway:
    ...
    def test_run_systemctl_raises_runtimeerror_when_missing(self, monkeypatch):
        ...
        with pytest.raises(RuntimeError, match="systemctl is not available"):
            gateway_cli._run_systemctl(["start", "sinria-gateway"])
    ...
    def test_run_systemctl_passes_through_on_success(self, monkeypatch):
        ...
        result = gateway_cli._run_systemctl(["status", "sinria-gateway"])
...
class TestLegacyHermesUnitDetection:
    ...
    renamed to ``sinria-gateway.service``. After PR #5646 (signal recovery
    ...
    (``sinria-gateway-<profile>.service``) or unrelated third-party services.
    """

    # Minimal ExecStart that looks like our gateway
    _OUR_UNIT_TEXT = (
        "[Unit]\nDescription=Sinria Gateway\n[Service]\n"
    ...
    def test_ignores_profile_unit_hermes_gateway_coder(self, tmp_path, monkeypatch):
        ...
        Teknium's concern — ``sinria-gateway-coder.service`` is our standard
        ...
        for base in (user_dir, system_dir):
            (base / "sinria-gateway-coder.service").write_text(
                self._OUR_UNIT_TEXT, encoding="utf-8"
            )
            (base / "sinria-gateway-orcha.service").write_text(
                self._OUR_UNIT_TEXT, encoding="utf-8"
            )
            (base / "sinria-gateway.service").write_text(
                self._OUR_UNIT_TEXT, encoding="utf-8"
            )
    ...
    def test_print_legacy_unit_warning_uses_sinria_hint(self, tmp_path, monkeypatch, capsys):
        ...
        assert "sinria gateway migrate-legacy" in out
        assert "hermes gateway migrate-legacy" not in out
...
class TestRemoveLegacyHermesUnits:
    ...
    def test_system_scope_without_root_defers_removal_with_sinria_hint(self, tmp_path, monkeypatch, capsys):
        ...
        out = capsys.readouterr().out
        assert "sudo sinria gateway migrate-legacy" in out
        assert "sudo hermes gateway migrate-legacy" not in out
    ...
    def test_does_not_touch_profile_units_during_migration(
        ...
        """Teknium's constraint: profile units (sinria-gateway-coder.service)
        ...
        profile_unit = user_dir / "sinria-gateway-coder.service"
        profile_unit.write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
        default_unit = user_dir / "sinria-gateway.service"
...
class TestSystemScopeWizardPreCheck:
    ...
    def _setup_units(tmp_path, monkeypatch, system_present: bool, user_present: bool):
        ...
        usr_dir.mkdir()
        if system_present:
            (sys_dir / "sinria-gateway.service").write_text("[Unit]\n")
        if user_present:
            (usr_dir / "sinria-gateway.service").write_text("[Unit]\n")
        monkeypatch.setattr(
            gateway_cli,
            "get_systemd_unit_path",
            lambda system=False: (sys_dir if system else usr_dir) / "sinria-gateway.service",
        )
...
class TestSystemScopeRemediationOutput:
    ...
    def test_start_remediation_uses_sinria_commands(self, capsys, monkeypatch):
        monkeypatch.setenv("HERMES_CLI_NAME", "sinria")
        monkeypatch.setattr(gateway_cli, "get_service_name", lambda: "sinria-gateway")
        ...
        assert "sudo sinria gateway uninstall --system" in out
        assert "sinria gateway install" in out
        assert "sinria gateway start" in out
        assert "sudo hermes gateway uninstall --system" not in out
        assert "sudo systemctl start sinria-gateway" in out
    ...
    def test_stop_remediation_uses_systemctl_stop(self, capsys, monkeypatch):
        ...
        assert "sudo systemctl stop sinria-gateway" in out
