...
def test_save_platform_tools_preserves_mcp_server_names():
    ...
    Regression test for https://github.com/NousResearch/sinria-agent/issues/1247
    """
    config = {
        "platform_toolsets": {
...
class TestPlatformToolsetConsistency:
    ...
    def test_gateway_toolset_includes_all_messaging_platforms(self):
        """sinria-gateway includes list should cover all messaging platforms."""
        from hermes_cli.tools_config import PLATFORMS
        from toolsets import TOOLSETS

        # Public-facing name is "Sinria", but the internal toolset key is
        # kept as "hermes-gateway" for backward compatibility (renaming it
        # would break existing configs/plugins that reference the key).
...
def test_numeric_mcp_server_name_does_not_crash_sorted():
    ...
    Regression test for https://github.com/NousResearch/sinria-agent/issues/6901
    """
    config = {
...
def test_reconfigure_provider_runs_post_setup_for_env_var_providers(
    ...
    # Cloud browser providers (Browser Use / Browserbase / Firecrawl) are no
    # longer hardcoded in TOOL_CATEGORIES["browser"]["providers"]; they are
    # injected at runtime via _plugin_browser_providers(). Source the row from
    # the same injection path (_visible_providers) the picker actually uses so
    # the test stays valid for both hardcoded (Camofox) and plugin providers.
    provider = next(
        p
