...
class TestDoctorMemoryProviderSection:
    ...
    def test_sinria_memory_and_skills_guidance_uses_sinria_commands(self, monkeypatch, tmp_path):
        monkeypatch.setenv("HERMES_CLI_NAME", "sinria")
        helper = TestDoctorMemoryProviderSection()

        def _missing_provider(_name):
            return None

        import plugins.memory as memory_mod
        monkeypatch.setattr(memory_mod, "load_memory_provider", _missing_provider)

        out = helper._run_doctor_and_capture(monkeypatch, tmp_path, provider="missing-provider")
        assert "run: sinria skills list" in out
        assert "run: sinria memory setup" in out
        assert "run: hermes skills list" not in out
        assert "run: hermes memory setup" not in out
...
class TestSinriaDoctorBranding:
    def test_ack_persist_failure_uses_sinria_home(self, monkeypatch):
        monkeypatch.setenv("HERMES_CLI_NAME", "sinria")
        monkeypatch.setattr(doctor_mod, "_DHH", "~/.sinria")

        fake_security = types.SimpleNamespace(
            ADVISORIES=[types.SimpleNamespace(id="ADV-1")],
            ack_advisory=lambda _ack: False,
        )
        monkeypatch.setitem(sys.modules, "hermes_cli.security_advisories", fake_security)

        buf = io.StringIO()
        with contextlib.redirect_stdout(buf), pytest.raises(SystemExit) as exc:
            doctor_mod.run_doctor(Namespace(fix=False, ack="ADV-1"))
        out = buf.getvalue()

        assert exc.value.code == 1
        assert "~/.sinria/config.yaml is writable" in out
        assert "~/.hermes/config.yaml is writable" not in out

    def test_security_advisory_summary_uses_sinria_ack_command(self, monkeypatch, tmp_path):
        home = tmp_path / ".hermes"
        home.mkdir(parents=True, exist_ok=True)
        monkeypatch.setenv("HERMES_HOME", str(home))
        monkeypatch.setenv("HERMES_CLI_NAME", "sinria")
        ...
        fake_hit = types.SimpleNamespace(
            ...
            advisory=types.SimpleNamespace(id="ADV-1", title="Critical issue"),
        )
        fake_security = types.SimpleNamespace(
            ADVISORIES=[fake_hit.advisory],
            ack_advisory=lambda _ack: True,
            detect_compromised=lambda: [fake_hit],
            filter_unacked=lambda hits: hits,
            full_remediation_text=lambda _hit: [],
            get_acked_ids=lambda: set(),
        )
        monkeypatch.setitem(sys.modules, "hermes_cli.security_advisories", fake_security)
        ...
        assert "`sinria doctor --ack ADV-1`" in out
        assert "`hermes doctor --ack ADV-1`" not in out
