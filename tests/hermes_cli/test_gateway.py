...
def _install_fake_gateway_run(monkeypatch, start_gateway):
    ...
    # /sinria-gateway.service"`` and runs ``systemctl --user daemon-reload``
...
def test_run_gateway_banner_uses_sinria_product_name(monkeypatch, capsys):
    calls = []
    ...
    out = capsys.readouterr().out
    assert calls == [(False, 0)]
    assert "Sinria Gateway Starting" in out
    assert "Hermes Gateway Starting" not in out
    assert "Messaging platforms + cron scheduler" in out
    assert "Press Ctrl+C to stop" in out
...
def test_run_gateway_refuses_root_in_official_docker(monkeypatch, tmp_path, capsys):
    ...
    # Pin the Sinria product name like production (sinria_cli/main.py sets
    # HERMES_CLI_NAME=sinria); without it _gateway_product_name() falls back
    # to "Hermes" and the asserted message text won't match.
    monkeypatch.setenv("HERMES_CLI_NAME", "sinria")
    ...
    assert exc_info.value.code == 1
    out = capsys.readouterr().out
    assert "Refusing to run the Sinria gateway as root" in out
    assert "/opt/hermes/docker/entrypoint.sh" in out


def test_run_gateway_root_guard_uses_sinria_product_name(monkeypatch, tmp_path, capsys):
    project_root = tmp_path / "opt" / "hermes"
    (project_root / "docker").mkdir(parents=True)
    (project_root / "docker" / "entrypoint.sh").write_text("#!/bin/sh\n")
    ...
    out = capsys.readouterr().out
    assert "Refusing to run the Sinria gateway as root" in out
    assert "Refusing to run the Hermes gateway as root" not in out
...
def test_systemd_install_checks_linger_status(monkeypatch, tmp_path, capsys):
    unit_path = tmp_path / "systemd" / "user" / "sinria-gateway.service"
...
def test_systemd_install_system_scope_skips_linger_and_uses_systemctl(monkeypatch, tmp_path, capsys):
    unit_path = tmp_path / "etc" / "systemd" / "system" / "sinria-gateway.service"
...
def test_conflicting_systemd_units_warning(monkeypatch, tmp_path, capsys):
    user_unit = tmp_path / "user" / "sinria-gateway.service"
    system_unit = tmp_path / "system" / "sinria-gateway.service"
    ...
    # otherwise the suggested command renders as "hermes gateway uninstall".
    monkeypatch.setenv("HERMES_CLI_NAME", "sinria")
    monkeypatch.setattr(
        gateway,
...
def test_install_linux_gateway_from_setup_system_choice_without_root_prints_followup(monkeypatch, capsys):
    ...
    assert "sudo sinria gateway install --system --run-as-user alice" in out
    assert "sudo sinria gateway start --system" in out



def test_sinria_scope_conflict_warning_uses_sinria_commands(monkeypatch, capsys, tmp_path):
    ...
    assert "hermes gateway uninstall" not in out
...
def test_sinria_gateway_service_definitions_pin_native_identity(monkeypatch, tmp_path):
    home = tmp_path / ".sinria"
    home.mkdir()
    monkeypatch.setenv("SINRIA_HOME", str(home))
    monkeypatch.setenv("SINRIA_CLI_NAME", "sinria")
    monkeypatch.setattr(gateway, "get_python_path", lambda: "/venv/bin/python")
    monkeypatch.setattr(gateway, "_detect_venv_dir", lambda: tmp_path / "venv")
    monkeypatch.setattr(gateway, "_build_service_path_dirs", lambda: ["/venv/bin"])
    ...
    assert f'Environment="SINRIA_HOME={home.resolve()}"' in unit
    assert 'Environment="SINRIA_CLI_NAME=sinria"' in unit
    assert f"<key>SINRIA_HOME</key>\n        <string>{home.resolve()}</string>" in plist
    assert "<key>SINRIA_CLI_NAME</key>\n        <string>sinria</string>" in plist


def test_sinria_gateway_process_scan_ignores_hermes_gateway(monkeypatch, tmp_path):
    home = tmp_path / ".sinria"
    home.mkdir()
    monkeypatch.setenv("SINRIA_HOME", str(home))
    monkeypatch.setenv("SINRIA_CLI_NAME", "sinria")
    ...
    ps_output = """101 HERMES_CLI_NAME=hermes HERMES_HOME=/tmp/.hermes /venv/bin/python -m hermes_cli.main gateway run --replace
102 SINRIA_CLI_NAME=sinria SINRIA_HOME={home} /venv/bin/python -m hermes_cli.main gateway run --replace
103 /usr/bin/python /Users/taro/bin/hermes gateway run
""".format(home=home.resolve())
    ...
    assert gateway._scan_gateway_pids(set()) == [102]
