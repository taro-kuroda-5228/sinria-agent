...
def test_list_groups_same_name_custom_providers_into_one_row(monkeypatch):
    ...
    monkeypatch.setattr("agent.models_dev.fetch_models_dev", lambda: {})
    monkeypatch.setattr(providers_mod, "HERMES_OVERLAYS", {})
    # Section 4 live-probes custom endpoints (even keyless ones like Ollama);
    # stub it so the configured model list — not a real localhost/ollama.com
    # /models response — is what we assert on.
...
def test_list_deduplicates_same_model_in_group(monkeypatch):
    ...
    monkeypatch.setattr(providers_mod, "HERMES_OVERLAYS", {})
    # Section 4 live-probes custom endpoints (even keyless ones like Ollama);
    # stub it so the configured model list is what we assert on.
    monkeypatch.setattr("hermes_cli.models.fetch_api_models", lambda *a, **k: None)
