"""Tests for Bug #12905 fix — stale OAuth token detection in model flow.

Bug 3: ``model`` with ``provider=anthropic`` skips OAuth re-authentication
when a stale ANTHROPIC_TOKEN exists in the runtime ``.env`` but no valid
