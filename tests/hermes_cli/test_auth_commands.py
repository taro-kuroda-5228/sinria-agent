...
def test_sinria_auth_status_missing_provider_guidance_uses_sinria_command(monkeypatch):
    ...
    assert "Example: `sinria auth status spotify`" in str(excinfo.value)
    assert "hermes auth status" not in str(excinfo.value)


def test_sinria_auth_add_unsupported_type_guidance_uses_sinria_command(monkeypatch):
    monkeypatch.setenv("HERMES_CLI_NAME", "sinria")
    ...
    with pytest.raises(SystemExit) as excinfo:
        auth_add_command(
            SimpleNamespace(provider="openrouter", auth_type="oauth", api_key=None, label=None)
        )

    assert "`sinria auth add openrouter` is not implemented" in str(excinfo.value)
    assert "`hermes auth add openrouter`" not in str(excinfo.value)
...
def test_auth_add_nous_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    # `auth add nous` must also populate providers.nous so the
...
def test_auth_add_nous_oauth_honors_custom_label(tmp_path, monkeypatch):
    """``auth add nous --type oauth --label <name>`` must preserve the
...
def test_auth_add_codex_clears_suppression_marker(tmp_path, monkeypatch):
    """Re-linking codex via ``auth add openai-codex`` must clear any suppression marker."""
    monkeypatch.setenv("HERMES_HOME", str(tmp_path / "hermes"))
    monkeypatch.setattr(
        "agent.credential_pool._seed_from_singletons",
        lambda provider, entries: (False, set()),
...
def test_auth_remove_env_seeded_suppresses_shell_exported_var(tmp_path, monkeypatch, capsys):
    """``auth remove xai 1`` must stick even when the env var is exported
    by the shell (not written into the runtime .env). Before PR for #13371 the
...
def test_auth_remove_env_seeded_dotenv_only_no_shell_hint(tmp_path, monkeypatch, capsys):
    """When the env var lives only in the runtime .env (not the shell), the
...
def test_auth_add_clears_env_suppression_for_provider(tmp_path, monkeypatch):
    """Re-adding a credential via ``auth add <provider>`` clears any
...
def test_auth_add_clears_all_suppressions_including_non_env(tmp_path, monkeypatch):
    """Re-adding a credential via ``auth add <provider>`` clears ALL
...
def test_auth_remove_codex_manual_device_code_suppresses_canonical(tmp_path, monkeypatch):
    """Removing a manual:device_code entry (from ``auth add openai-codex``)
