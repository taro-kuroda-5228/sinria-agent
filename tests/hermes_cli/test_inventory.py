...
def test_picker_hints_adds_warning_to_skeleton_rows():
    ...
    for row in skeleton_rows:
        ...
        # `sinria model` fallback.
        assert (
            row["warning"].startswith("paste ")
            or row["warning"].startswith("run `sinria model`")
        )
...
def test_picker_hints_use_sinria_model_command_for_non_api_key_providers(monkeypatch):
    """Bare Sinria should not tell users to run ``hermes model`` in
    local model-picker guidance."""
    ...
    non_api_key_rows = [
        ...
        if row.get("source") == "canonical" and row.get("auth_type") != "api_key"
    ]
    assert non_api_key_rows, "test setup: expected at least one OAuth/non-api-key provider"
    assert any(row["warning"].startswith("run `sinria model`") for row in non_api_key_rows)
    assert all("run `hermes model`" not in row["warning"] for row in non_api_key_rows)
