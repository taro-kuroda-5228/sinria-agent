...
def test_repeated_failures_spawn_hint_uses_sinria(monkeypatch):
    monkeypatch.setenv("HERMES_CLI_NAME", "sinria")
    task = _task(status="ready", assignee="debugger", consecutive_failures=3,
                 last_failure_error="Profile 'debugger' does not exist")
    runs = [
        _run(outcome="spawn_failed", run_id=1),
        _run(outcome="spawn_failed", run_id=2),
    ...
    diags = kd.compute_task_diagnostics(task, [], runs)
    actions = [a.label for a in diags[0].actions]
    assert any("sinria -p debugger doctor" in s for s in actions)
    assert all("hermes -p debugger doctor" not in s for s in actions)
...
def test_repeated_failures_timeout_hint_uses_sinria(monkeypatch):
    monkeypatch.setenv("HERMES_CLI_NAME", "sinria")
    task = _task(status="ready", consecutive_failures=3, last_failure_error="elapsed 600s > limit 300s")
    runs = [
        _run(outcome="timed_out", run_id=1),
        _run(outcome="timed_out", run_id=2),
        _run(outcome="timed_out", run_id=3),
    ...
    diags = kd.compute_task_diagnostics(task, [], runs)
    actions = [a.label for a in diags[0].actions]
    assert any("sinria kanban log t_demo00" in s for s in actions)
    assert all("hermes kanban log t_demo00" not in s for s in actions)
...
def test_repeated_crashes_hint_uses_sinria(monkeypatch):
    monkeypatch.setenv("HERMES_CLI_NAME", "sinria")
    task = _task(status="ready", assignee="crashy")
    runs = [
        _run(outcome="completed", run_id=1),
