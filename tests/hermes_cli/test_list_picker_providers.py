...
def test_current_custom_endpoint_passthrough_marks_current_row(monkeypatch):
    ...
    monkeypatch.setattr("hermes_cli.models.fetch_openrouter_models",
                        lambda *a, **kw: [])
    # Section 4 live-probes custom endpoints (here the configured Ollama
    # localhost URL); stub it so the configured model list — not a real
    # /models response from a running Ollama — is what we assert on.
