...
class TestTestCommand:
    def test_sinria_test_failure_hint_uses_sinria_gateway_run(self, capsys, monkeypatch):
        monkeypatch.setenv("HERMES_CLI_NAME", "sinria")
        webhook_command(_make_args(webhook_action="subscribe", name="t"))
        capsys.readouterr()
        ...
        webhook_command(_make_args(webhook_action="test", name="t"))
        out = capsys.readouterr().out
...
class TestWebhookEnabledGate:
    ...
    def test_sinria_subscribe_guidance_uses_sinria_gateway_run(self, capsys, monkeypatch):
        monkeypatch.setenv("HERMES_CLI_NAME", "sinria")
        webhook_command(_make_args(webhook_action="subscribe", name="allowed-sinria"))
        out = capsys.readouterr().out
        assert "sinria gateway run" in out
        assert "hermes gateway run" not in out
