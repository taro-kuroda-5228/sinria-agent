...
def test_claw_command_usage_uses_sinria(monkeypatch, capsys):
    monkeypatch.setenv("HERMES_CLI_NAME", "sinria")
    claw_mod.claw_command(Namespace(claw_action=None))
    out = capsys.readouterr().out
    assert "Usage: sinria claw <command> [options]" in out
    assert "Run 'sinria claw <command> --help' for options." in out
    assert "Migrate settings from OpenClaw to Sinria" in out
    assert "Usage: hermes claw <command> [options]" not in out



def test_claw_preview_hint_uses_sinria(monkeypatch, capsys):
    monkeypatch.setenv("HERMES_CLI_NAME", "sinria")
    claw_mod._print_migration_report({"preset": "full", "items": []}, dry_run=True)
    out = capsys.readouterr().out
    assert "sinria claw migrate --preset full" in out
    assert "hermes claw migrate --preset full" not in out



def test_claw_backup_restore_hint_uses_sinria(monkeypatch, tmp_path, capsys):
    monkeypatch.setenv("HERMES_CLI_NAME", "sinria")
    ...
    class _FakeMigrator:
        ...
        def preview(self):
            return {"preset": "full", "items": []}
        def migrate(self):
            if not self.execute:
                return {"preset": "full", "summary": {"migrated": 1}, "items": [{"kind": "soul", "status": "migrated", "destination": "/tmp/SOUL.md"}]}
            return {"preset": "full", "summary": {"migrated": 1}, "items": []}

    fake_mod = ModuleType("openclaw_to_hermes")
    fake_mod.Migrator = _FakeMigrator
    fake_mod.resolve_selected_options = lambda *_args, **_kwargs: None

    args = Namespace(
        source=str(tmp_path / "source"), preset="full", overwrite=False,
        migrate_secrets=False, no_backup=False, yes=True, dry_run=False,
        workspace_target="profiles", skill_conflict="preserve", selected=None,
    )
    Path(args.source).mkdir()

    with patch.object(claw_mod, "_find_migration_script", return_value=tmp_path / "script.py"), \
         patch.object(claw_mod, "_load_migration_module", return_value=fake_mod), \
         patch.object(claw_mod, "_warn_if_openclaw_running", return_value=False), \
         patch.object(claw_mod, "_warn_if_gateway_running", return_value=False), \
         patch("hermes_cli.backup.create_pre_migration_backup", return_value=archive), \
         patch("hermes_cli.backup._format_size", return_value="1 B"):
        claw_mod._cmd_migrate(args)

    out = capsys.readouterr().out
    assert "Restore with: sinria import pre-migration.zip" in out
    assert "Restore with: hermes import pre-migration.zip" not in out
