...
def _make_hermes_tree(root: Path) -> None:
    ...
    # sinria-agent repo (should be EXCLUDED)
    (root / "sinria-agent").mkdir(exist_ok=True)
    (root / "sinria-agent" / "run_agent.py").write_text("# big file\n")
    (root / "sinria-agent" / ".git").mkdir()
    (root / "sinria-agent" / ".git" / "HEAD").write_text("ref: refs/heads/main\n")
...
class TestShouldExclude:
    def test_excludes_hermes_agent(self):
        ...
        assert _should_exclude(Path("sinria-agent/run_agent.py"))
        assert _should_exclude(Path("sinria-agent/.git/HEAD"))
...
class TestBackup:
    ...
    def test_excludes_hermes_agent(self, tmp_path, monkeypatch):
        """Backup does NOT include sinria-agent/ directory."""
        hermes_home = tmp_path / ".hermes"
        hermes_home.mkdir()
        _make_hermes_tree(hermes_home)
        ...
        with zipfile.ZipFile(out_zip, "r") as zf:
            names = zf.namelist()
            agent_files = [n for n in names if "sinria-agent" in n]
            assert agent_files == [], f"sinria-agent files leaked into backup: {agent_files}"
    ...
    def test_default_output_path_uses_sinria_name(self, tmp_path, monkeypatch):
        hermes_home = tmp_path / ".hermes"
        hermes_home.mkdir()
        (hermes_home / "config.yaml").write_text("model: test\n")
        ...
        assert len(list(tmp_path.glob("sinria-backup-*.zip"))) == 1
        assert len(list(tmp_path.glob("hermes-backup-*.zip"))) == 0
...
class TestValidateBackupZip:
    ...
    def test_invalid_backup_reason_uses_sinria_identity(self, tmp_path, monkeypatch):
        monkeypatch.setenv("HERMES_CLI_NAME", "sinria")
        from hermes_cli.backup import _validate_backup_zip
        zip_path = tmp_path / "old.zip"
        self._make_zip(zip_path, ["hermes_state.db", "memory_store.db"])
        with zipfile.ZipFile(zip_path, "r") as zf:
            ok, reason = _validate_backup_zip(zf)
        assert not ok
        assert "Sinria backup" in reason
        assert "Hermes backup" not in reason
...
class TestRoundTrip:
    def test_backup_then_import(self, tmp_path, monkeypatch):
        ...
        # sinria-agent should NOT be present
        assert not (dst_home / "sinria-agent").exists()
...
class TestProfileRestoration:
    ...
    def test_import_uses_sinria_restore_guidance(self, tmp_path, monkeypatch, capsys):
        hermes_home = tmp_path / ".hermes"
        hermes_home.mkdir()
        monkeypatch.setenv("HERMES_HOME", str(hermes_home))
        monkeypatch.setenv("HERMES_CLI_NAME", "sinria")
        ...
        assert "sinria-agent codebase was not included in the backup" in out
        assert "sinria update" in out
        assert "sinria -p coder gateway install" in out
        assert "Done. Your Sinria configuration has been restored." in out
        assert "hermes-agent codebase was not included in the backup" not in out
        assert "hermes update" not in out
        assert "hermes -p coder gateway install" not in out
        assert "Done. Your Hermes configuration has been restored." not in out
...
class TestPreUpdateBackup:
    ...
    def test_backup_contents_match_full_backup(self, hermes_home):
        ...
        assert "skills/my-skill/SKILL.md" in names
        assert "profiles/coder/config.yaml" in names
        # sinria-agent repo excluded
        assert not any(n.startswith("sinria-agent/") for n in names)
