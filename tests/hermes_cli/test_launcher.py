"""Tests for the top-level `./sinria` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./sinria` should use `hermes_cli.main`, not the legacy Fire wrapper.

    The root launcher was renamed `./hermes` -> `./sinria`; it now routes
    through `sinria_cli.main`, which in turn delegates to the shared argparse
    entrypoint `hermes_cli.main` (stubbed below). The legacy Fire wrapper
    (`cli.main` / `fire.Fire`) must never be touched.
    """
    launcher_path = Path(__file__).resolve().parents[2] / "sinria"
    called = []
