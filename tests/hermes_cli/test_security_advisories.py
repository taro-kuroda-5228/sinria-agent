...
class TestRendering:
    ...
    def test_gateway_log_message_plural_uses_runtime_cli_name(self, fake_advisory, monkeypatch):
        monkeypatch.setattr(adv, "get_acked_ids", lambda: set())
        monkeypatch.setenv("HERMES_CLI_NAME", "sinria")
        hit1 = adv.AdvisoryHit(
            advisory=fake_advisory,
            package="fake-malicious-pkg",
            installed_version="6.6.6",
        )
        hit2 = adv.AdvisoryHit(
            advisory=adv.Advisory(
                id="ADV-2",
                title="Another advisory",
                summary="Summary",
                url="https://example.invalid/adv-2",
                remediation=("step",),
                published="2026-01-01",
                severity="high",
                compromised=(("other-pkg", frozenset({"1.2.3"})),),
            ),
            package="other-pkg",
            installed_version="1.2.3",
        )
        msg = adv.gateway_log_message([hit1, hit2])
        assert msg is not None
        assert "Run `sinria doctor` on the gateway host for details." in msg
        assert "Run `hermes doctor`" not in msg
