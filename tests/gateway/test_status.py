...
def _clear_native_sinria_env_for_legacy_status_tests(monkeypatch):
    """Keep legacy HERMES_HOME-based tests hermetic under a live Sinria shell.

    The user's real Sinria launcher exports SINRIA_HOME, which correctly wins
    over HERMES_HOME in production. Most older tests in this module only set
    HERMES_HOME, so clear native env by default and let Sinria-specific tests
    opt back in explicitly.
    """
    monkeypatch.delenv("SINRIA_HOME", raising=False)
    monkeypatch.delenv("SINRIA_CLI_NAME", raising=False)
    monkeypatch.delenv("SINRIA_GATEWAY_LOCK_DIR", raising=False)


class TestGatewayPidState:
    def test_write_pid_file_records_sinria_product_metadata(self, tmp_path, monkeypatch):
        ...
        assert payload["kind"] == "sinria-gateway"
        assert payload["product"] == "sinria"
        assert payload["cli_name"] == "sinria"
        assert payload["home"] == str(tmp_path)

    def test_lock_dir_prefers_sinria_native_override(self, tmp_path, monkeypatch):
        sinria_lock_dir = tmp_path / "sinria-locks"
        hermes_lock_dir = tmp_path / "hermes-locks"
        monkeypatch.setenv("SINRIA_GATEWAY_LOCK_DIR", str(sinria_lock_dir))
        monkeypatch.setenv("HERMES_GATEWAY_LOCK_DIR", str(hermes_lock_dir))

        assert status._get_lock_dir() == sinria_lock_dir

    def test_default_lock_dir_uses_sinria_state_namespace(self, tmp_path, monkeypatch):
        monkeypatch.setenv("SINRIA_CLI_NAME", "sinria")
        monkeypatch.setenv("XDG_STATE_HOME", str(tmp_path))
        monkeypatch.delenv("SINRIA_GATEWAY_LOCK_DIR", raising=False)
        monkeypatch.delenv("HERMES_GATEWAY_LOCK_DIR", raising=False)

        assert status._get_lock_dir() == tmp_path / "sinria" / "gateway-locks"

    def test_sinria_runtime_rejects_explicit_hermes_gateway_cmdline(self, monkeypatch):
        monkeypatch.setenv("SINRIA_CLI_NAME", "sinria")
        monkeypatch.setattr(
            status,
            "_read_process_cmdline",
            lambda pid: "HERMES_CLI_NAME=hermes /usr/local/bin/hermes gateway run --replace",
        )

        assert status._looks_like_gateway_process(12345) is False

    def test_sinria_runtime_accepts_unmarked_shared_module_for_legacy_compat(self, monkeypatch):
        monkeypatch.setenv("SINRIA_CLI_NAME", "sinria")
        monkeypatch.setattr(
            status,
            "_read_process_cmdline",
            lambda pid: "/venv/bin/python -m hermes_cli.main gateway run --replace",
        )

        assert status._looks_like_gateway_process(12345) is True

    def test_sinria_runtime_rejects_hermes_pid_record_metadata(self, monkeypatch):
        monkeypatch.setenv("SINRIA_CLI_NAME", "sinria")

        assert status._record_looks_like_gateway({
            "kind": "sinria-gateway",
            "product": "hermes",
...
class TestScopedLocks:
    ...
    def test_acquire_scoped_lock_replaces_pid_reused_by_unrelated_process(self, tmp_path, monkeypatch):
        ...
        lock_path.write_text(json.dumps({
            ...
            "start_time": None,
            "kind": "sinria-gateway",
            "argv": ["/Users/user/.hermes/sinria-agent/hermes_cli/main.py", "gateway", "run", "--replace"],
        }))
