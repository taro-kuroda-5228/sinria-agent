...
class TestSinriaRetentionPolicy:
    def test_append_to_transcript_skips_legacy_jsonl_when_policy_disables_raw_history(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            lambda: {
                "sinria": {
                    "policy": {
                        "active_profile": "minimal_local_history",
                        "profiles": {
                            "minimal_local_history": {
        ...
        store.append_to_transcript(sid, {"role": "user", "content": "sensitive hello"})

        assert store.get_transcript_path(sid).exists() is False
        db_messages = store._db.get_messages_as_conversation(sid)
        assert len(db_messages) == 1
        assert db_messages[0]["content"] == "sensitive hello"
