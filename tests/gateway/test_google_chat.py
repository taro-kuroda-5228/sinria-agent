...
def _install_fake_aiohttp(monkeypatch, session):
    # _standalone_send calls ClientSession(timeout=..., trust_env=True); accept
    # and ignore any kwargs (trust_env, connector, ...) so the fake stays in
    # lockstep with the production call signature.
    fake_aiohttp = types.SimpleNamespace(
        ClientSession=lambda timeout=None, **_kwargs: session,
