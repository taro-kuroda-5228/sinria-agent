...
# The comprehensive discord mock — including app_commands.Group/Command and
# the autocomplete decorator this module's flat /skill command needs — is
# installed once at collection time by tests/gateway/conftest.py. A per-file
# stub here used to overwrite that shared mock (``sys.modules["discord"] = …``)
# and break sibling discord test modules when run together. Rely on the shared
# conftest mock instead.
