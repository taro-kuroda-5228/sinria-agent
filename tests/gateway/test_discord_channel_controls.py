...
# The comprehensive discord mock is installed once, at collection time, by
# tests/gateway/conftest.py::_ensure_discord_mock (overwrite semantics). A
# per-file ``setdefault`` stub used to fight that shared mock when several
# discord test modules ran together — the first file's narrower stub would win
# and later files would import a discord missing attributes they need. Rely on
# the shared conftest mock instead.
