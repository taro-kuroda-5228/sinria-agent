...
def test_stop_is_idempotent_and_no_ticks_follow(caplog):
    caplog.set_level(logging.INFO, logger="gateway.memory_monitor")
    mm.start_memory_monitoring(interval_seconds=0.01)
    time.sleep(0.04)

    mm.stop_memory_monitoring(timeout=1.0)
    stopped_count = len(caplog.records)
    mm.stop_memory_monitoring(timeout=1.0)
    time.sleep(0.04)

    messages = [r.getMessage() for r in caplog.records]
    assert len(messages) == stopped_count
    assert sum("Periodic memory monitoring stopped" in m for m in messages) == 1
    assert mm.is_running() is False


def test_join_timeout_keeps_live_thread_visible(caplog):
    class _StuckThread:
        name = "stuck-memory-monitor"
        alive = True
    ...
    thread = _StuckThread()
    mm._monitor_thread = thread
    mm._stop_event = __import__("threading").Event()
    caplog.set_level(logging.WARNING, logger="gateway.memory_monitor")

    mm.stop_memory_monitoring(timeout=0.01)

    assert mm.is_running() is True
    assert any("did not stop" in r.getMessage() for r in caplog.records)

    # Let the fixture's finalizer observe a completed thread and clear state.
    thread.alive = False
    mm.stop_memory_monitoring(timeout=0.01)


def test_concurrent_stop_logs_one_shutdown(caplog):
    import threading
...
def test_periodic_timer_fires(caplog):
    caplog.set_level(logging.INFO, logger="gateway.memory_monitor")
    # Short interval so we can observe multiple ticks. Under xdist on busy
    # developer machines the timer thread can be delayed, so poll up to a
    # bounded deadline instead of assuming three ticks happen inside 450 ms.
