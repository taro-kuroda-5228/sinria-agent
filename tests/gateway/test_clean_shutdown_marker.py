...
class TestCleanShutdownMarker:
    ...
    def test_signal_shutdown_caps_drain_and_marks_resume_pending(self, tmp_path, monkeypatch):
        """External SIGTERM should restart quickly instead of leaving Sinria stopped."""
        monkeypatch.setattr("gateway.run._hermes_home", tmp_path)
        monkeypatch.delenv("HERMES_SIGNAL_SHUTDOWN_DRAIN_TIMEOUT", raising=False)
        ...
        runner._draining = False
        runner._stop_task = None
        runner._running_agents = {"agent:main:telegram:chat:123:u1": MagicMock()}
        runner._running_agents_ts = {}
        runner._pending_messages = {}
        runner._pending_approvals = {}
        runner._background_tasks = set()
        runner._shutdown_event = MagicMock()
        runner._restart_drain_timeout = 180
        runner._exit_code = None
        runner._exit_reason = None
        runner.adapters = {}
        runner.config = GatewayConfig()
        ...
        drain = AsyncMock(return_value=(dict(runner._running_agents), False))
        with patch("gateway.run.GatewayRunner._drain_active_agents", drain), \
             patch("gateway.run.GatewayRunner._finalize_shutdown_agents"), \
             patch("gateway.run.GatewayRunner._update_runtime_status"), \
             patch("gateway.status.remove_pid_file"), \
             patch("tools.process_registry.process_registry") as mock_proc_reg, \
        ...
        runner.session_store.mark_resume_pending.assert_called_once_with(
            "agent:main:telegram:chat:123:u1", "shutdown_timeout"
        )
        drain.assert_awaited_once_with(15.0)
