...
Regression test for https://github.com/NousResearch/sinria-agent/issues/18594.
...
class TestGetHermesHomeProfileWarning:
    def test_classic_mode_no_active_profile_no_warning(
        ...
        assert "SINRIA_HOME fallback" not in capsys.readouterr().err
    ...
    def test_named_profile_unset_home_warns_once(
        ...
        assert err.count("SINRIA_HOME fallback") == 1
        assert "'coder'" in err
        # The rebranded message dropped the bare "#18594" issue tag but still
        # spells out the contamination risk and the remedy (pass SINRIA_HOME
        # explicitly). Pin that diagnostic guidance instead of the issue number.
        assert "wrong profile" in err
        assert "SINRIA_HOME explicitly" in err

        # 3. One-shot: second and third calls don't re-warn
        fresh_constants.get_hermes_home()
        fresh_constants.get_hermes_home()
        err2 = capsys.readouterr().err
        assert "SINRIA_HOME fallback" not in err2
