"""Tests for ACP Registry metadata shipped with Sinria."""
...
def test_agent_json_matches_official_registry_required_fields():
    ...
    assert data["id"] == "sinria-agent"
    assert re.fullmatch(r"[a-z][a-z0-9-]*", data["id"])
    assert data["name"] == "Sinria"
    assert data["description"]
    assert data["repository"] == "https://github.com/taro-kuroda-5228/sinria-agent"
    assert data["website"] == "https://taro-kuroda-5228.github.io/sinria-agent/docs"
    assert data["authors"] == ["Medical Horizon"]
...
def test_agent_json_uses_uvx_distribution_without_local_command_fields():
    ...
    assert uvx["package"] == f"sinria-agent[acp]=={data['version']}"
    assert uvx["args"] == ["sinria-acp"]
...
def test_agent_json_pins_uvx_package_to_pyproject_version():
    ...
    assert _manifest()["distribution"]["uvx"]["package"] == (
        f"sinria-agent[acp]=={_pyproject_version()}"
    )
