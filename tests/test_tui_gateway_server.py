...
def test_browser_manage_connect_default_local_reports_launch_hint(monkeypatch):
    ...
    with patch.dict(sys.modules, {"tools.browser_tool": fake}):
        ...
        with (
            ...
            # Force the "no executable found" hint path deterministically.
            # get_chrome_debug_candidates=[] alone is not enough: on macOS,
            # manual_chrome_debug_command() falls back to an `open -a "Google
            # Chrome"` command (Darwin branch), so the "No Chrome/Chromium
            # executable" message would never appear. Mock the command builder
            # to None so the hint path is platform-independent.
            patch(
