"""Shared fixtures for the sinria-agent test suite.
...
_HERMES_BEHAVIORAL_VARS = frozenset({
    ...
    # a config.yaml is present (gateway/config.py writes these via
    # ``os.environ[...] = ...``, which monkeypatch can't undo), so individual
    # test bodies that construct GatewayConfig()/load the config leak these
    # values into later tests on the same xdist worker. This bit a developer
    # machine whose real config.yaml set discord.allowed_channels: a stray
    # DISCORD_ALLOWED_CHANNELS=<real channel id> survived into
    # test_discord_channel_controls and made every "channel processes
    # normally" assertion fail (the test channel wasn't in the whitelist).
    # Force-clear every config→env bridge key on each test setup so the leak
    # can't happen. Keep this list in sync with the os.environ[...] writes in
    # gateway/config.py.
    ...
    "FEISHU_ALLOW_BOTS",
    # API server bridge/env overrides — load_gateway_config() and adapter
    # construction read these directly. Real developer profiles commonly set
    # API_SERVER_ENABLED/MODEL_NAME (e.g. OpenAI-compatible local endpoint),
    # which must not leak into unrelated tests or flip config/model assertions.
    "API_SERVER_ENABLED",
    "API_SERVER_HOST",
    "API_SERVER_PORT",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    # 2. Blank behavioral HERMES_* / SINRIA_* vars that could change test semantics.
    for name in _HERMES_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)
    for name in (
        "SINRIA_HOME",
        # CLI-identity markers. The Sinria product entrypoint
        # (sinria_cli.main._pin_sinria_runtime_env) sets these via raw
        # os.environ.setdefault, NOT monkeypatch — so a test that exercises
        # that entrypoint (test_sinria_cli_entrypoint) leaks
        # HERMES_CLI_NAME=sinria / SINRIA_CLI_NAME=sinria /
        # HERMES_DISABLE_ACTIVE_PROFILE=1 into the worker process. Every
        # later branding/identity test in that worker then resolves the CLI
        # name as "sinria" instead of the default "hermes"
        # (recommended_update_command, _product_name, status/oauth/skin
        # wording, …) and flakes depending on xdist worker assignment.
        # Clear them per test; tests that need a Sinria identity set it
        # explicitly via monkeypatch.setenv.
        "HERMES_CLI_NAME",
        "SINRIA_CLI_NAME",
        "HERMES_DISABLE_ACTIVE_PROFILE",
        "SINRIA_GATEWAY_LOCK_DIR",
        "SINRIA_DISABLE_ACTIVE_PROFILE",
    ):
        monkeypatch.delenv(name, raising=False)
    ...
    # 3b. Repoint hermes_state.DEFAULT_DB_PATH at the per-test home. It is a
    #     module-level constant frozen at import time
    #     (DEFAULT_DB_PATH = get_hermes_home() / "state.db"), and SessionDB()
    #     defaults to it. Without this, every test that opens a SessionDB with
    #     no explicit path (goal store, session persistence) shares ONE state.db
    #     — whichever HERMES_HOME was active when hermes_state was first
    #     imported — so rows written by one test leak into another on the same
    #     xdist worker. Only patch when already imported; don't force the import.
    ...
    # 4c. Disable the external tirith security binary. tools.tirith_security
    #     resolves a `tirith` binary (from PATH or $HERMES_HOME/bin, else
    #     auto-downloads it from GitHub releases) and runs it as a real
    #     subprocess on EVERY command passed to check_all_command_guards().
    #     In a hermetic test run that is both non-deterministic and unwanted:
    #       * with no binary it attempts a network download (HERMES_HOME is a
    #         fresh tempdir per test → never cached);
    #       * with a dev-machine binary on PATH it spawns a subprocess whose
    #         startup latency occasionally exceeds a test's readiness budget,
    #         flaking the gateway approval E2E tests (the agent thread reaches
    #         its notify callback late and `notified` is still empty).
    #     check_command_security() short-circuits to "allow" when disabled, so
    #     the deterministic pattern-matching guards still run. Tests that
    #     exercise tirith itself patch tools.tirith_security._load_security_config
    #     directly and are unaffected by this env var.
...
@pytest.fixture(autouse=True)
def _scrub_leaked_credential_env(_hermetic_environment):
    """Delete credential-shaped env vars written RAW during a test.

    _hermetic_environment blanks credential vars at setup via
    monkeypatch.delenv — but monkeypatch RESTORES whatever value was present
    when it deleted. So if test A injects a credential straight into
    os.environ (e.g. cli.py / gateway/run.py copy a task config's
    ``api_key`` into ``os.environ[XAI_API_KEY]`` while exercising runtime
    credential inheritance), that raw value is invisible to monkeypatch and
    survives into later tests on the same xdist worker. Code like
    transcription's _get_provider()/get_env_value() then sees a stray
    XAI_API_KEY in os.environ and resolves it instead of the patched .env,
    flaking depending on worker assignment.

    This finalizer snapshots the credential keys present *after* hermetic
    setup (the clean baseline) and removes any credential-shaped key that
    appeared during the test, so raw writes cannot leak forward. It runs as
    a teardown only — it never touches env vars a test set via monkeypatch
    (those are restored independently) nor the baseline keys.
    """
    baseline = {name for name in os.environ if _looks_like_credential(name)}
    yield
    for name in [n for n in os.environ if _looks_like_credential(n)]:
        if name not in baseline:
            os.environ.pop(name, None)


# Modules that some tests importlib.reload() to re-read HERMES_CLI_NAME / env at
# import. reload() re-executes the module body in place, rebinding every global —
# including per-import secrets (web_server._SESSION_TOKEN) and functions other
# modules imported by reference (hermes_cli.config.get_env_value /load_env used by
# tools.xai_http; hermes_constants home helpers used by hermes_cli.goals). On a
# long-lived xdist worker that silently corrupts later tests (401s from token
# mismatch, dotenv lookups bypassing patched load_env, goal/home resolution
# reading stale state). We snapshot each module's __dict__ before every test and
# restore it after, so a reload's effects never escape the test that did it.
...
@pytest.fixture(autouse=True)
def _contain_module_reloads():
    """Revert importlib.reload() leakage of widely-imported shared modules."""
    saved = []
    for name in _RELOAD_GUARDED_MODULES:
        mod = sys.modules.get(name)
        if mod is not None:
            saved.append((mod, dict(mod.__dict__)))
    try:
        yield
    finally:
        for mod, original_dict in saved:
            if mod.__dict__ != original_dict:
                mod.__dict__.clear()
...
def _reset_module_state():
    ...
    # --- cli.CLI_CONFIG["delegation"] — frozen-at-import developer config ---
    # cli.CLI_CONFIG is computed ONCE when cli.py is first imported, using
    # whatever HERMES_HOME was active at that moment. If cli first imports
    # while the real ~/.hermes/config.yaml is visible, CLI_CONFIG freezes with
    # the developer's real `delegation` block (e.g. a local Ollama endpoint).
    # tools.delegate_tool._load_config() reads cli.CLI_CONFIG["delegation"]
    # FIRST, so that stale block leaks into every delegate_task() in the
    # worker — making test_child_inherits_runtime_credentials see Ollama creds
    # instead of inheriting from the parent. Drop the delegation override per
    # test; tests that need one set it explicitly via patch.
    try:
        import cli as _cli_mod
        if isinstance(getattr(_cli_mod, "CLI_CONFIG", None), dict):
            _cli_mod.CLI_CONFIG["delegation"] = {}
    except Exception:
        pass

    # --- hermes_cli.config._LOAD_CONFIG_CACHE — path-keyed config cache ---
    # The _load_config() fallback path calls hermes_cli.config.load_config(),
    # which caches per config-path. Clear it so a real-config load from one
    # test can't satisfy a later test's lookup before HERMES_HOME isolation
    # is re-applied.
    #
    # _env_cache is the sibling .env (dotenv) memoisation behind
    # get_env_value(); it is keyed on (env_path, mtime, size). A prior test
    # that calls the real load_env() under one HERMES_HOME caches that .env's
    # contents, and code reading get_env_value() in a later test can be served
    # the stale dict before HERMES_HOME isolation re-applies. Invalidate it too.
    try:
        from hermes_cli import config as _hconfig_mod
        _hconfig_mod._LOAD_CONFIG_CACHE.clear()
        _hconfig_mod._RAW_CONFIG_CACHE.clear()
        _hconfig_mod._env_cache = None
    except Exception:
        pass

    # --- hermes_cli.goals._DB_CACHE — per-HERMES_HOME goal DB handles ---
    # get_db() memoises a goal-store SQLite handle keyed by str(get_hermes_home()).
    # If a test sets a goal and a later test on the same worker resolves the same
    # home string (e.g. before its own HERMES_HOME isolation re-applies, or after
    # a module reload perturbs home resolution), it can read the prior test's
    # goal — making "/goal status shows none" assertions see a stale goal.
    # Drop the cached handles between tests.
    try:
        from hermes_cli import goals as _goals_mod
        _goals_mod._DB_CACHE.clear()
    except Exception:
        pass
...
# environment and finds the developer's live ``sinria-gateway`` process
...
#    ``check_output`` reject any ``systemctl ... <verb> sinria-gateway``
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart sinria-gateway"``,
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live sinria-gateway systemd unit. Mock "
