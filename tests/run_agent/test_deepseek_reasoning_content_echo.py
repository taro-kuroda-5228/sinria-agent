...
class TestNeedsKimiToolReasoning:
    ...
    def test_non_kimi_provider(self) -> None:
        ...
        # The model name contains 'kimi', so the model-substring rule fires
        # even though the host is openrouter. This is intentional (#15748):
        # Kimi models served through third-party providers (openrouter,
        # ollama-cloud, ...) still require reasoning_content on tool-call
        # turns; failing to pad it leaks a prior provider's chain of thought
        # and trips an HTTP 400 on the next replay. So this must be True.
        assert agent._needs_kimi_tool_reasoning() is True
