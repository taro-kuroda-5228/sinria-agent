...
def test_sinria_agent_init_missing_provider_guidance_uses_sinria_command(monkeypatch):
    ...
    with (
        ...
        AIAgent(quiet_mode=True, skip_context_files=True, skip_memory=True)

    message = str(excinfo.value)
    assert "Run `sinria model`" in message
    assert "run `sinria setup`" in message
...
class TestBuildApiKwargs:
    def test_basic_kwargs(self, agent):
        ...
        # The Sinria egress boundary (redact_model_provider_messages) returns a
        # redacted *copy* of the messages, so the result is equal but no longer
        # the same object — compare by value, not identity.
...
class TestRetryExhaustion:
    ...
    def test_api_error_returns_gracefully_after_retries(self, agent):
        ...
        assert "What happened" in result["final_response"]
        assert "What I already tried" in result["final_response"]
        assert "What you can do next" in result["final_response"]

    def test_sinria_model_provider_egress_block_reports_diagnosis_and_alternatives(self, agent):
        """Sinria egress guard blocks must be explained instead of looking like a shutdown."""
        self._setup_agent(agent)
        agent.provider = "openai-codex"
        agent.model = "gpt-5.1-codex-max"
        agent.client.chat.completions.create.side_effect = RuntimeError(
            "Sinria external egress guard: ask for model_provider"
        )
        with (
            patch.object(agent, "_persist_session"),
            patch.object(agent, "_save_trajectory"),
            ...
            result = agent.run_conversation("Sinria設計相談")
        assert result.get("completed") is False
        assert result.get("failed") is True
        final = result.get("final_response") or ""
        assert "Sinria external egress guard" in final
        assert "model_provider" in final
        assert "trusted cloud model" in final
        assert "local/internal model" in final
        assert "redact" in final
        assert "gateway is still running" in final
        assert "Risk assessment" in final
        assert "Safe recovery choices" in final
        assert "not an intentional shutdown" in final
...
class TestSystemPromptStability:
    ...
    def test_fresh_build_when_no_history(self, agent):
        ...
        assert "Sinria" in agent._cached_system_prompt
