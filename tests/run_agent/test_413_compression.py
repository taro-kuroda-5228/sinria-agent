...
class TestHTTP413Compression:
    ...
    def test_context_length_retry_rebuilds_request_after_compression(self, agent):
        ...
        # The system prompt is augmented with the Sinria "Context Share
        # Resolver" suffix at build time, so the content is "compressed prompt"
        # plus that appended block — compare by prefix instead of exact match.
        system_msg = request_payloads[1]["messages"][0]
        assert system_msg["role"] == "system"
        assert system_msg["content"].startswith("compressed prompt")
