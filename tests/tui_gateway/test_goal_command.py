...
def hermes_home(tmp_path, monkeypatch):
    ...
    # The goal store is a SessionDB whose path comes from
    # hermes_state.DEFAULT_DB_PATH — a module-level constant frozen at import
    # time (DEFAULT_DB_PATH = get_hermes_home() / "state.db"). SessionDB() takes
    # no path here, so without this every goal test reads/writes the SAME state.db
    # (whichever HERMES_HOME was active when hermes_state was first imported),
    # and since all tests share session_id="sid-test" a goal set by one test
    # leaks into another's "/goal status shows none" assertion (observed flake:
    # 'No active goal' vs '⏸ Goal (paused …): write a story'). Repoint
    # DEFAULT_DB_PATH at the per-test home so the goal store is isolated.
    import hermes_state

    monkeypatch.setattr(hermes_state, "DEFAULT_DB_PATH", home / "state.db")

    # Bust the goal-module DB cache so it re-resolves HERMES_HOME and reopens
    # against the per-test DEFAULT_DB_PATH.
