...
def _extract_setup_path_shim_block() -> str:
    ...
    match = re.search(
        r"(?P<block>mkdir -p \"\$command_link_dir\".*?chmod \+x \"\$command_link_dir/sinria\")",
        text,
        re.DOTALL,
    )
...
def test_setup_path_shim_block_removes_old_link_before_writing() -> None:
    ...
    rm_idx = block.find('rm -f "$command_link_dir/sinria"')
    cat_idx = block.find('cat > "$command_link_dir/sinria" <<EOF')
    assert rm_idx != -1, (
        "setup_path() must `rm -f` $command_link_dir/sinria before the "
...
def test_re_running_setup_path_block_preserves_pip_entry_point(tmp_path: Path) -> None:
    ...
    # The launcher symlink/shim was renamed hermes -> sinria; the block writes
    # "$command_link_dir/sinria", so the prior-install symlink must use that
    # name for the stomp repro to exercise the real path.
    shim_path = command_link_dir / "sinria"
    ...
    assert not shim_path.is_symlink(), (
        "command_link_dir/sinria must be replaced with a regular file, not "
