...
class TestVprintForceParameter:
    ...
    def test_error_messages_use_force_in_run_agent(self):
        """Verify that critical error _vprint calls include force=True.

        The conversation loop (and with it every ❌ fatal-error _vprint call)
        was extracted out of run_agent.py into agent/conversation_loop.py.
        The only ❌ left in run_agent.py is a module-level CLI bootstrap
        ``print`` in main() that fires before the agent object exists, so it
        legitimately can't route through _vprint. Scan the module that now
        owns the streaming-suppression error path instead; the intent
        (critical errors must use force=True so they show during streaming
        TTS) is unchanged.
        """
