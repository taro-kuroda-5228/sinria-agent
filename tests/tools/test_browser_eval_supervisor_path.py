...
def _wait_for_loop_running(loop, deadline_s: float = 5.0):
    """Block until ``loop.run_forever()`` is actually executing.

    ``thread.start()`` returns before the target reaches ``run_forever()``;
    under xdist CPU contention that window is wide enough for
    ``evaluate_runtime`` to see ``loop.is_running() == False`` and return
    "supervisor loop is not running" instead of exercising the branch under
    test.  Condition-based wait instead of sleep keeps this deterministic.
    """
    import time
    ...
    while not loop.is_running():
        if time.monotonic() > deadline:
            raise RuntimeError("event loop did not start running within deadline")
        time.sleep(0.001)
