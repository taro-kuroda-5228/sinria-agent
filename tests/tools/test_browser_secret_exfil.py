...
class TestBrowserSecretExfil:
    ...
    def test_blocks_sinria_confidential_browser_navigation_before_command(self):
        from tools.browser_tool import browser_navigate

        with patch("tools.browser_tool._run_browser_command") as mock_run:
            result = browser_navigate("https://example.com/search?q=患者ID12345社外秘")

        parsed = json.loads(result)
        assert parsed["success"] is False
        assert "Sinria external egress guard" in parsed["error"]
        assert "患者ID12345" not in json.dumps(parsed, ensure_ascii=False)
        mock_run.assert_not_called()

    def test_blocks_inline_patient_id_form_without_separator_and_redacts_error(self):
        from tools.browser_tool import browser_navigate

        with patch("tools.browser_tool._run_browser_command") as mock_run:
            result = browser_navigate("https://example.com/search?q=患者IDABC12345社外秘")
        ...
        assert "患者IDABC12345" not in rendered
        mock_run.assert_not_called()

    def test_allows_normal_url(self):
        """Normal URLs pass the secret check (may fail for other reasons)."""
        from tools.browser_tool import browser_navigate
        result = browser_navigate("https://github.com/NousResearch/sinria-agent")
        parsed = json.loads(result)
...
class TestWebExtractSecretExfil:
    ...
    @pytest.mark.asyncio
    async def test_blocks_sinria_confidential_url(self):
        from tools.web_tools import web_extract_tool

        result = await web_extract_tool(urls=["https://example.com/search?q=患者ID12345社外秘"])
        parsed = json.loads(result)
    ...
    @pytest.mark.asyncio
    async def test_blocks_inline_patient_id_form_without_separator_and_redacts_error(self):
        from tools.web_tools import web_extract_tool

        result = await web_extract_tool(urls=["https://example.com/search?q=患者IDABC12345社外秘"])
        parsed = json.loads(result)
        rendered = json.dumps(parsed, ensure_ascii=False)
        assert "error" in parsed
        assert "Sinria external egress guard" in parsed["error"]
