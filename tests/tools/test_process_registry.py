...
def test_sinria_process_notifications_redact_confidential_output(monkeypatch):
    """Sinria background-process notifications may cross platform boundaries."""
    monkeypatch.setenv("HERMES_CLI_NAME", "sinria")
    evt = {
        "type": "completion",
        "session_id": "proc_phi",
        "command": "python export_patient.py --patient-id P-12345",
        "exit_code": 0,
        "output": "患者ID P-12345 のカルテ要約: 佐藤太郎さんは要フォロー",
    }

    result = format_process_notification(evt)

    assert "患者ID" not in result
    assert "P-12345" not in result
    assert "佐藤太郎" not in result
    assert "[sinria-redacted-confidential-process-notification]" in result
