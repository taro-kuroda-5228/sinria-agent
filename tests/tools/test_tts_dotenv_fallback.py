...
def isolate_env(monkeypatch):
    ...
    # Force the xAI credential resolver onto its env/.env fallback. The xAI
    # TTS path (_generate_xai_tts → tools.xai_http.resolve_xai_http_credentials)
    # consults the managed-OAuth resolvers before get_env_value, so leaked xAI
    # OAuth token state from a sibling xdist test could short-circuit the
    # dotenv path under test. Stub both resolvers to "no managed credentials"
    # (the normal hermetic state) so the .env fallback is reached
    # deterministically. Non-xAI providers here don't touch these resolvers.
    monkeypatch.setattr(
