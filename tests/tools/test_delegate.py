...
class TestDelegateHeartbeat(unittest.TestCase):
    ...
    def test_heartbeat_touches_parent_activity_during_child_run(self):
        ...
        # Make run_conversation block long enough for heartbeats to fire even
        # on loaded xdist/macOS workers.
        def slow_run(**kwargs):
            time.sleep(0.5)
    ...
    def test_heartbeat_does_not_trip_idle_stale_while_inside_tool(self):
        ...
        # Run long enough that, at the patched heartbeat interval, the number
        # of expected ticks (~1.0s / 0.02s ≈ 50) sits FAR above the assertion
        # threshold even on loaded xdist/macOS workers. The wide margin is
        # deliberate: heartbeat ticks are driven by Event.wait() on a daemon
        # thread, so startup and individual ticks are subject to scheduler
        # jitter. Demanding only ">6" out of ~50 expected ticks makes the test
        # deterministic under CI load while still proving the in-tool branch
        # keeps firing (the in-tool stale limit is 40 cycles, so it cannot break
        # within this window — an early stop would cap touches near the idle
        # limit instead).
        run_seconds = 1.0
        ...
        with patch("tools.delegate_tool._HEARTBEAT_INTERVAL", heartbeat_interval), patch(
            "tools.delegate_tool._HEARTBEAT_STALE_CYCLES_IDLE", 3
        ...
        # A child wedged inside a tool keeps current_tool set, so the in-tool
        # threshold applies and the heartbeat must keep touching past the idle
        # stale limit. If the idle branch were wrongly used, the patched
        # 3-cycle idle limit would stop touches at or below 3.
        self.assertGreater(
            len(touch_calls), 3,
            f"Heartbeat stopped too early while child was inside a tool; "
            f"got {len(touch_calls)} touches over {run_seconds}s "
            f"at {heartbeat_interval}s interval",
        )
