...
class TestSendMessageTool:
    def test_sinria_guard_blocks_confidential_external_message_before_send(self):
        config, _telegram_cfg = _make_config()

        with patch("gateway.config.load_gateway_config", return_value=config), \
             patch("tools.interrupt.is_interrupted", return_value=False), \
             patch("agent.sinria_egress._load_sinria_egress_config", return_value={
                 "mode": "block",
                ...
                send_message_tool(
                    {
                        ...
                        "message": "社外秘 password=example-secret",
                    }
                )
            )

        assert result["error"].startswith("Sinria external egress guard")
