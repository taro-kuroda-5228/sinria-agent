"""
Tests for mcp_serve — Sinria MCP server.
...
class TestHelpers:
    ...
    def test_sinria_mcp_fallback_home_uses_sinria_when_constants_unavailable(self, tmp_path, monkeypatch):
        ...
        def guarded_import(name, *args, **kwargs):
            if name == "hermes_constants":
                raise ImportError("simulate hermes_constants unavailable")
        ...
        assert mcp_serve._get_sessions_dir() == tmp_path / ".sinria" / "sessions"

        channel_directory = tmp_path / ".sinria" / "channel_directory.json"
        channel_directory.parent.mkdir(parents=True)
        channel_directory.write_text(json.dumps({"home": {"platform": "local"}}), encoding="utf-8")

        assert mcp_serve._load_channel_directory() == {"home": {"platform": "local"}}
