...
class AIAgent:
    ...
    def _hydrate_todo_store(self, history: List[Dict[str, Any]]) -> None:
        """
        Recover todo state from the disk snapshot or conversation history.

        The gateway creates a fresh AIAgent per message, so the in-memory
        TodoStore is empty. The durable per-session snapshot is preferred
        (it survives context compression and history truncation); the
        history scan remains as the fallback for sessions without one.
        """
        if self._todo_store.load_from_disk():
            if not self.quiet_mode:
                self._vprint(
                    f"{self.log_prefix}📋 Restored "
                    f"{len(self._todo_store.read())} todo item(s) from disk"
                )
    ...
    def _build_system_prompt_parts(self, system_message: Optional[str] = None, current_user_message: Optional[str] = None) -> Dict[str, str]:
        """Forwarder — see ``agent.system_prompt.build_system_prompt_parts``."""
        from agent.system_prompt import build_system_prompt_parts
        return build_system_prompt_parts(self, system_message=system_message, current_user_message=current_user_message)

    def _build_system_prompt(self, system_message: Optional[str] = None, current_user_message: Optional[str] = None) -> str:
        """Forwarder — see ``agent.system_prompt.build_system_prompt``."""
        from agent.system_prompt import build_system_prompt
        return build_system_prompt(self, system_message=system_message, current_user_message=current_user_message)
