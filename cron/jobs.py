...
Jobs are stored in ~/.sinria/cron/jobs.json
Output is saved to ~/.sinria/cron/output/{job_id}/{timestamp}.md
"""
    ...
    if schedule["kind"] == "once":
        ...
        if not HAS_CRONITER:
            logger.warning(
                ...
                "reinstall sinria-agent or run 'pip install croniter' in your "
                "runtime env.",
                schedule.get("expr"),
            )
            return None
...
def create_job(
    ...
    Args:
        ...
        script: Optional path to a script whose stdout feeds the job. With
                ...
                ~/.sinria/scripts/; ``.sh`` / ``.bash`` files run via bash,
