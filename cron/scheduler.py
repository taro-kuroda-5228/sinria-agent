...
Uses a file-based lock (~/.sinria/cron/.tick.lock) so only one tick
...
# Without this, standalone invocations (e.g. after `sinria update` reloads
...
def _resolve_cron_enabled_toolsets(job: dict, cfg: dict) -> list[str] | None:
    ...
    2. Per-platform ``sinria tools`` config for the ``cron`` platform.
...
def _deliver_result(job: dict, content: str, adapters=None, loop=None) -> Optional[str]:
    ...
    for target in targets:
        ...
        try:
            from agent.sinria_egress import SinriaEgressBlocked, guard_cron_delivery_egress

            target_label = f"{platform_name}:{chat_id}"
            if thread_id is not None:
                target_label += f":{thread_id}"
            sinria_cfg = user_cfg.get("sinria", {}).get("egress", {}) if isinstance(user_cfg, dict) else {}
            guard_cron_delivery_egress(target_label, cleaned_delivery_content, config=sinria_cfg)
        except Exception as egress_exc:
            try:
                from agent.sinria_egress import SinriaEgressBlocked

                if isinstance(egress_exc, SinriaEgressBlocked):
                    msg = str(egress_exc)
                    logger.warning("Job '%s': %s", job["id"], msg)
                    delivery_errors.append(msg)
                    continue
                ...
                pass
            logger.debug("Sinria cron delivery egress guard skipped due to error: %s", egress_exc)
