# Contributing to Sinria

Thank you for contributing to Sinria! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
2. **Cross-platform compatibility** — macOS, different Linux distros, and WSL2 on Windows. We want Sinria to work everywhere.
...
Bundled skills (in `skills/`) ship with every Sinria install. They should be **broadly useful to most users**:

- Document handling, web research, common dev workflows, system administration
- Used regularly by a wide range of people

If your skill is official and useful but not universally needed (e.g., a paid service integration, a heavyweight dependency), put it in **`optional-skills/`** — it ships with the repo but isn't activated by default. Users can discover it via `sinria skills browse` (labeled "official") and install it with `sinria skills install` (no third-party warning, builtin trust).

If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it in the [Medical Horizon Discord](https://discord.gg/Medical-Horizon). Users can install it with `sinria skills install`.

---

## Memory Providers: Ship as a Standalone Plugin

**We are no longer accepting new memory providers into this repo.** The set of built-in providers under `plugins/memory/` (honcho, mem0, supermemory, byterover, hindsight, holographic, openviking, retaindb) is closed. If you want to add a new memory backend, publish it as a **standalone plugin repo** that users install into `~/.sinria/plugins/` (or via a pip entry point).

Standalone memory plugins:

- Implement the same `MemoryProvider` ABC (`agent/memory_provider.py`) — `sync_turn`, `prefetch`, `shutdown`, and optionally `post_setup(sinria_home, config)` for setup-wizard integration
...
- Integrate with `sinria memory setup` via `post_setup()` — no need to touch core code
...
```bash
git clone --recurse-submodules https://github.com/taro-kuroda-5228/sinria-agent.git
cd sinria-agent
...
```bash
mkdir -p ~/.sinria/{cron,sessions,logs,memories,skills}
cp cli-config.yaml.example ~/.sinria/config.yaml
touch ~/.sinria/.env

# Add at minimum an LLM provider key:
echo "OPENROUTER_API_KEY=***" >> ~/.sinria/.env
```
...
mkdir -p ~/.local/bin
ln -sf "$(pwd)/venv/bin/sinria" ~/.local/bin/sinria
...
├── cli.py                    # SinriaCLI class — interactive TUI, prompt_toolkit integration
├── model_tools.py            # Tool orchestration (thin layer over tools/registry.py)
├── toolsets.py               # Tool groupings and presets (sinria-cli, sinria-telegram, etc.)
├── sinria_state.py           # SQLite session database with FTS5 full-text search, session titles
...
│
├── sinria_cli/               # CLI command implementations
...
### User configuration (stored in `~/.sinria/`)

| Path | Purpose |
|------|---------|
| `~/.sinria/config.yaml` | Settings (model, terminal, toolsets, compression, etc.) |
| `~/.sinria/.env` | API keys and secrets |
| `~/.sinria/auth.json` | OAuth credentials (Nous Portal) |
| `~/.sinria/skills/` | All active skills (bundled + hub-installed + agent-created) |
| `~/.sinria/memories/` | Persistent memory (MEMORY.md, USER.md) |
| `~/.sinria/state.db` | SQLite session database |
| `~/.sinria/sessions/` | JSON session logs |
| `~/.sinria/cron/` | Scheduled job data |
| `~/.sinria/whatsapp/session/` | WhatsApp bridge credentials |
...
- **Session persistence**: All conversations are stored in SQLite (`sinria_state.py`) with full-text search and unique session titles. JSON logs go to `~/.sinria/sessions/`.
...
Four fields are supported under `metadata.sinria`:
...
The user may skip setup and keep loading the skill. Sinria only exposes metadata (`stored_as`, `skipped`, `validated`) to the model — never the secret value.
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `sinria setup` or update `~/.sinria/.env` locally.
...
2. **Tools referenced in SKILL.md prose must be native Sinria tools or MCP servers the skill explicitly expects.** When the skill needs a capability, point at the proper tool by name in backticks: `` `terminal` ``, `` `web_extract` ``, `` `web_search` ``, `` `read_file` ``, `` `write_file` ``, `` `patch` ``, `` `search_files` ``, `` `vision_analyze` ``, `` `browser_navigate` ``, `` `delegate_task` ``, `` `image_generate` ``, `` `text_to_speech` ``, `` `cronjob` ``, `` `memory` ``, `` `skill_view` ``, `` `todo` ``, `` `execute_code` ``.
...
4. **`author` credits the human contributor first.** For external contributions, the contributor's real name + GitHub handle goes first (`Jane Doe (jane-doe)`); "Sinria" is the secondary collaborator. If the contributor's commit shows "Sinria" as author because they used Sinria to draft the skill, replace it with their actual name — credit the human, not the tool.
...
- **No external dependencies unless absolutely necessary.** Prefer stdlib Python, curl, and existing Sinria tools (`web_extract`, `terminal`, `read_file`).
...
- **Test it.** Run `sinria --toolsets skills -q "Use the X skill to do Y"` and verify the agent follows the instructions correctly.

---

## Adding a Skin / Theme

Sinria uses a data-driven skin system — no code changes needed to add a new skin.

**Option A: User skin (YAML file)**

Create `~/.sinria/skins/<name>.yaml`:
...
Add to `_BUILTIN_SKINS` dict in `sinria_cli/skin_engine.py`. Use the same schema as above but as a Python dict. Built-in skins ship with the package and are always available.

**Activating:**
- CLI: `/skin mytheme` or set `display.skin: mytheme` in config.yaml
- Config: `display: { skin: mytheme }`

See `sinria_cli/skin_engine.py` for the full schema and existing skins as examples.

---

## Cross-Platform Compatibility

Sinria runs on Linux, macOS, and native Windows (plus WSL2). When writing code
...
1. **Never call `os.kill(pid, 0)` for liveness checks.** `os.kill(pid, 0)`
   ...
   **Preferred:** use `psutil` (a core dependency — always available):
   ...
   If you specifically need the sinria wrapper (it has a stdlib fallback
...
2. **Use `shutil.which()` before shelling out — don't assume Windows has
   ...
   `sinria_cli/gateway.py::_scan_gateway_pids` for the pattern.
...
10. **Detached background daemons on Windows need `pythonw.exe`, NOT
    ...
    See `sinria_cli/gateway_windows.py::_spawn_detached` for the reference
    implementation.
...
16. **Two different quoting schemes in one command line.** `subprocess.run
    ...
    helpers and never cross them. See `sinria_cli/gateway_windows.py::
...
Sinria has terminal access. Security matters.
...
2. **Test manually**: Run `sinria` and exercise the code path you changed
...
- Use [GitHub Issues](https://github.com/taro-kuroda-5228/sinria-agent/issues)
- Include: OS, Python version, Sinria version (`sinria version`), full error traceback
...
- **Discord**: [discord.gg/Medical-Horizon](https://discord.gg/Medical-Horizon) — for questions, showcasing projects, and sharing skills
