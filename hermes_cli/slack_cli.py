...
Today only ``slack manifest`` is implemented — it generates the
...
Typical workflow::
    ...
    $ <cli> slack manifest > slack-manifest.json
    # or:
    $ <cli> slack manifest --write
...
def _default_manifest_description() -> str:
    return f"Your {_product_name()} agent on Slack"


def _default_manifest_target() -> Path:
    home_name = ".sinria" if _cli_command_name() == "sinria" else ".hermes"
    return Path(os.environ.get("HERMES_HOME") or str(Path.home() / home_name)) / "slack-manifest.json"
...
def _build_full_manifest(bot_name: str, bot_description: str) -> dict:
    ...
    it stays in sync with the rest of the CLI. Other manifest sections
    (display info, OAuth scopes, socket mode) are set to sensible defaults
    for a typical deployment — users can tweak them in the Slack UI after
    pasting.
    """
    ...
    return {
        ...
        "display_information": {
            "name": bot_name[:35],
            "description": (bot_description or _default_manifest_description())[:140],
        ...
        "features": {
            ...
            "assistant_view": {
                "assistant_description": f"Chat with {_product_name()} in threads and DMs.",
            },
        },
        "oauth_config": {
            "scopes": {
...
def slack_manifest_command(args) -> int:
    ...
    Flags (all parsed in ``hermes_cli/main.py``):
      ...
      --name NAME     Override the bot display name (default: active product name)
      --description DESC  Override the bot description
      --slashes-only  Emit only the ``features.slash_commands`` array (for
                      merging into an existing manifest manually)
    """
    name = getattr(args, "name", None) or _product_name()
    description = getattr(args, "description", None) or _default_manifest_description()
    ...
    if write_target is not None:
        ...
        print(
            "\nNext steps:\n"
            f"  1. Open https://api.slack.com/apps and pick your {_product_name()} app\n"
