"""
Web UI server for the active CLI product.
...
try:
    ...
    # running `dashboard` needs fastapi+uvicorn; lazy install keeps
...
app = FastAPI(title=("Sinria Agent" if (os.getenv("HERMES_CLI_NAME") or "").strip().lower() == "sinria" else "Sinria"), version=__version__)
...
# In-browser Chat tab (/chat, /api/pty, …).  Off unless ``dashboard --tui``
...
async def update_hermes():
    ...
    try:
        ...
        _log.exception("Failed to spawn background update")
...
# the canonical runtime ``auth add <provider>`` command so the dashboard
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    1. ``~/.hermes/.anthropic_oauth.json`` — Sinria-managed PKCE flow
    ...
    if hermes_creds and hermes_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "hermes_pkce",
            "source_label": f"Sinria PKCE ({_HERMES_OAUTH_FILE})",
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    when they also have a separate Sinria-managed PKCE login.
    """
    try:
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    {
        ...
        "flow": "pkce",
        "cli_command": f"{_cli_command_name()} auth add anthropic",
    ...
    {
        "id": "nous",
        "name": _nous_portal_label(),
        "flow": "device_code",
        "cli_command": f"{_cli_command_name()} auth add nous",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": f"{_cli_command_name()} auth add openai-codex",
    ...
    },
    {
        ...
        "flow": "external",
        "cli_command": f"{_cli_command_name()} auth add qwen-oauth",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": f"{_cli_command_name()} auth add minimax-oauth",
...
async def disconnect_oauth_provider(provider_id: str, request: Request):
    ...
    # Anthropic and claude-code clear the same Sinria-managed PKCE file
...
# Guarded so the web UI still starts if anthropic_adapter is unavailable;
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    ...
    the system in the same state as ``auth add anthropic``.
    """
    from agent.anthropic_adapter import _HERMES_OAUTH_FILE
    payload = {
...
def _profile_setup_command(name: str) -> str:
    ...
    cli = (os.getenv("SINRIA_CLI_NAME") or os.getenv("HERMES_CLI_NAME") or "sinria").strip().lower() or "sinria"
    return f"{cli} setup" if name == "default" else f"{name} setup"
...
_BUILTIN_DASHBOARD_THEMES = [
    {"name": "default",       "label": "Sinria Teal",         "description": "Classic dark teal — the canonical Sinria look"},
    {"name": "default-large", "label": "Sinria Teal (Large)", "description": "Sinria Teal with bigger fonts and roomier spacing"},
