"""
Security advisory checker for Sinria.
...
- **Acknowledgeable.** Once the user has read and acted on an advisory they
  can dismiss it via ``doctor --ack <id>``; the ack is persisted to
...
run in environments where the rest of the CLI failed to import.
"""
...
ADVISORIES: tuple[Advisory, ...] = (
    Advisory(
        ...
        remediation=(
            ...
            f"After cleanup: {_cli_command_name()} doctor --ack shai-hulud-2026-05  to dismiss "
            "this warning.",
        ),
...
def _installed_version(pkg_name: str) -> Optional[str]:
    ...
    except ImportError:  # py<3.8 — the CLI requires 3.10+ but defensive.
        return None
    try:
...
def short_banner_lines(hits: list[AdvisoryHit]) -> list[str]:
    ...
    lines = [
        ...
        f"  Run '{_cli_command_name()} doctor' for remediation steps.",
    ]
    if len(hits) > 1:
...
def gateway_log_message(hits: list[AdvisoryHit]) -> Optional[str]:
    ...
    return (f"{len(fresh)} security advisories active "
            ...
            f"Run `{_cli_command_name()} doctor` on the gateway host for details.")
