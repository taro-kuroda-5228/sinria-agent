...
- Keep data: Remove code but keep the active runtime home (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove CLI-owned PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing sinria-agent or hermes PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Sinria" comment and following line
                if '# Sinria' in line or '# sinria-agent' in line:
                    skip_next = True
                    continue
...
def remove_wrapper_script():
    """Remove active CLI wrapper scripts if they exist."""
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains CLI module reference)
                content = wrapper.read_text()
                if 'hermes_cli' in content or 'sinria-agent' in content:
                    wrapper.unlink()
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``<cli> gateway run`` processes
...
def _hermes_path_markers(hermes_home: Path) -> list[str]:
    """Path-entry substrings that identify CLI-owned User-PATH entries."""
    root = str(hermes_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare sinria-agent install dir.
    markers = [root + "\\sinria-agent", root + "\\git", root + "\\node", root + "\\venv"]
...
def remove_path_from_windows_registry(hermes_home: Path) -> list[str]:
    """Strip CLI-owned entries from User-scope PATH in the registry.
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its runtime home directory.

    We shell out to ``<cli> -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current runtime home and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m hermes_cli.main` so we don't depend on a CLI wrapper
    #    that may be half-removed mid-uninstall.
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + active runtime home (configs, data, logs)
    - Keep data: removes code but keeps the active runtime home for future reinstall
    """
    ...
    print()
    uninstall_title = f"⚕ {_product_name()} Uninstaller"
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color(f"│ {uninstall_title:^55} │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print()
    if full_uninstall:
        print(color(f"⚠️  WARNING: This will permanently delete ALL {_product_short_name()} data!", Colors.RED, Colors.BOLD))
        ...
        print(f"This will remove the {_product_short_name()} code but keep your configuration and data.")
    ...
    if _is_windows():
        ...
        if removed_path_entries:
            ...
            log_info(f"No {_product_short_name()}-owned PATH entries in User environment")
        ...
        if removed_env:
            ...
            log_info(f"No {_product_short_name()}-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info(f"Removing {_cli_command_name()} command...")
    ...
    try:
        if project_root.exists():
            # If the install is inside the runtime home, just remove the sinria-agent subdir
    ...
    # 5. Optionally remove the runtime-home data directory (and named profiles)
    if full_uninstall:
    ...
    if not full_uninstall:
        ...
        if _is_windows():
            print(color("  irm https://raw.githubusercontent.com/taro-kuroda-5228/sinria-agent/main/scripts/install.ps1 | iex", Colors.DIM))
        else:
            print(color("  curl -fsSL https://raw.githubusercontent.com/taro-kuroda-5228/sinria-agent/main/scripts/install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print(f"Thank you for using {_product_name()}! ⚕")
    print()
