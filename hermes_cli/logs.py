"""``logs`` — view and filter runtime log files.

Supports tailing, following, session filtering, level filtering,
component filtering, and relative time ranges. All log files live
under the runtime-home logs directory.

Usage examples::

    <cli> logs                    # last 50 lines of agent.log
    <cli> logs -f                 # follow agent.log in real time
    <cli> logs errors             # last 50 lines of errors.log
    <cli> logs gateway -n 100     # last 100 lines of gateway.log
    <cli> logs --level WARNING    # only WARNING+ lines
    <cli> logs --session abc123   # filter by session ID substring
    <cli> logs --component tools  # only tool-related lines
    <cli> logs --since 1h         # lines from the last hour
    <cli> logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    if not log_path.exists():
        ...
        product = "Sinria" if cli == "sinria" else "Hermes"
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when {product} runs — try '{cli} chat' first)")
        sys.exit(1)
...
def list_logs() -> None:
    ...
    if not found:
        print(f"  (no log files yet — run '{_cli_command_name()} chat' to generate logs)")
