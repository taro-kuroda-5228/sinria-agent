"""claw — OpenClaw migration commands.

Usage:
    claw migrate              # Preview then migrate (always shows preview first)
    claw migrate --dry-run    # Preview only, no changes
    claw migrate --yes        # Skip confirmation prompt
    claw migrate --preset full --overwrite --migrate-secrets  # Full run w/ secrets
    claw migrate --no-backup  # Skip pre-migration snapshot
    claw cleanup              # Archive leftover OpenClaw directories
    claw cleanup --dry-run    # Preview what would be archived
"""
...
def _warn_if_gateway_running(auto_yes: bool) -> None:
    """Check if a Sinria gateway is running with connected platforms.
    ...
    print_info(f"Recommendation: stop the gateway first with '{_cli_cmd('stop')}'.")
    print()
...
def claw_command(args):
    ...
    if action == "migrate":
        ...
        print(f"Usage: {_cli_cmd('claw')} <command> [options]")
        print()
        print("Commands:")
        ...
        print()
        print(f"Run '{_cli_cmd('claw <command> --help')}' for options.")


def _cmd_migrate(args):
    """Run the OpenClaw → current-CLI migration."""
    ...
    print(
        color(
            f"│          ⚕ {_product_name()} — OpenClaw Migration                 │",
            Colors.MAGENTA,
        )
    )
    print(
    ...
    if not source_dir.is_dir():
        ...
        print_info(f"You can specify a custom path: {_cli_cmd('claw migrate --source /path/to/.openclaw')}")
        return
    ...
    if preview_conflicts > 0 and not overwrite:
        ...
        print_info(
            f"Each conflict is an item whose target already exists in {_display_home()}/. "
    ...
    if not auto_yes:
        if not sys.stdin.isatty():
            ...
            print_info(f"To execute, re-run with: {_cli_cmd('claw migrate --yes')}")
            return
    ...
    # restorable with `import`. Mirrors OpenClaw's
    ...
    if not no_backup:
        try:
            ...
            if backup_archive:
                ...
                print_info(f"Restore with: {_cli_cmd(f'import {backup_archive.name}')}")
        except Exception as e:
            print()
            print_error(f"Could not create pre-migration backup: {e}")
            print_info(
                f"Re-run with --no-backup to skip, or free up disk space under the {_product_name()} home."
            )
...
def _cmd_cleanup(args):
    ...
    print(
        color(
            f"│          ⚕ {_product_name()} — OpenClaw Cleanup                   │",
            Colors.MAGENTA,
        )
    )
    print(
    ...
    if running:
        ...
        if not auto_yes:
            ...
            if not prompt_yes_no("Proceed anyway?", default=False):
                print_info(f"Aborted. Stop OpenClaw first, then re-run: {_cli_cmd('claw cleanup')}")
                return
    ...
    for source_dir in dirs_to_check:
        ...
        if dry_run:
            ...
            print_info(f"To execute, re-run with: {_cli_cmd('claw cleanup --yes')}")
...
def _print_migration_report(report: dict, dry_run: bool):
    ...
    if dry_run:
        ...
        migrate_cmd = _cli_cmd(f"claw migrate --preset {report.get('preset', 'full')}")
        print_info(f"  {migrate_cmd}")
    elif migrated:
        print()
        ...
        if skipped_keys:
            ...
            print_info(f"  {_cli_cmd('claw migrate --migrate-secrets')}")
            print()
            print_info("Or add your key manually:")
            print_info(f"  {_cli_cmd('config set OPENROUTER_API_KEY sk-or-v1-...')}")
