"""CLI subcommand: `curator <subcommand>`.
...
def _curator_command() -> str:
    return f"{_cli_command_name()} curator"


def _skills_display_path() -> str:
...
def _cmd_status(args) -> int:
    ...
    try:
        if _cli_command_name() == "sinria" and not curator._should_write_sanitized_training_log():
            print("  report logging: suppressed by policy")
    except Exception:
        pass
...
def _cmd_run(args) -> int:
    ...
    if not synchronous:
        print(f"llm pass running in background — check `{_curator_command()} status` later")
    if dry:
        if synchronous:
            print(
                "dry-run: no changes applied. Read the report with "
                f"`{_curator_command()} status` and run `{_curator_command()} run` (no flag) to apply."
            )
        else:
            print(
...
def _cmd_archive(args) -> int:
    ...
    if skill_usage.get_record(args.skill).get("pinned"):
        print(
            ...
            f"`{_curator_command()} unpin {args.skill}`"
        )
        return 1
...
def _cmd_backup(args) -> int:
    ...
    if snap is None:
        ...
        return 1
    print(f"curator: snapshot created at {_skills_display_path()}.curator_backups/{snap.name}")
    return 0
...
def _cmd_rollback(args) -> int:
    ...
    if target_path is None:
        ...
        if not rows:
            print(
                f"curator: no snapshots exist under {_skills_display_path()}.curator_backups/ yet. "
                f"Take one with `{_curator_command()} backup` or wait for the next curator run."
            )
        else:
            print(
    ...
    print(
        f"\nThis will replace the current {_skills_display_path()} tree (a safety "
...
def register_cli(parent: argparse.ArgumentParser) -> None:
    ...
    p_backup = subs.add_parser(
        "backup",
        help=f"Take a manual tar.gz snapshot of {_skills_display_path()} "
    ...
    p_rollback = subs.add_parser(
        "rollback",
        help=f"Restore {_skills_display_path()} from a curator snapshot "
...
def cli_main(argv=None) -> int:
    ...
    parser = argparse.ArgumentParser(
        prog=_curator_command(),
        epilog=f"Skill tree: {_skills_display_path()}",
    )
    register_cli(parser)
