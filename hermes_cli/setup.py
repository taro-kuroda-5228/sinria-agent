"""
Interactive setup wizard for Sinria.
...
Config files are stored in the active runtime home (for example ~/.sinria/ in Sinria mode) for easy access.
"""
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color(f"⚕ {_product_short_name()} Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info(f"Configure {_product_short_name()} using environment variables or config commands:")
    print_info(f"  {_cli_cmd('config set model.provider custom')}")
    print_info(f"  {_cli_cmd('config set model.base_url http://localhost:8080/v1')}")
    print_info(f"  {_cli_cmd('config set model.default your-model-name')}")
    print_info(f"  {_cli_cmd('config set sinria.policy.active_profile dogfood_frontier')}")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Available Sinria policy profiles: dogfood_frontier, enterprise_guarded_cloud, sovereign_local_only")
    print_info(f"Run '{_cli_cmd('setup')}' in an interactive terminal to use the full wizard.")
    print()
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning(f"  Skipped (configure later with '{_cli_cmd('setup')}')")
...
def _print_setup_summary(config: dict, hermes_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, f"run '{_cli_cmd('setup')}' to configure"))
    ...
    # Video generation — opt-in via `sinria tools` → Video Generation.
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, f"run '{_cli_cmd('setup tts')}'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, f"run '{_cli_cmd('setup tts')}'"))
    else:
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, f"run '{_cli_cmd('setup terminal')}'"))
    ...
    if disabled_tools:
        print_warning(
            f"Some tools are disabled. Run '{_cli_cmd('setup tools')}' to configure them,"
        )
    ...
    print()
    print(f"   {color(_cli_cmd('setup'), Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color(_cli_cmd('setup model'), Colors.GREEN)}    Change model/provider")
    print(f"   {color(_cli_cmd('setup terminal'), Colors.GREEN)} Change terminal backend")
    print(f"   {color(_cli_cmd('setup gateway'), Colors.GREEN)}  Configure messaging")
    print(f"   {color(_cli_cmd('setup tools'), Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color(_cli_cmd('config'), Colors.GREEN)}         View current settings")
    print(
        f"   {color(_cli_cmd('config edit'), Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color(_cli_cmd('config set <key> <value>'), Colors.GREEN)}")
    ...
    print()
    print(f"   {color(_cli_command_name(), Colors.GREEN)}              Start chatting")
    print(f"   {color(_cli_cmd('gateway'), Colors.GREEN)}      Start messaging gateway")
    print(f"   {color(_cli_cmd('doctor'), Colors.GREEN)}       Check for issues")
    print()
...
# between `sinria tools` and `<cli> setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``<cli> model``)
    ...
    provider added to ``model`` is automatically available here.
    ...
    # Delegate to the shared model-selection flow — handles provider picker,
    ...
    try:
        ...
        print_info(f"You can try again later with: {_cli_cmd('model')}")
    ...
    if not quick and _supports_same_provider_pool_setup(selected_provider):
        try:
            ...
            print_info(
                f"{_product_short_name()} can keep multiple credentials for one provider and rotate between"
            )
            print_info(
    ...
    if _vision_needs_setup:
        ...
        print_info(f"doesn't currently provide one {_product_short_name()} can auto-use for vision,")
        ...
        if _vision_idx == 0:  # OpenRouter
            ...
            print_info(f"Skipped — add later with '{_cli_cmd('setup')}' or configure AUXILIARY_VISION_* settings")
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from the runtime .env."
            )
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via {_cli_cmd('setup model')} or {_dhh()}/.env to use xAI TTS. "
                        "Falling back to Edge TTS."
                    )
                    selected = "edge"
            else:
...
def setup_tts(config: dict):
    """Standalone TTS setup (for `<cli> setup tts`)."""
    _setup_tts_provider(config)
...
def setup_terminal_backend(config: dict):
    ...
    print_info(f"Choose where {_product_short_name()} runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        print_info("Requires the optional SDK: pip install 'sinria-agent[vercel]'")
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                print_warning("Install failed — run manually: pip install 'sinria-agent[vercel]'")
                if result.stderr:
                    print_info(f"  Error: {result.stderr.strip().splitlines()[-1]}")
...
def _apply_default_agent_settings(config: dict):
    ...
    print_info(f"  Run `{_cli_cmd('setup agent')}` later to customize.")
...
def _setup_telegram():
    ...
    print()
    print_info(f"📬 Home Channel: where {_product_short_name()} delivers cron job results,")
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                f"list? (recommended after `{_cli_command_name()} update`)",
                True,
            ):
...
def _write_slack_manifest_and_instruct():
    ...
    try:
        ...
        manifest = _build_full_manifest(
            bot_name=_product_short_name(),
            bot_description="Your Sinria agent on Slack",
        )
        ...
        print_info(
            f"   Re-run `{_cli_command_name()} slack manifest --write` anytime to refresh after "
            f"{_product_name().replace(' Agent', '')} adds new commands."
        )
        ...
        print_info(
            ...
            f"{_cli_command_name()} slack manifest --write"
        )
...
def _setup_matrix():
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        print()
        print_info(f"📬 Home Room: where {_product_short_name()} delivers cron job results and notifications.")
...
def _setup_mattermost():
    ...
    print()
    print_info(f"📬 Home Channel: where {_product_short_name()} delivers cron job results and notifications.")
...
def _setup_bluebubbles():
    ...
    print_info(f"Connects {_product_short_name()} to iMessage via BlueBubbles — a free, open-source")
...
def setup_gateway(config: dict):
    ...
    print_info(f"Connect to messaging platforms to chat with {_product_short_name()} from anywhere.")
    ...
    if not selected:
        print_info(f"No platforms selected. Run '{_cli_cmd('setup gateway')}' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     {_cli_cmd(f'config set {plat.upper()}_HOME_CHANNEL <channel_id>')}"
                )
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                try:
                    ...
                    print_info(f"  You can try manually: {_cli_cmd('gateway install')}")
            else:
                print_info(f"  You can install later: {_cli_cmd('gateway install')}")
                if supports_systemd:
                    print_info(f"  Or as a boot-time service: sudo {_cli_cmd('gateway install --system')}")
                print_info(f"  Or run in foreground:  {_cli_cmd('gateway')}")
        else:
            from hermes_constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info(f"   {_cli_cmd('gateway run')}          # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info(f"   {_cli_cmd('gateway')}              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both ``<cli> setup tools`` and ``<cli> tools`` use the same flow:
...
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": f"⚠ Gateway/messaging — this will configure {_product_short_name()} to use your OpenClaw messaging channels",
    "telegram": f"⚠ Telegram — this will point {_product_short_name()} at your OpenClaw Telegram bot",
    "slack": f"⚠ Slack — this will point {_product_short_name()} at your OpenClaw Slack workspace",
    "discord": f"⚠ Discord — this will point {_product_short_name()} at your OpenClaw Discord bot",
    "whatsapp": f"⚠ WhatsApp — this will point {_product_short_name()} at your OpenClaw WhatsApp connection",
    "config": f"⚠ Config values — OpenClaw settings may not map 1:1 to {_product_short_name()} equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color(f"  Would overwrite (conflicts with existing {_product_short_name()} config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color(f"  Note: OpenClaw config values may have different semantics in {_product_short_name()}.", Colors.YELLOW))
        print(color(f"  For example, OpenClaw's tool_call_execution: \"auto\" ≠ {_product_short_name()}'s yolo mode.", Colors.YELLOW))
...
def _offer_openclaw_migration(hermes_home: Path) -> bool:
    ...
    print_info(f"{_product_short_name()} can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            f"Skipping migration. You can run it later with: {_cli_cmd('claw migrate --dry-run')}"
        )
        return False
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            f"Migration cancelled. You can run it later with: {_cli_cmd('claw migrate')}"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing Sinria configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing Sinria config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in {_product_short_name()} (use {_cli_cmd('claw migrate --overwrite')} to force).")
    if skipped:
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      <cli> setup           — full or quick (auto-detected)
      <cli> setup model     — just model/provider
      <cli> setup tts       — just text-to-speech
      <cli> setup terminal  — just terminal backend
      <cli> setup gateway   — just messaging platforms
      <cli> setup tools     — just tool configuration
      <cli> setup agent     — just agent settings
    """
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│ {('⚕ ' + _product_short_name() + ' Setup — ' + label):^55} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            f"│ {('⚕ ' + _product_name() + ' Setup Wizard'):^55} │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    setup_subtitle = f"Let's configure your {_product_name()} installation."
    print(
        color(
            f"│ {setup_subtitle:^55} │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        print_success(f"You already have {_product_short_name()} configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info(
            f"Tip: jump straight to a section with '{_cli_cmd('setup model')}', "
            f"'{_cli_cmd('setup terminal')}',"
        )
        print_info(
            f"     '{_cli_cmd('setup gateway')}', '{_cli_cmd('setup tools')}', or "
            f"'{_cli_cmd('setup agent')}', or fill only missing items with --quick."
        )
        ...
        setup_mode = prompt_choice(f"How would you like to set up {_product_short_name()}?", [
    ...
    print()
    print_info(f"You can edit these files directly or use '{_cli_cmd('config edit')}'")
...
def _run_first_time_quick_setup(config: dict, hermes_home, is_existing: bool):
    ...
    the user can customize later via ``setup <section>``.
    """
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            f"Skip — set up later with '{_cli_cmd('setup gateway')}'",
        ],
        0,
    )
    ...
    print()
    print_info(f"  Configure all settings:    {_cli_cmd('setup')}")
    if gateway_choice != 0:
        print_info(f"  Connect Telegram/Discord:  {_cli_cmd('setup gateway')}")
    print()
...
def _run_quick_setup(config: dict, hermes_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info(f"Run '{_cli_cmd('setup')}' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info(f"Connect {_product_short_name()} to messaging apps to chat from anywhere.")
        print_info(f"You can configure these later with '{_cli_cmd('setup gateway')}'.")
