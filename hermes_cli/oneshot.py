...
configured for "cli" in `<cli> tools`.
Rules / memory / AGENTS.md / preloaded skills = same as a normal chat turn.
Approvals = auto-bypassed (HERMES_YOLO_MODE=1 is set for the call).
Working directory = the user's CWD (AGENTS.md etc. resolve from there as usual).

Model / provider selection mirrors `<cli> chat`:
...
def _cli_oneshot_name() -> str:
    return f"{_cli_command_name()} -z"
...
def _validate_explicit_toolsets(toolsets: object = None) -> tuple[list[str] | None, str | None]:
    ...
    try:
        ...
        return None, f"{_cli_oneshot_name()}: failed to validate --toolsets: {exc}\n"
    ...
    if any(name in {"all", "*"} for name in built_in):
        ...
        if ignored:
            sys.stderr.write(
                f"{_cli_oneshot_name()}: --toolsets all enables every toolset; "
    ...
    if unknown:
        sys.stderr.write(f"{_cli_oneshot_name()}: ignoring unknown --toolsets entries: {', '.join(unknown)}\n")
    if disabled:
        sys.stderr.write(
            f"{_cli_oneshot_name()}: ignoring disabled MCP servers (set enabled: true in config.yaml to use): "
            f"{', '.join(disabled)}\n"
        )

    if not valid:
        return None, f"{_cli_oneshot_name()}: --toolsets did not contain any valid toolsets.\n"
...
def run_oneshot(
    ...
    env_model_early = (os.getenv("SINRIA_INFERENCE_MODEL") or os.getenv("HERMES_INFERENCE_MODEL", "")).strip()
    if provider and not ((model or "").strip() or env_model_early):
        sys.stderr.write(
            f"{_cli_oneshot_name()}: --provider requires --model (or HERMES_INFERENCE_MODEL). "
...
def _create_session_db_for_oneshot():
    """Best-effort SessionDB for oneshot (``-z``) mode.

    Oneshot bypasses the main CLI's ``_init_agent()`` path, so it must wire the SQLite
...
def _run_agent(
    ...
    # Imports are local so they don't run when the CLI is invoked for
    ...
    env_model = (os.getenv("SINRIA_INFERENCE_MODEL") or os.getenv("HERMES_INFERENCE_MODEL", "")).strip()
