"""
Unified tool configuration for the local CLI.

`tools` and `setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to the runtime-home config under
...
CONFIGURABLE_TOOLSETS = [
    ...
    ("ml_training",     "🏋️  ML Training / Fine-tuning", "env checks, dataset validation, LoRA scaffolding"),
...
# who want it opt in via `tools` → Video Generation, which walks
...
# `tools` → X (Twitter) Search setup walks users through credential
# setup. The tool's check_fn means the schema still won't appear to the
# model if the credential later goes missing or expires.
_DEFAULT_OFF_TOOLSETS = {"moa", "ml_training", "homeassistant", "spotify", "discord", "discord_admin", "video", "video_gen", "x_search"}
...
# Platform-scoped toolsets: only appear in the `tools` checklist for
...
def _get_effective_configurable_toolsets():
    ...
    Without the dedupe, ``tools`` → "reconfigure existing" would
...
TOOL_CATEGORIES = {
    ...
    "x_search": {
        ...
        "setup_note": (
            f"{_tools_product_name()} routes X searches through xAI's built-in x_search "
...
def install_cua_driver(upgrade: bool = False) -> bool:
    ...
    * ``upgrade=True`` — always re-run the installer (or call ``cua-driver
      update`` if the binary supports it). Used by ``update`` and
    ...
    if _plat.system() != "Darwin":
        if upgrade:
            # Silent on non-macOS — `update` calls this for every
    ...
    try:
        ...
        if result.returncode == 0 and shutil.which("cua-driver"):
            if verbose:
                ...
                _print_info("      System Settings > Privacy & Security > Accessibility")
                _print_info("      System Settings > Privacy & Security > Screen Recording")
                _print_info(f"    Both must allow the terminal / {_tools_product_name()} process.")
            return True
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        if not node_modules.exists() and npm_bin:
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {_display_hermes_home()}/sinria-agent && npm install")
                if result.stderr:
                    _print_info(f"      {result.stderr.strip()[:200]}")
        elif not node_modules.exists():
            _print_warning("    Node.js not found - browser tools require: npm install (in sinria-agent directory)")
            return
        ...
        if _running_in_docker():
            ...
            _print_info(
                "      docker pull ghcr.io/nousresearch/sinria-agent:latest"
            )
            return
        ...
        if not camofox_dir.exists() and _npm_bin:
            ...
            # package.json so that `update` does not silently pull
        ...
        # to the runtime .env), then continues straight into PKCE. If they
        ...
        try:
            ...
            _print_info(f"    Run manually: {_auth_cmd('spotify')}")
            return
        ...
        try:
            ...
            _print_info(f"    Run later: {_auth_cmd('spotify')}")
        except Exception as exc:
        ...
        try:
            ...
            _print_info(f"    Run later: {_auth_cmd('add xai-oauth')}   (or set XAI_API_KEY)")
            return
        ...
        idx = prompt_choice(
            ...
            choices=[
                ...
                f"Skip — configure later via `{_auth_cmd('add xai-oauth')}`",
            ],
            default=0,
        )
        if idx == 0:
            if _run_xai_oauth_login_from_setup():
                ...
                _print_warning(
                    ...
                    f"Run later: {_auth_cmd('add xai-oauth')}"
                )
        elif idx == 1:
            ...
            if api_key:
                ...
                _print_warning(
                    f"    No API key provided. Run later: {_auth_cmd('add xai-oauth')}"
                )
        else:
...
def _get_platform_tools(
    ...
    *,
    ...
    if has_explicit_config:
        ...
        # through ``tools`` to flip the toolset on. Only fires when
    ...
    # otherwise saving via `tools` (which flips has_explicit_config
    ...
    # shipped as a bundled plugin but user must opt in via `tools`
    # so we don't ship 7 Spotify tool schemas to users who don't use it).
    # A plugin toolset is "known" for a platform once `tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by tools — default enabled
                enabled_toolsets.add(pts)
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `tools` and `setup tools`.
