...
on Japanese locales, etc.). The CLI's banners, tool output feed, and slash
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. the CLI is running
...
def configure_windows_stdio() -> bool:
    ...
    # Augment PATH with the Sinria-managed Git install directories so
    ...
    # install without Sinria-managed PortableGit).
    ...
    # stdin is re-configured for completeness; the CLI's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and the CLI's
    ``config edit`` flow both honour ``$EDITOR``. Users who prefer a
    ...
    Set this before launching the CLI (User env var in Windows Settings, or
    export in a PowerShell profile) and the CLI picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Sinria-managed tool directories to os.environ['PATH'].
    ...
    *spawned* processes only — already-running shells (including the one the
    ...
    Any subprocess the CLI spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Sinria-managed tool directories to our
    ...
    candidate_dirs = [
        ...
        # CLI venv Scripts directory — host of the hermes.exe shim itself,
        ...
        os.path.join(local_appdata, "hermes", "sinria-agent", "venv", "Scripts"),
