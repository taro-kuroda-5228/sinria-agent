...
Usage:
    hermes pairing list / sinria pairing list
    hermes pairing approve <platform> <code> / sinria pairing approve <platform> <code>
    hermes pairing revoke <platform> <user_id> / sinria pairing revoke <platform> <user_id>
    hermes pairing clear-pending / sinria pairing clear-pending
"""
...
def _pairing_cmd(command: str) -> str:
    return f"{_cli_command_name()} pairing {command}".strip()
...
def pairing_command(args):
    """Handle CLI pairing subcommands."""
    ...
    if action == "list":
        ...
        print(f"Usage: {_pairing_cmd('{list|approve|revoke|clear-pending}')}")
        print(f"Run '{_pairing_cmd('--help')}' for details.")
...
def _cmd_approve(store, platform: str, code: str):
    ...
    if result:
        ...
        print(
            ...
            f"{_display_home()}/platforms/pairing/_rate_limits.json\n".format(platform)
        )
    else:
        print(f"\n  Code '{code}' not found or expired for platform '{platform}'.")
        print(f"  Run '{_pairing_cmd('list')}' to see pending codes.\n")
