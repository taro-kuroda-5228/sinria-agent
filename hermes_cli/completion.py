...
def _profiles_home() -> str:
    return "$HOME/.sinria/profiles" if _cli_command_name() == "sinria" else "$HOME/.hermes/profiles"
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    profiles_home = _profiles_home()

    return f"""# {product} bash completion
# Add to ~/.bashrc:
#   eval "$({cli} completion bash)"

_hermes_profiles() {{
    local profiles_dir="{profiles_home}"
...
complete -F _hermes_completion {cli}
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef {cli}
# {product} zsh completion
# Add to ~/.zshrc:
...
_hermes_profiles() {{
    ...
    if [[ -d "{profiles_home}" ]]; then
        profiles+=("${{(@f)$(ls {profiles_home} 2>/dev/null)}}")
    fi
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        f"# {product} fish completion",
        "# Add to your config:",
        f"#   {cli} completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __hermes_profiles",
        "    echo default",
        f"    if test -d {profiles_home}",
        f"        ls {profiles_home} 2>/dev/null",
        "    end",
        "end",
        "",
        ...
        "",
        "# Complete profile names after -p / --profile",
        f"complete -c {cli} -f -s p -l profile"
