"""Profile distributions — shareable, packaged runtime profiles via git.

A distribution is a runtime profile published as a git repository (or
...
* ``profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``profile``, not a parallel tree):

    profile install <source> [--name N] [--alias] [--force] [--yes]
    profile update  <name>  [--force-config] [--yes]
    profile info    <name>
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "backups", "cache",
    # Infrastructure
    "sinria-agent", ".worktrees", "profiles", "bin", "node_modules",
    ...
    with tempfile.TemporaryDirectory(prefix="hermes_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                f"Use `{_profile_cmd('update')}` to upgrade in place, "
...
def update_distribution(
    ...
    if existing_manifest is None:
        raise DistributionError(
            ...
            f"Only profiles installed via `{_profile_cmd('install')}` can be updated."
        )
    if not existing_manifest.source:
        raise DistributionError(
            f"Profile '{canon}' has no recorded source.  Re-install with "
            f"`{_profile_cmd('install')} <source> --name {canon} --force`."
        )
