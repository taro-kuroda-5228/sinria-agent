...
OPENROUTER_MODELS: list[tuple[str, str]] = [
    ...
    ("openai/gpt-5.6",                         "alias to gpt-5.6-sol"),
    ("openai/gpt-5.6-sol",                     "frontier capability"),
    ("openai/gpt-5.6-terra",                   "balanced intelligence/cost"),
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    ...
    sinria-agent is tool-calling-first — every provider path assumes the model
    ...
    *,
    ...
    for preferred_id in preferred_ids:
        ...
        if live_item is None:
            continue
        # Hide models that don't advertise tool-calling support — sinria-agent
...
def _fetch_anthropic_models(timeout: float = 5.0) -> Optional[list[str]]:
    ...
    try:
        ...
        # Sort: latest/largest first.  Fable 5 is the current headline
        # subscription model and should stay at the top of the picker even
        # when the live Anthropic catalog is available.
        return sorted(models, key=lambda m: (
            "fable-5" not in m,   # Fable 5 first
            "opus" not in m,      # then opus
