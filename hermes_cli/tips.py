...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or the CLI resume shortcut.",
    ...
    "Ctrl+Z suspends the CLI to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "The `-c` flag resumes your most recent CLI session. `-c \"project name\"` resumes by title.",
    "The `-w` flag creates an isolated git worktree — perfect for parallel agent workflows.",
    "The `-w -q \"Fix issue #42\"` form combines worktree isolation with a one-shot query.",
    "The `chat -t web,terminal` form enables only specific toolsets for a focused session.",
    "The `chat -s github-pr-workflow` form preloads a skill at launch.",
    "The `chat -q \"query\"` form runs a single non-interactive query and exits.",
    "The `chat --max-turns 200` form overrides the default 90-iteration limit per turn.",
    "The `chat --checkpoints` form enables filesystem snapshots before every destructive file change.",
    "The `--yolo` flag bypasses all dangerous command approval prompts for the entire session.",
    "The `chat --source telegram` form tags the session for filtering in the sessions list.",
    "The `-p work chat` form runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "The `doctor --fix` command diagnoses and auto-repairs config and dependency issues.",
    "The `dump` command outputs a compact setup summary — great for bug reports.",
    "The `config set KEY VALUE` command auto-routes secrets to .env and everything else to config.yaml.",
    "The `config edit` command opens config.yaml in your default editor.",
    "The `config check` command scans for missing or stale configuration options.",
    "Sinria sessions browse opens an interactive session picker with search.",
    "Sinria sessions stats shows session counts by platform and database size.",
    "Sinria sessions prune --older-than 30 cleans up old sessions.",
    "The `skills search react --source skills-sh` command searches the skills.sh public directory.",
    "The `skills check` command scans installed hub skills for upstream updates.",
    "The `skills tap add myorg/skills-repo` command adds a custom GitHub skill source.",
    "The `skills snapshot export setup.json` command exports your skill configuration for backup or sharing.",
    "The `mcp add github --command npx` command adds MCP servers from the command line.",
    "The `mcp serve` command runs the CLI itself as an MCP server for other agents.",
    "The `auth add` command lets you add multiple API keys for credential pool rotation.",
    "The `completion bash >> ~/.bashrc` command enables tab completion for all commands and profiles.",
    "The `logs -f` command follows agent.log in real time. `--level WARNING --since 1h` filters output.",
    "The `backup` command creates a zip backup of your entire runtime home directory.",
    "The `profile create coder` command creates an isolated profile that becomes its own command.",
    "The `profile create work --clone` command copies your current config and keys to a new profile.",
    "The `update` command syncs new bundled skills to ALL profiles automatically.",
    "The `gateway install` command sets up the CLI as a system service (systemd/launchd).",
    "The `memory setup` command lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "The `webhook subscribe` command creates event-driven webhook routes with HMAC validation.",
    "Save money: `tools` disables unused tools, `skills config` trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "The `models` command routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as `-w`).",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Sinria tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — creating `profile create coder` creates the `coder` command.",
    "The `profile export coder -o backup.tar.gz` command creates a portable profile archive.",
    ...
    "When exiting, the CLI prints a resume command with session ID and stats.",
    "Sinria sessions export backup.jsonl exports all sessions for backup or analysis.",
    "The `-r SESSION_ID` form resumes any specific past session by its ID.",
    ...
    "The `skills install official/security/1password` command installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under the workspace-local plans directory.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: `cron add --skill blogwatcher \"Check for new posts\"`.",
    ...
    # --- Gateway & Messaging ---
    "The agent runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "The `gateway install` command sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage support to the CLI via a local macOS server.",
    ...
    "Context files (.sinria.md, .hermes.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "SOUL.md in the runtime home is the agent's primary identity — customize it to shape behavior.",
    "The agent loads project context from .sinria.md/.hermes.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    "MCP servers auto-generate toolsets at runtime — `tools` can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in the runtime home checkpoints dir — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "The `chat -Q` form enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "The `plugins install owner/repo` command installs plugins directly from GitHub.",
    ...
    "The `acp` command runs the CLI as an ACP server for VS Code, Zed, and JetBrains integration.",
    ...
    "The `login` command supports OAuth-based auth for Nous and OpenAI Codex providers.",
    ...
    "status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in the runtime home scripts dir and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make the agent your own.",
    "SOUL.md is auto-seeded with a default personality on first run. Edit the runtime-home SOUL.md to customize.",
    ...
    "Before a gateway session reset, the agent auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log in the runtime home tracks every install and removal operation.",
    ...
    "Every interrupt during an agent run is logged to the runtime-home interrupt debug log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — `profile create ls` would be rejected.",
    "The `profile create backup --clone-all` command copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "The `claw migrate --dry-run` command previews OpenClaw migration without writing anything.",
    ...
    '/goal <text> sets a standing Ralph-loop objective — the agent auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of CLI config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while the CLI is working.',
    ...
    '/reload reloads the runtime .env into the running session — pick up new API keys without restarting.',
    ...
    # --- Gateway Hooks ---
    'Gateway hooks live under the runtime home hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a BOOT.md checklist into the runtime home and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    'The `curator run --dry-run` command previews what the curator would archive or consolidate without mutating anything.',
    "The `curator pin <skill>` command hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'The `curator rollback` command restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'The `auth reset <provider>` command clears all cooldowns and exhaustion flags on a credential pool.',
    ...
    'dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'The `dashboard --tui` command embeds the full agent TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in the runtime-home dashboard themes dir with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in from the runtime-home dashboard plugins dir — no npm build required.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to the runtime-home piper cache.',
    ...
    '/reload-skills re-scans the runtime-home skills dir so drop-in skills appear without restarting the session.',
    ...
    '-z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'chat --ignore-user-config skips the runtime-home config.yaml — reproducible bug reports and CI runs.',
    "chat --source tool tags programmatic chats so they don't clutter Sinria sessions list.",
    'dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'Sinria sessions rename <ID> "new title" renames any past session; Sinria sessions delete <ID> removes one.',
    'import restores a session export or profile archive produced by sessions export or profile export.',
    'fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'status --deep runs the full health sweep across every component; plain status is the quick view.',
    ...
    'HERMES_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from the project-local plugins directory — trust-gated by design.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes the CLI block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from Sinria sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into the runtime-home dashboard plugins dir.',
]
...
def get_random_tip(exclude_recent: int = 0) -> str:
    ...
    """
    return _render_tip(random.choice(TIPS))
