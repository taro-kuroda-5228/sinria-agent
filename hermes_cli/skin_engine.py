...
Skins are defined as YAML files in the runtime-home skins directory or as built-in presets.
...
.. code-block:: yaml
    ...
    branding:
      agent_name: "Sinria"          # Banner title, status display
      welcome: "Welcome message"          # Shown at CLI startup
      goodbye: "Goodbye! ⚕"              # Shown on exit
      response_label: " ⚕ Sinria "       # Response box header label
...
.. code-block:: python
    ...
    print(skin.get_branding("agent_name"))  # "Sinria"

    set_active_skin("ares")               # Switch to built-in ares skin
    set_active_skin("mytheme")            # Switch to user skin from ~/.sinria/skins/
...
Drop a YAML file in ``~/.sinria/skins/<name>.yaml`` following the schema above.
...
def _apply_cli_branding_aliases(branding: Dict[str, str]) -> Dict[str, str]:
    """Rewrite default Hermes-facing branding for fork-compatible CLI names.

    Only rewrites the shipped Hermes labels/messages so custom skin branding
    keeps its own identity.
    """
    if _cli_command_name() != "sinria":
        return branding

    aliased = dict(branding)
    if aliased.get("agent_name") == "Sinria":
        aliased["agent_name"] = "Sinria Agent"
    welcome = aliased.get("welcome")
    if isinstance(welcome, str) and "Sinria" in welcome:
        aliased["welcome"] = welcome.replace("Sinria", "Sinria Agent")
    if aliased.get("response_label") == " ⚕ Hermes ":
        aliased["response_label"] = " ⚕ Sinria "
    return aliased
...
_BUILTIN_SKINS: Dict[str, Dict[str, Any]] = {
    "default": {
        "name": "default",
        "description": "Classic Sinria — gold and kawaii",
        "colors": {
        ...
        "branding": {
            "agent_name": "Sinria",
            "welcome": "Welcome to Sinria! Type your message or /help for commands.",
            "goodbye": "Goodbye! ⚕",
            "response_label": " ⚕ Sinria ",
            "prompt_symbol": "❯",
    ...
    "sinria": {
        "name": "sinria",
        "description": "Sinria — confidential-org blue identity",
        "colors": {
        ...
        "spinner": {
            ...
            "thinking_faces": ["(◆)", "(◈)", "(◇)", "(⌁)", "(◎)"],
            "thinking_verbs": [
                "sealing context", "mapping boundaries", "checking egress",
                "hardening paths", "routing safely", "reading the vault",
                "keeping secrets local", "drafting options",
            ],
            "wings": [
                ["⟪◆", "◆⟫"],
                ["⟪◇", "◇⟫"],
        ...
        "branding": {
            "agent_name": "Sinria Agent",
            "welcome": "Welcome to Sinria Agent! Type your message or /help for commands.",
            "goodbye": "Sinria session closed. ◆",
            "response_label": " ◆ Sinria ",
            "prompt_symbol": "◆",
            "help_header": "(◆) Sinria Commands",
        },
        "tool_prefix": "│",
        "banner_logo": """[bold #93C5FD]                 SINRIA AGENT[/]
[bold #93C5FD]███████╗██╗███╗   ██╗██████╗ ██╗ █████╗        █████╗  ██████╗ ███████╗███╗   ██╗████████╗[/]
[bold #60A5FA]██╔════╝██║████╗  ██║██╔══██╗██║██╔══██╗      ██╔══██╗██╔════╝ ██╔════╝████╗  ██║╚══██╔══╝[/]
[#3B82F6]███████╗██║██╔██╗ ██║██████╔╝██║███████║█████╗███████║██║  ███╗█████╗  ██╔██╗ ██║   ██║[/]
[#2563EB]╚════██║██║██║╚██╗██║██╔══██╗██║██╔══██║╚════╝██╔══██║██║   ██║██╔══╝  ██║╚██╗██║   ██║[/]
...
[#1E3A8A]⠀⠀⠀⠀⠀⠀⠀⠀⠀⠈⠻⣿⣿⣿⣿⠟⠁⠀⠀⠀⠀⠀⠀⠀⠀⠀[/]
[#60A5FA]⠀⠀⠀⠀⠀⠀⠀━━━━━━━━━━━━━━━━━━⠀⠀⠀⠀⠀⠀⠀[/]
[#93C5FD]⠀⠀⠀⠀⠀⠀⠀⠀confidential boundary⠀⠀⠀⠀⠀⠀⠀⠀[/]
[#3B82F6]⠀⠀⠀⠀⠀⠀⠀⠀⠀blue runtime online⠀⠀⠀⠀⠀⠀⠀⠀[/]""",
    },
    "ares": {
        "name": "ares",
...
def _build_skin_config(data: Dict[str, Any]) -> SkinConfig:
    ...
    branding = _apply_cli_branding_aliases(branding)
...
def init_skin_from_config(config: dict) -> None:
    ...
    if _cli_command_name() == "sinria" and (not isinstance(skin_name, str) or skin_name.strip() in {"", "default"}):
        set_active_skin("sinria")
    elif isinstance(skin_name, str) and skin_name.strip():
