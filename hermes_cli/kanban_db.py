...
``-p <profile>`` joins the same board as the dispatcher that
...
the profile's runtime home but are otherwise isolated: a worker spawned
...
* ``<root>/kanban/current`` — a one-line text file holding the slug of
  the "currently selected" board. Written by ``kanban boards
  switch <slug>``. When absent, the active board is ``default``.

In standard installs ``<root>`` is the default runtime home. In Docker / custom
deployments where ``HERMES_HOME`` points outside the default runtime home (e.g.
...
# enough that kebab-case names like ``atm10-server`` or ``sinria-agent``
...
def current_board_path() -> Path:
    ...
    One-line text file written by ``kanban boards switch <slug>``
...
def get_current_board() -> str:
    ...
    2. ``<root>/kanban/current`` on disk (set by ``kanban boards
...
def set_current_board(slug: str) -> Path:
    ...
    so that ``kanban boards switch <typo>`` returns an error
...
class DispatchResult:
    ...
    lane (a Claude Code terminal like ``orion-cc``) rather than a named
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    whose assignee maps to a real named profile.
    ...
    *,
    ...
    for row in ready_rows:
        ...
        if not row["assignee"]:
            ...
            continue
        # Skip ready tasks whose assignee is not a real named profile.
        # `_default_spawn` invokes ``-p <assignee>`` which fails
        # with "Profile 'X' does not exist" when the assignee names a
        # control-plane lane (e.g. an interactive Claude Code terminal
        # like ``orion-cc`` / ``orion-research``) rather than a named
...
def _resolve_hermes_argv() -> list[str]:
    ...
    2. ``sys.executable -m hermes_cli.main`` — fallback for setups where
       the CLI is launched from a venv and the ``hermes`` shim is not on
    ...
    *,
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Install Sinria or activate its venv before running the kanban dispatcher."
        )
...
def list_profiles_on_disk() -> list[str]:
    ...
    - the implicit ``default`` profile when the default runtime root exists
