"""CLI hooks — inspect and manage shell-script hooks.

Usage::

    <cli> hooks list
    <cli> hooks test <event> [--for-tool X] [--payload-file F]
    <cli> hooks revoke <command>
    <cli> hooks doctor

Consent records live under the runtime-home shell-hooks allowlist, and
hook definitions come from the ``hooks:`` block in runtime-home config.yaml
...
def hooks_command(args) -> None:
    """Entry point for ``hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print(f"Usage: {_cli_cmd()} hooks {{list|test|revoke|doctor}}")
        print(f"Run '{_cli_cmd()} hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in runtime-home config.yaml.")
        print(f"See `{_cli_cmd()} hooks --help` or")
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (dispatcher wire shape): {json.dumps(parsed)}")
    else:
