#!/usr/bin/env python3
"""
Sinria/Sinria CLI - Main entry point.

Usage examples:
    sinria                     # Interactive chat (default)
    sinria chat                # Interactive chat
    sinria gateway             # Run gateway in foreground
    sinria gateway start       # Start gateway as service
    sinria gateway stop        # Stop gateway service
    sinria gateway status      # Show gateway status
    sinria gateway install     # Install gateway service
    sinria gateway uninstall   # Uninstall gateway service
    sinria setup               # Interactive setup wizard
    sinria logout              # Clear stored authentication
    sinria status              # Show status of all components
    sinria cron                # Manage cron jobs
    sinria cron list           # List cron jobs
    sinria cron status         # Check if cron scheduler is running
    sinria doctor              # Check configuration and dependencies
    sinria honcho setup                    # Configure Honcho AI memory integration
    sinria honcho status                   # Show Honcho config and connection status
    sinria honcho sessions                 # List directory → session name mappings
    sinria honcho map <name>               # Map current directory to a session name
    sinria honcho peer                     # Show peer names and dialectic settings
    sinria honcho peer --user NAME         # Set user peer name
    sinria honcho peer --ai NAME           # Set AI peer name
    sinria honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    sinria honcho mode                     # Show current memory mode
    sinria honcho mode [hybrid|honcho|local]  # Set memory mode
    sinria honcho tokens                   # Show token budget settings
    sinria honcho tokens --context N       # Set session.context() token cap
    sinria honcho tokens --dialectic N     # Set dialectic result char cap
    sinria honcho identity                 # Show AI peer identity representation
    sinria honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    sinria honcho migrate                  # Step-by-step migration guide: OpenClaw native → Sinria + Honcho
    sinria version             Show version
    sinria update              Update to latest version
    sinria uninstall           Uninstall Sinria Agent
    sinria acp                 Run as an ACP server for editor integration
    sinria sessions browse     Interactive session picker with search

    sinria claw migrate --dry-run  # Preview migration without changes
"""
...
# When the user upgrades code via ``git pull`` (or an update
...
# this guard, the CLI can crash on import and the user can't run
# update to recover. Missing the bootstrap means UTF-8 stdio
...
def _add_accept_hooks_flag(parser) -> None:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "(equivalent to SINRIA_ACCEPT_HOOKS=1)."
        ),
    )
...
def _require_tty(command_name: str) -> None:
    ...
    if not sys.stdin.isatty():
        print(
            f"Error: '{_cli_cmd(command_name)}' requires an interactive terminal.\n"
...
# Falls back to the runtime root's active_profile file for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    ...
    # parent directory name (e.g. <runtime-root>/profiles/coder or
    # /opt/data/profiles/coder). If HERMES_HOME points to the runtime root
    # instead (e.g. systemd hardcodes HERMES_HOME=/root/.sinria), we must
    # still read active_profile — the user may have switched profiles via
    # `profile use` and the gateway should honour that choice.
    # See issue #22502.
    hermes_home_env = os.environ.get("SINRIA_HOME") or os.environ.get("HERMES_HOME", "")
    if profile_name is None and hermes_home_env:
        if os.environ.get("HERMES_DISABLE_ACTIVE_PROFILE") == "1":
            return
    ...
    if profile_name is not None:
        try:
            ...
            return
        os.environ["SINRIA_HOME"] = hermes_home
...
# Load the runtime .env first, then project root as dev fallback.
...
from hermes_constants import AI_GATEWAY_BASE_URL, OPENROUTER_BASE_URL, display_hermes_home, display_sinria_home
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo {_cli_command_name()} {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo {_cli_command_name()} {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _print_tui_exit_summary(
    ...
    print(f"  {_cli_command_name()} --tui --resume {target}")
    if title:
        print(f'  {_cli_command_name()} --tui -c "{title}"')
    print()
...
def _ensure_tui_node() -> None:
    ...
    if os.environ.get("SINRIA_HOME"):
        hermes_home = os.environ["SINRIA_HOME"]
        ...
        try:
            ...
            hermes_home = str(Path.home() / (".sinria" if _cli_command_name() == "sinria" else ".hermes"))
    try:
        ...
        result = subprocess.run(
            ...
            env={
                ...
                **({"SINRIA_HOME": hermes_home} if _cli_command_name() == "sinria" else {}),
            },
            capture_output=True,
            text=True,
            check=False,
...
def _launch_tui(
    ...
    if model:
        ...
        env["SINRIA_INFERENCE_MODEL"] = model
        env["HERMES_INFERENCE_MODEL"] = model
    if provider:
        env["HERMES_TUI_PROVIDER"] = provider
        env["SINRIA_INFERENCE_PROVIDER"] = provider
...
def cmd_chat(args):
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print(f"Use '{_cli_cmd('sessions list')}' to see available sessions.")
                sys.exit(1)
        else:
    ...
    if not _has_any_provider_configured():
        print()
        print(
            f"It looks like {_product_short_name()} isn't configured yet -- no API keys or providers found."
        )
        print()
    ...
    # user's runtime-home config.yaml and return built-in defaults. Set BEFORE
...
def cmd_whatsapp(args):
    ...
    if not current_mode:
        print()
        print(f"How will you use WhatsApp with {_product_short_name()}?")
        print()
    ...
    # has no creds.json.  Every subsequent `sinria gateway` then paid a 30s
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in {"y", "yes"}:
            ...
            print(f"  Start the gateway with: {_cli_cmd('gateway')}")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        # and `sinria gateway` skips it cleanly instead of paying a 30s
        ...
        if wa_mode == "bot":
            ...
            print()
            print(f"  Tip: Agent responses are prefixed with '⚕ {product_name}'")
        else:
            print("  Next steps:")
            print(f"    1. Start the gateway:  {cli_name} gateway")
        ...
        print()
        print(f"  Or install as a service: {_cli_cmd('gateway install')}")
    else:
        print(f"⚠ Pairing may not have completed. Run '{_cli_cmd('whatsapp')}' to try again.")
...
def cmd_postinstall(args):
    ...
    print(f"⚕ {_product_short_name()} post-install bootstrap")
    print()
...
def select_provider_and_model(args=None):
    ...
    effective_provider = (
        config_provider or os.getenv("SINRIA_INFERENCE_PROVIDER") or os.getenv("HERMES_INFERENCE_PROVIDER") or "auto"
    )
    ...
    if effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check '{_cli_cmd('model')}' for "
                f"available providers, or run '{_cli_cmd('doctor')}' to diagnose config "
                "issues."
            )
    ...
    # a leftover OPENAI_BASE_URL in the runtime .env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from the active home .env if the active provider is not 'custom'.
...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        print(f"  {_product_short_name()} only falls back to a lightweight backend (OpenRouter,")
...
def _model_flow_openrouter(config, current_model=""):
    ...
    # in-flow (K/R/C) instead of having to edit the runtime .env by hand. The
    # previous bypass-when-key-exists branch left no way to recover from a
    # bad paste short of re-running setup from scratch. OpenRouter
...
def _model_flow_ai_gateway(config, current_model=""):
    ...
    # Route through _prompt_api_key so users can replace a stale/broken key
    # in-flow (K/R/C) instead of having to edit the runtime .env by hand.
...
def _model_flow_xai_oauth(_config, current_model=""):
    ...
    # live only in the pool (e.g. after ``auth add xai-oauth``).  Fall
...
def _model_flow_google_gemini_cli(_config, current_model=""):
    ...
    Flow:
      ...
      5. Save to the runtime config.yaml.
    """
    from hermes_cli.auth import (
...
def _model_flow_custom(config):
    ...
    if probe.get("used_fallback") and probe.get("resolved_base_url"):
        ...
        print(
            ...
            f"{_product_name()} will still save it."
        )
    ...
    if model_name:
        ...
        print(f"Endpoint saved. Use `/model` in chat or `{_cli_cmd('model')}` to set a model.")
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    mode_options = [
        (
            "",
            "Auto-detect",
            "Use Sinria URL heuristics; best for standard OpenAI-compatible endpoints.",
        ),
        (
            "chat_completions",
            "Chat Completions",
...
def _model_flow_azure_foundry(config, current_model=""):
    ...
    * Manual entry when every probe fails (private endpoints, etc.).
    ...
    print(f"Anthropic-style API endpoints.  {_product_short_name()} will probe your")
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    print(f"  GitHub Copilot ACP delegates {_product_short_name()} turns to `copilot --acp`.")
    print(f"  {_product_short_name()} currently starts its own ACP subprocess for each request.")
    print(f"  {_product_short_name()} uses your selected model as a hint for the Copilot ACP session.")
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    """Shared API-key entry point for setup/model flows.
    ...
    recover from a malformed paste without editing the runtime ``.env`` by hand.
    ...
    if choice.startswith("c"):
        ...
        print(
            f"  API key cleared.  Re-run `{_cli_cmd('setup')}` to configure {pconfig.name} again."
        )
        return "", True
    ...
    if provider_id == "gemini" and existing_key:
        ...
        if probe_gemini_tier is not None:
            ...
            if tier == "free":
                ...
                print(
                    f"   {_product_name()} typically makes 3-10 API calls per user turn "
                ...
                print()
                print(
                    f"   To use Gemini with {_product_name()}, enable billing on your "
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            print(
                f"    {_product_name()} will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
            )
            return True
        return False
    ...
    try:
        ...
        print(f"    4. Re-run:               {_cli_cmd('model')}")
        print()
...
def cmd_login(args):
    """Authenticate Sinria CLI with a provider."""
...
def cmd_slack(args):
    ...
    Dispatches ``slack <subcommand>``. Currently supports:
    ...
    if sub in {None, ""}:
        ...
        print(
            f"usage: {_cli_command_name()} slack <subcommand>\n"
            "\n"
            "subcommands:\n"
            "  manifest   Generate a Slack app manifest with every gateway\n"
            "             command registered as a native slash\n"
            "\n"
            f"Run `{_cli_command_name()} slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1
...
def cmd_backup(args):
    """Back up Sinria home directory to a zip file."""
...
def cmd_import(args):
    """Restore a Sinria backup from a zip file."""
...
def cmd_version(args):
    """Show version."""
    cli_name = os.getenv("HERMES_CLI_NAME", "hermes").strip().lower()
    product_name = "Sinria Agent" if cli_name == "sinria" else "Sinria"
...
def cmd_uninstall(args):
    """Uninstall the active CLI product."""
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by update gateway mode so interactive prompts (stash restore,
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next update to stash the
    lockfile — repeatedly.
    """
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``web``).
    ...
    if r1.returncode != 0:
        _say(
            ...
            + ("" if fatal else f" ({_cli_command_name()} web will not be available)")
        )
        _relay(r1)
        if fatal:
...
def _find_stale_dashboard_pids() -> list[int]:
    """Return PIDs of ``sinria dashboard`` processes other than ourselves.

    ``sinria dashboard`` is a long-lived server process commonly started and
    forgotten. When update replaces files on disk, the running
    ...
    patterns = [
        ...
        "sinria dashboard",
        "hermes dashboard",  # legacy compatibility: detect stale pre-fork dashboard processes
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after an update.
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. Update is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent update invocations skip the block until a newer
    ...
    print(
        ...
        f"View anytime: {_cli_cmd('curator status')})"
    )
...
def _kill_stale_dashboard_processes(
    ...
    """Kill running ``sinria dashboard`` processes.

    Called at the end of update (default ``reason``) and also
    from ``dashboard --stop`` (which overrides ``reason``). The
    ...
    if killed:
        ...
        print(f"    {_cli_cmd('dashboard --port <port>')}")
...
def _update_via_zip(args):
    """Update the active CLI product by downloading a ZIP archive.
    ...
    branch = "main"
    zip_url = (
        f"https://github.com/taro-kuroda-5228/sinria-agent/archive/refs/heads/{branch}.zip"
    )

    print("→ Downloading latest version...")
    try:
        tmp_dir = tempfile.mkdtemp(prefix="hermes-update-")
        zip_path = os.path.join(tmp_dir, f"sinria-agent-{branch}.zip")
        ...
        # GitHub ZIPs extract to sinria-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"sinria-agent-{branch}")
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print(f"  Review the result afterward if {_product_short_name()} behaves unexpectedly.")
    ...
    if stash_selector is None:
        print(
            f"⚠ Local changes were restored, but {_product_short_name()} couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                f"⚠ Local changes were restored, but {_product_short_name()} couldn't drop the saved stash entry."
            )
    ...
    print(f"  Review `git diff` / `git status` if {_product_short_name()} behaves unexpectedly.")
    return True


# =========================================================================
# Fork detection and upstream management for update
# =========================================================================

OFFICIAL_REPO_URLS = {
    "https://github.com/taro-kuroda-5228/sinria-agent.git",
    "git@github.com:taro-kuroda-5228/sinria-agent.git",
    "https://github.com/taro-kuroda-5228/sinria-agent",
    "git@github.com:taro-kuroda-5228/sinria-agent",
}
OFFICIAL_REPO_URL = "https://github.com/taro-kuroda-5228/sinria-agent.git"
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print(f"ℹ Your fork is not tracking the official {_product_short_name()} upstream repository.")
        print(f"  This means you may miss updates from the upstream {_product_short_name()}/Hermes codebase.")
        print()
        try:
            response = (
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/taro-kuroda-5228/sinria-agent.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/taro-kuroda-5228/sinria-agent.git' to add later."
            )
...
def _invalidate_update_cache():
    ...
    update, every profile is now current.
    """
    homes = []
...
def _run_install_with_heartbeat(
    ...
    *,
    ...
    know update is still progressing even if pip/uv itself is silent.
    """
...
def _cli_exe_shims(scripts_dir: Path) -> list[Path]:
    ...
    return [
        ...
        scripts_dir / "sinria-gateway.exe",
    ]


def _quarantine_running_cli_exe(scripts_dir: Path) -> list[tuple[Path, Path]]:
    """Pre-empt Windows file lock on the running CLI shim.
    ...
    when the active CLI runs an update, its ``.exe`` shim is the live process,
    and uv fails with ``Access is denied. (os error 5)``.

    We rename live shims to ``*.exe.old.<unix-ms>`` first. uv then writes
    ...
    for shim in _cli_exe_shims(scripts_dir):
        if not shim.exists():
            continue
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    """Roll back ``_quarantine_running_cli_exe`` if uv didn't write replacements."""
...
def _refresh_active_lazy_features() -> None:
    ...
    first use. Update runs ``uv pip install -e .[all]`` which
    ...
    if failed:
        ...
        print(f"  `{_cli_cmd('update')}` once the upstream issue is resolved.")
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    On Windows, pre-renames live ``hermes.exe`` / ``sinria-gateway.exe`` shims
    in the venv Scripts dir before each install attempt so uv can write fresh
    copies (Windows blocks REPLACE on a running .exe but allows RENAME). See
    ``_quarantine_running_cli_exe`` for the rationale.
    """
    ...
    def _install(args: list[str]) -> None:
        ...
        if scripts_dir is not None:
            moved = _quarantine_running_cli_exe(scripts_dir)
        try:
...
def _update_node_dependencies() -> None:
    ...
    for label, path in paths:
        ...
        # streamed output to the runtime update log so nothing is lost.
...
class _UpdateOutputStream:
    """Stream wrapper used during CLI update to survive terminal loss.

    Wraps the process's original stdout/stderr so that:

    * Every write is also mirrored to an append-only runtime update log file
      that users can inspect after the terminal disconnects.
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes CLI update safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run update in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       the runtime update log and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In update gateway mode the update is already
    ...
    try:
        ...
        log_file.write(
            f"\n=== {_cli_command_name()} update started "
...
def _cmd_update_check():
    """Implement update --check: fetch and report without installing."""
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on update without requiring a reinstall. The installer's
    assumption that ``/usr/local/bin`` is on PATH for every standard shell
    breaks on those distros in non-login interactive shells (su, sudo -s,
    tmux panes, some web terminals): /etc/bashrc doesn't add /usr/local/bin
    and /root/.bash_profile doesn't either. Symptom: the active CLI prints
    ``command not found`` even though the symlink lives in /usr/local/bin.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v <cli>'`` already resolves. Idempotent.
    """
    ...
    # /usr/local/bin/<cli>, code at /usr/local/lib/sinria-agent).
    fhs_link = Path(f"/usr/local/bin/{_cli_command_name()}")
    ...
    path_comment = (
        f"# {_product_name()} — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _run_pre_update_backup(args) -> None:
    ...
    HERMES_HOME directories. The ``--backup`` flag on update
    ...
    # Render path using display_hermes_home so the user sees the active runtime home.
    try:
        from hermes_constants import get_sinria_home, display_sinria_home

        home = get_sinria_home()
        try:
            display_path = f"{display_sinria_home()}/{out_path.relative_to(home)}"
        except ValueError:
    ...
    print(f"  Restore:  {_cli_cmd(f'import {out_path}')}")
...
def cmd_update(args):
    """Update the active CLI product to the latest version.
    ...
    if is_managed():
        managed_error(f"update {_product_name()}")
        return
...
def _cmd_update_pip(args):
    """Update Sinria via pip (for PyPI installs)."""
    ...
    uv = shutil.which("uv")
    if uv:
        cmd = [uv, "pip", "install", "--upgrade", "sinria-agent"]
    else:
        cmd = [sys.executable, "-m", "pip", "install", "--upgrade", "sinria-agent"]
    ...
    print(f"✓ Update complete! Restart {_cli_command_name()} to use the new version.")
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    print(f"⚕ Updating {_product_name()}...")
    print()
    ...
    try:
        ...
        if needs_migration:
            ...
            if response in {"", "y", "yes", "auto"}:
                ...
                if (gateway_mode or assume_yes or response == "auto") and missing_env:
                    print(f"  ℹ API keys require manual entry: {_cli_cmd('config migrate')}")
            else:
                print()
                print(f"Skipped. Run '{_cli_cmd('config migrate')}' later to configure.")
        else:
        ...
        # Tying the refresh to update gives users a predictable
        ...
        # When running as update gateway mode (spawned by the gateway's
        ...
        try:
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            unit = parts[
                                0
                            ]  # e.g. sinria-gateway.service or sinria-gateway-coder.service
                            ...
                            # path in `sinria gateway restart`
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print(f"    Restart manually: {_cli_cmd('gateway run')}")
                    if unmapped_count > 1:
                        print(
                            f"    (or: {_cli_command_name()} -p <profile> gateway run  for each profile)"
                        )
        ...
        # Warn if legacy Sinria gateway unit files are still installed.
        # When both hermes.service (from a pre-rename install) and the
        # current sinria-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every update surfaces the issue until the user migrates.
        try:
            from hermes_cli.gateway import (
            ...
            if supports_systemd_services() and has_legacy_hermes_units():
                print()
                print(f"⚠ Legacy {_product_short_name()} gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_hermes_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (hermes.service) fight the current")
                print("  sinria-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print(f"    {_cli_cmd('gateway migrate-legacy')}")
                print()
        ...
        print(f"  {_cli_cmd('model')}              # Select provider and model")
...
def cmd_profile(args):
    ...
    if action is None:
        # Bare `profile` — show current profile status
        profile_name = get_active_profile_name()
        dhh = display_sinria_home()
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    print(f"Alias:          {p.name} → {_cli_command_name()} -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default ({display_sinria_home()})")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  {_cli_cmd(f'profile alias {name} --name <custom>')}"
                    )
                    print(f"  Or access via flag:     {_cli_command_name()} -p {name} chat")
                else:
        ...
        if dist_name:
            ...
            print(f"  (run `{_cli_cmd(f'profile info {name}')}` for full manifest)")
        if wrapper.exists():
        ...
        try:
            ...
            if plan.has_cron:
                print(
                    ...
                    f"  Review them with:  {_cli_command_name()} -p {plan.manifest.name} cron list"
                )
            print(f"\n  Use with:      {_cli_command_name()} -p {plan.manifest.name} chat")
        ...
        try:
            ...
            if current is None:
                print(
                    ...
                    f"Only profiles installed via `{_cli_cmd('profile install')}` can be updated."
                )
                sys.exit(1)
            ...
            if plan.has_cron:
                print(
                    ...
                    f"{_cli_command_name()} -p {plan.manifest.name} cron list"
                )
        except (DistributionError, ValueError) as e:
            print(f"Error: {e}")
            sys.exit(1)
        ...
        if data.get("hermes_requires"):
            print(f"Requires:     {_product_short_name()} {data['hermes_requires']}")
        if data.get("source"):
...
def _render_distribution_plan(plan) -> None:
    ...
    if mf.hermes_requires:
        print(f"  Requires: {_product_short_name()} {mf.hermes_requires}")
...
def _report_dashboard_status() -> int:
    """Print ``sinria dashboard`` PIDs and return the count.

    Uses the same detection logic as ``_find_stale_dashboard_pids`` (the
    current process is excluded, but since ``dashboard --status``
    runs in a short-lived CLI process that never matches the pattern,
    the exclusion is irrelevant here).
    """
    pids = _find_stale_dashboard_pids()
    if not pids:
        print(f"No {_cli_command_name()} dashboard processes running.")
        return 0

    print(f"{len(pids)} {_cli_command_name()} dashboard process(es) running:")
    for pid in pids:
...
def cmd_dashboard(args):
    ...
    if getattr(args, "stop", False):
        ...
        if not pids:
            print(f"No {_cli_command_name()} dashboard processes running.")
            sys.exit(0)
        # Reuse the same SIGTERM-grace-SIGKILL path used after update.
...
def cmd_logs(args):
    """View and filter agent log files."""
...
def main():
    """Main entry point for the active CLI product."""
    ...
    # Update runs on Windows. Silent no-op on non-Windows or when
    # there's nothing to clean. See ``_quarantine_running_cli_exe``.
    try:
    ...
    model_parser.add_argument(
        "--client-id",
        default=None,
        help=f"OAuth client id to use for Nous login (default: {_cli_command_name()}-cli)",
    )
    model_parser.add_argument(
    ...
    fallback_subparsers.add_parser(
        "add",
        help=f"Pick a provider + model (same picker as `{_cli_cmd('model')}`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    proxy_start.add_argument(
        "--provider",
        default="nous",
        help=f"Upstream provider (default: nous). See `{_cli_cmd('proxy providers')}`.",
    )
    proxy_start.add_argument(
        "--host",
        default=None,
    ...
    setup_parser = subparsers.add_parser(
        ...
        description=f"Configure {_product_name()} with an interactive wizard. "
        f"Run a specific section: {_cli_cmd('setup')} model|tts|terminal|gateway|tools|agent",
    )
    ...
    setup_parser.add_argument(
        ...
        f"compatibility — a bare '{_cli_cmd('setup')}' now does this.",
    )
    setup_parser.add_argument(
        "--quick",
        action="store_true",
    ...
    slack_parser = subparsers.add_parser(
        ...
        description=f"Slack integration helpers for {_product_short_name()}.",
    )
    ...
    slack_manifest.add_argument(
        "--name",
        default=None,
        help=f'Bot display name (default: "{_product_short_name()}")',
    )
    slack_manifest.add_argument(
        "--description",
        default=None,
    ...
    login_parser = subparsers.add_parser(
        ...
        description=f"Run OAuth device authorization flow for {_product_name()}",
    )
    ...
    login_parser.add_argument(
        "--client-id", default=None, help=f"OAuth client id to use (default: {_cli_command_name()}-cli)"
    )
    ...
    auth_spotify = auth_subparsers.add_parser(
        "spotify",
        help=f"Authenticate {_product_short_name()} with Spotify via PKCE",
        description=f"Authenticate {_product_short_name()} with Spotify via PKCE",
    )
    ...
    status_parser = subparsers.add_parser(
        ...
        description=f"Display status of {_product_name()} components",
    )
    ...
    cron_create.add_argument(
        "--script",
        help=(
            f"Path to a script under {display_sinria_home()}/scripts/. Default mode: "
    ...
    cron_edit.add_argument(
        "--script",
        help=(
            f"Path to a script under {display_sinria_home()}/scripts/. Pass empty string to clear. "
    ...
    hooks_parser = subparsers.add_parser(
        ...
        description=(
            f"Inspect shell-script hooks declared in {display_sinria_home()}/config.yaml, "
            f"test them against synthetic payloads, and manage the first-use "
            f"consent allowlist at {display_sinria_home()}/shell-hooks-allowlist.json."
        ),
    )
    ...
    doctor_parser = subparsers.add_parser(
        ...
        description=f"Diagnose issues with {_product_name()} setup",
    )
    ...
    doctor_parser.add_argument(
        ...
        help=(
            ...
            f"advisory will no longer trigger startup banners. Run `{_cli_cmd('doctor')}` "
            "first to see active advisories and their IDs."
        ),
    )
    ...
    dump_parser = subparsers.add_parser(
        ...
        description=f"Output a compact, plain-text summary of your {_product_short_name()} setup "
    ...
    debug_epilog = "\n".join([
        "Examples:",
        f"    {_cli_cmd('debug share')}              Upload debug report and print URL",
        f"    {_cli_cmd('debug share')} --lines 500  Include more log lines",
        f"    {_cli_cmd('debug share')} --expire 30  Keep paste for 30 days",
        f"    {_cli_cmd('debug share')} --local      Print report locally (no upload)",
        f"    {_cli_cmd('debug share')} --no-redact  Disable upload-time secret redaction",
        f"    {_cli_cmd('debug delete')} <url>       Delete a previously uploaded paste",
    ])
    debug_parser = subparsers.add_parser(
        "debug",
        help="Debug tools — upload logs and system info for support",
        description=f"Debug utilities for {_product_name()}. Use '{_cli_cmd('debug share')}' to "
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help=f"Delete a paste uploaded by '{_cli_cmd('debug share')}'",
    )
    ...
    backup_parser = subparsers.add_parser(
        "backup",
        help=f"Back up {product_name.split()[0]} home directory to a zip file",
        description=f"Create a zip archive of your entire {product_name.split()[0]} configuration, "
        "skills, sessions, and data (excludes the local agent codebase). "
        "Use --quick for a fast snapshot of just critical state files.",
    )
    backup_parser.add_argument(
        "-o",
        "--output",
        help=f"Output path for the zip file (default: ~/{_cli_command_name()}-backup-<timestamp>.zip)",
    )
    backup_parser.add_argument(
        "-q",
        "--quick",
    ...
    checkpoints_parser = subparsers.add_parser(
        "checkpoints",
        help=f"Inspect / prune / clear {home_display}/checkpoints/",
        description="Manage the filesystem checkpoint store — the shadow git "
        f"repo {cli_name} uses to snapshot working directories before "
    ...
    import_parser = subparsers.add_parser(
        "import",
        help=f"Restore a {_product_short_name()} backup from a zip file",
        description=f"Extract a previously created {_product_short_name()} backup into your "
        f"{_product_short_name()} home directory, restoring configuration, skills, "
        "sessions, and data",
    )
    ...
    config_parser = subparsers.add_parser(
        ...
        description=f"Manage {_product_name()} configuration",
    )
    ...
    skills_record = skills_subparsers.add_parser(
        "record",
        help="Turn local workflow notes into a privacy-first Sinria skill",
        description=(
            "Create an inspectable/editable Sinria skill from a local workflow recording, "
            "notes file, or stdin. Raw recordings are not stored by default; any optional "
            "reference notes are sanitized before writing."
        ),
    )
    skills_record.add_argument("input", help="Path to workflow notes/recording text, or '-' for stdin")
    skills_record.add_argument("--name", required=True, help="Skill name, e.g. billing-report-download")
    skills_record.add_argument("--description", required=True, help="One-line skill description")
    skills_record.add_argument("--category", default="", help="Optional one-segment category folder")
    skills_record.add_argument("--trigger", default=None, help="When this workflow skill should be used")
    skills_record.add_argument(
        "--required-input",
        action="append",
        default=[],
        help="Input needed when replaying (repeatable)",
    )
    skills_record.add_argument(
        "--skills-dir",
        default=None,
        help=f"Override skills root (default: {display_sinria_home()}/skills)",
    )
    skills_record.add_argument(
        "--save-demo",
        action="store_true",
        help="Also save sanitized notes under references/recording.md (raw content is never stored)",
    )
    skills_record.add_argument(
        "--force",
        action="store_true",
        help="Replace an existing skill directory with the same name",
    )

    skills_record_video = skills_subparsers.add_parser(
        "record-video",
        help="Record or import a local screen video and turn it into a privacy-first Sinria skill",
        description=(
            "Create an inspectable/editable Sinria skill from a local screen/video recording. "
            "Raw video bytes and local paths are not copied into the skill; only sanitized metadata, "
            "review checkpoints, and replay instructions are written."
        ),
    )
    skills_record_video.add_argument("--video", default=None, help="Existing local screen/video recording to convert")
    skills_record_video.add_argument(
        "--record-duration",
        type=int,
        default=None,
        help="Record the local screen for N seconds first (macOS ffmpeg backend; local only)",
    )
    skills_record_video.add_argument("--output", default=None, help="Output path when using --record-duration")
    skills_record_video.add_argument("--name", default=None, help="Skill name, e.g. browser-export-replay (optional with --interactive)")
    skills_record_video.add_argument("--description", default=None, help="One-line skill description (optional with --interactive)")
    skills_record_video.add_argument("--category", default="", help="Optional one-segment category folder")
    skills_record_video.add_argument("--notes", default=None, help="Optional sanitized operator notes to guide step extraction")
    skills_record_video.add_argument("--notes-file", default=None, help="Optional local notes file to guide step extraction")
    skills_record_video.add_argument(
        "--interactive",
        action="store_true",
        help="Prompt for purpose, safety gates, completion condition, and required replay inputs before writing the skill",
    )
    skills_record_video.add_argument(
        "--required-input",
        action="append",
    ...
    skills_record_video.add_argument(
        "--save-demo",
        action="store_true",
        help="Also save sanitized extracted notes under references/recording.md (raw video is never stored)",
    )
    skills_record_video.add_argument(
        "--force",
        action="store_true",
    ...
    skills_reset = skills_subparsers.add_parser(
        ...
        description=(
            f"Clear a bundled skill's entry from the sync manifest ({display_sinria_home()}/skills/.bundled_manifest) "
            f"so future '{_cli_cmd('update')}' runs stop marking it as user-modified. Pass --restore to also "
    ...
    plugins_install.add_argument(
        "identifier",
        help=f"Git URL or owner/repo shorthand (e.g. anpicasso/{_cli_command_name()}-plugin-chrome-profiles)",
    )
    plugins_install.add_argument(
        "--force",
        "-f",
    ...
    _install_enable_group.add_argument(
        "--no-enable",
        action="store_true",
        help=f"Install disabled (skip confirmation prompt); enable later with `{_cli_cmd('plugins enable <name>')}`",
    )
    ...
    def cmd_memory(args):
        ...
        if sub == "off":
            ...
            if not existing:
                print(
                    f"\n  Nothing to reset — no memory files found in {display_sinria_home()}/memories/\n"
                )
                return
            ...
            print(f"  Files were in: {display_sinria_home()}/memories/\n")
        else:
    ...
    tools_parser = subparsers.add_parser(
        ...
        description=(
            ...
            f"Run '{_cli_cmd('tools')}' with no subcommand for the interactive configuration UI."
        ),
    )
    ...
    # tools disable <name...> [--platform cli]
    ...
    # tools enable <name...> [--platform cli]
    ...
    computer_use_parser = subparsers.add_parser(
        ...
        description=(
            ...
            f"Use `{_cli_cmd('computer-use install')}` to fetch and run the\n"
            "upstream cua-driver installer. This is equivalent to the\n"
            f"post-setup hook that `{_cli_cmd('tools')}` runs when you first\n"
    ...
    def cmd_computer_use(args):
        ...
        if action == "status":
            ...
            if path:
                ...
                print(f"  Refresh to latest: {_cli_cmd('computer-use install --upgrade')}")
                return
            print("cua-driver: not installed")
            print(f"  Run: {_cli_cmd('computer-use install')}")
            return
    ...
    mcp_parser = subparsers.add_parser(
        "mcp",
        help=f"Manage MCP servers and run {_product_short_name()} as an MCP server",
        description=(
            f"Manage MCP server connections and run {_product_name()} as an MCP server.\n\n"
            "MCP servers provide additional tools via the Model Context Protocol.\n"
            f"Use '{_cli_cmd('mcp add')}' to connect to a new server, or\n"
            f"'{_cli_cmd('mcp serve')}' to expose {_product_short_name()} conversations over MCP."
        ),
    )
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve",
        help=f"Run {_product_short_name()} as an MCP server (expose conversations to other agents)",
    )
    ...
    # flag is omitted, causing `mcp add ...` to fall through to
    # interactive chat.
    ...
    sessions_export.add_argument(
        ...
        default="raw",
        help="Export format: raw session backup (default) or ML-ready Sinria agent training JSONL",
    )
    sessions_export.add_argument(
        "--include-reasoning",
        action="store_true",
        help="Include explicit stored provider reasoning fields in training exports; hidden model internals are not reconstructed",
    )
    ...
    def cmd_sessions(args):
        ...
        if action == "list":
            ...
            if getattr(args, "format", "raw") == "training":
                ...
                if args.session_id:
                    ...
                    if not resolved_session_id:
                        ...
                        return
                examples = db.export_training_examples(
                    source=args.source,
                ...
                if args.output == "-":
                    for example in examples:
                        sys.stdout.write(_json.dumps(example, ensure_ascii=False) + "\n")
                else:
                    with open(args.output, "w", encoding="utf-8") as f:
                        for example in examples:
                            f.write(_json.dumps(example, ensure_ascii=False) + "\n")
                    suffix = " with explicit reasoning" if args.include_reasoning else ""
                    print(f"Exported {len(examples)} training examples{suffix} to {args.output}")
                return

            if args.include_reasoning:
                print("Warning: --include-reasoning only applies to --format training; raw export is unchanged.")
    ...
    claw_parser = subparsers.add_parser(
        ...
        description=f"Migrate settings, memories, skills, and API keys from OpenClaw to {_product_short_name()}",
    )
    ...
    claw_migrate = claw_subparsers.add_parser(
        "migrate",
        help=f"Migrate from OpenClaw to {_product_short_name()}",
    ...
    claw_migrate.add_argument(
        "--no-backup",
        action="store_true",
        help=f"Skip the pre-migration zip snapshot of {display_sinria_home()}/ (by default a "
        f"single restore-point archive is written to {display_sinria_home()}/backups/ "
        f"before apply; restorable with '{_cli_cmd('import')}'.",
    )
    claw_migrate.add_argument(
    ...
    update_parser = subparsers.add_parser(
        "update",
        help=f"Update {_product_name()} to the latest version",
    ...
    update_parser.add_argument(
        ...
        action="store_true",
        default=False,
        help=f"Assume yes for interactive prompts (config migration, stash restore). API-key entry is skipped; run '{_cli_cmd('config migrate')}' separately for those.",
    )
    ...
    uninstall_parser = subparsers.add_parser(
        "uninstall",
        help=f"Uninstall {_product_name()}",
        description=f"Remove {_product_name()} from your system. Can keep configs/data for reinstall.",
    )
    ...
    acp_parser = subparsers.add_parser(
        "acp",
        help=f"Run {product_name} as an ACP (Agent Client Protocol) server",
        description=f"Start {product_name} in ACP mode for editor integration (VS Code, Zed, JetBrains)",
    )
    ...
    acp_parser.add_argument(
        ...
        action="store_true",
        dest="acp_version",
        help=f"Print {_product_short_name()} ACP version and exit",
    )
    acp_parser.add_argument(
        "--check",
        action="store_true",
    ...
    acp_parser.add_argument(
        "--setup",
        action="store_true",
        help=f"Run interactive {_product_short_name()} provider/model setup for ACP terminal auth",
    )
    acp_parser.add_argument(
        "--setup-browser",
        action="store_true",
        help=f"Install agent-browser + Playwright Chromium into {display_sinria_home()}/node/ "
    ...
    def cmd_acp(args):
        """Launch the local CLI as an ACP server."""
        try:
    ...
    profile_parser = subparsers.add_parser(
        "profile",
        help=f"Manage profiles — multiple isolated {_product_short_name()} instances",
        description=f"Manage profiles — multiple isolated {_product_short_name()} instances",
    )
    ...
    profile_create.add_argument(
        "--no-skills",
        action="store_true",
        help=f"Create an empty profile with no bundled skills (opts out of `{_cli_cmd('update')}` skill sync)",
    )
    ...
    profile_install = profile_subparsers.add_parser(
        ...
        description=(
            "Install a profile distribution. SOURCE can be a git URL "
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description=f"Launch the {_product_name()} web dashboard for managing config, API keys, and sessions",
    )
    ...
    # for `sinria dashboard` cmdlines and SIGTERM them directly — the same
    # cleanup path update uses for stale dashboards.
    dashboard_parser.add_argument(
        "--stop",
        action="store_true",
        help=f"Stop all running {_cli_command_name()} dashboard processes and exit",
    )
    dashboard_parser.add_argument(
        "--status",
        action="store_true",
        help=f"List running {_cli_command_name()} dashboard processes and exit",
    )
    ...
    logs_parser = subparsers.add_parser(
        "logs",
        help=f"View and filter {_product_short_name()} log files",
        description="View, tail, and filter agent.log / errors.log / gateway.log",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=f"""\
Examples:
    {_cli_command_name()} logs                    Show last 50 lines of agent.log
    {_cli_command_name()} logs -f                 Follow agent.log in real time
    {_cli_command_name()} logs errors             Show last 50 lines of errors.log
    {_cli_command_name()} logs gateway -n 100     Show last 100 lines of gateway.log
    {_cli_command_name()} logs --level WARNING    Only show WARNING and above
    {_cli_command_name()} logs --session abc123   Filter by session ID
    {_cli_command_name()} logs --component tools  Only show tool-related lines
    {_cli_command_name()} logs --since 1h         Lines from the last hour
    {_cli_command_name()} logs --since 30m -f     Follow, starting from 30 min ago
    {_cli_command_name()} logs list               List available log files with sizes
""",
    )
