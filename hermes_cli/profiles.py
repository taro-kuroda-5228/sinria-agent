"""
Profile management for multiple isolated CLI instances.

Each profile is a fully independent HERMES_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under the runtime-home profiles directory by default.

The "default" profile is the runtime home itself — backward compatible,
zero migration needed.

Usage::

    <cli> profile create coder            # fresh profile + bundled skills
    <cli> profile create coder --clone    # also copy config, .env, SOUL.md, skills
    <cli> profile create coder --clone-all  # full copy of source profile
    coder chat                            # use via wrapper alias
    <cli> -p coder chat                   # or via flag
    <cli> profile use coder               # set as sticky default
    <cli> profile delete coder            # remove profile + alias + service
"""
...
def _profile_cmd(command: str) -> str:
    return f"{_cli_command_name()} profile {command}".strip()
...
# default profile runtime home. Named profiles never contain these
# directories at root, so the exclusion is gated to avoid silently dropping
# user data from a named-profile source.
#
# Rationale per item:
#   sinria-agent  — git repo checkout (~84 MB source + ~3 GB venv)
...
# Marker file written by `profile create --no-skills`. When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `update`'s
# all-profile sync, the web dashboard) skip bundled-skill seeding for that
# profile. The user can still install skills manually via `skills install`
# or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Two categories:
      1. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         infrastructure directories that only the default runtime profile
         ever contains. Gated on ``source_dir`` actually
...
# Directories/files to exclude when exporting the default runtime profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "sinria-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
def _get_profiles_root() -> Path:
    ...
    the default runtime home, profiles live under ``HERMES_HOME/profiles/`` so
...
def _get_default_hermes_home() -> Path:
    ...
    In standard deployments this is the default runtime home.
    In Docker/custom deployments where HERMES_HOME is outside that default home
...
def validate_profile_name(name: str) -> None:
    ...
    collisions (for example a profile name matching the base CLI command) or get refused
    at alias-creation time anyway. ``default`` is a special pass-through —
    it's a valid alias for the built-in root profile.
    """
    if name == "default":
        return  # special alias for the default runtime home
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the {_product_name()} installation itself or a common system binary.  "
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    if canon in _HERMES_SUBCOMMANDS:
        return f"'{canon}' conflicts with a {_cli_command_name()} subcommand"
        ...
        if result.returncode == 0:
            ...
            if existing_path == str(wrapper_dir / canon):
                try:
                    ...
                    if f"{_cli_command_name()} -p" in content:
...
def create_wrapper_script(name: str) -> Optional[Path]:
    ...
    try:
        wrapper_path.write_text(f'#!/bin/sh\nexec {_cli_command_name()} -p {canon} "$@"\n')
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``profile list`` for an unrelated profile.
    """
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            f"Cannot create a profile named 'default' — it is the built-in profile ({_display_home()})."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling runtime-home profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
    ...
    # Write the opt-out marker so seed_profile_skills() and `update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                f"(`{_profile_cmd('create --no-skills')}`).\n"
                f"Delete this file to re-enable sync on the next `{_cli_command_name()} update`.\n",
                encoding="utf-8",
            )
        except OSError:
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Profiles that opted out of bundled skills (via ``profile create
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            f"Cannot delete the default profile ({_display_home()}).\n"
            f"To remove everything, use: {_cli_command_name()} uninstall"
        )
...
def set_active_profile(name: str) -> None:
    ...
    Writes to the runtime-home ``active_profile`` marker. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: {_profile_cmd(f'create {canon}')}"
        )
...
def get_active_profile_name() -> str:
    ...
    Returns ``"default"`` if HERMES_HOME is not set or points to the default runtime home.
    Returns the profile name if HERMES_HOME points into the runtime-home profiles tree.
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile is the runtime home itself — its parent is ~/ and its
        # directory name is the hidden runtime-home directory, not "default". We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            f"Specify it explicitly: {_profile_cmd('import <archive> --name <name>')}"
        )
    ...
    # dir. Importing as "default" would target the runtime home itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            f"Cannot import as 'default' — that is the built-in root profile ({_display_home()}). "
            f"Specify a different name: {_profile_cmd('import <archive> --name <name>')}"
        )
