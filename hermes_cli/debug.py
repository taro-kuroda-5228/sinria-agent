"""``debug`` tools for the local CLI.

Currently supports:
    <cli> debug share    Upload debug report (system info + logs) to a
                         ...
                         runtime log files are not leaked into the public
                         paste service. Pass ``--no-redact`` to disable.
"""
...
_REDACTION_BANNER = (
    "[debug share: log content redacted at upload time. "
...
def _pending_file() -> Path:
    """Path to the runtime-home ``pastes/pending.json`` file.
...
def _save_pending(entries: list[dict]) -> None:
    ...
    try:
        ...
        # Non-fatal — worst case the user has to run ``debug delete``
        # manually.
        pass
...
def _sweep_expired_pastes(now: Optional[float] = None) -> tuple[int, int]:
    ...
    intended to be called from every ``debug`` invocation with
    minimal noise.
    """
    entries = _load_pending()
    if not entries:
...
⚠️  This will upload the following to a public paste service:
  • System info (OS, Python version, CLI version, provider, which API keys
...
_GATEWAY_PRIVACY_NOTICE = (
    ...
    "Full logs are NOT included from the gateway — use `debug share` "
...
def delete_paste(url: str) -> bool:
    ...
    req = urllib.request.Request(
        ...
        headers={"User-Agent": "sinria-agent/debug-share"},
    )
...
def _schedule_auto_delete(urls: list[str], delay_seconds: int = _AUTO_DELETE_SECONDS):
    ...
    every ``debug share`` invocation added ~20 MB of resident Python
    interpreters that never exited until the sleep completed.

    The replacement is stateless: we append to the runtime-home pending paste registry
    and the gateway's cron ticker sweeps expired entries once per hour.
    ``debug share`` also runs an opportunistic sweep as a fallback
...
def _delete_hint(url: str) -> str:
    ...
    paste_id = _extract_paste_id(url)
    if paste_id:
        return f"{_cli_command_name()} debug delete {url}"
...
def _upload_paste_rs(content: str) -> str:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "sinria-agent/debug-share",
        },
    )
    with urllib.request.urlopen(req, timeout=30) as resp:
...
def _capture_dump() -> str:
    """Run ``dump`` and return its stdout as a string."""
...
def collect_debug_report(
    *,
    ...
    dump_text
        Pre-captured dump output. If empty, ``dump`` is run
        internally.
...
def run_debug_share(args):
    ...
    if redact:
        logger.info(
            "debug share: applied force-mode redaction to log snapshots before upload"
        )
    ...
    print(f"To delete now:  {_cli_command_name()} debug delete <url>")

    print(f"\nShare these links with the support team for help.")
...
def run_debug_delete(args):
    ...
    if not urls:
        print(f"Usage: {_cli_command_name()} debug delete <url> [<url> ...]")
        print("  Deletes paste.rs pastes uploaded by 'debug share'.")
        return
...
def run_debug(args):
    ...
    # Opportunistic sweep of expired pastes on every ``debug`` call.
    ...
    # best-effort — any failure is swallowed so ``debug`` stays
    ...
    if subcmd == "share":
        ...
        # Default: show help
        print(f"Usage: {_cli_command_name()} debug <command>")
        print()
        print("Commands:")
