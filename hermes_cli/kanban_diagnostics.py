...
def _profile_cmd(profile: str, command: str) -> str:
    return f"{_cli_command_name()} -p {profile} {command}".strip()


def _kanban_cmd(command: str) -> str:
    return f"{_cli_command_name()} kanban {command}".strip()
...
def _rule_repeated_failures(task, events, runs, now, cfg) -> list[Diagnostic]:
    ...
    if most_recent_outcome == "spawn_failed" and assignee and assignee != "default":
        ...
        actions.append(DiagnosticAction(
            kind="cli_hint",
            label=f"Verify profile: {_profile_cmd(assignee, 'doctor')}",
            payload={"command": _profile_cmd(assignee, 'doctor')},
            suggested=True,
        ))
        actions.append(DiagnosticAction(
            kind="cli_hint",
            label=f"Fix profile auth: {_profile_cmd(assignee, 'auth')}",
            payload={"command": _profile_cmd(assignee, 'auth')},
        ))
        ...
        if task_id:
            actions.append(DiagnosticAction(
                kind="cli_hint",
                label=f"Check logs: {_kanban_cmd(f'log {task_id}')}",
                payload={"command": _kanban_cmd(f'log {task_id}')},
                suggested=True,
            ))
...
def _rule_stranded_in_ready(task, events, runs, now, cfg) -> list[Diagnostic]:
    ...
    * Dispatcher is misconfigured (wrong board, wrong HERMES_HOME).
    ...
    works for runtime profiles, registered lanes, external workers, and
