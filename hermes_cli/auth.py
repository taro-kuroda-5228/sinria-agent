"""
Multi-provider authentication system for the local CLI.
...
def _auth_cmd(command: str) -> str:
    return f"{_cli_command_name()} auth {command}".strip()
...
SPOTIFY_DOCS_URL = "https://sinria-agent.nousresearch.com/docs/user-guide/features/spotify"
SPOTIFY_DASHBOARD_URL = "https://developer.spotify.com/dashboard"
SPOTIFY_ACCESS_TOKEN_REFRESH_SKEW_SECONDS = 120

XAI_OAUTH_DOCS_URL = "https://sinria-agent.nousresearch.com/docs/guides/xai-grok-oauth"
OAUTH_OVER_SSH_DOCS_URL = "https://sinria-agent.nousresearch.com/docs/guides/oauth-over-ssh"
...
def _resolve_api_key_provider_secret(
    ...
    for env_var in pconfig.api_key_env_vars:
        # Check both os.environ and the runtime .env file
...
def format_auth_error(error: Exception) -> str:
    ...
    if error.relogin_required:
        return f"{error} Run `{_cli_command_name()} model` to re-authenticate."
...
def _load_global_auth_store() -> Dict[str, Any]:
    ...
    runtime-root auth store even when HERMES_HOME is set to a profile
...
def read_credential_pool(provider_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    ``auth add <provider>`` inside the profile, profile entries
...
def is_provider_explicitly_configured(provider_id: str) -> bool:
    ...
    # not by the user explicitly configuring anthropic in the local CLI.
...
def _get_config_hint_for_unknown_provider(provider_name: str) -> str:
    ...
    try:
        ...
        lines = [f"Config issue detected — run '{_cli_command_name()} doctor' for full diagnostics:"]
        for ci in issues:
    ...
    *,
    ...
    if normalized != "auto":
        ...
        if _config_hint:
            ...
            msg += f" Check '{_cli_command_name()} model' for available providers, or run '{_cli_command_name()} doctor' to diagnose config issues."
    ...
    cli_name = (os.getenv("SINRIA_CLI_NAME") or os.getenv("HERMES_CLI_NAME") or "sinria").strip().lower() or "sinria"
    raise AuthError(
...
#
# Tokens live in the runtime auth/google_oauth.json path (managed by agent.google_oauth).
...
def get_gemini_oauth_auth_status() -> Dict[str, Any]:
    """Return a status dict for `auth list` / `status`."""
    try:
...
# Spotify auth — PKCE tokens stored in the runtime auth store
# =============================================================================
...
def _refresh_spotify_oauth_state(
    ...
    *,
    ...
    if not refresh_token:
        raise AuthError(
            f"Spotify refresh token missing. Run `{_auth_cmd('spotify')}` again.",
            provider="spotify",
    ...
    if response.status_code >= 400:
        detail = response.text.strip()
        raise AuthError(
            f"Spotify token refresh failed. Run `{_auth_cmd('spotify')}` again."
            + (f" Response: {detail}" if detail else ""),
            provider="spotify",
            code="spotify_refresh_failed",
            relogin_required=True,
...
def resolve_spotify_runtime_credentials(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError(
                f"Spotify is not authenticated. Run `{_auth_cmd('spotify')}` first.",
                provider="spotify",
    ...
    if not access_token:
        raise AuthError(
            f"Spotify access token missing. Run `{_auth_cmd('spotify')}` again.",
            provider="spotify",
...
def _spotify_interactive_setup(redirect_uri_hint: str) -> str:
    ...
    print("       App name:     anything (e.g. sinria-agent)")
...
def login_spotify_command(args) -> None:
    ...
    print()
    print(f"Open this URL to authorize {_product_name()}:")
    print(authorize_url)
    print()
...
# =============================================================================
# OpenAI Codex auth — tokens stored in the runtime auth store (not ~/.codex/)
#
# The local CLI maintains its own Codex OAuth session separate from the Codex CLI
# and VS Code extension. This prevents refresh token rotation conflicts
# where one app's refresh invalidates the other's session.
# =============================================================================

def _read_codex_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    """Read Codex OAuth tokens from the runtime auth store.
    ...
    if not state:
        raise AuthError(
            f"No Codex credentials stored. Run `{_cli_command_name()} auth` to authenticate.",
    ...
    if not isinstance(tokens, dict):
        raise AuthError(
            f"Codex auth state is missing tokens. Run `{_cli_command_name()} auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(access_token, str) or not access_token.strip():
        raise AuthError(
            f"Codex auth is missing access_token. Run `{_cli_command_name()} auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
            f"Codex auth is missing refresh_token. Run `{_cli_command_name()} auth` to re-authenticate.",
            provider="openai-codex",
...
def refresh_codex_oauth_pure(
    ...
    *,
    ...
    """Refresh Codex OAuth tokens without mutating local auth state."""
    ...
    if response.status_code != 200:
        ...
        if code == "refresh_token_reused":
            message = (
                ...
                f"then run `{_cli_command_name()} auth` to re-authenticate."
            )
            relogin_required = True
...
def _refresh_codex_auth_tokens(
    ...
    Saves the new tokens to the runtime auth store automatically.
    """
...
def _resolve_codex_runtime_credentials_from_pool() -> Optional[Dict[str, Any]]:
    """Resolve Codex runtime credentials from the credential pool.

    Some auth flows persist Codex OAuth credentials as a pool-only
    ``manual:device_code`` entry.  Older runtime paths still read the legacy
    ``providers.openai-codex`` singleton first; if that singleton is absent,
    use the already-validated pool entry instead of forcing an interactive
    re-login.  This keeps unattended gateway/cron work running after the user
    ...
    return {
        "provider": "openai-codex",
        "base_url": (str(getattr(entry, "base_url", "") or "").strip().rstrip("/") or DEFAULT_CODEX_BASE_URL),
        "api_key": access_token,
        "source": getattr(entry, "source", "credential_pool") or "credential_pool",
...
def resolve_codex_runtime_credentials(
    *,
    ...
    """Resolve runtime credentials from the local Codex token store."""
    try:
        data = _read_codex_tokens()
    except AuthError as exc:
        if exc.provider == "openai-codex" and exc.code in {
            "codex_auth_missing",
    ...
    if should_refresh:
        # Re-read under lock to avoid racing with other local CLI processes
...
# =============================================================================
# xAI Grok OAuth — tokens stored in the runtime auth store
# =============================================================================
...
def _read_xai_oauth_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    ...
    if not state:
        raise AuthError(
            f"No xAI OAuth credentials stored. Select xAI Grok OAuth (SuperGrok Subscription) in `{_cli_command_name()} model`.",
            provider="xai-oauth",
    ...
    if not isinstance(tokens, dict):
        raise AuthError(
            f"xAI OAuth state is missing tokens. Re-authenticate with `{_cli_command_name()} model`.",
            provider="xai-oauth",
    ...
    if not access_token:
        raise AuthError(
            f"xAI OAuth state is missing access_token. Re-authenticate with `{_cli_command_name()} model`.",
            provider="xai-oauth",
    ...
    if not refresh_token:
        raise AuthError(
            f"xAI OAuth state is missing refresh_token. Re-authenticate with `{_cli_command_name()} model`.",
            provider="xai-oauth",
...
def _xai_validate_oauth_endpoint(url: str, *, field: str) -> str:
    ...
    output is cached in the runtime auth store. A single MITM during initial
    ...
    if host != "x.ai" and not host.endswith(".x.ai"):
        raise AuthError(
            ...
            f"discovery; re-authenticate with `{_cli_command_name()} model` to re-fetch.",
            provider="xai-oauth",
            code="xai_discovery_invalid",
        )
    return url
...
def refresh_xai_oauth_pure(
    ...
    *,
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
            f"xAI OAuth is missing refresh_token. Re-authenticate with `{_cli_command_name()} model`.",
            provider="xai-oauth",
            code="xai_auth_missing_refresh_token",
            relogin_required=True,
        )
    ...
    # written by an older build (or hand-edited) may carry a non-xAI
    ...
    # with a clear error so the user can re-run `<cli> model` to refetch.
...
# so a new `<cli> --profile <name> auth add nous --type oauth` can one-tap
...
# ``get_default_hermes_root()`` returns the default runtime root on Linux/macOS,
...
def _nous_shared_auth_dir() -> Path:
    ...
    Linux/macOS classic installs land under the runtime shared directory, native
...
def _nous_shared_store_path() -> Path:
    ...
    # real user's runtime root, refuse rather than silently corrupt cross-profile
...
def _refresh_access_token(
    *,
    ...
    # called POST /api/oauth/token with the local refresh_token without
    # persisting the rotated token back to auth.json — the server then
    # retires the original RT, the next refresh uses it, and the whole
    ...
    if "reuse" in lowered or "reuse detected" in lowered:
        description = (
            ...
            f"custom self-heal hook, or another {_product_name()} install sharing "
            f"{_display_home()}/auth.json) called POST /api/oauth/token with {_product_name()}'s "
            "refresh token without persisting the rotated token back.\n"
            f"Nous refresh tokens are single-use — only {_product_name()} may call the "
            f"refresh endpoint. For health checks, use `{_cli_command_name()} auth status` instead.\n"
            f"Re-authenticate with: {_cli_command_name()} auth add nous"
        )
    ...
    *,
    ...
    for item in data:
        ...
        if isinstance(model_id, str) and model_id.strip():
            mid = model_id.strip()
            # Skip "hermes" model families — they're not reliable for agentic tool-calling
...
def resolve_nous_access_token(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError(
                f"{_product_name()} is not logged into Nous Portal.",
                provider="nous",
                relogin_required=True,
            )
...
def persist_nous_credentials(
    ...
    *,
    ...
    Historically ``auth add nous`` wrote a ``manual:device_code`` pool
    ...
    ``auth add nous --label <name>``).  It gets embedded in the
    ...
    # these credentials via `auth add nous --type oauth`. Best-
...
def resolve_nous_runtime_credentials(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError(f"{_product_name()} is not logged into Nous Portal.",
...
# ~350ms even on the failure path, and read-only UI surfaces (`tools`, status panels,
# subscription-feature checks) call it many times per render — `tools` → "All Platforms"
# was firing the refresh ~31× during one menu paint, racking up >13s of HTTP and burning
# single-use refresh tokens. Cache the snapshot for a few seconds, keyed on the auth.json
# mtime so that auth login/logout/add/remove invalidates naturally on the next call.
...
def get_codex_auth_status() -> Dict[str, Any]:
    ...
    Checks the credential pool first (where `auth` stores credentials),
    then falls back to the legacy provider state.
    """
    # Check credential pool first — this is where `auth` and
    # `model` store device_code tokens.
    try:
        from agent.credential_pool import load_pool
        pool = load_pool("openai-codex")
        if pool and pool.has_credentials():
    ...
    # Fall back to the older provider-state path
    try:
...
def login_command(args) -> None:
    """Deprecated: use model/auth/setup commands instead."""
    print(f"The '{_cli_command_name()} login' command has been removed.")
    print(f"Use '{_cli_command_name()} auth' to manage credentials,")
    print(f"'{_cli_command_name()} model' to select a provider, or '{_cli_command_name()} setup' for full setup.")
    raise SystemExit(0)
...
def _login_openai_codex(
    ...
    *,
    ...
    """OpenAI Codex login via device code flow. Tokens stored in the runtime auth store."""

    del args, pconfig  # kept for parity with other provider login helpers

    # Check for existing local-CLI-owned credentials
    if not force_new_login:
        try:
            ...
            if isinstance(_resolved_key, str) and _resolved_key and not _codex_access_token_is_expiring(_resolved_key, 60):
                print(f"Existing Codex credentials found in {_product_name()} auth store.")
                try:
    ...
    if not force_new_login:
        ...
        if cli_tokens:
            ...
            print(f"{_product_name()} will create its own session to avoid conflicts with Codex CLI / VS Code.")
            try:
            ...
            if do_import in {"y", "yes"}:
                ...
                print(f"{_product_name()} will keep working independently with its own session.")
                print(f"  Config updated: {config_path} (model.provider=openai-codex)")
                return

    # Run a fresh device code flow — the local CLI gets its own OAuth session
    print()
    print("Signing in to OpenAI Codex...")
    print(f"({_product_name()} creates its own session — won't affect Codex CLI or VS Code)")
    print()
...
def _login_xai_oauth(
    ...
    *,
    ...
    if not force_new_login:
        try:
            ...
            if isinstance(api_key, str) and api_key and not _xai_access_token_is_expiring(api_key, 60):
                print(f"Existing xAI OAuth credentials found in {_product_name()} auth store.")
                try:
                    reuse = input("Use existing credentials? [Y/n]: ").strip().lower()
                except (EOFError, KeyboardInterrupt):
                    reuse = "y"
    ...
    print(f"({_product_name()} creates its own local OAuth session)")
    print()
...
def _xai_oauth_build_authorize_url(
    *,
    ...
    # `referrer=sinria-agent` lets xAI attribute local-CLI-originated logins
...
def _xai_oauth_loopback_login(
    *,
    ...
    try:
        ...
        print(f"Open this URL to authorize {_product_name()} with xAI:")
        print(authorize_url)
        print()
...
def _minimax_save_auth_state(auth_state: Dict[str, Any]) -> None:
    """Persist MiniMax OAuth state to the runtime auth store."""
    with _auth_store_lock():
        auth_store = _load_auth_store()
...
def _minimax_oauth_login(
    *, region: str = "global", open_browser: bool = True,
    ...
    print(f"Starting {_product_name()} login via MiniMax ({region}) OAuth...")
...
def resolve_minimax_oauth_runtime_credentials(
    *, min_token_ttl_seconds: int = MINIMAX_OAUTH_REFRESH_SKEW_SECONDS,
    ...
    if not state or not state.get("access_token"):
        raise AuthError(
            f"Not logged into MiniMax OAuth. Run `{_cli_command_name()} model` and select MiniMax (OAuth).",
...
def _nous_device_code_login(
    *,
    ...
    print(f"Starting {_product_name()} login via {pconfig.name}...")
    print(f"Portal: {portal_base_url}")
    if insecure:
    ...
    try:
        ...
        if exc.code == "subscription_required":
            ...
            print()
            print(f"After subscribing, run `{_cli_command_name()} model` again to finish setup.")
            raise SystemExit(1)
        raise
...
def _login_nous(args, pconfig: ProviderConfig) -> None:
    ...
    try:
        ...
        try:
            ...
            if model_ids:
                ...
                if free_tier:
                    ...
                    # as free so users on older builds still see
        ...
        if not selected_model:
            ...
            print(f"  Run `{_cli_command_name()} model` again to switch to Nous Portal.")
            return
...
def logout_command(args) -> None:
    ...
    if clear_provider_auth(target) or should_reset_config:
        ...
        if should_reset_config and os.getenv("OPENROUTER_API_KEY"):
            print(f"{_product_name()} will use OpenRouter for inference.")
        elif should_reset_config:
            print(f"Run `{_cli_command_name()} model` or configure an API key to use {_product_name()}.")
        else:
