"""
CLI fallback — manage the fallback provider chain.
...
Subcommands:
  fallback [list]   Show the current fallback chain (default when no subcommand)
  fallback add      Pick provider + model via the same picker as `model`,
                    then append the selection to the chain
  fallback remove   Pick an entry to delete from the chain
  fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in the runtime ``config.yaml`` (top-level, list of
...
def _fallback_cmd(command: str) -> str:
    return f"{_cli_command_name()} fallback {command}".strip()


def _model_cmd() -> str:
    return f"{_cli_command_name()} model"
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        ...
        # the active CLI's model command to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print(f"  Add one with:  {_fallback_cmd('add')}")
        print()
        return
    ...
    print("  Docs: https://sinria-agent.nousresearch.com/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as the CLI model command, then append the selection to the chain."""
    ...
    print(f"  `{_model_cmd()}` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print(f"  Run `{_fallback_cmd('list')}` to view, or `{_fallback_cmd('remove')}` to delete.")
