"""
Backup and import commands for the CLI.

`backup` creates a zip archive of the active home
root directory (excluding the agent repo and transient files).

`import` restores from a backup zip, overlaying onto
the current HERMES_HOME root.
"""
...
def _cli_command_name() -> str:
    name = os.environ.get("HERMES_CLI_NAME", "hermes").strip().lower()
...
def _default_backup_basename(stamp: str) -> str:
    prefix = "sinria" if _cli_command_name() == "sinria" else "hermes"
...
_EXCLUDED_DIRS = {
    "sinria-agent",     # the codebase repo — re-clone instead
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to the runtime root) should be skipped."""
...
def run_backup(args) -> None:
    """Create a zip backup of the active runtime home."""
    hermes_root = get_default_hermes_root()

    if not hermes_root.is_dir():
        print(f"Error: {_product_name()} home directory not found at {hermes_root}")
        sys.exit(1)
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / _default_backup_basename(stamp)
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / _default_backup_basename(stamp)
    ...
    for dirpath, dirnames, filenames in os.walk(hermes_root, followlinks=False):
        ...
        for fname in filenames:
            ...
            # Skip the output zip itself if it happens to be inside the runtime root
            try:
    ...
    print(f"\nRestore with: {_cli_command_name()} import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    ...
    if not found:
        return False, (
            f"zip does not appear to be a {_product_name()} backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip the config under a hidden runtime-home directory instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a runtime-home dir name
...
def run_import(args) -> None:
    """Restore a runtime-home backup from a zip file."""
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        if (has_config or has_env) and not args.force:
            print()
            print(f"Warning: Target directory already has {_product_name()} configuration.")
        ...
        if profiles_dir.is_dir():
            try:
                ...
                if any(profiles_dir.iterdir()):
                    ...
                    print(f"  Run: {_cli_command_name()} profile list  (after installing {_cli_command_name()})")

        # Guidance
        print()
        if not (hermes_root / "sinria-agent").is_dir():
            codebase_name = "sinria-agent" if _cli_command_name() == "sinria" else "sinria-agent"
            print(f"Note: The {codebase_name} codebase was not included in the backup.")
            print(f"  If this is a fresh install, run: {_cli_command_name()} update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  {_cli_command_name()} -p {pname} gateway install")

        print(f"Done. Your {_product_name()} configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and backup --quick)
# ---------------------------------------------------------------------------
...
# — update snapshots this set before pulling so approved-user lists
...
def run_quick_backup(args) -> None:
    """CLI entry point for backup --quick."""
...
def create_pre_update_backup(
    ...
    update should continue even if the backup fails.
    """
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    restorable with ``import <archive>``.  Writes to
    ...
    # Reuses the shared backups/ directory so `import` and the
