"""CLI for the Kanban board — ``<cli> kanban …`` subcommand.
...
def _check_dispatcher_presence() -> tuple[bool, str]:
    ...
    if pid and not dispatch_on:
        return (
            ...
            f"standalone daemon (`{_cli_cmd('kanban daemon --force')}`)."
        )
    return (
        False,
        "No gateway is running — the task will sit in 'ready' until you "
        "start it. Run:\n"
        f"    {_cli_cmd('gateway start')}\n"
...
def build_parser(parent_subparsers: argparse._SubParsersAction) -> argparse.ArgumentParser:
    ...
    p_daemon = sub.add_parser(
        "daemon",
        help=f"DEPRECATED — dispatcher now runs in the gateway. Use `{_cli_cmd('gateway start')}`.",
    )
    ...
    p_asg = sub.add_parser(
        ...
        help="List known profiles + per-profile task counts "
             "(union of runtime-home profiles and current assignees on the board)",
    )
...
def kanban_command(args: argparse.Namespace) -> int:
    ...
    if not action:
        ...
        if parser is not None:
            ...
            print(
                f"usage: {_cli_cmd('kanban')} <action> [options]\n"
                f"Run '{_cli_cmd('kanban --help')}' for the full list of actions.",
                file=sys.stderr,
            )
        return 0
    ...
    if board_override:
        ...
        if normed != kb.DEFAULT_BOARD and not kb.board_exists(normed):
            print(
                ...
                f"Create it with `{_cli_cmd(f'kanban boards create {normed}')}`.",
                file=sys.stderr,
            )
            return 1
...
def _cmd_boards_list(args: argparse.Namespace) -> int:
    ...
    if not boards:
        print(f"(no boards — create one with `{_cli_cmd('kanban boards create <slug>')}`)")
        return 0
    ...
    if len(boards) > 1:
        print(f"Switch boards with `{_cli_cmd('kanban boards switch <slug>')}`.")
    return 0
...
def _cmd_boards_create(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "switch", False):
        ...
        switch_cmd = _cli_cmd(f"kanban boards switch {meta['slug']}")
        print(f"  Use `{switch_cmd}` to make it current.")
    return 0
...
def _cmd_init(args: argparse.Namespace) -> int:
    ...
    if profiles:
        ...
        print("No profiles found under the runtime-home profiles directory.")
        print(f"Create one with `{_cli_cmd('-p <name> setup')}` before assigning tasks.")
    print()
    print("Next step: start the gateway so ready tasks actually get picked up.")
    print(f"  {_cli_cmd('gateway start')}")
    print()
    print(
...
def _cmd_assignees(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "json", False):
        ...
        return 0
    if not data:
        print(f"(no assignees — create a profile with `{_cli_cmd('-p <name> setup')}`)")
        return 0
    # Header
...
def _cmd_daemon(args: argparse.Namespace) -> int:
    ...
    if not getattr(args, "force", False):
        print(
            f"{_cli_cmd('kanban daemon')}: DEPRECATED — the dispatcher now runs\n"
            "inside the gateway. To use kanban:\n"
            "\n"
            f"    {_cli_cmd('gateway start')}       # starts the gateway + embedded dispatcher\n"
            "\n"
        ...
        if health_state["bad_ticks"] >= HEALTH_WINDOW:
            ...
            if now - health_state["last_warn_at"] >= 300:
                print(
                    ...
                    f"`{_cli_cmd('kanban list --status ready')}` / "
                    f"`{_cli_cmd('kanban list --status blocked')}` for recent "
    ...
    def _ready_queue_nonempty() -> bool:
        ...
        task whose assignee maps to a real profile (i.e. one the
