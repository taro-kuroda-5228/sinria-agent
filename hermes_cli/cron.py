...
def _gateway_install_guidance() -> tuple[str, str]:
    cli_name = _cli_command_name()
    return (
        f"{cli_name} gateway install",
        f"sudo {cli_name} gateway install --system",
    )
...
def cron_list(show_all: bool = False):
    ...
    if not find_gateway_pids():
        gateway_install_cmd, gateway_install_system_cmd = _gateway_install_guidance()
        print(color("  ⚠  Gateway is not running — jobs won't fire automatically.", Colors.YELLOW))
        print(color(f"     Start it with: {gateway_install_cmd}", Colors.DIM))
        print(color(f"                    {gateway_install_system_cmd}  # Linux servers", Colors.DIM))
        print()
...
def cron_status():
    ...
    if pids:
        ...
        print(f"    {gateway_install_cmd}    # Install as a user service")
        print(f"    {gateway_install_system_cmd}  # Linux servers: boot-time system service")
...
def cron_command(args):
    ...
    print(f"Usage: {_cli_command_name()} cron [list|create|edit|pause|resume|run|remove|status|tick]")
    sys.exit(1)
