...
The project docs site hosts a JSON manifest of curated models for providers
...
1. ``get_catalog()`` — returns a parsed manifest dict.
   ...
   - Reads disk cache at ``~/.sinria/cache/model_catalog.json``
     (the legacy runtime-home cache path remains a compatibility alias).
...
def reset_cache() -> None:
    """Clear the in-process cache. Used by tests and ``model --refresh``."""
    global _catalog_cache, _catalog_cache_source_mtime
    _catalog_cache = None
