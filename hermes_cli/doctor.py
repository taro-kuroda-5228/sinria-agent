"""
Doctor command for the local CLI.

Diagnoses issues with local CLI setup.
"""
...
_DHH = display_hermes_home()  # user-facing display path for the active runtime home
...
def _nous_portal_label() -> str:
    return "Medical Horizon Portal" if _cli_command_name() == "sinria" else "Nous Portal"

# Load environment variables from the runtime .env so API key checks work
...
def _cli_command_name() -> str:
    """Return the command name used for user-facing local CLI guidance."""
    return (os.getenv("SINRIA_CLI_NAME") or os.getenv("HERMES_CLI_NAME") or "sinria").strip().lower() or "sinria"


def _doctor_product_name() -> str:
    return "Sinria" if _cli_command_name() == "sinria" else "Hermes"


def _doctor_agent_name() -> str:
    return f"{_doctor_product_name()} Agent"


def _doctor_cmd(command: str) -> str:
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when the runtime .env contains provider auth/base URL settings."""
...
def run_doctor(args):
    ...
    doctor_title = f"🩺 {_doctor_product_name()} Doctor"
    print(color(f"│{doctor_title:^57}│", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
        ...
        if fresh_hits:
            for hit in fresh_hits:
                ...
                manual_issues.append(
                    ...
                    f"`{_doctor_cmd(f'doctor --ack {hit.advisory.id}')}`."
                )
    ...
    # Check the runtime .env (primary location for user config)
    ...
    # Check runtime-home config.yaml (primary) or project cli-config.yaml (fallback)
            ...
            if provider and provider != "auto":
                if catalog_provider is None or (
                    ...
                    issues.append(
                        ...
                        f"Fix: run '{_doctor_cmd('config set model.provider <valid_provider>')}'"
                    )
                    ...
                    if pconfig and getattr(pconfig, "auth_type", "") == "api_key":
                        ...
                        if not configured:
                            check_fail(
                                ...
                                f"(check {_DHH}/.env or run '{_doctor_cmd('setup')}')",
                            )
                            issues.append(
                                f"No credentials found for provider '{runtime_provider}'. "
                                f"Run '{_doctor_cmd('setup')}' or set the provider's API key in {_DHH}/.env, "
                                f"or switch providers with '{_doctor_cmd('config set model.provider <name>')}'"
                            )
                except Exception:
                    pass
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    try:
                        ...
                        issues.append(f"Run '{_doctor_cmd('setup')}' to migrate config")
                else:
                    issues.append(f"Run '{_doctor_cmd('doctor --fix')}' or '{_doctor_cmd('setup')}' to migrate config")
            else:
        ...
        try:
            ...
            if stale_root_keys:
                ...
                if should_fix:
                    ...
                    issues.append(f"Stale root-level provider/base_url in config.yaml — run '{_doctor_cmd('doctor --fix')}'")
        except Exception:
            pass
    ...
    try:
        ...
        portal_auth_label = f"{_nous_portal_label()} auth"
        if nous_status.get("logged_in"):
            check_ok(portal_auth_label, "(logged in)")
        else:
            check_warn(portal_auth_label, "(not logged in)")
    ...
    if _safe_which("codex"):
        ...
        # Native OAuth uses the CLI's own device-code flow — the Codex CLI is
    ...
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", f"(create it to give {_doctor_product_name()} a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text(
                f"# {_doctor_agent_name()} Persona\n\n"
                f"<!-- Edit this file to customize how {_doctor_product_name()} communicates. -->\n\n"
                f"You are {_doctor_product_name()}, a helpful AI assistant.\n",
                encoding="utf-8",
            )
        ...
        try:
            ...
            if wal_size > 50 * 1024 * 1024:  # 50 MB
                ...
                if should_fix:
                    ...
                    issues.append(f"Large WAL file — run '{_doctor_cmd('doctor --fix')}' to checkpoint")
    ...
    if sys.platform != "win32":
        ...
        if _venv_bin is None:
            check_warn(
                ...
                f"({_cli_name} not in venv/bin/ or .venv/bin/ — reinstall with pip install -e '.[all]')"
            )
            manual_issues.append(
            ...
            if _cmd_link.is_symlink():
                ...
                if _target == _expected:
                    check_ok(f"{_cmd_link_display}/{_cli_name} → correct target")
                else:
                    check_warn(
                        f"{_cmd_link_display}/{_cli_name} points to wrong target",
                        f"(→ {_target}, expected → {_expected})"
                    )
                    if should_fix:
                        _cmd_link.unlink()
                        _cmd_link.symlink_to(_venv_bin)
                        check_ok(f"Fixed symlink: {_cmd_link_display}/{_cli_name} → {_venv_bin}")
                        fixed_count += 1
                    else:
                        issues.append(f"Broken symlink at {_cmd_link_display}/{_cli_name} — run '{_doctor_cmd('doctor --fix')}'")
            elif _cmd_link.exists():
                # It's a regular file, not a symlink — possibly a wrapper script
                check_ok(f"{_cmd_link_display}/{_cli_name} exists (non-symlink)")
            else:
                check_fail(
                    f"{_cmd_link_display}/{_cli_name} not found",
                    f"({_cli_name} command may not work outside the venv)"
                )
                if should_fix:
                    _cmd_link_dir.mkdir(parents=True, exist_ok=True)
                    _cmd_link.symlink_to(_venv_bin)
                    check_ok(f"Created symlink: {_cmd_link_display}/{_cli_name} → {_venv_bin}")
                    fixed_count += 1
                    ...
                    issues.append(f"Missing {_cmd_link_display}/{_cli_name} symlink — run '{_doctor_cmd('doctor --fix')}'")
    ...
    if terminal_env == "vercel_sandbox":
        ...
        if importlib.util.find_spec("vercel") is not None:
            ...
            check_fail("vercel SDK not installed", "(pip install 'sinria-agent[vercel]')")
            issues.append("Install the Vercel optional dependency: pip install 'sinria-agent[vercel]'")
            ...
            if r.status_code == 402:
                return _ConnectivityResult(
                    ...
                    ["OpenRouter account has insufficient credits. "
                     f"Fix: run '{_doctor_cmd('config set model.provider <provider>')}' "
    ...
    if hub_dir.exists():
        ...
        check_warn("Skills Hub directory not initialized", f"(run: {_doctor_cmd('skills list')})")
    ...
    if not _active_memory_provider:
        ...
        try:
            ...
            if not _honcho_cfg_path.exists():
                check_warn("Honcho config not found", f"run: {_doctor_cmd('memory setup')}")
            elif not hcfg.enabled:
                check_info(f"Honcho disabled (set enabled: true in {_honcho_cfg_path} to activate)")
            elif not (hcfg.api_key or hcfg.base_url):
                check_fail("Honcho API key or base URL not set", f"run: {_doctor_cmd('memory setup')}")
                issues.append(f"No Honcho API key — run '{_doctor_cmd('memory setup')}'")
            else:
        ...
        try:
            ...
            if mem0_key:
                ...
                check_fail("Mem0 API key not set", f"(set MEM0_API_KEY in .env or run {_doctor_cmd('memory setup')})")
        ...
        try:
            ...
            if _provider and _provider.is_available():
                ...
                check_warn(f"{_active_memory_provider} configured but not available", f"run: {_doctor_cmd('memory status')}")
            else:
                check_warn(f"{_active_memory_provider} plugin not found", f"run: {_doctor_cmd('memory setup')}")
        except Exception as _e:
