"""
Gateway subcommand for the local CLI.

Handles: <cli> gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
# display_sinria_home is imported lazily at call sites to avoid ImportError
# when the compatibility constants module is cached from a pre-update version during `update`.
...
def _cli_entrypoint_module() -> str:
    """Return the product-native module used by service managers.

    ``hermes_cli.main`` remains an internal compatibility entrypoint, but
    generated Sinria launchd/systemd service definitions should launch through
    ``sinria_cli.main`` so process/runtime displays show Sinria ownership.
    """
    return "sinria_cli.main" if _cli_command_name() == "sinria" else "hermes_cli.main"


def _service_entrypoint_argv() -> list[str]:
    """Return the preferred product-native gateway service entrypoint argv."""
    cli_name = _cli_command_name()
    resolved = shutil.which(cli_name)
    if resolved and not (cli_name == "sinria" and "/.hermes/" in resolved.replace("\\", "/")):
        return [resolved]
    local_launcher = PROJECT_ROOT / cli_name
    if local_launcher.exists() and os.access(local_launcher, os.X_OK):
        return [str(local_launcher)]
    return [get_python_path(), "-m", _cli_entrypoint_module()]


def _service_exec_command(profile_arg: str = "") -> str:
    argv = _service_entrypoint_argv()
    if profile_arg:
...
def _plist_string(value: str) -> str:
    import html
    return f"<string>{html.escape(value, quote=True)}</string>"


def _service_uses_python_module_entrypoint() -> bool:
    argv = _service_entrypoint_argv()
    return len(argv) >= 3 and argv[1] == "-m"


def _sinria_visible_service_path(path: str) -> bool:
    if _cli_command_name() != "sinria":
        return True
    return "/.hermes/" not in path.replace("\\", "/")



def _gateway_cmd(command: str, *, sudo: bool = False) -> str:
    prefix = "sudo " if sudo else ""
    return f"{prefix}{_cli_command_name()} gateway {command}".strip()
        ...
        for scope_args in [["systemctl", "--user"], ["systemctl"]]:
            try:
                result = subprocess.run(
                    scope_args + ["list-units", "sinria-gateway*",
...
def _get_parent_pid(pid: int) -> int | None:
    ...
    the CLI that invoked this scan from a real gateway process.
    """
    if pid <= 1:
        return None
    try:
...
def _graceful_restart_via_sigusr1(pid: int, drain_timeout: float) -> bool:
    ...
    relaunch the process after the graceful exit.  launchd intentionally keeps
    the gateway alive even after clean exits so transient shutdowns do not cut
    off messaging-platform contact.
...
def _get_ancestor_pids() -> set[int]:
    ...
    ``sinria gateway status`` from falsely counting the ``hermes`` CLI that
...
def _scan_gateway_pids(exclude_pids: set[int], all_profiles: bool = False) -> list[int]:
    ...
    # scan (e.g. ``sinria gateway status``) is never mistaken for a running
    # gateway.  See #13242.
    ...
    current_cli_name = _cli_command_name()

    def _matches_current_product(command: str) -> bool:
        """Avoid treating different product gateways as the same process.

        Both products may still execute through the compatibility module
        ``hermes_cli.main``. Product identity therefore comes from explicit
        CLI/env/home markers, not just the Python module name.
        """
        if current_cli_name == "sinria":
            if "HERMES_CLI_NAME=hermes" in command or "SINRIA_CLI_NAME=hermes" in command:
                return False
            if (
                " hermes gateway" in command
                or "/hermes gateway" in command
                or command.startswith("hermes gateway")
            ):
                return False
            # Explicit Sinria markers are preferred.  If a process is
            # unmarked, keep matching it for backward compatibility with
            # pre-isolation Sinria gateways; explicit opposite-product markers
            # above still prevent Sinria from sweeping a known other-product process.
            return True
        if "SINRIA_CLI_NAME=sinria" in command or "HERMES_CLI_NAME=sinria" in command:
            return False
        if " sinria gateway" in command or command.startswith("sinria gateway"):
            return False
        return True
        ...
        if is_windows():
            ...
            for line in result.stdout.split("\n"):
                ...
                if line.startswith("CommandLine="):
                    ...
                    if any(p in current_cmd for p in patterns) and _matches_current_product(current_cmd) and (
                ...
                try:
                    ...
                    for entry in os.listdir("/proc"):
                        ...
                        try:
                            ...
                            if any(p in cmdline for p in patterns) and _matches_current_product(cmdline) and (
            ...
            if not _found_via_proc:
                ...
                for line in result.stdout.split("\n"):
                    ...
                    if pid is None:
                        continue
                    if any(pattern in command for pattern in patterns) and _matches_current_product(command) and (
...
def find_gateway_pids(exclude_pids: set | None = None, all_profiles: bool = False) -> list:
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``update``
            needs this because a code update affects every profile.
            When ``False`` (default), only PIDs belonging to the current
            active profile are returned.
    """
...
def find_profile_gateway_processes(
    ...
    """Return running gateway PIDs mapped to profiles via PID files."""
    _exclude = set(exclude_pids or set())
...
def launch_detached_profile_gateway_restart(profile: str, old_pid: int) -> bool:
    ...
    # when the user closes the terminal, leaving ``update`` users
    # with no running gateway until they re-invoke the gateway command
...
def _sync_hermes_home_from_systemd_unit(system: bool) -> None:
    """When acting on a system-scope unit, adopt its configured runtime home.

    Under ``sudo``, runtime-home env vars are stripped and ``HOME=/root``, so
    :func:`get_hermes_home` can fall back to root's default home — the wrong
    profile. The unit file pins ``SINRIA_HOME`` plus legacy ``HERMES_HOME`` for
    the actual gateway process, so we mirror that into our own environment to make
    ``read_runtime_status`` / ``get_running_pid`` read the correct files.
    """
    if not system:
        return
    env = _read_systemd_unit_environment(system=True)
    unit_home = (env.get("SINRIA_HOME") or env.get("HERMES_HOME") or "").strip()
    if not unit_home:
        return
    current = (os.environ.get("SINRIA_HOME") or os.environ.get("HERMES_HOME") or "").strip()
    if current == unit_home:
        return
    os.environ["SINRIA_HOME"] = unit_home
...
def _wait_for_systemd_service_restart(
    *,
    ...
    print(
        ...
        f"  Check status: {_gateway_cmd('status', sudo=system)}\n"
...
def _print_systemd_start_limit_wait(system: bool = False) -> None:
    ...
    print(f"  Wait for the start-limit window to expire, then run: {_gateway_cmd(f'restart{scope_flag}', sudo=system)}")
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    print(f"  This is usually a manual foreground/tmux/nohup run, so `{_gateway_cmd('status').rsplit(' ', 1)[0]}`")
...
def _print_other_profiles_gateway_status() -> None:
    ...
    Shown at the bottom of ``sinria gateway status`` output so users with
...
def _windows_gateway_should_absorb_console_controls() -> bool:
    ...
    Foreground ``sinria gateway run`` must remain interruptible from
    ...
    detached = (os.getenv("SINRIA_GATEWAY_DETACHED") or os.getenv("HERMES_GATEWAY_DETACHED") or "").strip().lower()
...
_SERVICE_BASE = "sinria-gateway"
SERVICE_DESCRIPTION = f"{_gateway_product_name()} Agent Gateway - Messaging Platform Integration"
...
def _profile_arg(hermes_home: str | None = None) -> str:
    ...
    For a runtime-home named profile, returns ``"--profile <name>"``.
...
def get_service_name() -> str:
    ...
    The default runtime home returns ``sinria-gateway`` (backward compatible).
    A named profile returns ``sinria-gateway-<profile>``.
...
# Legacy service names from older installs that predate the
# sinria-gateway rename. Kept as an explicit allowlist (NOT a glob) so
# profile units (sinria-gateway-*.service) and unrelated third-party
...
def _find_legacy_hermes_units() -> list[tuple[str, Path, bool]]:
    """Return ``[(unit_name, unit_path, is_system)]`` for legacy Sinria gateway units.

    Detects unit files installed by older versions that used a
    different service name (e.g. ``hermes.service`` before the rename to
    ``sinria-gateway.service``). When both a legacy unit and the current
    ``sinria-gateway.service`` are active, they fight over the same bot
    ...
    * Explicit allowlist of legacy names (no globbing). Profile units such
      as ``sinria-gateway-coder.service`` and unrelated third-party
...
def has_legacy_hermes_units() -> bool:
    """Return True when any legacy Sinria gateway unit files exist."""
    return bool(_find_legacy_hermes_units())


def print_legacy_unit_warning() -> None:
    """Warn about legacy Sinria gateway unit files if any are installed.
    ...
    if not legacy:
        return
    print_warning("Legacy Sinria gateway unit(s) detected from an older install:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
        print_info(f"    {path}  ({scope} scope)")
    print_info("  These run alongside the current sinria-gateway service and")
    print_info("  cause SIGTERM flap loops — both try to use the same bot token.")
    print_info("  Remove them with:")
    print_info(f"    {_gateway_cmd('migrate-legacy')}")
...
def remove_legacy_hermes_units(
    ...
    """Stop, disable, and remove legacy Sinria gateway unit files.
    ...
    legacy = _find_legacy_hermes_units()
    if not legacy:
        print("No legacy Sinria gateway units found.")
        return 0, []
    ...
    print()
    print("Legacy Sinria gateway unit(s) found:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
    ...
    if interactive and not prompt_yes_no("Remove these legacy units?", True):
        print(f"Skipped. Run again with: {_gateway_cmd('migrate-legacy')}")
        return 0, [p for _, p, _ in legacy]
    ...
    if system_units:
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd removal path, guarded by `if system == "Linux"` / systemd-only branch
            ...
            print_info(f"  Re-run with: {_gateway_cmd('migrate-legacy', sudo=True)}")
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info(f"    {_gateway_cmd('uninstall')}")
    print_info(f"    {_gateway_cmd('uninstall --system', sudo=True)}")
...
def install_linux_gateway_from_setup(force: bool = False) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd install wizard, never invoked on Windows
            print_warning(f"  System service install requires sudo, so {_gateway_product_name()} can't create it from this user session.")
            if run_as_user:
                print_info(f"  After setup, run: {_gateway_cmd(f'install --system --run-as-user {run_as_user}', sudo=True)}")
            else:
                print_info(f"  After setup, run: {_gateway_cmd('install --system --run-as-user <your-user>', sudo=True)}")
            print_info(f"  Then start it with: {_gateway_cmd('start --system', sudo=True)}")
            return scope, False
...
def _launchd_user_home() -> Path:
    ...
    Profile mode often sets ``HOME`` to a profile-scoped directory, but
...
def get_launchd_plist_path() -> Path:
    """Return the launchd plist path, scoped per product/profile.

    Default runtime homes use ``ai.<product>.gateway.plist``.
    Named profiles use ``ai.<product>.gateway-<profile>.plist``.
    """
    suffix = _profile_suffix()
    product = "sinria" if _cli_command_name() == "sinria" else "hermes"
    name = f"ai.{product}.gateway-{suffix}" if suffix else f"ai.{product}.gateway"
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.sinria/sinria-agent  -> /home/alice/.sinria/sinria-agent
      /opt/sinria                 -> /opt/sinria  (kept as-is)
...
def _runtime_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current runtime home to the equivalent under a target user's home.

    When installing a system service via sudo, get_hermes_home() resolves via
    root's environment. This translates it to the target user's equivalent path:
      /root/.sinria                    → /home/alice/.sinria
    ...
    # Profile or subdir of the default runtime home → preserve relative structure.
    try:
        relative = current_home.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path outside the default runtime home — keep as-is.
        return str(current_home)


def _hermes_home_for_target_user(target_home_dir: str) -> str:
    """Legacy compatibility alias for older callers/tests."""
    return _runtime_home_for_target_user(target_home_dir)
...
def _build_service_path_dirs(project_root: Path | None = None) -> list[str]:
    ...
    runtime_node = runtime_home / "node" / "bin"
    if runtime_node.is_dir():
        candidates.append(str(runtime_node))
    runtime_nm = runtime_home / "node_modules" / ".bin"
    if runtime_nm.is_dir():
        candidates.append(str(runtime_nm))
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    path_entries = [p for p in _build_service_path_dirs() if _sinria_visible_service_path(p)]
    resolved_node = shutil.which("node")
    if resolved_node:
        resolved_node_dir = str(Path(resolved_node).resolve().parent)
        if resolved_node_dir not in path_entries and _sinria_visible_service_path(resolved_node_dir):
    ...
    if system:
        ...
        virtual_env_line = f'Environment="VIRTUAL_ENV={venv_dir}"' if _service_uses_python_module_entrypoint() else ""
        return f"""[Unit]
...
User={username}
Group={group_name}
ExecStart={_service_exec_command(profile_arg)}
...
{virtual_env_line}
Environment="SINRIA_HOME={runtime_home}"
Environment="HERMES_HOME={runtime_home}"
Environment="SINRIA_CLI_NAME={_cli_command_name()}"
Environment="HERMES_CLI_NAME={_cli_command_name()}"
Restart=always
RestartSec=5
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    print(f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current {_gateway_product_name()} install")
    return True
...
def _print_system_scope_remediation(action: str) -> None:
    ...
    print_info(f"         {_gateway_cmd('uninstall --system', sudo=True)}")
    print_info(f"         {_gateway_cmd('install')}")
    print_info(f"         {_gateway_cmd('start')}")


def _get_restart_drain_timeout() -> float:
    """Return the configured gateway restart drain timeout in seconds."""
    raw = (os.getenv("SINRIA_RESTART_DRAIN_TIMEOUT") or os.getenv("HERMES_RESTART_DRAIN_TIMEOUT") or "").strip()
    if not raw:
        cfg = read_raw_config()
...
def systemd_install(force: bool = False, system: bool = False, run_as_user: str | None = None):
    ...
    # before installing the new sinria-gateway.service. If both remain, they
    ...
    print()
    print("Next steps:")
    print(f"  {_gateway_cmd(f'start{scope_flag}', sudo=system)}              # Start the service")
    print(f"  {_gateway_cmd(f'status{scope_flag}', sudo=system)}             # Check status")
...
def _require_service_installed(action: str, system: bool = False) -> None:
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {_gateway_cmd(f'install{scope_flag}', sudo=system)}")
        sys.exit(1)
...
def systemd_stop(system: bool = False):
    ...
    try:
        ...
        print(
            ...
            f"check `{_gateway_cmd('status')}` or logs for final shutdown state."
        )
        return
    ...
    if pid is not None:
        ...
        try:
            ...
            print(
                ...
                f"check `{_gateway_cmd('status')}` or logs for final state."
            )
            return
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(f"  Run: {_gateway_cmd(f'restart{scope_flag}', sudo=system)}  # auto-refreshes the unit")
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {_gateway_cmd(f'start{scope_flag}', sudo=system)}")
    ...
    if active_state == "activating" and sub_state == "auto-restart":
        ...
        print(f"  Run after the start-limit window expires: {_gateway_cmd(f'restart{scope_flag}', sudo=system)}")
        ...
        print(f"  Run: systemctl {'--user ' if not system else ''}reset-failed {get_service_name()} && {_gateway_cmd(f'start{scope_flag}', sudo=system)}")
...
def get_launchd_label() -> str:
    """Return the launchd service label, scoped per product/profile."""
    suffix = _profile_suffix()
    product = "sinria" if _cli_command_name() == "sinria" else "hermes"
    return f"ai.{product}.gateway-{suffix}" if suffix else f"ai.{product}.gateway"
...
def generate_launchd_plist() -> str:
    ...
    priority_dirs = [p for p in _build_service_path_dirs() if _sinria_visible_service_path(p)]
    resolved_node = shutil.which("node")
    if resolved_node:
        resolved_node_dir = str(Path(resolved_node).resolve().parent)
        if resolved_node_dir not in priority_dirs and _sinria_visible_service_path(resolved_node_dir):
            priority_dirs.append(resolved_node_dir)
    sane_path = ":".join(
        dict.fromkeys(
            priority_dirs
            + [p for p in os.environ.get("PATH", "").split(":") if p and _sinria_visible_service_path(p)]
        )
    )

    # launchd defaults ExitTimeOut to ~5 seconds. That is much shorter than
    # Sinria's graceful drain window (agent.restart_drain_timeout, default
    # 180s), so a normal service restart can be escalated to SIGKILL while an
    # agent turn is still being drained/resume-marked. Match the systemd unit:
    # drain timeout plus cleanup headroom for adapter disconnect, SessionDB
    # close, and tool subprocess cleanup.
    launchd_exit_timeout = max(60, int(_get_restart_drain_timeout() or 0)) + 30

    # Build ProgramArguments array, including --profile when using a named profile
    prog_args = [_plist_string(part) for part in _service_entrypoint_argv()]
    if profile_arg:
    ...
    cli_name = _cli_command_name()
    profile_isolation_xml = ""
    virtual_env_xml = ""
    if _service_uses_python_module_entrypoint():
        virtual_env_xml = f"""
        <key>VIRTUAL_ENV</key>
        <string>{venv_dir}</string>"""
    if cli_name == "sinria":
        profile_isolation_xml = """
        <key>HERMES_DISABLE_ACTIVE_PROFILE</key>
        <string>1</string>"""
...
<dict>
    ...
    <dict>
        ...
        <key>HERMES_HOME</key>
        <string>{runtime_home}</string>
        <key>SINRIA_CLI_NAME</key>
        <string>{cli_name}</string>
        <key>HERMES_CLI_NAME</key>
        <string>{cli_name}</string>{profile_isolation_xml}
    </dict>
    ...
    <key>ExitTimeOut</key>
    <integer>{launchd_exit_timeout}</integer>
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print(f"  {_gateway_cmd('status')}             # Check status")
    from hermes_constants import display_sinria_home as _dsh
    print(f"  tail -f {_dsh()}/logs/gateway.log  # View logs")
...
def launchd_stop():
    ...
    # `sinria gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def launchd_status(deep: bool = False):
    ...
    if launchd_plist_is_current():
        print(f"✓ Service definition matches the current {_gateway_product_name()} install")
    else:
        print(f"⚠ Service definition is stale relative to the current {_gateway_product_name()} install")
        print(f"  Run: {_gateway_cmd('start')}")
...
def _is_official_docker_checkout() -> bool:
    return (
        str(PROJECT_ROOT) in {"/opt/sinria", "/opt/hermes"}
...
def _guard_official_docker_root_gateway() -> None:
    ...
    if not hasattr(os, "geteuid") or os.geteuid() != 0:
        return
    if _truthy_env(os.getenv("SINRIA_ALLOW_ROOT_GATEWAY") or os.getenv("HERMES_ALLOW_ROOT_GATEWAY")):
        return
    ...
    print(
        ...
        "$SINRIA_HOME and break later non-root dashboard/gateway runs."
    )
    print("  Set SINRIA_ALLOW_ROOT_GATEWAY=1 only if you intentionally accept this risk.")
    sys.exit(1)
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False):
    ...
    # from sibling CLI processes, but foreground `sinria gateway run` still
    ...
    # /restart) rather than through `sinria gateway restart` which already
    ...
    # `sinria gateway start/restart` — leaving the gateway vulnerable to
    ...
    title = f"⚕ {_gateway_product_name()} Gateway Starting..."
    print("┌─────────────────────────────────────────────────────────┐")
    print(f"│{title:^57}│")
    print("├─────────────────────────────────────────────────────────┤")
...
_PLATFORMS = [
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            f"4. Alternatively, provide user ID + password and {_gateway_product_name()} will log in directly",
    ...
    },
    {
        ...
        "vars": [
            ...
            {"name": "MATTERMOST_HOME_CHANNEL", "prompt": "Home channel ID (for cron/notification delivery, or empty to set later with /set-home)", "password": False,
             "help": f"Channel ID where {_gateway_product_name()} delivers cron results and notifications."},
    ...
    },
    {
        ...
        "setup_instructions": [
            "1. Use a dedicated email account for your Sinria agent",
        ...
        "vars": [
            {"name": "EMAIL_ADDRESS", "prompt": "Email address", "password": False,
             "help": f"The email address {_gateway_product_name()} will use (e.g., agent@example.com)."},
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            f"5. {_gateway_product_name()} connects via the BlueBubbles REST API and receives",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            f"4. Enter them below and {_gateway_product_name()} will connect automatically over WebSocket",
        ],
        "vars": [
...
def _all_platforms() -> list[dict]:
    ...
    ``setup gateway`` without needing the gateway to be running.
    ...
    # User-installed platform plugins under the runtime-home plugins directory still require
        ...
        if var.get("is_allowlist"):
            ...
            if value:
                ...
                access_choices = [
                    ...
                    f"Use DM pairing (unknown users request access, you approve with '{_cli_command_name()} pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info(f"  Approve with: {_cli_command_name()} pairing approve <platform> <code>")
                else:
                    print_info(f"  Skipped — configure later with '{_gateway_cmd('setup')}'")
            continue
...
def _setup_weixin():
    ...
    print()
    print_info(f"  1. {_gateway_product_name()} will open Tencent iLink QR login in this terminal.")
    print_info("  2. Use WeChat to scan and confirm the QR code.")
    print_info(f"  3. {_gateway_product_name()} will store the returned account_id/token in {_display_home()}/.env.")
    ...
    if not check_weixin_requirements():
        ...
        print_info(f"  Install them, then rerun `{_gateway_cmd('setup')}`.")
        return
    ...
    if access_idx == 0:
        ...
        print_info(f"  Unknown DM users can request access and you approve them with `{_cli_command_name()} pairing approve`.")
    elif access_idx == 1:
...
def _setup_feishu():
    ...
    if access_idx == 0:
        ...
        print_success("  DM pairing enabled.")
        print_info(f"  Unknown users can request access; approve with `{_cli_command_name()} pairing approve`.")
    elif access_idx == 1:
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info("    signal-cli link -n \"SinriaAgent\"")
...
def _configure_platform(platform: dict) -> None:
    ...
    is needed here. User-installed platform plugins under the runtime-home
    plugins directory must already be in ``plugins.enabled`` before they
    appear in this menu.
    """
    entry = platform.get("_registry_entry")
    ...
    if required:
        print_info(f"  Set these env vars in {_display_home()}/.env: {', '.join(required)}")
    else:
...
def gateway_setup():
    ...
    print()
    setup_title = f"⚕ {_gateway_product_name()} Gateway Setup"
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA))
    print(color(f"│ {setup_title:^55} │", Colors.MAGENTA))
    print(color("├─────────────────────────────────────────────────────────┤", Colors.MAGENTA))
            ...
            if supports_systemd_services() and _system_scope_wizard_would_need_root():
                ...
                try:
                    if supports_systemd_services():
                        ...
                        print_info(f"Start manually: {_gateway_cmd('run')}")
                except UserSystemdUnavailableError as e:
            ...
            if supports_systemd_services() or is_macos() or is_windows():
                ...
                if prompt_yes_no(f"  Install the gateway as a {platform_name} service?{wsl_note} (runs in background, starts on boot)", True):
                    try:
                        ...
                        print_info(f"  You can try manually: {_gateway_cmd('install')}")
                else:
                    print_info(f"  You can install later: {_gateway_cmd('install')}")
                    if supports_systemd_services():
                        print_info(f"  Or as a boot-time service: {_gateway_cmd('install --system', sudo=True)}")
                    print_info(f"  Or run in foreground:  {_gateway_cmd('run')}")
            elif is_wsl():
                print_info("  WSL detected but systemd is not running.")
                print_info(f"  Run in foreground: {_gateway_cmd('run')}")
                print_info(f"  For persistence:   tmux new -s {_cli_command_name()} '{_gateway_cmd('run')}'")
                print_info("  To enable systemd: add systemd=true to /etc/wsl.conf, then 'wsl --shutdown'")
            elif is_termux():
                from hermes_constants import display_sinria_home as _dsh
                print_info("  Termux does not use systemd/launchd services.")
                print_info(f"  Run in foreground: {_gateway_cmd('run')}")
                print_info(f"  Or start it manually in the background (best effort): nohup {_gateway_cmd('run')} >{_dsh()}/logs/gateway.log 2>&1 &")
            else:
                print_info("  Service install not supported on this platform.")
                print_info(f"  Run in foreground: {_gateway_cmd('run')}")
    else:
        print()
        print_info(f"No platforms configured. Run '{_gateway_cmd('setup')}' when ready.")
...
def gateway_command(args):
    ...
    try:
        ...
        # The direct ``sinria gateway install|uninstall|start|stop|restart``
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if is_termux():
            ...
            print(f"Run manually: {_gateway_cmd('run')}")
            sys.exit(1)
        if supports_systemd_services():
            if is_wsl():
                print_warning("WSL detected — systemd services may not survive WSL restarts.")
                print_info(f"  Consider running in foreground instead: {_gateway_cmd('run')}")
                print_info(f"  Or use tmux/screen for persistence: tmux new -s {_cli_command_name()} '{_gateway_cmd('run')}'")
                print()
            ...
            print()
            print(f"  {_gateway_cmd('run')}                              # direct foreground")
            print(f"  tmux new -s {_cli_command_name()} '{_gateway_cmd('run')}'         # persistent via tmux")
            print(f"  nohup {_gateway_cmd('run')} > {_display_home()}/logs/gateway.log 2>&1 &  # background")
            sys.exit(1)
        elif is_container():
            ...
            print()
            print(f"To run the gateway: {_gateway_cmd('run')}")
            sys.exit(0)
        else:
        ...
        if is_termux():
            ...
            print(f"Stop manual runs with: {_gateway_cmd('stop')}")
            sys.exit(1)
        if supports_systemd_services():
        ...
        if supports_systemd_services():
            ...
            print()
            print(f"Or run the gateway directly: {_gateway_cmd('run')}")
            sys.exit(0)
        else:
            print("Not supported on this platform.")
            sys.exit(1)
        ...
        if not service_available:
            ...
            if service_configured:
                ...
                print(f"  Fix the service, then retry: {_gateway_cmd('start')}")
                sys.exit(1)
        ...
        if supports_systemd_services() and (get_systemd_unit_path(system=False).exists() or get_systemd_unit_path(system=True).exists()):
            ...
            if pids:
                ...
                if is_termux():
                    ...
                    print(f"  {_gateway_cmd('install')}")
                else:
                    print("To install as a service:")
                    print(f"  {_gateway_cmd('install')}")
                    print(f"  {_gateway_cmd('install --system', sudo=True)}")
            else:
                ...
                print()
                print("To start:")
                print(f"  {_gateway_cmd('run')}      # Run in foreground")
                if is_termux():
                    print(f"  nohup {_gateway_cmd('run')} > {_display_home()}/logs/gateway.log 2>&1 &  # Best-effort background start")
                elif is_wsl():
                    print(f"  tmux new -s {_cli_command_name()} '{_gateway_cmd('run')}'         # persistent via tmux")
                    print(f"  nohup {_gateway_cmd('run')} > {_display_home()}/logs/gateway.log 2>&1 &  # background")
                elif is_windows():
                    print(f"  {_gateway_cmd('install')}  # Install as Windows Scheduled Task (auto-start on login)")
                else:
                    print(f"  {_gateway_cmd('install')}  # Install as user service")
                    print(f"  {_gateway_cmd('install --system', sudo=True)}  # Install as boot-time system service")
        ...
        # Stop, disable, and remove legacy Sinria gateway unit files from
