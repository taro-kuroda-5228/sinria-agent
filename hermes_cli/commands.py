"""Slash command definitions and autocomplete for the CLI.
...
def _skills_display_path() -> str:
    return "~/.sinria/skills/" if _cli_command_name() == "sinria" else "~/.hermes/skills/"
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("rollback", "List or restore filesystem checkpoints", "Session",
               args_hint="[number]"),
    CommandDef("snapshot", f"Create or restore state snapshots of {_product_short_name()} config/state", "Session",
    ...
    CommandDef("steer", "Inject a message after the next tool call without interrupting", "Session",
               args_hint="<prompt>"),
    CommandDef("goal", f"Set a standing goal {_product_short_name()} works on across turns until achieved", "Session",
    ...
    CommandDef("busy", f"Control what Enter does while {_product_short_name()} is working", "Configuration",
    ...
    CommandDef("reload-mcp", "Reload MCP servers from config", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", f"Re-scan {_skills_display_path()} for newly installed or removed skills",
    ...
    CommandDef("update", f"Update {_product_name()} to the latest version", "Info",
               gateway_only=True),
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    entries.append(("hermes", f"Talk to {_product_short_name()} or run a subcommand", "[subcommand] [args]"))
    seen.add("hermes")
...
def slack_app_manifest(request_url: str = "https://sinria-agent.local/slack/commands") -> dict[str, Any]:
