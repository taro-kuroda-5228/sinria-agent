"""``plugins`` CLI subcommand — install, update, remove, and list plugins.

Plugins are installed from Git repositories into the runtime-home plugins directory.
...
def _plugins_cmd(command: str = "") -> str:
    return f"{_cli_command_name()} plugins {command}".strip()


def _gateway_restart_cmd() -> str:
    return f"{_cli_command_name()} gateway restart"
...
def _resolve_git_executable() -> Optional[str]:
    ...
    Matches other local CLI subprocess resolution: :func:`shutil.which` first,
...
def _missing_requires_env_names(manifest: dict) -> list[str]:
    """Return declared ``requires_env`` names that are unset in the runtime .env."""
...
def _prompt_plugin_env_vars(manifest: dict, console) -> None:
    ...
    Already-set variables are skipped. Values are saved to the runtime ``.env``.
    """
    requires_env = manifest.get("requires_env") or []
    if not requires_env:
        return
...
def _install_plugin_core(identifier: str, *, force: bool) -> tuple[Path, dict, str]:
    """Clone Git plugin into the runtime-home plugins directory.
        ...
        if mv is not None:
            ...
            if mv_int > _SUPPORTED_MANIFEST_VERSION:
                ...
                raise PluginOperationError(
                    ...
                    f"Run {recommended_update_command()} to update the CLI.",
                ) from None
        ...
        if target.exists():
            if not force:
                raise PluginOperationError(
                    ...
                    f"or run `{_plugins_cmd(f'update {plugin_name}')}`.",
                )
...
def cmd_install(
    ...
    if not (target / "plugin.yaml").exists() and not (target / "plugin.yml").exists() and not (
        ...
        console.print(
            ...
            f"or __init__.py. It may not be a valid plugin for this CLI.",
        )
    ...
    if should_enable:
        ...
        console.print(
            ...
            f"Run `{_plugins_cmd(f'enable {installed_name}')}` to activate.[/dim]",
        )

    console.print("[dim]Restart the gateway for the plugin to take effect:[/dim]")
    console.print(f"[dim]  {_gateway_restart_cmd()}[/dim]")
    console.print()
...
def _discover_all_plugins() -> list:
    ...
    user types into ``plugins enable <key>``. For category-namespaced
...
def cmd_list() -> None:
    ...
    if not entries:
        ...
        console.print(f"[dim]Install with:[/dim] {_plugins_cmd('install owner/repo')}")
        return
    ...
    console.print(table)
    console.print()
    console.print(f"[dim]Interactive toggle:[/dim] {_plugins_cmd()}")
    console.print(f"[dim]Enable/disable:[/dim] {_plugins_cmd('enable/disable <name>')}")
...
def cmd_toggle() -> None:
    ...
    if not sys.stdin.isatty():
        ...
        console.print(
            f"[dim]Try: {_plugins_cmd('list')} or {_plugins_cmd('enable <name>')}[/dim]"
        )
        return
...
def _user_installed_plugin_dir(name: str) -> Optional[Path]:
    """Resolved path under the runtime-home plugins directory if it exists."""
    plugins_dir = _plugins_dir()
    try:
        target = _sanitize_plugin_name(name, plugins_dir)
    except ValueError:
...
def dashboard_update_user_plugin(name: str) -> dict[str, Any]:
    """``git pull`` inside the runtime-home plugins directory."""
...
def dashboard_remove_user_plugin(name: str) -> dict[str, Any]:
    """Delete a plugin tree under the runtime-home plugins directory only."""
    plugins_dir = _plugins_dir()
