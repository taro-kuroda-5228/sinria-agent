...
Implements ``mcp add/remove/list/test/configure`` for interactive
MCP server lifecycle management (issue #690 Phase 2).

Relies on tools/mcp_tool.py for connection/discovery and keeps
configuration in the runtime-home config under the ``mcp_servers`` key.
"""
...
def cmd_mcp_add(args):
    ...
    if not url and not command:
        ...
        _info("Examples:")
        _info(f'  {cli} mcp add ink --url "https://mcp.ml.ink/mcp"')
        _info(f'  {cli} mcp add github --command npx --args @modelcontextprotocol/server-github')
        _info(f'  {cli} mcp add myserver --preset mypreset')
        return
    ...
    try:
        ...
        if _confirm("Save config anyway (you can test later)?", default=False):
            ...
            _info(f"Fix the issue, then: {_cli_command_name()} mcp test " + name)
        return
...
def cmd_mcp_list(args=None):
    ...
    if not servers:
        ...
        print()
        _info("Add one with:")
        _info(f'  {cli} mcp add <name> --url <endpoint>')
        _info(f'  {cli} mcp add <name> --command <cmd> --args <args...>')
        print()
        return
...
def cmd_mcp_login(args):
    ...
    if server_config.get("auth") != "oauth":
        ...
        _info(f"Use `{_cli_command_name()} mcp remove` + `{_cli_command_name()} mcp add` to reconfigure auth.")
        return
...
def cmd_mcp_configure(args):
    ...
    if not _sys.stdin.isatty():
        print(f"Error: '{_cli_command_name()} mcp configure' requires an interactive terminal.", file=_sys.stderr)
        _sys.exit(1)
    name = args.name
    servers = _get_mcp_servers()
...
def mcp_command(args):
    """Main dispatcher for ``mcp`` subcommands."""
    ...
    if handler:
        ...
        cli = _cli_command_name()
        _info(f"{cli} mcp serve                              Run as MCP server")
        _info(f"{cli} mcp add <name> --url <endpoint>        Add an MCP server")
        _info(f"{cli} mcp add <name> --command <cmd>         Add a stdio server")
        _info(f"{cli} mcp add <name> --preset <preset>       Add from a known preset")
        _info(f"{cli} mcp remove <name>                      Remove a server")
        _info(f"{cli} mcp list                               List servers")
        _info(f"{cli} mcp test <name>                        Test connection")
        _info(f"{cli} mcp configure <name>                   Toggle tools")
        _info(f"{cli} mcp login <name>                       Re-authenticate OAuth")
        print()
