"""
Status command for the Sinria/Sinria CLI.

Shows the status of all agent components.
"""
...
def _nous_gateway_label() -> str:
    return "Medical Horizon Tool Gateway" if _cli_command_name() == "sinria" else "Nous Tool Gateway"


def _nous_subscription_label() -> str:
    return "Medical Horizon subscription" if _cli_command_name() == "sinria" else "Nous subscription"
...
def redact_key(key: str) -> str:
    ...
    "(not set)" placeholder in dim color to match ``sinria config``'s
...
def _agent_product_name() -> str:
    """Return the product name used in local status output."""
    return "Sinria Agent" if _cli_command_name() == "sinria" else "Sinria"


def _sinria_policy_view(config: dict) -> dict | None:
    sinria_cfg = config.get("sinria") if isinstance(config, dict) else None
...
def show_status(args):
    """Show status of all agent components."""
    show_all = getattr(args, 'all', False)
    deep = getattr(args, 'deep', False)
    cli_name = _cli_command_name()
    product_name = _agent_product_name()
    status_title = f"⚕ {product_name} Status"
    ...
    policy_view = _sinria_policy_view(config) if cli_name == "sinria" else None
    if policy_view:
        ...
        print(f"  Egress:       {active_cfg.get('external_send', '(not set)')}")
        print(f"  Confidential: {active_cfg.get('confidential_external_send', '(not set)')}")
        print(f"  Transcript:   {'local mirror' if raw_history_enabled else 'SQLite only'}")
        print(f"  Curator log:  {'sanitized' if training_log_enabled else 'disabled'}")
    ...
    nous_label = "logged in" if nous_logged_in else f"not logged in (run: {cli_name} auth add nous --type oauth)"
    portal_label = _nous_portal_label()
    print(
        f"  {portal_label:<22}  {check_mark(nous_logged_in)} "
        f"{nous_label}"
    )
    ...
    codex_label = "logged in" if codex_logged_in else f"not logged in (run: {cli_name} model)"
    print(
    ...
    minimax_label = "logged in" if minimax_logged_in else f"not logged in (run: {cli_name} auth add minimax-oauth)"
    print(
    ...
    if managed_nous_tools_enabled():
        ...
        print()
        print(color(f"◆ {_nous_gateway_label()}", Colors.CYAN, Colors.BOLD))
        portal_label = _nous_portal_label()
        if not features.nous_auth_present:
            print(f"  {portal_label}   ✗ not logged in")
        else:
            print(f"  {portal_label}   ✓ managed tools available")
        for feature in features.items():
            if feature.managed_by_nous:
                state = f"active via {_nous_subscription_label()}"
            elif feature.active:
    ...
    if terminal_env == "ssh":
        ...
        sdk_label = "installed" if sdk_ok else "missing (install: pip install 'sinria-agent[vercel]')"
    ...
    try:
        ...
        if snapshot.has_process_service_mismatch:
            ...
            print(f"  Start with:   {_cli_command_name()} gateway")
