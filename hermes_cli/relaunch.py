"""
Unified self-relaunch for the CLI.

Preserves critical flags (--tui, --dev, --profile, --model, etc.) across
process replacement so that ``sessions browse`` or post-setup relaunch
doesn't silently drop the user's UI mode or other preferences.

Also works when the CLI command is not on PATH (e.g. ``nix run`` or ``python -m``).
"""
...
def _extract_inherited_flags(argv: Sequence[str]) -> list[str]:
    """Pull out flags that should carry over into a self-relaunched CLI process."""
    flags: list[str] = []
    i = 0
    while i < len(argv):
        arg = argv[i]
...
def resolve_hermes_bin() -> Optional[str]:
    """Find the active CLI entry point.
...
def relaunch(
    ...
    *,
    ...
    relaunch contract wants: "run the CLI again as if the user had typed
    the new argv".
    ...
    *emulates* exec by spawning the child and exiting the parent, but
    ...
    That's functionally equivalent — the user sees "the old process exited,
    then the new CLI process started" — just with two PIDs in play instead of one.
    """
    ...
    if sys.platform == "win32":
        ...
        try:
            ...
            # cryptic. Common causes: the CLI shim is not on PATH yet (install
            # hasn't propagated User PATH into this shell) or a stale shim.
            cli = _cli_command_name()
            print(
                f"\n{_product_name()} relaunch failed: {exc}\n"
                f"Command: {' '.join(new_argv)}\n"
                f"Fix: open a new terminal so PATH picks up, then re-run {cli}.",
                file=sys.stderr,
            )
            sys.exit(1)
    else:
