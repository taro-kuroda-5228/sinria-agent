"""
Configuration management for the active CLI product.

Config files are stored in the active home directory for easy access:
- <home>/config.yaml  - All settings (model, toolsets, terminal, etc.)
- <home>/.env         - API keys and secrets

This module provides configuration loading, editing, persistence, and
setup-wizard support for the CLI.
"""
...
def _config_cmd(command: str) -> str:
    return f"{_cli_command_name()} config {command}".strip()


def _general_cmd(command: str) -> str:
    return f"{_cli_command_name()} {command}".strip()


def _display_home_str() -> str:
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  {_config_cmd('')}: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade sinria-agent"
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    cli_name = (os.getenv("SINRIA_CLI_NAME") or os.getenv("HERMES_CLI_NAME") or "sinria").strip() or "sinria"
    if method == "nixos":
        return "sudo nixos-rebuild switch"
    if method == "homebrew":
        return "brew upgrade sinria-agent"
    if method == "pip":
        ...
        uv = shutil.which("uv")
        if uv:
            return "uv pip install --upgrade sinria-agent"
        return "pip install --upgrade sinria-agent"
    return f"{cli_name} update"
...
def format_managed_message(action: str = "modify this installation") -> str:
    ...
    if managed_system == "NixOS":
        ...
        return (
            f"Cannot {action}: this {product_name} installation is managed by NixOS "
            f"(HERMES_MANAGED={env_hint}).\n"
            "Edit services.sinria-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this {product_name} installation is managed by Homebrew "
            f"(HERMES_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade sinria-agent"
        )

    return (
        f"Cannot {action}: this {product_name} installation is managed by {managed_system}.\n"
        f"Use your package manager to upgrade or reinstall {product_name}."
    )
...
def get_container_exec_info() -> Optional[dict]:
    ...
    container_name = info.get("container_name", "sinria-agent")
...
DEFAULT_CONFIG = {
    ...
    "sinria": {
        "product_name": "Sinria",
        "policy": {
            "active_profile": "dogfood_frontier",
            "internal_confidential_data": "allowed",
            "internal_storage": "allowed",
            "protects": "external_leakage",
            "profiles": {
                "dogfood_frontier": {
                    "description": "Use trusted frontier providers for dogfooding while keeping local raw history and external-send guardrails.",
                    "provider_trust": "trusted_frontier",
                    "external_send": "ask",
                ...
                "enterprise_guarded_cloud": {
                    "description": "Allow approved cloud providers with stronger redaction and review gates for confidential organizational work.",
                ...
                "sovereign_local_only": {
                    "description": "Restrict model traffic to local or sovereign infrastructure only; no external provider sends.",
    ...
    "browser": {
        ...
        "use_real_profile": False,  # Opt-in: snapshot and attach to the user's local browser profile
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in product-branded header.
    ...
    "code_execution": {
        ...
        #     sinria-agent's own python (sys.executable). Maximum isolation
    ...
    # update model picker lists without shipping a sinria-agent release.
    ...
    "sessions": {
        ...
        # default of ``Sinria sessions prune``.
        "retention_days": 90,
    ...
    # `sinria tools`. These settings tune the backing Responses API call.
    "x_search": {
...
OPTIONAL_ENV_VARS = {
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": f"Azure Foundry base URL (set via '{_general_cmd('model')}' for endpoint-specific config)",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'sinria-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    _KNOWN_KEYS = {
        ...
        "discover_models", "allow_global_key_fallback",
    }
    ...
    allow_global_key_fallback = entry.get("allow_global_key_fallback")
    if isinstance(allow_global_key_fallback, bool):
        normalized["allow_global_key_fallback"] = allow_global_key_fallback
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", f"Run '{_general_cmd('setup')}' to create a valid config")]
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append(f"  \033[2mRun '{_general_cmd('doctor')}' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("HERMES_HOME", display_hermes_home())
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print(f"  Set later with: {_config_cmd('set <key> <value>')}")
...
def read_raw_config() -> Dict[str, Any]:
    """Read the active home config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from the active home config.yaml.
...
def save_config(config: Dict[str, Any]):
    """Save configuration to the active home config.yaml."""
    with _CONFIG_LOCK:
        if is_managed():
...
def load_env() -> Dict[str, str]:
    """Load environment variables from the active home .env.
    ...
    interactive menu render (tools, setup, status
    panels). Sanitisation is O(lines × known-keys), so re-parsing the
    same file on every call was burning ~300ms of CPU per tools
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite the active home .env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in the active home .env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from the active home .env and os.environ.
...
def reload_env() -> int:
    """Re-read the active home .env into os.environ. Returns count of vars updated.
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from the active home .env or environment."""
...
def _sinria_policy_view(config: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    """Return the active Sinria policy profile details, if configured."""
    sinria_cfg = config.get("sinria")
    if not isinstance(sinria_cfg, dict):
        return None
    policy = sinria_cfg.get("policy")
    ...
    if not active or not isinstance(profiles, dict):
        return None
    active_cfg = profiles.get(active)
    if not isinstance(active_cfg, dict):
        return None
    return {"active": active, "config": active_cfg}
...
def show_config():
    ...
    print()
    product_name = _product_name().replace(" Agent", "")
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color(f"│        ⚕ {product_name} Configuration{' ' * max(0, 33 - len(product_name))}│", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    policy_view = _sinria_policy_view(config) if _cli_command_name() == "sinria" else None
    if policy_view:
        print()
        print(color("◆ Sinria Policy", Colors.CYAN, Colors.BOLD))
        active = policy_view["active"]
        active_cfg = policy_view["config"]
        print(f"  Active:       {active}")
        print(f"  Trust:        {active_cfg.get('provider_trust', '(not set)')}")
        print(f"  External:     {active_cfg.get('external_send', '(not set)')}")
        print(f"  Confidential: {active_cfg.get('confidential_external_send', '(not set)')}")
        raw_history_enabled = bool(active_cfg.get('retain_raw_history_locally'))
        training_log_enabled = bool(active_cfg.get('retain_sanitized_training_log'))
        print(f"  Raw history:  {'local' if raw_history_enabled else 'disabled'}")
        print(f"  Training log: {'sanitized' if training_log_enabled else 'disabled'}")
        print(f"  JSONL mirror: {'enabled' if raw_history_enabled else 'suppressed by policy'}")
        print(f"  Curator logs: {'enabled' if training_log_enabled else 'suppressed by policy'}")
        try:
            from agent.sinria_egress import resolve_sinria_boundary_control
            boundary = resolve_sinria_boundary_control(config)
            matrix = boundary.get('data_policy_matrix', {}) if isinstance(boundary, dict) else {}
            registry = boundary.get('provider_trust_registry', {}) if isinstance(boundary, dict) else {}
            print(f"  Boundary:     {boundary.get('deployment_mode', '(not set)')}")
            print(f"  Data classes: {'/'.join(matrix.keys())}")
            print(f"  Provider reg: {'/'.join(registry.keys())}")
        except Exception:
            pass
    ...
    print(color(f"  {cli_name} config edit     # Edit config file", Colors.DIM))
    print(color(f"  {cli_name} config set <key> <value>", Colors.DIM))
    if cli_name == "sinria":
        print(color(f"  {cli_name} config set sinria.policy.active_profile <profile>", Colors.DIM))
    print(color(f"  {cli_name} setup           # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            cli_name = _cli_command_name()
            print(f"Usage: {cli_name} config set <key> <value>")
            print()
            print("Examples:")
            print(f"  {cli_name} config set model anthropic/claude-sonnet-4")
            print(f"  {cli_name} config set terminal.backend docker")
            print(f"  {cli_name} config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print(f"    Run '{_config_cmd('migrate')}' to add them")
        ...
        print(f"  {cli_name} config           Show current configuration")
        print(f"  {cli_name} config edit      Open config in editor")
        print(f"  {cli_name} config set <key> <value>   Set a config value")
        print(f"  {cli_name} config check     Check for missing/outdated config")
        print(f"  {cli_name} config migrate   Update config with new options")
        print(f"  {cli_name} config path      Show config file path")
        print(f"  {cli_name} config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``sinria config`` UI so users
