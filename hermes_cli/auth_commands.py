...
def _cli_command_name() -> str:
    value = os.environ.get("SINRIA_CLI_NAME") or os.environ.get("HERMES_CLI_NAME", "")
    return "sinria" if str(value).lower() == "sinria" else "hermes"
...
def _display_source(source: str) -> str:
    raw = source.split(":", 1)[1] if source.startswith("manual:") else source
    # Keep the stored source id as hermes_pkce for legacy compatibility, but
    # don't expose the upstream fork name in Sinria-facing auth output.
    return "sinria_pkce" if raw == "hermes_pkce" else raw
...
def auth_add_command(args) -> None:
    ...
    if provider == "nous":
        ...
        # <runtime-root>/shared/nous_auth.json (written by any previous
        # successful login), offer to import it instead of running the
        # full device-code flow. This makes `<cli> --profile <name>
    ...
    if provider == "openai-codex":
        # Clear any existing suppression marker so a re-link after `auth
    ...
    command_name = _cli_command_name()
    raise SystemExit(f"`{command_name} auth add {provider}` is not implemented for auth type {requested_type} yet.")
...
def auth_remove_command(args) -> None:
    ...
    # Unified removal dispatch. Every credential source the CLI reads from
...
def auth_status_command(args) -> None:
    ...
    if not provider:
        command_name = _cli_command_name()
        raise SystemExit(f"Provider is required. Example: `{command_name} auth status spotify`.")
...
def _interactive_auth() -> None:
    """Interactive credential pool management when `auth` is called bare."""
