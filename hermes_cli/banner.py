...
Pure display functions with no main-CLI state dependency.
"""
...
def _fetch_pypi_latest(package: str = "sinria-agent") -> Optional[str]:
...
def check_for_updates() -> Optional[int]:
    """Check whether a CLI update is available.
    ...
    if embedded_rev:
        ...
        # $HERMES_HOME/sinria-agent/ may be a stale copy from --clone-all;
        # Path(__file__) always resolves to the actual installed checkout.
        repo_dir = Path(__file__).parent.parent.resolve()
        if not (repo_dir / ".git").exists():
            repo_dir = hermes_home / "sinria-agent"
        if not (repo_dir / ".git").exists():
...
def _resolve_repo_dir() -> Optional[Path]:
    """Return the active CLI git checkout, or None if this isn't a git install.

    Prefers the running code's location over the profile-scoped path
    because ``$HERMES_HOME/sinria-agent/`` may be a stale copy carried
    over by ``--clone-all``.
    """
    repo_dir = Path(__file__).parent.parent.resolve()
    if not (repo_dir / ".git").exists():
...
def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    ...
    active checkout. Cached per-process. Release URL always points at the
    canonical Sinria repo (forks don't get a link).
    """
    ...
    try:
        ...
        if behind is not None and behind != 0:
            ...
            if behind > 0:
                ...
                # UPDATE_AVAILABLE_NO_COUNT: nix-built CLI binary; we know an update
