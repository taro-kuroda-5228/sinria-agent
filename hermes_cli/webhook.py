"""Manage dynamic webhook subscriptions from the CLI.

Usage:
    <cli> webhook subscribe <name> [options]
    <cli> webhook list
    <cli> webhook remove <name>
    <cli> webhook test <name> [--payload '{"key": "value"}']

Subscriptions persist to the runtime-home webhook_subscriptions.json and are
...
def _webhook_cmd(command: str) -> str:
    return f"{_cli_command_name()} webhook {command}".strip()


def _gateway_run_cmd() -> str:
    return f"{_cli_command_name()} gateway run"
...
def _setup_hint() -> str:
  ...
  Then start the gateway: {_gateway_run_cmd()}
"""
...
def webhook_command(args):
    """Entry point for the webhook subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print(f"Usage: {_webhook_cmd('{subscribe|list|remove|test}')}")
        print(f"Run '{_webhook_cmd('--help')}' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print(f"  The gateway must be running to receive events ({_gateway_run_cmd()}).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print(f"  Create one with: {_webhook_cmd('subscribe <name>')}")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or f'{{"test": true, "event_type": "test", "message": "Hello from {_cli_command_name()} webhook test"}}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print(f"  Is the gateway running? ({_gateway_run_cmd()})")
