#!/usr/bin/env python3
"""
Skills Hub CLI — Unified interface for the Sinria Skills Hub.

Powers both:
  - `sinria skills <subcommand>` (CLI argparse entry point)
...
from hermes_constants import display_sinria_home


def _cli_command_name() -> str:
    import os
    name = os.environ.get("SINRIA_CLI_NAME") or os.environ.get("HERMES_CLI_NAME") or "sinria"
    name = name.strip().lower()
    return name or "sinria"


def _cli_cmd(args: str) -> str:
    return f"{_cli_command_name()} {args}"
...
def _existing_categories() -> List[str]:
    """Return sorted subdirectory names under the Sinria skills directory that look
...
def _prompt_for_category(c: Console, existing: List[str]) -> str:
    ...
    if existing:
        ...
        c.print(
            f"[bold]Category[/] [dim](optional — press Enter to install flat at {display_sinria_home()}/skills/<name>/)[/]"
        )
    try:
...
def do_search(query: str, source: str = "all", limit: int = 10,
    ...
    c.print(table)
    c.print(f"[dim]Use: {_cli_cmd('skills inspect <identifier>')} to preview, "
            f"{_cli_cmd('skills install <identifier>')} to install[/]\n")
...
def do_browse(page: int = 1, page_size: int = 20, source: str = "all",
    ...
    if official_count > 0 and page == 1:
        c.print(f"[bright_cyan]★ {official_count} official optional Sinria skill(s)[/]")
    c.print()
    ...
    c.print(f"[dim]Tip: '{_cli_cmd('skills search <query>')}' searches deeper across all registries[/]\n")
...
def do_install(identifier: str, category: str = "", force: bool = False,
    ...
    if bundle.source == "url" and (not bundle.name or bundle_meta.get("awaiting_name")):
        if name_override and _is_valid_installed_skill_name(name_override):
            ...
            c.print(
                ...
                f"  [bold]{_cli_cmd(f'skills install {url} --name <your-name>')}[/]\n\n"
    ...
    if not force and not skip_confirm:
        ...
        if bundle.source == "official":
            c.print(Panel(
                "[bold bright_cyan]This is an official optional Sinria skill maintained by Medical Horizon.[/]\n\n"
                "It ships with sinria-agent but is not activated by default.\n"
                "Installing will copy it to your skills directory where the agent can use it.\n\n"
                f"Files will be at: [cyan]{display_sinria_home()}/skills/{category + '/' if category else ''}{bundle.name}/[/]",
...
def do_inspect(identifier: str, console: Optional[Console] = None) -> None:
    ...
    if bundle and "SKILL.md" in bundle.files:
        ...
        c.print(Panel(preview, title="SKILL.md Preview", subtitle=f"{_cli_cmd('skills install <id>')} to install"))
...
def do_list(source_filter: str = "all",
    ...
    config — ``sinria -p <profile> skills list`` reads that profile's
    ``skills.disabled`` list because the profile flag swaps the runtime home at process
...
def do_tap(action: str, repo: str = "", console: Optional[Console] = None) -> None:
    ...
    if action == "list":
        ...
        if not repo:
            c.print(f"[bold red]Error:[/] Repo required. Usage: {_cli_cmd('skills tap add owner/repo')}\n")
            return
        if mgr.add(repo):
        ...
        if not repo:
            c.print(f"[bold red]Error:[/] Repo required. Usage: {_cli_cmd('skills tap remove owner/repo')}\n")
            return
        if mgr.remove(repo):
    ...
    if target == "github":
        if not repo:
            c.print("[bold red]Error:[/] --repo required for GitHub publish.\n"
                    f"Usage: {_cli_cmd('skills publish <path> --to github --repo owner/repo')}\n")
            return

        auth = GitHubAuth()
        if not auth.is_authenticated():
            c.print("[bold red]Error:[/] GitHub authentication required.\n"
                    f"Set GITHUB_TOKEN in {display_sinria_home()}/.env or run 'gh auth login'.\n")
            return
    ...
    try:
        resp = httpx.post(
            ...
            json={
                ...
                "body": f"Submitting the `{skill_name}` skill via Sinria Skills Hub.\n\n"
                        f"This skill was scanned by the Sinria Skills Guard before submission.",
...
def do_record(
    ...
    *,
    ...
    """Create a local Sinria skill from a sanitized workflow recording/notes file."""
    from hermes_cli.skill_recorder import DEFAULT_SKILLS_DIR, read_recording_input, write_recorded_skill

    c = console or _console
    transcript = read_recording_input(input_path)
    result = write_recorded_skill(
        skills_dir=Path(skills_dir).expanduser() if skills_dir else DEFAULT_SKILLS_DIR,
        name=name,
    ...
    if not result.get("success"):
        c.print(f"[bold red]Error:[/] {result.get('error', 'Failed to create recorded workflow skill.')}\n")
        return result

    c.print(f"[bold green]Created recorded workflow skill:[/] {result['path']}")
    c.print(f"[dim]SKILL.md:[/] {result['skill_md']}")
    c.print(f"[dim]Replay checklist:[/] {result['replay_checklist']}")
    c.print(f"[dim]Review bundle:[/] {result['review_bundle']}")
    if save_demo and result.get("sanitized_recording"):
        c.print(f"[yellow]Saved sanitized notes only:[/] {result['sanitized_recording']}")
    else:
        c.print("[dim]Privacy:[/] raw recording was not stored; only sanitized replay instructions were written.")
    _print_record_replay_next_steps(c, name=name, video=False)
    return result


def _read_optional_notes(notes: str | None = None, notes_file: str | None = None) -> str:
    if notes_file:
...
def _collect_record_video_guidance(
    *,
    name: str | None,
    ...
    """Collect reusable intent/safety metadata without storing raw screen text."""
    console.print("[bold]Sinria record-video guide[/]")
    console.print("[dim]Enter reusable intent and safety gates only; do not paste credentials, PHI/PII, or raw customer text.[/]")
    skill_name = (name or _prompt_text("Skill name", "recorded-video-replay")).strip().lower()
    skill_description = (description or _prompt_text("One-line description", "Replay a screen-recorded workflow safely.")).strip()
    purpose = _prompt_text("Purpose / operator intent")
    prohibited = _prompt_text("Must not do without approval", "No external send; no production write; no raw private data storage")
    done_when = _prompt_text("Done when / completion condition")
    inputs = list(required_inputs or [])
    while True:
        item = _prompt_text("Required replay input (blank to finish)")
        if not item:
            break
        inputs.append(item)
    guided_notes = "\n".join(
        ...
        for line in [
            ...
            f"Must not: {prohibited}" if prohibited else "",
            f"Done when: {done_when}" if done_when else "",
            notes,
        ]
        if line
    ).strip()
    return skill_name, skill_description, guided_notes, inputs


def _print_record_replay_next_steps(c: Console, *, name: str, video: bool = False) -> None:
    replay_target = "screen-recorded workflow" if video else "workflow"
    replay_cmd = _cli_cmd(
        f"chat --skills {name} -q 'Replay this {replay_target} for <current inputs>. Verify the result before reporting done.'"
    )
    c.print("[bold]Next steps[/]")
    c.print("  1. Review SKILL.md plus references/review-bundle.json and references/replay-checklist.md.")
    if video:
        c.print("  2. Open Company OS Review and approve metadata only; do not upload raw video.")
    else:
        c.print("  2. Open Company OS Review if this skill should be shared or reused by the team.")
    c.print("  3. Replay locally only after filling current inputs; external action remains blocked until a separate approval.")
    c.print(f"[dim]Replay:[/] {replay_cmd}\n")
...
def do_record_video(
    ...
    *,
    ...
    """Create a local Sinria skill from a local screen/video recording.

    If record_duration is supplied, capture a short local screen video first.
    The raw video is never copied into the skill directory; only sanitized
    metadata and replay instructions are written.
    """
    from hermes_cli.skill_recorder import (
        DEFAULT_SKILLS_DIR,
        record_local_screen_video,
        write_recorded_video_skill,
    )

    c = console or _console
    if record_duration is not None:
        capture_root = Path(output).expanduser() if output else DEFAULT_SKILLS_DIR.parent / "captures" / f"{name}.mp4"
        try:
            video = record_local_screen_video(capture_root, duration=int(record_duration))
        except Exception as exc:
            result = {"success": False, "error": str(exc)}
            c.print(f"[bold red]Error:[/] {result['error']}\n")
            return result
    elif video_path:
        video = Path(video_path).expanduser()
    else:
        result = {"success": False, "error": "Pass --video <local recording> or --record-duration <seconds>."}
        c.print(f"[bold red]Error:[/] {result['error']}\n")
        return result

    guidance_notes = _read_optional_notes(notes, notes_file)
    if interactive:
        name, description, guidance_notes, required_inputs = _collect_record_video_guidance(
            name=name,
            description=description,
    ...
    if not name or not description:
        result = {"success": False, "error": "Pass --name and --description, or use --interactive to be guided through them."}
        c.print(f"[bold red]Error:[/] {result['error']}\n")
        return result
    ...
    if not result.get("success"):
        c.print(f"[bold red]Error:[/] {result.get('error', 'Failed to create video-recorded workflow skill.')}\n")
        return result

    c.print(f"[bold green]Created video-recorded workflow skill:[/] {result['path']}")
    c.print(f"[dim]SKILL.md:[/] {result['skill_md']}")
    c.print(f"[dim]Video capture metadata:[/] {result['video_capture']}")
    if result.get("video_step_extraction"):
        c.print(f"[dim]Video step extraction:[/] {result['video_step_extraction']}")
    c.print(f"[dim]Replay checklist:[/] {result['replay_checklist']}")
    c.print("[dim]Privacy:[/] raw video was not copied into the skill; only sanitized metadata and replay instructions were written.")
    _print_record_replay_next_steps(c, name=name, video=True)
    return result
...
def skills_command(args) -> None:
    """Router for the active Sinria CLI skills subcommands."""
    ...
    if action == "browse":
        ...
        do_record(
            ...
            required_inputs=getattr(args, "required_input", None) or [],
            skills_dir=getattr(args, "skills_dir", None),
            save_demo=getattr(args, "save_demo", False),
        ...
        do_record_video(
            ...
            record_duration=getattr(args, "record_duration", None),
        ...
        if snap_action == "export":
            ...
            _console.print(f"Usage: {_cli_cmd('skills snapshot')} [export|import]\n")
    elif action == "tap":
        tap_action = getattr(args, "tap_action", None)
        repo = getattr(args, "repo", "") or getattr(args, "name", "")
        if not tap_action:
            _console.print(f"Usage: {_cli_cmd('skills tap')} [list|add|remove]\n")
            return
        do_tap(tap_action, repo=repo)
    else:
        _console.print(f"Usage: {_cli_cmd('skills')} [browse|search|install|record|inspect|list|check|update|audit|uninstall|reset|publish|snapshot|tap]\n")
        _console.print(f"Run '{_cli_cmd('skills <command> --help')}' for details.\n")
