...
store under the runtime home. Actions:

    checkpoints               # same as `status`
    checkpoints status        # total size, project count, breakdown
    checkpoints list          # per-project checkpoint counts + workdir
    checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    checkpoints clear [-f]    # nuke the entire base (asks first)
    checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    checkpoints
    checkpoints prune --retention-days 3 --max-size-mb 200
    checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``checkpoints`` parser."""
