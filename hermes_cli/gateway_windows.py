...
_TASK_DESCRIPTION = f"{_product_name()} Gateway - Messaging Platform Integration"
...
def get_task_name() -> str:
    ...
    Default profile: base gateway task name
    Named profile X: base gateway task name with a profile suffix
    """
    _assert_windows()
...
def get_task_script_path() -> Path:
    ...
    Lives under the local app-data gateway-service directory
...
def _report_gateway_start(via: str) -> None:
    ...
    if pids:
        ...
        print(f"  Then check status: {_cli_command_name()} gateway status")
...
def _print_next_steps() -> None:
    ...
    print()
    print("Next steps:")
    print(f"  {cli} gateway status                      # Check status")
...
def status(deep: bool = False) -> None:
    ...
    if not task_installed and not startup_installed and not pids:
        print()
        print("To install:")
        print(f"  {_cli_command_name()} gateway install")
