"""CLI subcommand: ``hermes send`` / ``sinria send`` — pipe text from shell
scripts to any configured messaging platform (Telegram, Discord, Slack,
Signal, SMS, etc.).
...
def _gateway_setup_cmd() -> str:
    return f"{_cli_command_name()} gateway setup"
...
def _read_message_body(
    ...
    if file_path:
        ...
        try:
            ...
            print(f"{_cli_cmd('send')}: cannot read {file_path}: {exc}", file=sys.stderr)
    ...
    *,
    ...
    if json_mode:
        ...
        if payload.get("error"):
            print(f"{_cli_cmd('send')}: {payload['error']}", file=sys.stderr)
...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    try:
        ...
        print(f"{_cli_cmd('send')}: failed to load channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT

    try:
        raw = load_directory()
    except Exception as exc:
        print(f"{_cli_cmd('send')}: failed to read channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT
    ...
    if platform_filter:
        ...
        if not filtered:
            print(
                f"{_cli_cmd('send')}: no targets found for platform '{platform_filter}'. "
                f"Configured: {', '.join(sorted(platforms)) or '(none)'}",
                file=sys.stderr,
            )
            print(f"Try `{_cli_cmd('send --list')}` to inspect available targets.", file=sys.stderr)
            return _FAILURE_EXIT
        platforms = filtered
    ...
    if not any(platforms.values()):
        ...
        print(f"Set one up with `{_gateway_setup_cmd()}`, or run the gateway once so")
        print(f"channel discovery can populate {_display_home()}/channel_directory.json.")
        return _SUCCESS_EXIT
...
def _load_hermes_env() -> None:
    """Populate ``os.environ`` from the active home ``.env`` AND bridge top-level
    ...
    startup that the send subcommand must replicate when invoked standalone:

    1. ``load_dotenv(<home>/.env)`` — brings bot tokens into the env.
    2. Bridge top-level simple values from ``<home>/config.yaml`` into
    ...
    intentionally reimplement the minimum needed here so the send subcommand
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    # Bridge the active home .env and config.yaml into os.environ so the
    ...
    if not target:
        example_telegram = _cli_cmd('send --to telegram "hello"')
        example_discord = _cli_cmd('send --to discord:#ops --file report.md')
        example_list = _cli_cmd('send --list')
        print(
            f"{_cli_cmd('send')}: --to PLATFORM[:channel[:thread]] is required\n"
            "Examples:\n"
            f"  {example_telegram}\n"
            f"  {example_discord}\n"
            f"  {example_list}      # list available targets",
            file=sys.stderr,
        )
        sys.exit(_USAGE_EXIT)
    ...
    if message is None or not message.strip():
        print(
            f"{_cli_cmd('send')}: no message provided. Pass text as a positional "
    ...
    # Import lazily so CLI help stays fast and does not pull in
...
def register_send_subparser(subparsers) -> argparse.ArgumentParser:
    ...
    """
    example_deploy = _cli_cmd('send --to telegram "deploy finished"')
    example_pipe = _cli_cmd('send --to telegram:-1001234567890')
    example_discord = _cli_cmd('send --to discord:#ops --file /tmp/report.md')
    example_slack = _cli_cmd('send --to slack:#eng --subject "[CI]" --file build.log')
    example_list = _cli_cmd('send --list')
    example_list_tg = _cli_cmd('send --list telegram')
    parser = subparsers.add_parser(
        "send",
        help="Send a message to a configured platform (scripts, cron jobs, CI).",
        description=(
            f"Pipe text from any shell script to any messaging platform {_product_name()} "
            "is already configured for. Reuses the gateway's platform "
            f"credentials ({_display_home()}/.env + {_display_home()}/config.yaml) — no LLM, "
        ...
        epilog=(
            "Examples:\n"
            f"  {example_deploy}\n"
            f"  echo \"RAM 92%\" | {example_pipe}\n"
            f"  {example_discord}\n"
            f"  {example_slack}\n"
            f"  {example_list}                  # all platforms\n"
            f"  {example_list_tg}         # filter by platform\n"
            "\n"
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help=f"List available targets. Optional positional filter: `{_cli_cmd('send --list telegram')}`.",
    )
