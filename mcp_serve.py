...
def _fallback_runtime_home() -> Path:
    """Return the runtime home when hermes_constants cannot be imported."""
    explicit_home = os.environ.get("HERMES_HOME")
    if explicit_home:
        return Path(explicit_home)
    cli_name = os.environ.get("HERMES_CLI_NAME", "hermes").lower()
    return Path.home() / (".sinria" if cli_name == "sinria" else ".hermes")
...
def _get_sessions_dir() -> Path:
    ...
    try:
        ...
        return _fallback_runtime_home() / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        ...
        directory_file = _fallback_runtime_home() / "channel_directory.json"
...
class EventBridge:
    ...
    def _poll_once(self, db):
        ...
        try:
            ...
            db_file = _fallback_runtime_home() / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    ...
    mcp = FastMCP(
        "hermes",
        instructions=(
            "Sinria messaging bridge. Use these tools to interact with "
